...
#include <boost/optional/optional_io.hpp> /* required to output std::optional in assertions */
...
TEST(is_hdd, rotational_drive) {
  ...
  EXPECT_EQ(tools::is_hdd(hdd), std::optional<bool>(true));
}
...
TEST(is_hdd, ssd) {
  ...
  EXPECT_EQ(tools::is_hdd(ssd), std::optional<bool>(false));
}

TEST(is_hdd, unknown_attrs) {
  EXPECT_EQ(tools::is_hdd("/dev/null"), std::nullopt);
}
#endif
