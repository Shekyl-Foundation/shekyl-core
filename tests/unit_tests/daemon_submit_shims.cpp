...
// ─────────────────────────────────────────────────────────────────────────
// F-11 probe (§50). The trigger under test is tx_pool.cpp's loop detection:
//
//     if (tx_relay == relay_method::stem && meta.dandelionpp_stem)
//       tx_relay = relay_method::fluff;
//
// Two questions, settled by execution rather than inferred from the source:
// does an inbound stem re-send of an already-stem-held tx force an immediate
// fluff, and can the sender's identity gate it? These live here rather than
// in txpool_relay_timers.cpp because reaching that line requires a tx that
// passes `check_tx_inputs` — a first attempt using a zero-fee minimal blob
// was rejected on fee grounds long before the branch and was vacuous by
// input in all three of its cases, including the one that appeared to pass.
//
// DISABLED — vacuous a SECOND time, and this is why they are disabled rather
// than deleted or left green. The re-send is rejected before the branch: this
// fixture `commit`s the tx first, so the re-delivery hits the key-image gate
// that sits ahead of `check_tx_inputs`. The reachability assertions inside
// `probe_resend` are what caught it; without them the run reported "held in
// stem, unchanged" and the peer-blindness test PASSED, both meaning nothing.
//
// One structural fact does survive and is not in question:
// `tx_memory_pool::add_tx` takes no peer identity, and `add_new_tx`'s
// short-circuit uses `relay_category::legacy` (= broadcasted + none), which
// EXCLUDES stem — so a re-sent stem-held tx does reach `add_tx` in
// production. What remains unexecuted is only whether it then clears the
// key-image and input gates to reach the promotion.
//
// A valid harness must deliver the tx to a pool that holds it in stem state
// WITHOUT having inserted its key images from a local commit — i.e. seed the
// pool entry at the DB level (as `txpool_relay_timers` does) while still
// supplying a tx that passes `check_tx_inputs`. Neither existing fixture does
// both. See DAEMON_RELAY_PRIVACY.md §50.
// ─────────────────────────────────────────────────────────────────────────

namespace
{
  //! Put `s` into the pool in stem state, then re-deliver it with `incoming`.
  //! \return the relay method the pool holds afterwards.
  relay_method probe_resend(ShimFixture& fx, const SubmitTx& s, const relay_method incoming)
  {
    shekyl_submit_facts_ffi fresh;
    uint8_t fresh_ki = 0;
    EXPECT_EQ(fx.commit(s, fresh, fresh_ki), SHEKYL_SUBMIT_OK);

    // Move it to the stem state the trigger keys on, and confirm the
    // precondition rather than assume it — otherwise a fixture change makes
    // the whole probe vacuous without saying so.
    {
      txpool_tx_meta_t meta{};
      EXPECT_TRUE(fx.db->get_txpool_tx_meta(s.txid, meta));
      meta.set_relay_method(relay_method::stem);
      fx.db->update_txpool_tx(s.txid, meta);
      txpool_tx_meta_t check{};
      EXPECT_TRUE(fx.db->get_txpool_tx_meta(s.txid, check));
      EXPECT_TRUE(check.dandelionpp_stem)
        << "precondition: the tx must be held in dandelionpp_stem state";
    }

    cryptonote::transaction tx = s.tx;
    tx_verification_context tvc{};
    const bool accepted = fx.bap.txpool.add_tx(tx, s.txid, s.blob, s.weight, tvc,
      incoming, /*relayed=*/true, /*version=*/1, /*nic_verified_hf_version=*/1);

    // REACHABILITY, asserted rather than assumed: the loop-detection branch
    // sits in the `else` of `if (!ch_inp_res)`, so a tx that fails input
    // checks never reaches it and every assertion below would be vacuous.
    // A first attempt at this probe was vacuous exactly this way.
    EXPECT_TRUE(accepted) << "add_tx rejected the re-send — the branch under "
                             "test was never reached, and this probe is vacuous";
    EXPECT_FALSE(tvc.m_verifivation_failed)
      << "verification failed, so the re-send took the !ch_inp_res path";

    txpool_tx_meta_t after{};
    EXPECT_TRUE(fx.db->get_txpool_tx_meta(s.txid, after));
    return after.get_relay_method();
  }
}

// Q1: does one inbound stem message force the holder to broadcast?
TEST(txpool_stem_probe, DISABLED_an_inbound_stem_resend_promotes_a_stem_held_tx_to_fluff)
{
  ShimFixture fx;
  const SubmitTx s = fx.make_tx(0, 0);
  EXPECT_EQ(relay_method::fluff, probe_resend(fx, s, relay_method::stem))
    << "if this is fluff, one message from any peer makes the holder broadcast";
}

// Q2, the decisive one: can identity discriminate a probe from a genuine
// loop? `tx_memory_pool::add_tx` takes (tx, id, blob, weight, tvc, tx_relay,
// relayed, version, hf) — NO peer, no connection id, no source. The protocol
// handler holds `context.m_connection_id` and drops it at
// `handle_incoming_tx`. So every caller is indistinguishable at the decision
// site by construction, and this asserts the consequence: two deliveries that
// differ only in who sent them cannot differ in outcome, because "who sent
// it" is not an input.
TEST(txpool_stem_probe, DISABLED_the_promotion_cannot_depend_on_who_sent_it)
{
  ShimFixture predecessor;
  const SubmitTx a = predecessor.make_tx(0, 0);
  const relay_method from_predecessor = probe_resend(predecessor, a, relay_method::stem);

  ShimFixture stranger;
  const SubmitTx b = stranger.make_tx(0, 0);
  const relay_method from_stranger = probe_resend(stranger, b, relay_method::stem);

  EXPECT_EQ(from_predecessor, from_stranger)
    << "identical outcomes confirm the trigger is peer-blind: it is a "
       "membership oracle on the stem set, and no identity-based "
       "discriminator can exist at this layer";
}

// Bound on the oracle: a tx already in fluff state must not react, because
// upgrade_relay_method(stem) fails (stem < fluff). This is what limits the
// probe to one usable bit per transaction.
TEST(txpool_stem_probe, DISABLED_a_fluff_held_tx_does_not_react_to_a_stem_resend)
{
  ShimFixture fx;
  const SubmitTx s = fx.make_tx(0, 0);
  shekyl_submit_facts_ffi fresh;
  ...
  EXPECT_EQ(relay_method::fluff, after.get_relay_method())
    << "already-fluffed txs do not re-react: the oracle is one bit per tx";
}
