...
TEST(txpool_relay_timers, local_holds_past_min_relay_time)
{
  ...
  // THE discriminating case: relayed 400 s ago — past MIN_RELAY_TIME (300 s),
  // short of the derived interval. Reverting the base to MIN_RELAY_TIME turns
  // this green→red, which is the whole point of the case.
  fx.put(relay_method::local, now - 400, now - 400);
  EXPECT_FALSE(fx.relayable())
