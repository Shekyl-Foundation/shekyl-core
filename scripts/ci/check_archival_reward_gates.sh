...
# Staker-inflow redirect operand tripwire (F-B1b / F-B1c-c2, gating round
# §9.9). The redirect block in handle_block_to_main_chain must:
#
...
#     leg, re-mintable through emission claims once redirected into
#     budget(E): an inflation surface);
#   - key the burn-vs-accrue decision on bl.major_version, the block's own
...
REDIRECT_BLOCK="$(awk '/Staker-inflow redirect \(ARCHIVAL_BUDGET_SCHEDULE/,/m_db->add_block\(/' "$BLOCKCHAIN_CPP")"
REDIRECT_CODE="$(rg -v '^\s*//' <<<"$REDIRECT_BLOCK" || true)"
if [[ -z "$REDIRECT_BLOCK" ]]; then
  echo "FAIL: staker-inflow redirect block not found in $BLOCKCHAIN_CPP" >&2
  echo "  (if the anchors moved, update this tripwire; do not let it pass silently)" >&2
  FAIL=1
else
  if rg -n 'get_block_reward' <<<"$REDIRECT_CODE"; then
    echo "FAIL: get_block_reward call inside the staker-inflow redirect block" >&2
  ...
  if rg -n 'get_current_version|get_ideal_version\(' <<<"$REDIRECT_CODE"; then
    echo "FAIL: tip-relative or table-only version read inside the redirect block" >&2
    echo "  (the redirect keys on bl.major_version — the block's own consensus-checked" >&2
    echo "   version; see F-B1b in gating round §9.9)" >&2
    FAIL=1
  fi
  if ! rg -q 'bl\.major_version' <<<"$REDIRECT_CODE" \
     || ! rg -q 'compute_emission_split' <<<"$REDIRECT_CODE"; then
    echo "FAIL: redirect-block anchors drifted (bl.major_version / compute_emission_split" >&2
