...
//! `rust/shekyl-wallet-state/schemas/`.
//!
...
mod tests {
    ...
    /// Snapshot directory, relative to the `shekyl-wallet-state` crate root.
    ...
    fn check_or_update_snapshot(name: &str, schema: &'static NamedType) {
        ...
        let expected = fs::read_to_string(&path).unwrap_or_else(|e| {
            panic!(
                "missing snapshot {}: {e}\n\
                 hint: run `UPDATE_SNAPSHOTS=1 cargo test -p shekyl-wallet-state \
