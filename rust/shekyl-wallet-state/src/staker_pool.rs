...
/// and kept in sync during scanning. Persisted inside the ledger block of
/// the wallet file.
...
mod tests {
    ...
    fn reward_skips_zero_blocks() {
        ...
        pool.insert(102, make_record(0, 0, 0));
    ...
    fn json_roundtrip() {
        ...
        let back: StakerPoolState = serde_json::from_str(&s).unwrap();
        assert_eq!(pool, back);
    }
    ...
    fn postcard_roundtrip() {
        ...
        pool.insert(101, make_record(100, 50, 1000));
        pool.insert(102, make_record(200, 100, 2000));
        let bytes = postcard::to_allocvec(&pool).unwrap();
        let back: StakerPoolState = postcard::from_bytes(&bytes).unwrap();
        assert_eq!(pool, back);
    }
}
