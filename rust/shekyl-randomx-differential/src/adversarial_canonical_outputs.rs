...
//! likewise. The substrate that operationalizes this scope is the
//! consolidated set:
//!
//! - **Compile-time meta-pin tamper guard.** [`FAMILY_1_RECIPE_SHA256`]
//!   is asserted byte-equal against `SHA-256(FAMILY_1_RECIPE_OUTPUTS)`
//!   by the [`tests::family_1_recipe_sha256_meta_pin`] unit test;
//!   the cheap unit test runs on default `cargo test` and catches
//!   any tamper of the canonical array's contents.
//! - **In-module shape backstop.** The `#[ignore]`-gated
//!   [`adversarial::canonical::tests::compute_corpus_canonicals_full_corpus_shape`](super::adversarial::canonical)
//!   test re-derives canonicals against the C4 starter corpus and
//!   asserts the shape invariant (one non-zero entry per recipe).
//! - **Regeneration path.** The
//!   [`gen_canonical_outputs`](../../../rust/shekyl-randomx-differential/src/bin/gen_canonical_outputs.rs)
//!   binary provides the second independent computation path
//!   (C-reference-derived) per the
//!   [`canonical_outputs`](super::canonical_outputs)
//!   regeneration-discipline precedent.
//!
//! A dedicated `tests/adversarial_canonical_runtime.rs` integration
//! test that re-derives and asserts byte-equality against
//! [`FAMILY_1_RECIPE_OUTPUTS`] was scoped in early planning (as a
//! "C7" landing) but consolidated into the substrate above; per
//! [`21-reversion-clause-discipline.mdc`](../../../.cursor/rules/21-reversion-clause-discipline.mdc),
//! this disposition reopens only if evaluator-drift regressions
//! surface that the compile-time pin + in-module shape backstop
//! don't catch — at which point the deferred integration test
//! lands as the load-bearing regression guard.
//!
...
/// fails likewise. The substrate that operationalizes this scope
/// is the consolidated set enumerated in this module's top-of-file
/// rustdoc (compile-time [`FAMILY_1_RECIPE_SHA256`] meta-pin
/// tamper guard + `#[ignore]`-gated
/// [`adversarial::canonical::tests::compute_corpus_canonicals_full_corpus_shape`](super::adversarial::canonical)
/// shape backstop + `gen_canonical_outputs` regeneration helper).
///
/// # Regeneration
///
...
mod tests {
    ...
    /// between commit and use; complements the in-module
    /// `#[ignore]`-gated shape backstop
    /// [`adversarial::canonical::tests::compute_corpus_canonicals_full_corpus_shape`](super::adversarial::canonical)
    /// per the consolidated substrate enumerated in this module's
    /// top-of-file rustdoc).
    #[test]
    ...
    /// `gen_canonical_outputs` helper; the in-module
    /// `#[ignore]`-gated shape backstop
    /// [`adversarial::canonical::tests::compute_corpus_canonicals_full_corpus_shape`](super::adversarial::canonical)
    /// is the substrate-level check — if a recipe is reordered
    /// without regenerating the canonical SHAs, the
    /// shape-backstop's per-index non-zero assertion + the
    /// compile-time [`FAMILY_1_RECIPE_SHA256`] meta-pin combine
    /// to surface the drift, even though the comments may have
    /// gone stale. Per
    /// [`19-validation-surface-discipline.mdc`](../../../.cursor/rules/19-validation-surface-discipline.mdc),
    ...
    /// (that's the consolidated runtime/regeneration substrate
    /// enumerated in this module's top-of-file rustdoc); it
