...
def main() -> int:
    ...
    daemon_commit = subprocess.check_output(
    ...
    fixture = {
        ...
        "fixture_bytes_commit": daemon_commit,
        "audited_at": datetime.now(timezone.utc).strftime("%Y-%m-%d"),
        "audited_daemon_commit": daemon_commit,
        "generator_script": "gen_ct2_fixture.py",
        "regen_required": False,
        "regen_audit_note": (
            "Fresh generation at audited_daemon_commit. Regenerate only when "
            "consensus coinbase leaf/root rules change."
        ),
        "network": "regtest",
