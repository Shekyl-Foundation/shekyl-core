...
/// TJ-F liveness face (RED — the D1 red-test pattern;
/// `ARCHIVAL_TEST_EQUALS_JOB_SEQUENCING.md` §4 TJ-F, §7):
/// **verification must succeed from `R_k` plus responder-supplied material
/// with no leaf-store read.** Today it cannot: the wire
/// (`ArchivalServeCreditResponse`) carries only the challenged 128-byte
/// `leaf_bytes`, and consensus fills `leaf_layer_scalars` from the local
/// segment leaf store (`path.rs`'s own doc states this) — which is exactly
/// fact §2.5's consensus-required universal retention. A pruned validator has
/// only what the wire carries, modeled here as the empty leaf-layer: the call
/// must still verify. It fails today (`LeafNotInOpening`); TJ-A's response
/// format (carrying the leaf-layer chunk or equivalent) turns it green, and
/// nothing else can.
#[test]
#[ignore = "RED: documents TJ-F's liveness face — verification requires local leaf-store reads; \
un-ignores when TJ-A's challenge/response spec lands (ARCHIVAL_TEST_EQUALS_JOB_SEQUENCING.md)"]
fn tj_f_red_verification_succeeds_without_a_leaf_store() {
    let blocks = main_chain();
    let mut client = CurveTreeClient::new();
    for blk in &blocks {
        let txs = [TxLeafInputs {
    ...
    // A pruned validator's whole world: the wire-carried material plus R_k.
    // No leaf store exists to read the leaf-layer chunk from.
    let wire_carried_leaf_layer: Vec<[u8; 32]> = Vec::new();

    verify_segment_path(
        &leaf_bytes,
        &wire_carried_leaf_layer,
        &opening,
        &reference.curve_tree_root,
    )
    .expect("TJ-F liveness: a response must verify from R_k + responder-supplied material alone");
}
