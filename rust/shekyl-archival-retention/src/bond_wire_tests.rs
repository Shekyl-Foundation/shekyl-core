...
fn endpoint_update_vin(hybrid_pk: &[u8]) -> ArchivalBondPostVin {
    ArchivalBondPostVin::endpoint_update(
        hybrid_pk.to_vec(),
        p_canonical_id_from_hybrid_pubkey(hybrid_pk).to_bytes(),
        [0x4E; ENDPOINT_BYTES],
    )
...
fn bond_post_complete_tree_has_no_shard_list_on_wire() {
    ...
    let wire = vin.serialize().unwrap();
    let decoded = ArchivalBondPostVin::read(&mut wire.as_slice()).unwrap();
    let holdings = decoded.holdings().expect("JoinMarket carries holdings");
...
#[test]
fn kind_four_is_endpoint_update() {
    assert!(matches!(
...
fn endpoint_update_round_trips_as_exactly_the_endpoint() {
    ...
    assert_eq!(decoded, vin);
    assert!(decoded.bond_spend_pk().is_none());
    assert!(decoded.holdings().is_none());
    assert_eq!(decoded.bond_credit(), None);
    assert_eq!(decoded.bond_debit(), None);
    assert_eq!(decoded.bonded_total_atomic(), None);
}

/// EU-D9 KAT (a), as a parse test: bytes spelling a holdings list and a term
/// after a kind-4 endpoint are trailing bytes, not a parse of a vin with a
/// term. Write-side refusal of those fields is the type — EndpointUpdate
/// has no holdings or amount fields to emit.
#[test]
fn endpoint_update_trailing_holdings_or_term_are_not_part_of_the_vin() {
    let hybrid_pk = vec![0xAB; HYBRID_PUBKEY_CANONICAL_BYTES];
