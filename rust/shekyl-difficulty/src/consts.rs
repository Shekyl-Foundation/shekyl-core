...
//! with the C++ side per `docs/completed/DAA_LWMA1.md` §4 and §6.2. The
...
/// `docs/completed/DAA_LWMA1.md` §5.5 (which co-tunes FTL with the
...
/// recommendation per `docs/completed/DAA_LWMA1.md` §4 / §5.5.
...
// Per docs/completed/DAA_LWMA1.md §4 Round 4 disposition.
const _: () = assert!(
    N == 90,
...
// Per docs/completed/DAA_LWMA1.md §4 (T = 120 second target block time).
const _: () = assert!(
    T_SECONDS == 120,
...
// Per docs/completed/DAA_LWMA1.md §5.5 (FTL = N*T/20 = 540 seconds).
const _: () = assert!(
    FTL_SECONDS == 540,
...
// Per docs/completed/DAA_LWMA1.md §5.5 (MTP window = 11 blocks).
const _: () = assert!(
    MTP_WINDOW == 11,
