...
//! `shekyl-engine-file`'s `save_state` hands to the envelope's
...
//!
//! The bench lives in `shekyl-engine-state` (not `shekyl-engine-file`)
...
fn build_ledger(n: usize) -> WalletLedger {
    ...
    // `shekyl-engine-state::invariants`) fires inside
