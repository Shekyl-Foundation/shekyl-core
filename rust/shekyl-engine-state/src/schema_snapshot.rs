...
//! 3.4). Every type that lands on disk — [`WalletLedger`] and the six inner
//! blocks [`LedgerBlock`], [`BookkeepingBlock`], [`TxMetaBlock`],
//! [`SyncStateBlock`], [`StakingBlock`], and [`SendJournalBlock`], plus the
//! `P`-isolated
...
//! - `send_journal_block.snap`     ↔  [`SEND_JOURNAL_BLOCK_VERSION`]
...
//! [`SEND_JOURNAL_BLOCK_VERSION`]: crate::send_journal_block::SEND_JOURNAL_BLOCK_VERSION
...
mod tests {
    ...
    #[test]
    fn send_journal_block_schema_matches_snapshot() {
    ...
    fn rendered_schemas_are_self_parseable() {
        for schema in [
            ...
            <SendJournalBlock as Schema>::SCHEMA,
        ] {
