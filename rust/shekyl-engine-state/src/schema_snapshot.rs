...
mod tests {
    ...
    /// **The gate asserts its own arming state (rule 47).**
    ///
    /// `UPDATE_SNAPSHOTS` is a kill switch for every assertion in this module:
    /// the ten per-type snapshot tests stop comparing and write instead (still
    /// reporting pass, deliberately, so a bootstrap run is not a failure), and
    /// `version_bump_registry_covers_every_snapshot` returns early. One
    /// environment variable disarms the whole file, and a disarmed run is
    /// indistinguishable from a clean one in the output — which is precisely
    /// the shape [`47-gate-subject-assertion`] exists to refuse: green and
    /// vacuous must not look the same.
    ///
    /// It is `var_os(..).is_some()`, not a truthy check, so `UPDATE_SNAPSHOTS=`,
    /// `=0` and `=false` all disarm it too. A reader assuming a truthy test
    /// would mis-judge how easily this fires.
    ///
    /// Nothing else covers the hole. The paired-bump job in
    /// `.github/workflows/schema-snapshot.yml` fires on a `.snap` file
    /// *changing in the diff*; under the kill switch CI rewrites the snapshot
    /// in its own working tree and commits nothing, so that job sees no change
    /// either. **Two independent-looking checks with one shared blind spot** —
    /// worse than a single weak check, because a reviewer counting checks sees
    /// two and feels covered.
    ///
    /// The remedy keeps the bootstrap workflow intact rather than removing it:
    /// a developer running `UPDATE_SNAPSHOTS=1` locally still regenerates every
    /// snapshot and now also gets exactly one loud failure here telling them to
    /// re-run without the variable to actually assert. CI, which never sets it,
    /// *proves* the assertions were armed instead of assuming it — the
    /// assumption the sibling test's comment states as fact ("CI's
    /// assert-snapshots job runs *without* the flag") with nothing checking it.
    #[test]
    fn snapshot_assertions_are_armed() {
        assert!(
            env::var_os("UPDATE_SNAPSHOTS").is_none(),
            "UPDATE_SNAPSHOTS is set, so every schema-snapshot assertion in this \
             module is disarmed: the per-type tests OVERWRITE their `.snap` \
             instead of comparing, and the registry-coverage guard returns early. \
             Snapshots were still regenerated by this run. Re-run WITHOUT \
             `UPDATE_SNAPSHOTS` to assert them — a green run with this variable \
             set proves nothing about the persisted wire schema."
        );
    }
