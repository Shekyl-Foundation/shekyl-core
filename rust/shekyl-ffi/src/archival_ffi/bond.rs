...
/// the presence coupling is the selector's shape, not a check. EndpointUpdate
/// does not come through this function.
///
/// # Safety
...
/// len is 0.
/// The value-moving payload the shared marshal builds. Carries exactly the
/// kind-coupled operand its kind can hold: JoinMarket commits the `EU-D3`
/// endpoint; Release, Rebond and HoldingsUpdate have no endpoint field, and
/// EndpointUpdate has its own entry and payload. A wrong kind at an entry is
/// therefore refused by that entry's own wrong-kind code before any operand
/// is marshaled, never as a coupling on an operand the kind has no slot for.
#[derive(Clone, Copy)]
...
impl ValueMovingKind {
    ...
    /// marshal; JoinMarket needs an endpoint those entries do not carry, and
    /// EndpointUpdate is not value-moving.
    fn endpoint_free(kind: BondPostKind) -> Option<Self> {
        match kind {
            ...
            BondPostKind::JoinMarket | BondPostKind::EndpointUpdate => None,
        }
    }
}
...
/// `ENDPOINT_BYTES` is the key, anything else is the coupling refusal (the
/// wire carries the raw key or nothing — there is no other length).
///
/// # Safety
/// When `len > 0`, `ptr` must be valid for `len` bytes for the duration of the call.
unsafe fn endpoint_from_raw(
    ptr: *const u8,
    len: usize,
) -> Result<Option<[u8; ENDPOINT_BYTES]>, u8> {
    if len == 0 {
        return Ok(None);
    }
    if len != ENDPOINT_BYTES {
...
pub unsafe extern "C" fn shekyl_archival_verify_join_market_bond_post(
    ...
    // EU-D3: JoinMarket commits an endpoint; absent (len 0) is the coupling
    // refusal, as is any length other than the key's.
    let endpoint = match unsafe { endpoint_from_raw(endpoint_ptr, endpoint_len) } {
        Ok(Some(e)) => e,
        Ok(None) => return SHEKYL_ARCHIVAL_BOND_POST_ERR_ENDPOINT_COUPLING,
...
pub unsafe extern "C" fn shekyl_archival_verify_release_bond_post(
    ...
    // A kind this entry cannot marshal (out of range, JoinMarket, EndpointUpdate)
    // is the entry's own wrong-kind verdict; the verify re-checks Release by name.
...
unsafe fn bond_post_record_marshal_prologue(
    ...
    // (`EU-D3`); a kind it cannot marshal without one (JoinMarket), or that is
    // not value-moving (EndpointUpdate), is the caller's wrong-kind verdict.
