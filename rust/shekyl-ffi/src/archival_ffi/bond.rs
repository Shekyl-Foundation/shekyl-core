...
unsafe fn bond_post_vin_from_raw(
    ...
    bond_credit: u64,
    bond_debit: u64,
    endpoint: Option<[u8; ENDPOINT_BYTES]>,
...
pub unsafe extern "C" fn shekyl_archival_verify_release_bond_post(
    ...
    let post_kind = match BondPostKind::from_u8(post_kind) {
        Ok(k) => k,
        Err(_) => return SHEKYL_ARCHIVAL_BOND_POST_ERR_POST_KIND_NOT_RELEASE,
    };
    let vin = match bond_post_vin_from_raw(
        post_kind,
        holdings_kind,
...
unsafe fn bond_post_record_marshal_prologue(
    ...
    let vin = unsafe {
        bond_post_vin_from_raw(
            ...
            bond_credit,
            bond_debit,
            // The prologue serves HoldingsUpdate and Rebond, which carry no
            // endpoint (`EU-D3`); a conforming C++ caller has nothing to pass.
            None,
        )
    }?;
