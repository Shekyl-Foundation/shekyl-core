...
//! orchestrator in `shekyl-engine-file` owns all of that. Before this
...
/// [`shekyl_engine_prefs::PrefsError`] variant plus JSON
/// serialization/deserialization failures at the FFI boundary
/// (caller-supplied JSON for `shekyl_engine_prefs_set_json` did not
...
// ---------------------------------------------------------------------------
//
// The FFI surface for `shekyl-engine-prefs` uses JSON as the wire
...
///   panicking).
#[no_mangle]
pub unsafe extern "C" fn shekyl_engine_prefs_get_json(
    h: *mut ShekylWallet,
    out_buf: *mut u8,
    out_cap: usize,
    out_len_required: *mut usize,
...
#[no_mangle]
pub unsafe extern "C" fn shekyl_engine_prefs_set_json(
    h: *mut ShekylWallet,
    json_ptr: *const u8,
    json_len: usize,
    out_error: *mut u32,
    ...
    // every nested struct in `shekyl-engine-prefs::schema` ensures
...
pub unsafe extern "C" fn shekyl_wallet_extract_rederivation_inputs(
    ...
    match w.inner.extract_rederivation_inputs() {
        ...
        Err(shekyl_engine_file::ExtractRederivationInputsError::ViewOnly) => {
            set_err(out_error, SHEKYL_WALLET_ERR_CAPABILITY_VIEW_ONLY_NO_SPEND);
            false
        }
        Err(shekyl_engine_file::ExtractRederivationInputsError::HardwareOffload) => {
            set_err(
                out_error,
...
mod tests {
    ...
    /// `shekyl-engine-file`'s own tests.
    ...
    /// path is covered in `shekyl-engine-file`'s own tests, and here
    ...
    /// Call `shekyl_engine_prefs_get_json` with the two-call sizing
    ...
    fn get_prefs_json(h: *mut ShekylWallet) -> (Vec<u8>, bool) {
        unsafe {
            ...
            let mut err = 0u32;
            let ok = shekyl_engine_prefs_get_json(
                h,
                std::ptr::null_mut(),
                0,
                &raw mut need,
            ...
            let mut err2 = 0u32;
            let ok2 = shekyl_engine_prefs_get_json(
                h,
                buf.as_mut_ptr(),
                buf.len(),
                &raw mut got,
    ...
    fn prefs_set_then_get_round_trips_through_disk() {
        ...
        unsafe {
            let mut err = 0u32;
            let ok = shekyl_engine_prefs_set_json(h, json.as_ptr(), json.len(), &raw mut err);
    ...
    fn prefs_get_surfaces_tamper_and_quarantines() {
        ...
        unsafe {
            let mut err = 0u32;
            assert!(shekyl_engine_prefs_set_json(
                h,
                json.as_ptr(),
                json.len(),
                &raw mut err,
