...
pub extern "C" fn shekyl_daemon_rpc_start(
    ...
    let core = match unsafe { shekyl_daemon_rpc::core::CoreRpc::from_raw(rpc_server_ptr) } {
    ...
    let config = shekyl_daemon_rpc::server::ServerConfig {
        bind_address: bind,
        restricted,
        ..Default::default()
    };
    ...
    rt.spawn(async move {
        if let Err(e) =
            shekyl_daemon_rpc::server::run_server(core, config, shutdown_for_server).await
        {
...
pub extern "C" fn shekyl_daemon_rpc_stop(handle: *mut ShekylDaemonRpcHandle) {
    ...
    // Recover the Notify and signal shutdown
