...
fn flipped_version_byte_detected_as_auth_failure() {
    ...
    assert_eq!(
        rc, -2,
        "expected auth failure (-2) from flipped version byte, got {rc}"
    );
}
...
fn flipped_ciphertext_body_detected_as_auth_failure() {
    ...
    assert_eq!(
        rc, -2,
        "expected auth failure (-2) from flipped body byte, got {rc}"
    );
}
...
fn empty_ciphertext_returns_format_error() {
    ...
    assert_eq!(
        rc, -3,
        "expected format error (-3) for empty input, got {rc}"
    );
}
