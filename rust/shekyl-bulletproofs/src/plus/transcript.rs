...
use shekyl_curve_primitives::{keccak256, keccak256_to_scalar};
...
pub(crate) static TRANSCRIPT: LazyLock<[u8; 32]> = LazyLock::new(|| {
    biased_hash_to_point(keccak256(b"bulletproof_plus_transcript"))
        .compress()
        .to_bytes()
});

pub(crate) fn initial_transcript(commitments: core::slice::Iter<'_, EdwardsPoint>) -> Scalar {
    let commitments_hash = keccak256_to_scalar(
        commitments
