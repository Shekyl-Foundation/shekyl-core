...
//!   stable.
//!
//! Wired as a `#[path]` child of `engine/local_keys.rs`, so `use super::*`
//! and `super::` paths resolve into the workflow module and private items
...
fn compute_test_key_image(output_key: [u8; 32], spend_key_x: [u8; 32]) -> [u8; 32] {
    ...
    let x_scalar =
        Scalar::from_canonical_bytes(spend_key_x).expect("spend_key_x from bundle is canonical");
