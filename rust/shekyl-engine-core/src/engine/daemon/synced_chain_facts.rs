...
impl SyncedChainFacts {
    ...
    /// [`ChainCount`] as the dispatch clock (height-semantics Phase 2b).
    /// The number is the Phase 1 pin (a 3-block chain reports 3); flipping
    /// to [`Self::tip`] would move every stamp down by one block. Prefer
    /// [`Self::tip`] when the consumer wants an ordinal — a consumer that
    /// wants a height and reaches for this is reintroducing the confusion
