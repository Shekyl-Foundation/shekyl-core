...
//!
//! Message handlers live in sibling modules (`persona`, `bond`, `claim`, …) and
//! reach actor state through `pub(crate)` fields (transfer-style; rule 36 still
//! holds — secrets never leave the stake_engine tree).
...
use crate::engine::pscan::scan_step::{
    FundingOutputMatch, KeyImageWatchSet,
};
...
use super::helpers::{
    draw_entry_gap_guarded, derive_funding_key_image,
};
use super::types::*;
