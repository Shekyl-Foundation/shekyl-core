...
// `config/consensus_constants.json` (generated by
// `rust/shekyl-engine-core/build.rs`). The C++ side at
// `src/cryptonote_config.h` consumes the same JSON via
// `cmake/generate_consensus_constants.py`, so the wallet-side
// validation cannot drift from the daemon-side consensus check
// (`src/cryptonote_core/blockchain.cpp` ~line 3632). Bug 3 of the
// 2026-05-05 FFI constant-drift audit motivated the JSON authority;
// pre-fix this file hand-defined `MIN_AGE = 10` while C++ hand-defined
// `MIN_AGE = 5`, silently rejecting valid reference blocks for ~3
// weeks (the Rust multisig SpendIntent added in `744ab6407`, 23 days
...
mod tests {
    ...
    fn validate_temporal_rejects_ref_block_too_fresh() {
        // ref_height = 900, tip = 903 → age = 3 < MIN_AGE (5),
        // unambiguously "too fresh". Previously used tip = 905 (age = 5),
        // which was the boundary value `age < 5` (false) under the
        // post-Bug-3 fixed value — would accept rather than reject and
        // mask any future regression that lowered MIN_AGE further. This
        // wording survives the JSON-authority migration: as long as
        // MIN_AGE > 3 (which the consensus-review sentinel pins), this
        // test correctly fires.
        let intent = make_test_intent();
