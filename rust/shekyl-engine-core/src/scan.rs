...
//!
//! `ScanResult` is the value the Phase 2a `Engine::refresh()`
//! pipeline produces from a scanner pass and that
//! `apply_scan_result` merges into [`shekyl_engine_state::WalletLedger`]
//! plus [`shekyl_engine_state::LedgerIndexes`] under the wallet's
//! `&mut self` borrow. The type lives in `shekyl-engine-core` (not
...
//!   [`shekyl_engine_state::LedgerIndexes::ingest_block`] via the
...
//! [`shekyl_engine_state::TransferDetails`]. Those fields are
...
/// [`crate::engine::RefreshError::ConcurrentMutation`] when:
///
...
/// persisted [`shekyl_engine_state::TransferDetails`].
