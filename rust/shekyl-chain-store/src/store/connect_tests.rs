...
use super::connect_fixtures::{
    candidate, coinbase, connect_genesis, facts, judge, spend, FixtureSubstrate, GENESIS_RULES,
};
...
fn genesis_connect_writes_every_row_of_the_write_set_at_the_lmdb_layouts() {
    ...
    assert_eq!(
        snap.open_table(HF_VERSIONS)
            ...
            .expect("g")
            .map(|g| g.value().decode().expect("decodes")),
        Some(RuleSetInForce(GENESIS_RULES.id())),
...
fn two_blocks_in_one_batch_with_a_spend_and_a_burn() {
    ...
    let out: Result<(Connected, Connected), TestErr> = store.write(|batch| {
        ...
        let c0 = batch.connect(judge(&view, g)?, facts(0, 0), GENESIS_RULES)?;
        // Block 1 is validated against a view that already holds block 0
        // and connected in the same batch.
        let c1 = batch.connect(judge(&view, b1)?, facts(1, 25), GENESIS_RULES)?;
        Ok((c0, c1))
    });
...
fn pop_by_replay_returns_the_store_to_the_state_before_the_block() {
    ...
    let out: Result<Connected, TestErr> = store.write(|batch| {
        let view = batch.chain_view();
        Ok(batch.connect(judge(&view, b1)?, facts(1, 4), GENESIS_RULES)?)
    });
...
fn a_block_whose_parent_is_not_the_tip_is_si2() {
    ...
    let out: Result<Connected, TestErr> = store.write(|batch| {
        ...
        batch.connect(first, facts(1, 0), GENESIS_RULES)?; // the tip moves
        Ok(batch.connect(second, facts(1, 0), GENESIS_RULES)?) // stale: SI-2
    });
...
fn a_fakechain_verdict_is_refused_under_genesis_in_force() {
    ...
    let out: Result<Connected, TestErr> = store.write(|batch| {
        ...
        Ok(batch.connect(valid, facts(0, 0), GENESIS_RULES)?)
    });
    let want = StoreCannot::RuleSetNotInForce {
        height: 0,
        judged: seven.id(),
        in_force: GENESIS_RULES.id(),
    };
    assert_eq!(out, Err(TestErr::Store(StoreError::from(want).to_string())));
    assert_eq!(
        seven.id(),
        GENESIS_RULES.id(),
        "the premise: same id, different set"
    );
    assert!(
        StoreError::from(want)
            .to_string()
            .contains("same id, different set"),
        "the refusal says why two equal ids are not one set"
    );
    cleanup(&path);
}
...
fn a_fakechain_verdict_connects_when_the_fakechain_set_is_in_force() {
    // RD-F13's repair (DRS-E2 RD-Q10): with `in_force: RuleSetId` no id
    // could name a `Fixed` set, so the refusal above was also the proof
    // that no path accepted. `connect` now takes the set; the schedule
    // that named it hands it over, and the belt still lands GENESIS's id.
    let seven = RuleSet::fakechain(core::num::NonZeroU128::new(7).expect("non-zero"));
    let path = tmp("connect-fakechain-accepts");
    let store = ChainStore::create(&path, EPOCH).expect("create");
    let out: Result<Connected, TestErr> = store.write(|batch| {
        let view = batch.chain_view();
        let cand = candidate(0, BlockHash::NULL, Vec::new());
    ...
    let connected = out.expect("a Fakechain verdict connects under its own set");
    assert_eq!(connected.height, BlockHeight::from_raw(0));
    let snap = store.begin_read().expect("read");
    assert_eq!(
        snap.open_table(HF_VERSIONS)
            .expect("t")
        ...
        "the CEN-B3 belt lands the id, which Fakechain shares with GENESIS"
    );
    cleanup(&path);
}
...
fn plant_then_connect_is_si9(
    ...
    let out: Result<Connected, TestErr> = store.write(|batch| {
        let view = batch.chain_view();
        Ok(batch.connect(judge(&view, b1)?, facts(1, 0), GENESIS_RULES)?)
    });
...
fn a_key_image_spent_in_an_earlier_block_is_si1() {
    ...
    let out: Result<Connected, TestErr> = store.write(|batch| {
        let view = batch.chain_view();
        Ok(batch.connect(judge(&view, b2)?, facts(2, 0), GENESIS_RULES)?)
    });
...
fn a_pass_through_connect_taints_the_file_s_provenance_and_a_derived_one_does_not() {
    ...
    let out: Result<Connected, TestErr> = store.write(|batch| {
        let view = batch.chain_view();
        Ok(batch.connect(judge(&view, g)?, derived, GENESIS_RULES)?)
    });
    out.expect("genesis");
    ...
    let out: Result<(), TestErr> = store.write(|batch| {
        let view = batch.chain_view();
        batch.connect(judge(&view, b1.clone())?, facts(1, 0), GENESIS_RULES)?;
        Err(TestErr::Abort)
    });
    ...
    let out: Result<Connected, TestErr> = store.write(|batch| {
        let view = batch.chain_view();
        Ok(batch.connect(judge(&view, b1)?, partial, GENESIS_RULES)?)
    });
    out.expect("block 1");
