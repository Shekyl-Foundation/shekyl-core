...
use shekyl_crypto_pq::account::{
    ...
    MASTER_SEED_BYTES, RAW_SEED_BYTES, SeedFormat,
};
...
fn blob_to_tier2_expected(blob: &AllKeysBlob, master_seed: &[u8; MASTER_SEED_BYTES]) -> Tier2Expected {
    Tier2Expected {
...
fn assert_tier2_expected(blob: &AllKeysBlob, master_seed: &[u8; MASTER_SEED_BYTES], exp: &Tier2Expected) {
    ...
    assert_eq!(got.pqc_public_key_hex, exp.pqc_public_key_hex, "pqc_public_key_hex");
    assert_eq!(
        got.classical_address_bytes_hex,
        exp.classical_address_bytes_hex,
...
fn run_tier1(v: &Tier1Vector) {
    match v.kind.as_str() {
        ...
        "derive_spend_wide" | "derive_view_wide" | "derive_kem_d_z" => {
            let master = decode_hex(
                v.master_seed_hex.as_ref().expect("master_seed_hex"),
                &v.id,
            );
...
fn run_tier2(v: &Tier2Vector) {
    ...
    let (master_seed, blob) = match fmt {
        SeedFormat::Bip39 => {
            ...
            let (seed, blob) =
                generate_account_from_bip39(words, passphrase, net).expect("generate_account_from_bip39");
            (seed, blob)
        }
        SeedFormat::Raw32 => {
    ...
    // Rederive path must reproduce the same public material.
    let blob2 = rederive_account(&master_seed, net, fmt).expect("rederive_account");
    assert_eq!(
        blob.spend_pk.as_canonical_bytes(),
        blob2.spend_pk.as_canonical_bytes(),
        "{} rederive spend_pk",
        v.id
    );
    assert_eq!(
        blob.pqc_public_key,
        blob2.pqc_public_key,
        "{} rederive pqc_public_key",
        v.id
    );
}
...
fn build_tier1_vectors() -> Vec<serde_json::Value> {
    ...
    let spend_mainnet = derive_spend_wide(&master_11, DerivationNetwork::Mainnet, SeedFormat::Bip39);
    ...
    let sk_mainnet =
        wide_reduce_to_scalar(&derive_spend_wide(&master_11, DerivationNetwork::Mainnet, SeedFormat::Bip39));
