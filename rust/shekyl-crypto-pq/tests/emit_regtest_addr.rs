...
// regenerate rust/shekyl-wire/tests/vectors/regtest_coinbase_h{1,2}.block via
// capture_coinbase.py. Fakechain mirrors Mainnet address encoding, so the address
// parses on the FAKECHAIN daemon (account.rs::to_address_network). Deterministic
// from the fixed seed so the corpus is reproducible.
//
//   cargo test -p shekyl-crypto-pq --test emit_regtest_addr -- --ignored --nocapture
#[test]
...
fn emit_regtest_mining_address() {
    ...
    let spend: [u8; 32] = blob.classical_address_bytes[1..33].try_into().unwrap();
    let view: [u8; 32] = blob.classical_address_bytes[33..65].try_into().unwrap();
