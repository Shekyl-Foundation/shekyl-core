...
pub fn advance_already_generated(
    ...
    // formula pays the tail at EMISSION_CURVE_ASYMPTOTE") whose policy conclusion the
...
mod tests {
    ...
    fn c2a_prime_layer2_b_accum_matches_projected_already_generated() {
        ...
        for height in 0..1000 {
            ...
            ag = ag.saturating_add(q_full).min(p.emission_curve_asymptote);
        }
    }
    ...
    fn advance_already_generated_passes_the_asymptote() {
        ...
        assert_eq!(
            advance_already_generated(p.emission_curve_asymptote, 1, &p),
            p.emission_curve_asymptote + 1
        );
        assert_eq!(
            advance_already_generated(p.emission_curve_asymptote - 1, 50, &p),
