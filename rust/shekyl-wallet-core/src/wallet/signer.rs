...
//! Signer-kind type parameter for the [`Wallet`](super::Wallet)
//! orchestrator.
//!
//! The Phase 1 plan locks the wallet's solo / multisig dispatch into the
//! type system: `Wallet<S: WalletSignerKind>`, with `S` ranging over
...
//!
//! [`WalletSignerKind`] is sealed: only this crate may add variants.
//! Downstream code parameterizes `Wallet<S>` with `SoloSigner` (or, in
...
/// [`Wallet`](super::Wallet). Sealed: see module docs.
///
