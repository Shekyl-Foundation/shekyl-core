...
//! [`Wallet::refresh`](super::Wallet) is the only async lifecycle
...
//!
//! [`Wallet::open_full`] returns an [`OpenedWallet`] sum rather than a
//! plain `Wallet<S>` so the rebuilt-state recovery path
//! ([`OpenOutcome::StateLost`](shekyl_wallet_file::OpenOutcome::StateLost))
...
use shekyl_wallet_prefs::{LoadOutcome as PrefsLoadOutcome, WalletPrefs};
use shekyl_wallet_state::{LedgerIndexes, WalletLedger};

use super::error::{IoError, KeyError, OpenError};
use super::{Capability, DaemonClient, SoloSigner, Wallet, WalletSignerKind};
...
impl<S: WalletSignerKind> std::fmt::Debug for OpenedWallet<S> {
...
impl<S: WalletSignerKind> OpenedWallet<S> {
    ...
    pub fn into_wallet(self) -> Wallet<S> {
        match self {
            Self::Loaded(w) => w,
            Self::Restored { wallet, .. } => wallet,
        }
...
pub struct WalletCreateParams<'a> {
...
#[cfg(test)]
impl<'a> WalletCreateParams<'a> {
    /// Test-only helper. Builds a FULL-capability `WalletCreateParams`
...
// ---------------------------------------------------------------------------
// Wallet<SoloSigner> :: create / open_full / open_view_only / open_hardware_offload
// ---------------------------------------------------------------------------

impl Wallet<SoloSigner> {
    ...
    pub fn create(params: WalletCreateParams<'_>, daemon: DaemonClient) -> Result<Self, OpenError> {
        let WalletCreateParams {
            base_path,
            credentials,
            network,
            capability,
    ...
    pub fn open_full(
    ...
    ) -> Result<OpenedWallet<SoloSigner>, OpenError> {
        let (file, outcome) =
            WalletFile::open(base_path, credentials.password(), network, overrides)
                .map_err(|e| map_wallet_file_error(e, network))?;
        ...
        let prefs = match file.load_prefs() {
            ...
            Ok(PrefsLoadOutcome::Tampered { prefs, .. }) => {
                warn!(
                    target: "shekyl_wallet_core::lifecycle",
        ...
        Ok(match restored_from {
            None => OpenedWallet::Loaded(wallet),
            Some(from_height) => OpenedWallet::Restored {
                wallet,
                from_height,
            },
        })
...
fn extract_failure_detail(
    _err: &shekyl_wallet_file::ExtractRederivationInputsError,
) -> &'static str {
...
// ---------------------------------------------------------------------------
// Wallet<S> :: change_password / close (signer-kind-agnostic)
// ---------------------------------------------------------------------------
...
impl<S: WalletSignerKind> Wallet<S> {
    ...
    ///    on `<base>.keys` (see
    ///    `shekyl_wallet_file::handle::WalletFile::drop`).
    ...
    /// (`Wallet<S>.keys: AllKeysBlob`); there is no wrapper layer
...
mod tests {
    ...
    /// the daemon is held on the `Wallet<S>` for refresh / submit
    ...
    fn create_full_then_open_full_round_trips_state() {
        ...
        let params = WalletCreateParams::for_test_full(&fix.base_path, &creds, &seed);
        let network = params.network;
        let created =
            Wallet::<SoloSigner>::create(params, dummy_daemon()).expect("create FULL wallet");
        ...
        let opened = Wallet::<SoloSigner>::open_full(
            &fix.base_path,
            &creds,
            network,
            dummy_daemon(),
        ...
        assert!(matches!(opened, OpenedWallet::Loaded(_)));
    ...
    fn open_full_with_wrong_password_returns_incorrect_password() {
        ...
        let params = WalletCreateParams::for_test_full(&fix.base_path, &creds, &seed);
        let network = params.network;
        Wallet::<SoloSigner>::create(params, dummy_daemon())
        ...
        let err = Wallet::<SoloSigner>::open_full(
            &fix.base_path,
            &bad_creds,
            network,
            dummy_daemon(),
    ...
    fn change_password_rewraps_envelope_then_reopen_uses_new_password() {
        ...
        let params = WalletCreateParams::for_test_full(&fix.base_path, &creds_old, &seed);
        let network = params.network;
        let mut wallet =
            Wallet::<SoloSigner>::create(params, dummy_daemon()).expect("create FULL wallet");
        wallet
        ...
        let _ = Wallet::<SoloSigner>::open_full(
            &fix.base_path,
            &creds_new,
            network,
            dummy_daemon(),
    ...
    fn open_full_after_state_file_deleted_returns_restored_from_height() {
        ...
        let state_path = {
            ...
            // `shekyl_wallet_file` writes `<base>.keys` and
        ...
        match opened {
            ...
            OpenedWallet::Loaded(_) => panic!("expected Restored, got Loaded"),
        }
    }
    ...
    fn open_view_only_returns_capability_not_yet_implemented() {
        ...
        let err = Wallet::<SoloSigner>::open_view_only(
            &fix.base_path,
            &creds,
            network,
            dummy_daemon(),
    ...
    fn open_hardware_offload_returns_capability_not_yet_implemented() {
        ...
        let err = Wallet::<SoloSigner>::open_hardware_offload(
            &fix.base_path,
            &creds,
            network,
            dummy_daemon(),
    ...
    /// Sanity-check that `for_test_full` produces a `WalletCreateParams`
    /// callable through `Wallet::create` without surprises. Functionally
