...
///
/// [`inherited`] is the single Rust mirror of the `cryptonote_config.h` relay
/// block. These two are **not** mirrors: they replace `CRYPTONOTE_NOISE_BYTES`
/// (3 KiB) and `CRYPTONOTE_MAX_FRAGMENTS` (20), both of which are deleted from
/// C++ by the same change that adds this module — discharging the FOLLOWUP
/// `levin_notify.cpp` recorded beside its own guard ("*the constants should
/// cross to Rust and `NoiseQueues`' window should be derived from them, at
/// which point this assertion moves with them*").
///
...
/// [`inherited::noise_windows_in_epoch`]`(300)`** — the epoch *ceiling* on the
/// cap rather than a cap derived from the transactions it has to carry. It was
/// a number satisfying a constraint nobody had connected to its subject, so it
/// was unattached rather than wrong.
pub mod carrier {
    ...
    /// [`inherited::noise_windows_in_epoch`]`(300) = 20` windows, and the levin
    /// packet limit affords far more. Both are asserted in
    /// `tests/carrier_window.rs` against the real constants.
...
mod r1_tests {
    ...
    fn a_full_noise_message_fits_exactly_at_the_cadence_boundary() {
        ...
        let exact = carrier::MAX_FRAGMENTS * per_send;
        assert_eq!(
