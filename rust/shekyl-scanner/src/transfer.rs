...
//!
//! The canonical type now lives in [`shekyl_engine_state::transfer`] so that the
...
//! by the [`crate::ledger_ext::TransferDetailsExt`] extension trait, which
//! must be in scope for `TransferDetails::from_wallet_output(&out, h)` to resolve.

pub use shekyl_engine_state::transfer::{FcmpPrecomputedPath, TransferDetails, SPENDABLE_AGE};
