...
use crate::source::{Seq, Sequenced};
...
impl<T> Sequencer<T> {
    ...
    #[must_use]
    pub const fn new(first: Seq) -> Self {
        Self {
            next: first,
    ...
    pub fn push(&mut self, item: Sequenced<T>) -> Result<(), SequenceError> {
        ...
        self.pending.insert(item.seq, item.item);
        Ok(())
    }
...
mod tests {
    ...
    fn drain(s: &mut Sequencer<&'static str>) -> Vec<(u64, &'static str)> {
        ...
        while let Some(item) = s.pop_ready() {
            out.push((item.seq.to_raw(), item.item));
        }
        out
    }
    ...
    fn releases_in_order_and_never_past_a_gap() {
        let mut s = Sequencer::new(Seq::FIRST);
        s.push(Sequenced::new(seq(2), "c")).unwrap();
        s.push(Sequenced::new(seq(0), "a")).unwrap();
        assert_eq!(drain(&mut s), vec![(0, "a")], "1 is missing: 2 waits");
        assert_eq!(s.pending(), 1);
        s.push(Sequenced::new(seq(1), "b")).unwrap();
    ...
    fn a_released_position_and_a_pending_one_are_both_refused() {
        ...
        s.push(Sequenced::new(seq(0), "a")).unwrap();
        assert_eq!(
            s.push(Sequenced::new(seq(0), "a again")),
            Err(SequenceError::Duplicate(seq(0)))
        );
        drain(&mut s);
        assert_eq!(
            s.push(Sequenced::new(seq(0), "a late")),
    ...
    fn starts_where_told() {
        ...
        s.push(Sequenced::new(seq(5), "e")).unwrap();
    ...
    fn seq_saturates_at_the_top() {
        ...
        assert!(Seq::FIRST < Seq::FIRST.next());
    }
}
