...
impl From<ChainArg> for CorpusNet {
    fn from(c: ChainArg) -> Self {
        match c {
            ChainArg::Mainnet => CorpusNet::Mainnet,
            ChainArg::Testnet => CorpusNet::Testnet,
            ChainArg::Stagenet => CorpusNet::Stagenet,
            ChainArg::Regtest => CorpusNet::Fakechain,
        }
    }
}
...
enum Command {
    ...
    Fetch {
        ...
        /// (`CorpusNet`) and checked by `replay --chain`.
        #[arg(long, value_enum)]
        chain: ChainArg,
        /// First height.
...
async fn real_main(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    match cli.command {
        ...
        Command::Replay {
            ...
            let substrate = Arc::new(ProductionSubstrate::with_metrics(
