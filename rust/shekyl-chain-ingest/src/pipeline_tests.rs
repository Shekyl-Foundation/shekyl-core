...
use shekyl_chain_rules::{Corrupt, FormAttempt, Retry, RuleSet};
...
/// Form one corpus block under GENESIS with the mock substrate, claiming
/// `seed`.
fn formed(
    cb: CorpusBlock,
    seed: BlockHash,
...
fn corpus_block(
    height: u64,
    b: &shekyl_wire::Block,
    txs: &[shekyl_wire::Transaction],
) -> CorpusBlock {
    CorpusBlock {
        height: h(height),
        candidate: shekyl_chain_rules::Candidate::new(b.clone(), txs.to_vec()),
    }
}
...
async fn the_connector_stays_stopped_after_a_halt_and_does_not_come_back() {
    ...
    let (h1, f1) = formed(corpus_block(1, &chain[1].0, &chain[1].1), chain[0].0.hash());
    let err = connector
        .ask(Apply(vec![(h1, *f1)]))
        .await
    ...
    let (h1b, f1b) = formed(corpus_block(1, &chain[1].0, &chain[1].1), chain[0].0.hash());
    let again = connector
    ...
    let (h1c, f1c) = formed(corpus_block(1, &chain[1].0, &chain[1].1), chain[0].0.hash());
    assert!(matches!(
        connector
...
async fn a_corpus_replays_end_to_end_and_the_redb_digest_matches_the_trace_checkpoint() {
    ...
    assert_eq!(ours.n_spent, 4, "one spend per non-genesis block");
...
async fn a_wrong_seed_is_a_driver_defect_surfaced_on_first_occurrence() {
    ...
    let (h1, f1) = formed(corpus_block(1, &chain[1].0, &chain[1].1), wrong);
    let err = connector
        .ask(Apply(vec![(h1, *f1)]))
        .await
...
async fn a_corrupt_record_observed_by_the_validator_halts_through_refuse_corrupt() {
    ...
    let rest = {
        let mut w = crate::corpus::CorpusWriter::new(Vec::new(), h(n + 1)).expect("header");
        let (bb, bodies) = wire(&chain[n_us + 1].0, &chain[n_us + 1].1);
    ...
    // closure returned, so the fault is the SI-10 row itself — carrying the
    // validator's observation.
    match err {
        PipelineFault::Connector(RunFault::Store(StoreError::InvariantViolated(
            StoreInvariant::DifficultyRecordIncoherent(observed),
        ))) => {
            assert_eq!(
                observed,
                Corrupt::CumulativeDifficultyNotMonotone { at: h(50) }
            );
        }
        other => panic!("{other:?}"),
    }
...
async fn a_rewind_pops_behind_the_barrier_and_the_fork_connects() {
    ...
    let events = vec![
        IngestEvent::Extend(Box::new(corpus_block(0, &main[0].0, &main[0].1))),
        IngestEvent::Extend(Box::new(corpus_block(1, &main[1].0, &main[1].1))),
        IngestEvent::Extend(Box::new(corpus_block(2, &main[2].0, &main[2].1))),
        IngestEvent::Rewind { to: h(1) },
        IngestEvent::Extend(Box::new(corpus_block(2, &fork2, &[spend(0x22)]))),
        IngestEvent::Extend(Box::new(corpus_block(3, &fork3, &[spend(0x23)]))),
    ];
