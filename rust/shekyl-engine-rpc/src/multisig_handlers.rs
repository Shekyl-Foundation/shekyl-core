...
//! hex-encoded. DKG is handled via the `shekyl-engine-core` API directly
...
fn rpc_err(msg: impl Into<String>) -> EngineError {
    EngineError {
        code: -1,
        message: msg.into(),
    }
...
fn handle_register_group(ms: &mut MultisigState, params: Value) -> Result<Value, EngineError> {
...
fn handle_list_groups(ms: &MultisigState) -> Result<Value, EngineError> {
...
fn hex_to_32(s: &str, field: &str) -> Result<[u8; 32], EngineError> {
...
fn handle_create_signing(ms: &mut MultisigState, params: Value) -> Result<Value, EngineError> {
...
fn handle_sign_preprocess(ms: &mut MultisigState, params: Value) -> Result<Value, EngineError> {
...
fn handle_sign_add_preprocess(ms: &mut MultisigState, params: Value) -> Result<Value, EngineError> {
...
fn handle_sign_nonce_sums(ms: &mut MultisigState, params: Value) -> Result<Value, EngineError> {
...
fn handle_sign_own(ms: &mut MultisigState, params: Value) -> Result<Value, EngineError> {
    let p: SignGroupParams =
        serde_json::from_value(params).map_err(|e| rpc_err(format!("invalid params: {e}")))?;
    let group = ms
        .groups
...
fn handle_sign_add_shares(ms: &mut MultisigState, params: Value) -> Result<Value, EngineError> {
...
fn handle_sign_aggregate(ms: &mut MultisigState, params: Value) -> Result<Value, EngineError> {
    let p: SignGroupParams =
        serde_json::from_value(params).map_err(|e| rpc_err(format!("invalid params: {e}")))?;
    let group = ms
        .groups
