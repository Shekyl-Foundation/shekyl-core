...
// `shekyl-engine-state` and don't need a separate use here.
#[cfg(feature = "rust-scanner")]
...
pub fn dispatch(wallet: &Wallet2, method: &str, params: Value) -> Result<Value, EngineError> {
...
fn lock_state(
    scanner: &ScannerState,
) -> Result<std::sync::MutexGuard<'_, crate::scanner_state::LiveLedger>, EngineError> {
    scanner.state.lock().map_err(|e| EngineError {
        code: -1,
...
#[cfg(feature = "rust-scanner")]
fn scanner_get_balance(scanner: &ScannerState, _params: Value) -> Result<Value, EngineError> {
    let state = lock_state(scanner)?;
    let (ledger, _indexes) = &*state;
    let height = ledger.height();
    let summary = ledger.balance(height);
    serde_json::to_value(&summary).map_err(|e| EngineError {
        code: -1,
...
#[cfg(feature = "rust-scanner")]
fn scanner_get_height(scanner: &ScannerState) -> Result<Value, EngineError> {
    let state = lock_state(scanner)?;
    Ok(serde_json::json!({ "height": state.0.height() }))
}

#[cfg(feature = "rust-scanner")]
fn scanner_get_transfers(scanner: &ScannerState, params: Value) -> Result<Value, EngineError> {
...
#[cfg(feature = "rust-scanner")]
fn scanner_incoming_transfers(scanner: &ScannerState, params: Value) -> Result<Value, EngineError> {
...
fn parse_key_image(params: &Value) -> Result<[u8; 32], EngineError> {
    ...
    let ki_bytes = hex::decode(key_image).map_err(|e| EngineError {
        code: -1,
...
#[cfg(feature = "rust-scanner")]
fn scanner_freeze(scanner: &ScannerState, params: Value) -> Result<Value, EngineError> {
...
#[cfg(feature = "rust-scanner")]
fn scanner_thaw(scanner: &ScannerState, params: Value) -> Result<Value, EngineError> {
    let ki = parse_key_image(&params)?;
    let mut state = lock_state(scanner)?;
    let (ledger, indexes) = &mut *state;
