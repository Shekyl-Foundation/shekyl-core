...
/// RTN-3 / `PDM-Q-F26`: `pqc_auth_hash` **is** the txid's third component —
/// `keccak256(varint(count) ‖ auths)`, not `keccak256` of the raw
/// `txs_pqc_auths` segment — and mixing it with prefix, base, and prunable
/// reproduces [`Transaction::hash`].
#[test]
fn pqc_auth_hash_is_the_txid_third_component() {
    let tx = build_tx(true);
    let Ct::Fcmp { pqc_auths, .. } = &tx.ct else {
        panic!("KAT spend is FCMP");
    };
    assert!(
        !pqc_auths.is_empty(),
        "the KAT spend must be 4-part (rule 47: subject exists)"
    );

    let h_pqc = tx
        .pqc_auth_hash()
        .expect("4-part spend carries the third component");

    let segments = tx.write_segments().expect("Vec write is infallible");
    let mut with_count = Vec::new();
    shekyl_wire::varint::write_varint(pqc_auths.len(), &mut with_count)
        .expect("Vec write is infallible");
    with_count.extend_from_slice(&segments.pqc_auths);
    assert_eq!(
        PqcAuthHash::from_bytes(keccak256(&with_count)),
        h_pqc,
        "pqc_auth_hash must be keccak256(varint(count) ‖ auths)"
    );
    assert_ne!(
        PqcAuthHash::from_bytes(keccak256(&segments.pqc_auths)),
        h_pqc,
        "the raw txs_pqc_auths segment has no count prefix and must not be \
         the third component"
    );
...
#[test]
fn coinbase_pqc_auth_hash_is_none() {
    // 3-part: no third component. A sentinel would label the miner tx with
    // a value the chain never committed.
    let tx = Transaction {
        prefix: TxPrefix {
            unlock_time: 0,
        ...
        ct: Ct::Null(CtBase {
            ...
            commitments: vec![BASEPOINT],
        }),
    };
    assert_eq!(tx.pqc_auth_hash(), None);
}
