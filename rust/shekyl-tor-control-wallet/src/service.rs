...
use shekyl_tor_control_client::control::{
    parse_socks_listeners, BootstrapReadiness, BootstrapState, Command, ManagedTor, SocksPort,
...
mod tests {
    ...
    // The SOCKS-listener parse KATs moved with `parse_socks_listeners` to
    // `shekyl_tor_control_client::control::listeners` (PWD-E7 piece 2
    // pre-step) — the parse is neutral protocol shape, shared by both
    // supervisors, and its tests travel with it.
