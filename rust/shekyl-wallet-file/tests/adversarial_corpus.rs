...
//! [`shekyl_wallet_state::WalletLedger::check_invariants`] gate — must
...
//! [`shekyl_wallet_state::invariants`] each carry their own
...
use shekyl_wallet_file::paths::{keys_path_from, state_path_from};
use shekyl_wallet_file::payload::{
...
fn ledger_invariant_orphan_tx_key_is_refused() {
    ...
    let w = WalletLedger::new(
        shekyl_wallet_state::LedgerBlock::empty(),
        shekyl_wallet_state::BookkeepingBlock::empty(),
        tx_meta,
        shekyl_wallet_state::SyncStateBlock::empty(),
    );
    ...
    match open_and_capture_error(&base, TEST_PW) {
        WalletFileError::Ledger(WalletLedgerError::InvariantFailed { invariant, .. }) => {
            assert_eq!(
                invariant,
                shekyl_wallet_state::invariants::INV_TX_KEYS_NO_ORPHANS
            );
        }
