...
pub const INPUTS_UPPER_BOUND: usize = const_max(
    MAX_MINER_TRANSACTION_INPUTS,
    NON_MINER_TRANSACTION_INPUTS_UPPER_BOUND,
);
...
impl TransactionPrefix {
    ...
    pub fn read<R: Read>(r: &mut R) -> io::Result<TransactionPrefix> {
        ...
        let max_outputs = if is_miner_tx {
            None
        } else {
            Some(MAX_NON_MINER_TRANSACTION_OUTPUTS)
        };
        ...
        let max_extra = if is_miner_tx {
            None
        } else {
            Some(MAX_NON_MINER_TRANSACTION_SIZE)
        };
