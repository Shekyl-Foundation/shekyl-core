...
use crate::{fcmp::bulletproofs::Bulletproof, io::*};
...
///
/// The amount_tag detects KEM ciphertext corruption at decode time. It is derived
/// from the HKDF output secrets and verified by the scanner during decryption.
...
impl ProofBase {
    ...
    /// Read a ProofBase.
    pub fn read<R: Read>(outputs: usize, r: &mut R) -> io::Result<Option<(ProofType, ProofBase)>> {
...
impl PrunableProof {
    ...
    pub fn read<R: Read>(inputs: usize, r: &mut R) -> io::Result<PrunableProof> {
        ...
        if fcmp_proof_len > MAX_FCMP_PROOF_SIZE {
            Err(io::Error::other(
                "FCMP++ proof exceeds maximum allowed size",
            ))?;
        }
        ...
        if n_pqc_auths != inputs {
            Err(io::Error::other(
                "pqc_auths count does not match input count",
            ))?;
        }
        ...
        for _ in 0..n_pqc_auths {
            ...
            if auth_len > MAX_PQC_AUTH_SIZE {
                Err(io::Error::other(
                    "PQC auth signature exceeds maximum allowed size",
                ))?;
            }
...
impl Proofs {
    ...
    pub fn read<R: Read>(inputs: usize, outputs: usize, r: &mut R) -> io::Result<Option<Proofs>> {
        let Some((_proof_type, base)) = ProofBase::read(outputs, r)? else {
            return Ok(None);
        };
        Ok(Some(Proofs {
            base,
            prunable: PrunableProof::read(inputs, r)?,
        }))
    }
}
