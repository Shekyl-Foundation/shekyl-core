...
use crate::{
  ...
  address::{Network, AddressType, SubaddressIndex, ShekylAddress},
};
...
impl ViewPair {
  ...
  pub fn legacy_address(&self, network: Network) -> ShekylAddress {
    ShekylAddress::new(network, AddressType::Legacy, self.spend, self.view())
  }

  /// Derive a legacy integrated address from this ViewPair.
  ///
  /// Subaddresses SHOULD be used instead.
  pub fn legacy_integrated_address(&self, network: Network, payment_id: [u8; 8]) -> ShekylAddress {
    ShekylAddress::new(network, AddressType::LegacyIntegrated(payment_id), self.spend, self.view())
  }

  /// Derive a subaddress from this ViewPair.
  pub fn subaddress(&self, network: Network, subaddress: SubaddressIndex) -> ShekylAddress {
    let (spend, view) = self.subaddress_keys(subaddress);
    ShekylAddress::new(network, AddressType::Subaddress, spend, view)
  }
}
