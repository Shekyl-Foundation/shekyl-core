...
async fn initialize_rpcs() -> (SimpleRequestRpc, SimpleRequestRpc, ShekylAddress) {
  ...
  // Fund the new wallet
  let address = ShekylAddress::from_str(Network::Mainnet, &address.address).unwrap();
...
test!(
  ...
  (
    |_, mut builder: Builder, _| async move {
      ...
      builder
        .add_payment(ShekylAddress::from_str(Network::Mainnet, &addr.address).unwrap(), 1000000);
  ...
  (
    |_, mut builder: Builder, _| async move {
      ...
      builder.add_payments(&[
        (ShekylAddress::from_str(Network::Mainnet, &addrs.addresses[0]).unwrap(), 1000000),
        (ShekylAddress::from_str(Network::Mainnet, &addrs.addresses[1]).unwrap(), 2000000),
      ]);
...
test!(
  ...
  (
    |_, mut builder: Builder, _| async move {
      ...
      builder.add_payment(ShekylAddress::from_str(Network::Mainnet, &addr).unwrap(), 1000000);
