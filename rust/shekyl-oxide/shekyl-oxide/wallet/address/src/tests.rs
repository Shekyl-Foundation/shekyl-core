...
use crate::{Network, AddressType, ShekylAddress};
...
#[test]
fn standard_address() {
  let addr = ShekylAddress::from_str(Network::Mainnet, STANDARD).unwrap();
...
fn integrated_address() {
  let addr = ShekylAddress::from_str(Network::Mainnet, INTEGRATED).unwrap();
  assert_eq!(addr.network(), Network::Mainnet);
...
#[test]
fn subaddress() {
  let addr = ShekylAddress::from_str(Network::Mainnet, SUBADDRESS).unwrap();
  assert_eq!(addr.network(), Network::Mainnet);
    ...
    [(Network::Mainnet, 'C'), (Network::Testnet, 'K'), (Network::Stagenet, 'F')]
  {
    for _ in 0 .. 100 {
      ...
      for features in 0 .. (1 << 3) {
        ...
        let addr = ShekylAddress::new(network, kind, spend, view);

        assert_eq!(addr.to_string().chars().next().unwrap(), first);
        assert_eq!(ShekylAddress::from_str(network, &addr.to_string()).unwrap(), addr);
...
fn featured_vectors() {
  ...
  for vector in vectors {
    ...
    let addr = ShekylAddress::from_str(network, &vector.address).unwrap();
    assert_eq!(addr.spend, spend);
    assert_eq!(addr.view, view);
