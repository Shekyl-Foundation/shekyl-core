...
use shekyl_base58::{encode_check, decode_check};
...
const SHEKYL_MAINNET_BYTES: AddressBytes = match AddressBytes::new(18, 19, 42, 70) {
  Some(bytes) => bytes,
  None => panic!("mainnet byte constants conflicted"),
};
// https://github.com/monero-project/monero/blob/cc73fe71162d564ffda8e549b79a350bca53c454
//   /src/cryptonote_config.h#L277-L281
const SHEKYL_STAGENET_BYTES: AddressBytes = match AddressBytes::new(24, 25, 36, 86) {
  Some(bytes) => bytes,
  None => panic!("stagenet byte constants conflicted"),
};
// https://github.com/monero-project/monero/blob/cc73fe71162d564ffda8e549b79a350bca53c454
//   /src/cryptonote_config.h#L262-L266
const SHEKYL_TESTNET_BYTES: AddressBytes = match AddressBytes::new(53, 54, 63, 111) {
  Some(bytes) => bytes,
...
/// The bytes used for distinguishing Shekyl addresses.
pub const SHEKYL_BYTES: NetworkedAddressBytes = match NetworkedAddressBytes::new(
  SHEKYL_MAINNET_BYTES,
  SHEKYL_STAGENET_BYTES,
  SHEKYL_TESTNET_BYTES,
) {
  Some(bytes) => bytes,
  None => panic!("Shekyl network byte constants conflicted"),
};
...
/// Instantiation of the Address type with Shekyl's network bytes.
pub type ShekylAddress = Address<{ SHEKYL_BYTES.to_const_generic() }>;
