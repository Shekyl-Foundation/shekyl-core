...
use crypto_bigint::{Encoding, Limb, Word, U128, U256};
...
impl Field for HelioseleneField {
    ...
    fn invert(&self) -> CtOption<Self> {
        ...
        fn step(a: &mut U256, b: &mut U256, u: &mut U256, v: &mut U256, limbs: usize) {
            ...
            *b = select(b, a, both, limbs);
            ...
            let a_diff_b = {
                ...
                for l in 0..limbs {
                    ...
                    let carry_bool;
                    (limb, carry_bool) =
                        (a_sub_b.as_limbs()[l] ^ a_lt_b).0.overflowing_add(carry.0);
                    (a_diff_b.as_limbs_mut()[l], carry) =
                        (Limb(limb), Limb(Word::from(carry_bool)));
                }
                a_diff_b
            };
            ...
            */
            const MODULUS_XOR_TWO_MODULUS: U256 = U256::from_be_hex(
...
impl PrimeField for HelioseleneField {
    ...
    const TWO_INV: Self = Self(U256::from_be_hex(
        "3ffffffffffffffffffffffffffffffffb9aa40e8cb4f98bfcc285b46f88efaa",
    ));
    ...
    const ROOT_OF_UNITY: Self = Self(U256::from_be_hex(
        "7ffffffffffffffffffffffffffffffff735481d1969f317f9850b68df11df52",
    ));
