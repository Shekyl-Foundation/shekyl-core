...
use generalized_bulletproofs::{
    ...
    transcript::*,
    Generators, PedersenCommitment, PedersenVectorCommitment,
};
...
fn test_zero_arithmetic_circuit() {
    ...
    let proof = {
        statement
            .clone()
            .prove(&mut OsRng, &mut transcript, witness)
            .unwrap();
        transcript.complete()
    };
    ...
    statement
        .verify(&mut OsRng, &mut verifier, &mut transcript)
        .unwrap();
...
fn fuzz_test_arithmetic_circuit() {
    ...
    for i in 0..100 {
        ...
        let commitments = transcript.write_commitments(
            C.iter()
                .map(|C| C.commit(generators.g_bold_slice(), generators.h()).unwrap())
                .collect(),
            V.iter()
                .map(|V| V.commit(generators.g(), generators.h()))
                .collect(),
        );
        ...
        let witness =
            ArithmeticCircuitWitness::<Ristretto>::new(aL, aR, C.clone(), V.clone()).unwrap();
