...
use std_shims::{collections::BTreeMap, vec, vec::Vec};
...
impl<F: PrimeField> Sub<&LinComb<F>> for LinComb<F> {
    ...
    fn sub(mut self, constraint: &Self) -> Self {
        ...
        self.WL
            .extend(constraint.WL.iter().map(|(i, weight)| (*i, -*weight)));
        self.WR
            .extend(constraint.WR.iter().map(|(i, weight)| (*i, -*weight)));
        self.WO
            .extend(constraint.WO.iter().map(|(i, weight)| (*i, -*weight)));
        for (i, sparse_vec) in &constraint.WCG {
        ...
        self.WV
            .extend(constraint.WV.iter().map(|(i, weight)| (*i, -*weight)));
        self.c -= constraint.c;
        self
    }
}
