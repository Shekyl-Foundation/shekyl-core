...
pub use crate::lincomb::{LinComb, Variable};
use crate::{
    inner_product::{IpProveError, IpStatement, IpVerifyError, IpWitness, P},
    lincomb::accumulate_vector,
    transcript::*,
    BatchVerifier, PedersenCommitment, PedersenVectorCommitment, PointVector, ProofGenerators,
    ScalarVector,
};
    ...
    C::F: FromUniformBytes<64>,
{
    fn yz_challenges(&self, y: C::F, z_1: C::F) -> YzChallenges<C> {
        ...
        for _ in 1..q {
            z.0.push(
                *z.0.last()
                    .expect("always non-empty Vec didn't have a last element")
                    * z_1,
            );
        }
        z.0.truncate(q);
    ...
    pub fn prove<R: RngCore + CryptoRng>(
        ...
        #[cfg(debug_assertions)]
        {
            ...
            for (commitment, opening) in self.C.0.iter().zip(witness.c.iter()) {
                if Some(*commitment)
                    != opening.commit(self.generators.g_bold_slice(), self.generators.h())
                {
                    Err(AcProveError::InconsistentWitness)?;
                }
            }
                ...
                let eval = constraint
                    ...
                    .chain(
                        constraint
                            .WV
                            .iter()
                            .map(|(i, weight)| *weight * witness.v[*i].value),
                    )
        ...
        let AI = {
            let alg = witness
                ...
                .iter()
                .enumerate()
                .map(|(i, aL)| (*aL, self.generators.g_bold(i)));
            let arh = witness
                .aR
                .0
                .iter()
                .enumerate()
                .map(|(i, aR)| (*aR, self.generators.h_bold(i)));
        ...
        let AO = {
            let aog = witness
                ...
                .iter()
                .enumerate()
                .map(|(i, aO)| (*aO, self.generators.g_bold(i)));
        ...
        let S = {
            let slg =
                sL.0.iter()
                    .enumerate()
                    .map(|(i, sL)| (*sL, self.generators.g_bold(i)));
            let srh =
                sR.0.iter()
                    .enumerate()
                    .map(|(i, sR)| (*sR, self.generators.h_bold(i)));
    ...
    let js = ni + 1; // 3 if c = 0
        ...
        */
        ...
        for (t, tau) in t.0[0..ni].iter().zip(tau_before_ni.iter()) {
            transcript.push_point(&multiexp(&[
                (*t, self.generators.g()),
                (*tau, self.generators.h()),
            ]));
        }
        ...
        .prove(
            transcript,
            IpWitness::new(l, r).expect("created an invalid IpWitness"),
        )
        .map_err(|e| match e {
    ...
    C::F: FromUniformBytes<64>,
{
    ...
    pub fn verify<R: RngCore + CryptoRng>(
        ...
        let AI = transcript
            .read_point::<C>()
            .map_err(|_| AcVerifyError::IncompleteProof)?;
        let AO = transcript
            .read_point::<C>()
            .map_err(|_| AcVerifyError::IncompleteProof)?;
        let S = transcript
        ...
        let tau_x = transcript
            .read_scalar::<C>()
            .map_err(|_| AcVerifyError::IncompleteProof)?;
        let u = transcript
            .read_scalar::<C>()
            .map_err(|_| AcVerifyError::IncompleteProof)?;
        let t_caret = transcript
            .read_scalar::<C>()
            .map_err(|_| AcVerifyError::IncompleteProof)?;
        ...
        // Lines 88-90, modified per Generalized Bulletproofs as needed w.r.t. `t`
        {
            ...
            for pair in V_weights.0.into_iter().zip(self.V.0) {
                verifier
                    .additional
                    .push((-verifier_weight * pair.0, pair.1));
            }
            ...
            for (i, T) in T_after_ni.into_iter().enumerate() {
                verifier
                    .additional
                    .push((-verifier_weight * x[ni + 1 + i], T));
            }
        }
