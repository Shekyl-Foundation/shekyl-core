...
impl Transcript {
    ...
    pub fn new(context: [u8; 32]) -> Self {
        ...
        Self {
            digest,
            transcript: Vec::with_capacity(1024),
        }
    }
...
impl<'a> VerifierTranscript<'a> {
    ...
    pub fn read_commitments<C: Ciphersuite>(
        ...
        self.digest
            .update(u64::try_from(C).map_err(io::Error::other)?.to_le_bytes());
        let mut C_vec = Vec::with_capacity(C);
        for _ in 0..C {
            C_vec.push(self.read_point::<C>()?);
        }
        self.digest
            .update(u64::try_from(V).map_err(io::Error::other)?.to_le_bytes());
