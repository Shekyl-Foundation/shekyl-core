...
use crate::{transcript::*, BatchVerifier, PointVector, ProofGenerators, ScalarVector};
    ...
    C::F: FromUniformBytes<64>,
{
    ...
    pub(crate) fn prove(
        ...
        let (mut g_bold, mut h_bold, u, mut P, mut a, mut b) = {
            ...
            let h_bold =
                PointVector::<C>(generators.h_bold_slice().to_vec()).mul_vec(&h_bold_weights);
    ...
    pub(crate) fn verify(
        ...
        for _ in 0..lr_len {
            L.push(
                transcript
                    .read_point::<C>()
                    .map_err(|_| IpVerifyError::IncompleteProof)?,
            );
            R.push(
                transcript
                    .read_point::<C>()
