...
impl<C: DivisorCurve> From<C> for Projective<C> {
    fn from(point: C) -> Self {
        let Some((x, y)) = C::to_xy(point) else {
...
impl<C: DivisorCurve> Add for Projective<C> {
    ...
    fn add(self, p2: Self) -> Self {
        ...
        let res =
            <_>::conditional_select(&res, &self.double(), same_x_coordinate & same_y_coordinate);
...
impl<C: DivisorCurve> XyPoint<C::FieldElement> for Projective<C> {
    ...
    fn batch_to_xy(points: &[Self]) -> Vec<(C::FieldElement, C::FieldElement)> {
        ...
        points
            .iter()
            .zip(z)
            .map(|(p, z_inv)| (p.x * z_inv, p.y * z_inv))
            .collect()
    }
}
...
mod ed25519_test {
    ...
    fn projective() {
        ...
        assert_eq!(
            Projective::<EdwardsPoint>::IDENTITY + projective,
            projective,
            "identity + point"
        );
        assert_eq!(
            projective + Projective::<EdwardsPoint>::IDENTITY,
            projective,
            "point + identity"
        );
        assert_eq!(
        ...
        assert_eq!(
            Projective::<EdwardsPoint>::IDENTITY.double(),
            Projective::<EdwardsPoint>::IDENTITY
        );
    }
}
