...
type Denom<C> = (
    CtOption<<C as DivisorCurve>::FieldElement>,
    CtOption<<C as DivisorCurve>::FieldElement>,
);
...
impl<F: Zeroize + PrimeFieldBits> ScalarDecomposition<F> {
    ...
    pub fn scalar_mul_divisor<C: Zeroize + DivisorCurve<Scalar = F>>(
        ...
        let res =
            new_divisor::<C>(&divisor_points, C::interpolator_for_scalar_mul().borrow()).unwrap();
...
mod ed25519 {
    ...
    use crate::{Interpolator, Projective};
    ...
    impl crate::DivisorCurve for EdwardsPoint {
        ...
        fn to_xy(point: Self) -> Option<(Self::FieldElement, Self::FieldElement)> {
            ...
            const MODULUS: DynResidueParams<{ U256::LIMBS }> = DynResidueParams::new(
                &U256::ONE.shl_vartime(255).wrapping_sub(&U256::from_u64(19)),
            );
            ...
            let wei_x = (edwards_y_plus_one
                * one_minus_edwards_y.invert().expect(
                    "couldn't map non-identity Ed25519 point's y coordinate to Wei25519 x",
                ))
                + Y_TO_X_MAP_CONST;
