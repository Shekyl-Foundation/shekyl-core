...
pub use generalized_bulletproofs::arithmetic_circuit_proof::{LinComb, Variable};
use generalized_bulletproofs::{
    arithmetic_circuit_proof::{
        AcStatementError, ArithmeticCircuitStatement, ArithmeticCircuitWitness,
    },
    transcript::{Commitments, Transcript as ProverTranscript, VerifierTranscript},
...
impl<C: Ciphersuite> Circuit<C> {
    ...
    pub fn prove(
        ...
        Self {
            muls: 0,
            constraints: vec![],
            prover: Some(ProverData {
                aL: vec![],
                aR: vec![],
                C: vector_commitments,
                V: commitments,
            }),
        }
    }
