...
use generalized_bulletproofs::{
    arithmetic_circuit_proof::{AcProveError, AcStatementError, AcVerifyError},
...
impl<C> Fcmp<C> {
    ...
    pub const fn ipa_rows(inputs: usize, layers: usize) -> (usize, usize) {
        ...
        let c1_rows = inputs
            * (C1_LEAVES_ROWS_PER_INPUT + (non_leaves_c1_branches * C1_BRANCH_ROWS_PER_INPUT));
        ...
        (
            const_max(c1_rows, C1_TARGET_ROWS),
            const_max(c2_rows, C2_TARGET_ROWS),
        )
    }
}
    ...
    <C::C2 as Ciphersuite>::F: FromUniformBytes<64>,
{
    ...
    fn input(
        ...
        c1_circuit.first_layer(
            ...
            (if layers == 1 {
                ...
                c1_branches.next().unwrap()
            })
            .chunks(leaf_tuple_width)
            .map(|chunk| {
                assert_eq!(chunk.len(), leaf_tuple_width);
                chunk.to_vec()
            })
            .collect(),
        );
        ...
        let (c1_chain, c2_chain) = if root_is_c1 {
            (Some(root), None)
        } else {
            (None, Some(root))
        };
        let these_c1_branches = these_c1_branches
            .map(Some)
            .chain(core::iter::once(c1_chain))
            .flatten();
        let these_c2_branches = these_c2_branches
            .map(Some)
            .chain(core::iter::once(c2_chain))
            .flatten();
            ...
            these_c1_branches.zip(c2_commitments)
        {
            ...
            c1_circuit.additional_layer(
                ...
                <C::C2 as Ciphersuite>::G::to_xy(prior_commitment)
                    .ok_or(FcmpError::IdentityPoint)?,
                prior_blind_opening,
                (hash_x, hash_y),
                branch,
            );
            ...
            these_c2_branches.zip(c1_commitments)
        {
            ...
            c2_circuit.additional_layer(
                ...
                <C::C1 as Ciphersuite>::G::to_xy(prior_commitment)
                    .ok_or(FcmpError::IdentityPoint)?,
                prior_blind_opening,
                (hash_x, hash_y),
                branch,
        ...
        <C::C2 as Ciphersuite>::G: GroupEncoding<Repr = [u8; 32]>,
    {
        let tree: TreeRoot<C::C1, C::C2> = match &branches.root {
            ...
            RootBranch::C2(branches) => {
                ...
                for (scalar, point) in branches
                    .iter()
                    .zip(params.curve_2_generators.g_bold_slice())
                {
                    items.push((*scalar, *point));
                }
        ...
        if matches!(tree, TreeRoot::C1(_)) {
            let s = *root_blind_r_C1.unwrap()
                + (transcript.challenge::<C::C1>() * root_blind_C1.unwrap());
            ...
            let s = *root_blind_r_C2.unwrap()
                + (transcript.challenge::<C::C2>() * root_blind_C2.unwrap());
            root_blind_pok[..32].copy_from_slice(&root_blind_R);
            root_blind_pok[32..].copy_from_slice(s.to_repr().as_ref());
        }
        ...
        let mut c1_circuit = Circuit::<C::C1>::prove(
            c1_tape
                ...
                .map(|(g_values, mask)| PedersenVectorCommitment {
                    g_values,
                    mask: *mask,
                })
                .collect(),
        ...
        let TranscriptedBlinds {
            c1: transcripted_blinds_c1,
            c2: transcripted_blinds_c2,
        } = transcripted_blinds;
        ...
        let (c1_statement, c1_witness) = c1_circuit.statement(
            params
                .curve_1_generators
                .reduce(c1_padded_pow_2)
                .ok_or(FcmpError::NotEnoughGenerators)?,
            commitments_1,
        )?;
    ...
    pub fn verify<R: RngCore + CryptoRng>(
        ...
        for _ in inputs {
            for i in 0..(layers - 1) {
                if (i % 2) == 0 {
                    c1_branches.push(c1_tape.append_branch(
                        if i == 0 {
                            leaf_layer_len
                        } else {
                            LAYER_ONE_LEN
        ...
        for _ in 0..(if c1_branches.is_empty() {
            ...
            commitment_blind_claims_1.push(
                c1_tape
                    .append_claimed_point::<C::C2Parameters>(None, None, None, None)
                    .0,
            );
        }
            ...
            + (inputs.len() * usize::from(u8::from(matches!(tree, TreeRoot::C2(_))))))
        {
            commitment_blind_claims_2.push(
                c2_tape
                    .append_claimed_point::<C::C1Parameters>(None, None, None, None)
                    .0,
            );
        }
        ...
        // Verify the root blind PoK
        {
            let claimed_root = if (layers % 2) == 1 {
                TreeRoot::<C::C1, C::C2>::C1(match root[0] {
                    Variable::CG {
                        commitment: i,
                        index: _,
                    } => params.curve_1_hash_init + proof_1_vcs.C()[i],
                ...
                TreeRoot::<C::C1, C::C2>::C2(match root[0] {
                    Variable::CG {
                        commitment: i,
                        index: _,
                    } => params.curve_2_hash_init + proof_2_vcs.C()[i],
                    _ => panic!("branch wasn't present in a vector commitment"),
                })
            };
            match (claimed_root, tree) {
                (TreeRoot::C1(claimed), TreeRoot::C1(actual)) => {
                    ...
                    verifier_1
                        .additional
                        .push((batch_verifier_weight * c, claimed - actual));
        ...
        let mut c1_commitments = proof_1_vcs
            ...
            .iter()
            .copied()
            .zip(core::iter::repeat(None))
            .zip(commitment_blind_claims_2);
        let mut c2_commitments = proof_2_vcs
            .C()
            .iter()
            .copied()
            .zip(core::iter::repeat(None))
            .zip(commitment_blind_claims_1);
