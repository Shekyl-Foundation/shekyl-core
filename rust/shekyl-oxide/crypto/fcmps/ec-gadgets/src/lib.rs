...
impl<C: Ciphersuite> EcGadgets<C> for Circuit<C> {
    fn on_curve(&mut self, curve: &CurveSpec<C::F>, (x, y): (Variable, Variable)) -> OnCurve {
        ...
        let (_x, _x_2, x2) = self.mul(
            Some(LinComb::from(x)),
            Some(LinComb::from(x)),
            x_eval.map(|x| (x, x)),
        );
        let (_x, _x_2, x3) = self.mul(
            Some(LinComb::from(x2)),
            Some(LinComb::from(x)),
            x_eval.map(|x| (x * x, x)),
        );
    ...
    fn incomplete_add_fixed(&mut self, a: (C::F, C::F), b: OnCurve, c: OnCurve) -> OnCurve {
        // Check b.x != a.0
        {
            ...
            self.inequality(
                bx_lincomb,
                &LinComb::empty().constant(a.0),
                bx_eval.map(|bx| (bx, a.0)),
            );
        }
        ...
        let (slope, _r, o) = self.mul(
            None,
            Some(x1_minus_x0),
            slope_eval.map(|slope| (slope, x1_minus_x0_eval.unwrap())),
        );
        ...
        self.equality(
            o.into(),
            &LinComb::empty().term(-C::F::ONE, y2).constant(-y0),
        );

        // slope * slope = x0 + x1 + x2
        let (_slope, _slope_2, o) = self.mul(
            Some(slope.into()),
            Some(slope.into()),
            slope_eval.map(|slope| (slope, slope)),
        );
        self.equality(
            o.into(),
            &LinComb::from(x1).term(C::F::ONE, x2).constant(x0),
        );
