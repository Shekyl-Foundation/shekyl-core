...
use generic_array::typenum::{Diff, Quot, Sum, U1, U2};

use ciphersuite::{
    group::ff::{BatchInverter, Field as _, FromUniformBytes, PrimeField},
    Ciphersuite,
};
    ...
    for GeneratorTable<F, Parameters>
{
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt.debug_struct("GeneratorTable")
            .field("x", &self.0[0].0)
            .field("y", &self.0[0].1)
...
fn divisor_challenge_eval<C: Ciphersuite, Parameters: DiscreteLogParameters>(
    ...
    let witness = circuit
        .eval(&p_d)
        .map(|p_d| (p_d, circuit.eval(&p_n).unwrap() * p_d.invert().unwrap()));
...
pub trait EcDlogGadgets<C: Ciphersuite> {
    ...
    fn discrete_log_challenge<T: Transcript, Parameters: DiscreteLogParameters>(
        ...
        Vec<ChallengedGenerator<C::F, Parameters>>,
    );
    ...
    fn discrete_log_challenge<T: Transcript, Parameters: DiscreteLogParameters>(
            ...
            C::F: FromUniformBytes<64>,
        {
            loop {
                ...
                return (
                    c_x,
                    if bool::from(c_y.is_odd()) != odd_y_coordinate {
                        -c_y
                    } else {
                        c_y
                    },
        ...
        let mut inversions =
            vec![C::F::ZERO; 3 + (generators.len() * Parameters::ScalarBits::USIZE)];
        ...
        for challenge_inversion in &inversions {
            ...
            assert!(
                !bool::from(challenge_inversion.is_zero()),
                "trying to invert 0"
            );
        }
