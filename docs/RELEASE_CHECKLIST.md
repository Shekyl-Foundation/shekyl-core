- [ ] **Stressnet stable for 4 consecutive weeks** (Phase 7.7)
  ...
  - [ ] **`DEFAULT_EXIT_GAP_WINDOW` sealed from the stressnet's own rate read** (F-W3 sentinel, same pattern as `K_COVER`, sealed by a different event: the Phase 7.7 stressnet is where `ρ_x`, `N_x`, `σ_L` get read — `docs/design/ARCHIVAL_EXIT_STANDOFF_FD4_WINDOW.md` §5.4). The read runs early in the stressnet window; the value is fixed by the frozen §5.4 decision rule (smallest `SETTLEMENT_EPOCH_BLOCKS` multiple ≥ `max(` X-1 bound at the 10th percentile of the joint `(N_P, c)` read with exit-derived `N_t`, `2 × SEB` X-3 anchor-merge floor `)`); the provisional flag is cleared and the **sealed draw runs live for the remainder of the 4-week window** — sealing later would leave mainnet genesis as the exit draw's first live execution. Post-seal the value is soft-frozen (changing a shipped window splits the wallet population into two draw distributions — §16.1 partition trap).
