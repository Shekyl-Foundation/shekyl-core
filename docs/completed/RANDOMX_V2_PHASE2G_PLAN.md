...
| Field | Value |
|-------|-------|
| Status | **Landed as PR #75 (merge `33d22a83b`, 2026-05-25).** Design Rounds 1–4 complete; differential-test harness shipped. |
| Parent plan | [`docs/design/RANDOMX_V2_PLAN.md`](../design/RANDOMX_V2_PLAN.md) — Track A Phase 2g todo (`phase2g-differential-harness`, line 30) |
...
constant (per [`RANDOMX_V2_PLAN.md`](../design/RANDOMX_V2_PLAN.md) §6 line
...
— [`RANDOMX_V2_PLAN.md`](../design/RANDOMX_V2_PLAN.md) §6 lines 236–243.
...
— [`RANDOMX_V2_PLAN.md`](../design/RANDOMX_V2_PLAN.md) §7 line 248.
...
- **(b)** `tests/` folder of an existing crate (rejected by
  construction — [`RANDOMX_V2_PLAN.md`](../design/RANDOMX_V2_PLAN.md) §7
...
Per [`RANDOMX_V2_PLAN.md`](../design/RANDOMX_V2_PLAN.md) §6:
...
- **Release-gate suite** also absorbs the 600k-block initial-sync
  ...
  synthetic chain harness per [`RANDOMX_V2_RUST.md`](../design/RANDOMX_V2_RUST.md)
  §8 lines 449–462.
- **Documentation closure** per [`91-documentation-after-plans.mdc`](../../.cursor/rules/91-documentation-after-plans.mdc):
  ...
  [`RANDOMX_V2_PLAN.md`](../design/RANDOMX_V2_PLAN.md) `phase2g-differential-harness`
...
| Round 8 (Post-merge substrate-triage amendment — R8-D1 verifier-divergence root-cause closure + R8-D2 cache-vs-VM-flag asymmetry pin + R8-D3 missed-altitude finding queued for rule-26) | 2026-05-26 | Post-merge substrate-triage round opened against the V3.0 FOLLOWUP "Investigate `shekyl-pow-randomx::compute_hash` divergence from C reference at large data sizes" that landed at the §11 Round 3 + Phase 2g implementation-PR (#75) closing summary as "deferred to V3.0 with detection-by-harness as M4-validation evidence." The FOLLOWUP's investigation entry hypothesized the divergence as Rust-side ("most likely failure modes are in the per-chunk Blake2b seed expansion or the AES-round chain across multi-chunk inputs"); two diagnostic substrate findings inverted the hypothesis: **(i)** an ignore-gated triage test ([`tests/divergence_triage.rs`](../../rust/shekyl-randomx-differential/tests/divergence_triage.rs)) ran T16's seedhash + data triple through Rust, through the C oracle, and against the T16 static fixture, producing Outcome B (Rust ≡ fixture; C oracle diverges) — i.e., Rust matched the spec-vector fixture byte-for-byte; only the C oracle diverged. **(ii)** T16's fixture-metadata then named the substrate (the fixture was generated with `RANDOMX_FLAG_V2`); the C reference's `external/randomx-v2/src/randomx.cpp:79` `(JIT | LARGE_PAGES)` mask at `randomx_alloc_cache` then named the asymmetry: cache memory is V2-flag-invariant (the mask drops the V2 bit at alloc), so passing `RANDOMX_FLAG_V2` at the cache surface is inert; only `randomx_create_vm` honors the V2 bit. The harness's [`c_oracle.rs::COracleSession`](../../rust/shekyl-randomx-differential/src/c_oracle.rs) and [`gen_canonical_outputs.rs`](../../rust/shekyl-randomx-differential/src/bin/gen_canonical_outputs.rs) both called `randomx_create_vm(RANDOMX_FLAG_DEFAULT, ...)`, which selects v1 (PROGRAM_SIZE = 256), against a Rust verifier implementing v2 (PROGRAM_SIZE = 384). The divergence was systematic by construction at every data size that triggered an opcode-stream long enough for v1/v2's PROGRAM_SIZE delta to register in the AES-round chain; T16's static 192-byte fixture happened to land below that threshold and pass, masking the substrate gap until C7's nightly-cadence corpus pushed past it. **R8-D1**: V3.0 FOLLOWUP closed by replacement. `randomx-v2-sys/src/lib.rs` exposes `pub const RANDOMX_FLAG_V2: RandomxFlags = 128` with cache-vs-VM-flag-split docs that name both callsites (cache alloc + VM create) and the upstream evidence (`randomx.cpp:79` for the mask + `randomx_vm.cpp` for the v2 algorithm selection); `c_oracle.rs` and `gen_canonical_outputs.rs` switch the VM-create callsite to pass `RANDOMX_FLAG_V2`; `canonical_outputs.rs` regenerates under the v2 flag (generator-version pin bumped `v1-c5a-nightly-1024` → `v2-flag-nightly-1024`; `CANONICAL_RANDOM_HASHES` repopulated; `CANONICAL_CACHE_SHAS` unchanged, as predicted by the mask); end-to-end `--mode=correctness` re-runs the nightly corpus (32 seedhashes × 32 data = 1024 random pairs) with three-way agreement (Rust ≡ C ≡ canonical; exit 0). The Rust verifier was correct throughout the investigation window; **no Rust implementation work was required**. The audit trail of the substrate-triage moves to `docs/FOLLOWUPS.md` "Recently resolved (audit trail)" section per the file's resolution convention. **R8-D2 (cache-vs-VM-flag-asymmetry pin)**: the upstream `(JIT | LARGE_PAGES)` mask at `randomx_alloc_cache` is the structural reason the cache-vs-VM flag split exists. Future additions to `randomx-v2-sys::RandomxFlags` carry a load-bearing-flag-callsite-disposition obligation in their docs: each `pub const` flag names which of `randomx_alloc_cache` / `randomx_create_vm` / `randomx_calculate_hash` honors it, cites the upstream evidence for the asymmetry, and (where the flag is silently inert at a callsite) names that the wrong-callsite behavior is silent. Three load-bearing artifacts at three altitudes carry the discipline: (a) `RANDOMX_FLAG_V2`'s pub-const-doc enumerates the cache-vs-VM split with `randomx.cpp:79` cite; (b) `c_oracle.rs`'s module-level safety prose explains the cache-vs-VM flag selection in the same terms; (c) `gen_canonical_outputs.rs`'s inline safety comment at the `randomx_create_vm` callsite cross-references the FOLLOWUP closure so a future regeneration cannot re-introduce the v1 flag without confronting the prior resolution. The reopening criterion for relaxing any of these three docstrings is substrate-anchored per [`21-reversion-clause-discipline.mdc`](../../.cursor/rules/21-reversion-clause-discipline.mdc) (e.g., upstream restructuring that collapses the cache-vs-VM flag distinction at `randomx_alloc_cache`); preference-anchored relaxation is rejected. **R8-D3 (missed-altitude finding queued for rule-26 amendment).** The R4-D5 close documented the harness's "light-mode VM lifecycle shape" at one altitude (cache + VM lifecycle; light vs. full mode) but not at the other (v1 vs. v2 algorithm selection). The R4 implementation-correctness checklist enumerated the C-side FFI surface (R4-D4 seven `extern "C"` signatures), the C-side build mode (R4-D9 Release), and the cache-vs-VM lifecycle (R4-D5), but did not enumerate the **algorithm-selection-flag callsite disposition** — the substrate question "which of the C-side flag arguments are load-bearing for selecting the correct algorithm version, and against which substrate did the chosen flag value land?" The forward-action queued for [`26-sub-pr-design-discipline.mdc`](../../.cursor/rules/26-sub-pr-design-discipline.mdc)'s next amendment: pre-implementation rounds add a **"FFI algorithm-selection-flag callsite disposition pass"** — for each FFI flag argument the implementation passes to an upstream library, enumerate (a) which upstream callsites honor the flag, (b) what the silent-default behavior is at each callsite where the flag is silently inert, (c) the source citation for the asymmetry, and (d) the test-coverage shape that catches a wrong-callsite-flag regression (here: the static-fixture three-way comparison, which is what the diagnostic-triage test became). The pass joins R5-D1 (surface enumeration), R5-D2 (cross-invariant impact analysis), R6-D2 (methodology-vs-surface-contract reconciliation), R6-D3 (C-FFI-link language enumeration), R6-D4 (committed-canonical sizing), and R7-D5 (substrate-derived-constant-validation) in the rule-26 queue. The accumulated queue now spans seven discipline classes; the rule-26 amendment after 2g closes records all seven. **Detection-validation framing.** Per Phase 2g §4.6 M4 (defense-in-depth detection capability), the harness's catch of the substrate gap — and the diagnostic-triage test's ability to bisect it to the cache-vs-VM-flag asymmetry in a single Outcome B / A pivot — is end-to-end validation of M4 against a real substrate gap. The FOLLOWUP's open-window between PR #75 merge (2026-05-25) and this closure (2026-05-26) is the discovery-cadence working: detection is the property the harness ships with; resolution is a separate substrate-anchored disposition that follows. **Round count.** R8 is the second post-implementation-PR substrate-completeness round in 2g's history (R5 + R6 + R7 were pre-implementation; R8 is post-implementation, opened against a substrate finding the harness itself produced). The §0 "≤3 substantive close-rounds" calibration is preserved (architecture at Round 2; threats at Round 3; implementation correctness across Round 4 + the R5/R6/R7 substrate-completeness cluster; R8 is a post-merge substrate-triage closure, not a new design round). **None of R8-D1 / R8-D2 / R8-D3 reopens a Round-1 / 2 / 3 / 4 / 5 / 6 / 7 disposition;** R4-D5's light-mode-shape decision is preserved (cache + VM lifecycle unchanged); R4-D4's seven-symbol FFI surface is preserved (the new `RANDOMX_FLAG_V2` constant is `pub const`, not `extern "C"`); §5.3.1's production-verifier-surface prohibition is preserved (the resolution required no verifier-side change). The amendment is substrate-completeness work — the **fifth instance of the "substrate-completeness amendment" round-shape** in 2g's history (first: Round 3 active-threat-surface; second: R5-D1 carve-out; third: R5-D2 refinement; fourth: R6 substrate-corrections cluster; fifth: R8 post-merge substrate-triage). The shape's auditability is now established across five diverse instances spanning extension, carve-out, refinement, substrate-correction, and post-merge substrate-triage; the cadence demonstrates the discipline's continued operation per [`16-architectural-inheritance.mdc`](../../.cursor/rules/16-architectural-inheritance.mdc)'s "continuous discipline as inheritance prevention" framing. |
