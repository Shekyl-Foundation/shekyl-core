...
> ## ⛔ SUPERSEDED — HISTORICAL RECORD ONLY (not genesis SoT)
>
> **This document is the retired confidential-claim / Decision 3C / entitlement
> staking design.** Genesis uses **transfer-shaped admission** (principal ↔ `P`)
> and membership-only reward emission — **not** this doc's §6.4 claim wire,
> `txin_stake_claim_v2`, or 3C staking subtree.
>
> - **Do not implement** anything whose only specification is this file.
> - **Do not treat grep hits on this basename as authoritative.** Prefer the
>   living docs: [`PHASE_2B_STAKE_LIFECYCLE.md`](../design/PHASE_2B_STAKE_LIFECYCLE.md),
...
> - A redirect stub remains at
>   [`docs/design/CONFIDENTIAL_STAKING.md`](../design/CONFIDENTIAL_STAKING.md)
>   so `docs/design/` greps hit the tombstone first.
>
> Archived under the verified-complete archive policy
> ([`IMPLEMENTATION_INDEX.md`](../design/IMPLEMENTATION_INDEX.md) §8) because
> leaving the full body in `docs/design/` caused agents to read it as current
> staking design.

**Status at archive (2026-07-26):** Genesis reward path superseded (2026-06).
Historical — confidential-claim design retained for audit trail only. Round 1
closed (2026-06-02) on economics and **(B)**. **(C)** closed on 3C (2026-06-04,
§6.4.3). **(A)** collapsed to reserve-DLEQ + bounded remainder (§6.4.1). §6.4.4
rejected re-stake-in-claim-tx; §6.4.4 reopening is valid for transfer-shaped
admission per rule 21 (reasons 1–3 were confidential-substrate only).

This doc historically superseded cleartext
[`STAKER_REWARD_DISBURSEMENT.md`](../STAKER_REWARD_DISBURSEMENT.md) for the
confidential-claim era. Principal/unstake membership pins may still inform
optional admission transfer discipline where not superseded by Phase-2b §2.4.

**Process discipline (historical):** [`26-sub-pr-design-discipline.mdc`](../../.cursor/rules/26-sub-pr-design-discipline.mdc).
**Binding constraint:** [`00-mission.mdc`](../../.cursor/rules/00-mission.mdc)
priority-1 then priority-2.

**Origin:** design session 2026-06-02. Code-site citations are to
`blockchain.cpp` and the `shekyl-staking` / `shekyl-economics` FFI verified that
session; FCMP++ / SAL / witness layout were **source-reviewed** for §6.4
(2026-06-02; `main` @ `34b2b3c`); Round 1 closed 2026-06-02 (§14).
...
(§6.4.2; wallet §8.5 in [`PHASE_2B_STAKE_LIFECYCLE.md`](../design/PHASE_2B_STAKE_LIFECYCLE.md)).
...
transiently ([`PHASE_2B_STAKE_LIFECYCLE.md`](../design/PHASE_2B_STAKE_LIFECYCLE.md) §3.3.1).
...
`nullifier_seed` / HKDF-stream requirement ([`PHASE_2B_STAKE_LIFECYCLE.md`](../design/PHASE_2B_STAKE_LIFECYCLE.md)
§8.5).
...
block-loading impact ([`FA-6_VIEW_TAG_ML_KEM.md`](../design/FA-6_VIEW_TAG_ML_KEM.md)).
...
close in Round 2 (§6.4.3). Wallet mirror: [`PHASE_2B_STAKE_LIFECYCLE.md`](../design/PHASE_2B_STAKE_LIFECYCLE.md)
§9 Round 1.
...
| Doc / site | Use |
|-----------|-----|
| [`PHASE_2B_STAKE_LIFECYCLE.md`](../design/PHASE_2B_STAKE_LIFECYCLE.md) | Wallet mirror (§2.3); R0-D1–D7; `N_S = x·G_S` §8.5; §6.4 review pins |
