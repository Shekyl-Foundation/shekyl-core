...
| Implementation | 2026-05-22 | Implementation landed on `feat/randomx-v2-phase2d` in five commits versus the §8 ceiling of seven. **SHA → §8 mapping:** (1) `bd7cea464` consolidates §8 commits 1+2+3+4 (`F128` newtype, integer helpers, `fpu_rounding` module, CFROUND wiring, integer dispatch arms, FP dispatch arms, CBRANCH/PC substrate) — §8's "prefer single commit 4 landing all FP arms atomically to avoid half-real hash semantics" guidance already pulled FP into one commit, and the integer-before-FP bisection nicety was orthogonal: pre-`bd7cea464` `dev` retains Phase 2c stub-NOP dispatch, so bisection across the consolidated commit reduces to "real dispatch vs. stub dispatch," which is the only consensus-relevant bisection question. (2) `26fc49d6c` covers §8 commit 5b (T8 NOP-hash → real-hash expectation flip realized via T16's sibling real-dispatch fixture) plus the CI-grep half of §8 commit 6 (`scripts/ci/check_randomx_fpu_rounding.sh` + `build.yml` wire-in) plus the post-substrate-finding FPU-state reset at `compute_hash` entry/exit and around determinism tests — bundled because the CI grep guards the fpu_rounding surface that 5b's real dispatch first stresses end-to-end. (3) `44674b296` covers the `BENCH_RESULTS.md`-update half of §8 commit 6 (303.60 ms median, +2.6% vs. Phase 2c stub-NOP baseline; under §9's informational ±10% threshold) plus the substantive CHANGELOG entry, kept separate from `26fc49d6c` so the bench delta is independently bisectable from the dispatch-real-hash flip. (4) `043076f18` covers §8 commit 5a — `phase2d/` C generator (`tests/vectors/reference/_generator/phase2d/{gen.cpp,Makefile,README.md}`) emits T9–T15 binaries with `.meta.txt` provenance under `tests/vectors/reference/vm/`; Rust spec-vector tests `{t9,t10,t11,t12,t13,t14,t15}_vm_..._matches_fork_reference` in `src/vm.rs#mod tests` drive `dispatch_instruction` against the canonical Phase 2d fixture (matching `phase2d/gen.cpp`'s `CANONICAL_*` tables) and assert byte-equality against the pinned fork's `executeInstruction` output. (5) `4fc0606d1` post-gate fmt-only cleanup: six mechanical `cargo fmt --check` divergences across `dispatch_instruction`'s IAddRs/IMulRcp/IRorR/IRolR arms (from `bd7cea464`) plus `CANONICAL_E_MASK_PD` (from `043076f18`) surfaced together when the §9 Format gate ran post-T9-T15 land; addressing them in a single fmt-only commit keeps each prior commit's semantic scope intact per `15-deletion-and-debt.mdc`'s "fix mechanical formatting errors in a file already being modified" carve-out. **§9 gate confirmation (HEAD = `4fc0606d1`):** Format `cargo fmt --check` ✓; Lint `cargo clippy --all-targets -D warnings` ✓; Test `cargo test -p shekyl-pow-randomx --release -- --test-threads=1` ✓ (89 passed, 2 ignored — T6/T7 stub-NOP vectors superseded by T16, 1 ignored — T17 per-hash latency placeholder per 2c F8); Doc `cargo doc -p shekyl-pow-randomx --no-deps` ✓; FPU unsafe grep `scripts/ci/check_randomx_fpu_rounding.sh` ✓; Debug ≡ release inherited Gate 2 unchanged. Phase 2d byte-equality surface against fork pin `aaafe71` is now exhausted (per-opcode T9–T15 + end-to-end T16). §8 commit 5b's T8 NOP→real-hash expectation flip is realized by T16's superseding real-hash vector with Phase 2c T8 stub-NOP and T6/T7 tagged `#[ignore]` with explanatory comment, rather than a literal re-emission of the T8 fixture; the consensus-affecting flip is independently bisectable to `26fc49d6c`. |
