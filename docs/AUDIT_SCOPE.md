...
>
> **Pointer (2026-08-26):** spend-path hybrid remains Ed25519 + ML-DSA-65 (this
> doc). Address v2 adds a 48-byte SLH-DSA-192s field for message signing —
> [`design/WALLET_MESSAGE_SIGNING.md`](design/WALLET_MESSAGE_SIGNING.md) SM-R-8.
> Lock-tier confidential *claim* verification is **out of scope** (retired;
> see the replacement section below).
...
3. **FCMP++ proof binding to Y_prover** — **OUT OF SCOPE (Option-D deleted, MS-5 PR-B):**
   Option E′ has no mandatory prover; `rotating_prover_index`, `group_id`, and the
   `ProverOutput`/equivocation machinery are deleted (`PQC_MULTISIG.md` §11 banner).
   Retained here only so the audit index stays contiguous.
...
4. **Rotating prover assignment grinding resistance** — **OUT OF SCOPE (deleted, see item 3)**
...
## Confidential stake claim verifier — RETIRED

Lock-tier confidential staking (`txin_stake_claim_v2`, 5-scalar staking
subtree, `h_bind`, entitlement remainder) is **deleted**, not deferred.
Do not audit or implement that surface.
...
The former in-scope list (5-scalar `h_bind`, window arithmetic, `ρ` remainder,
ClaimLinkability, `txin_stake_claim_v2` transcript) is historical only. Git
history of this file before 2026-08-26 has the Round-1 pin text.
