...
| Exit timing seam (release) | S4 ships the **entry** seam only. | §10 (line 457–462) |
...
- **Independently timed.** The recovery transfer must not be tied to the bond
  lifecycle (not at bond-post, not at release); it must be a separate,
...
- **Not the default.** The default path (cover-stays-with-P) never produces a
  ...
  operation; it does not share code paths with the bond request or release.
...
| Exit timing seam (release) | §10 (line 457–462); release slice | ✓ named |
