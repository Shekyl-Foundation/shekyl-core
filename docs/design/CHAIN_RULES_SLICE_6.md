...
| 7 | **LANDED 2026-09-25, three commits (cost 3; exempt from the §5.1 falsifier as a cross-crate cutover).** I17 as **(c)**. **(i) The gate as a KAT that outlives the oracle** (`pqc_signing_preimage_v1.json`, 772 KB): the review of 2026-09-25 named the flaw in a live comparison — the moment the C++ assembly becomes a call, the test compares Rust against itself and passes forever for no reason — so the payloads were **captured** from the C++ oracle, in the only commit where it still existed, over **eight daemon-accepted transactions** chosen for the shapes a divergence could hide in: one, two and six inputs (six twice — a captured chain's and the live oracle's), the bond post's `to_key ‖ bond_post`, the emission's `to_key ‖ to_key ‖ emission` (mixed archival arms, whose pseudo-out count is the *spend* subset), and the serve-credit form, which has **no** preimage — 21 payloads, byte-for-byte, and their hashes. **Why the serve-credit entry is empty (checked at source 2026-09-25, on the review's ask):** not because the form is unsigned. Consensus forbids it `pqc_auths` (`blockchain.cpp:3580–3588`, *"must not carry pqc_auths"*; Rust twin CEN-H20) and routes it around `verify_transaction_pqc_auth` (`:4277–4278`); its authentication is a hybrid Ed25519+ML-DSA-65 countersignature over the **pass record** — `response.signature_preimage(R_k, leaf_index, leaf, path)` under `SCHEME_DOMAIN_SERVE_CREDIT`, by the bond's registered key, the Ed25519 leg on the vin and the ML-DSA leg in the pruned record (`hybrid_countersignature`, `archival-retention/src/wire.rs:127`; verified in `shekyl_archival_verify_serve_credit_vin`, `archival_ffi/serve_credit.rs:245–270`; CEN-J10). The spend preimage is the wrong object for it, not a missing one. The KAT's module doc, the fixture's `description`, `PqcSigningPreimage::of`'s doc and `Ct::Fcmp`'s field doc all said *"no per-input authentication"* / *"the countersignature rides the vin"* — the first false, the second half-true (one leg does) — and now say this. **The phrase, swept (2026-09-25, on the review's ask that commit 10 sweep the pattern, not two cells):** the origin is gate-2 §5's *"hybrid signature lives on the vin"*, true until **RF-D2** split the legs across the kept/pruned boundary (`archival-retention/src/wire.rs:123–125` records the split) — a comment that outlived its architecture (rule 16), quoted forward into the C++ (`blockchain.cpp:3585`'s log string, `tx_verification_utils.cpp:101`), the census (I18 `:474` *"signature lives on the vin"*, J10 `:493` *"the vin-borne hybrid signature"*), `ARCHIVAL_PASS_RECORD_CARRIER.md:104,113` (quoting both), `CHAIN_RULES_CRATE.md:1456`, and the store's `amendments_tests.rs:324`. Three hits were this PR's own and are fixed in the same commit as the sweep: the fixture's per-subject `source` string (the emitter's copy had been corrected, the fixture's had not — so a re-emit would not have been a no-op; the KAT now asserts `description` and every `source` equal the emitter's, so that class of drift is red), `tx_signing_ffi.rs`'s module doc, and `fcmp_spend_roundtrip.rs:246`. **Commit 10 sweeps the rest by the phrase:** the two census cells and `CHAIN_RULES_CRATE.md:1456` are its; the C++ strings stay (rule 20 — a log string is not logic, and the sentence is true of the leg it names) with the census cells corrected above them; `ARCHIVAL_PASS_RECORD_CARRIER.md` and the store test are the archival and store lanes' surfaces, named here so the sweep hands them over rather than editing across lanes. Every subject is real: the captured chains under `shekyl-chain-ingest/tests/vectors` (six-input and two-input spends existed there already; nothing was built to fit), the live-oracle spend, the serve-credit parity capture. The fixture is documented against `FCMP_SPEND_SIGNING_PREIMAGE.md` §1.1 as **the specification's output** for those bytes, not "what the C++ produced" — the same bytes today, the claim that survives the C++'s deletion. The C++ leg asserted the assembly against it and was deleted with the assembly; the Rust leg holds the derivation, payload bytes per input and hashes. **(ii) The derivation written to our standards** (`transaction/signing_preimage.rs`): the wire's body was a transcription of the C++ — prefix blob, base blob, prunable blob hashed, a header spliced into a loop over every key, comments explaining what the C++ did. It is now `PqcSigningPreimage`: the three bindings every input shares (the pruned segment, the txid's `PrunableHash` component, the fold of key hashes), derived once from the transaction's typed parts, and `payload(auth)` / `signed_hash(auth)` composing each input's — documented as what a signature must bind. Byte identity is the constraint on the output; the KAT held across the rewrite, fixture unchanged. **(iii) The cutover:** `shekyl_tx_pqc_signing_payload_hashes` (coarse, rule 40: the whole transaction in, every input's hash out; codes OK / NULL_PTR / MALFORMED / CAPACITY) in `shekyl-ffi/src/tx_signing_ffi.rs`; `tx_pqc_verify.cpp` derives every hash once through it and verifies each signature against its slot — `get_transaction_signed_payload` deleted (−100 lines), the header's assembly description with it; `fcmp.cpp`'s MSW-6 test signs over the FFI's hashes and verifies through the rewired path with real solo and multisig signatures (54 C++ tests green). The rules crate adopts the derivation as **I17**, a definition recorded where `tx_against` derives it beside I12's anchor, staged for I18 (commit 8). Coverage 71 → 72. The FFI's name is `shekyl_tx_pqc_signing_payload_hashes` (the row above said `_payloads`; it returns hashes). **(iv) The hash is typed (found by CI, 2026-09-25):** the RTN-7 gate refused `signed_hash() -> [u8; 32]` on the new wire surface, and the old allowlist entry's own reopening clause — *"reopen if a second consumer appears"* — had fired in (iii): the daemon and the validator now consume what only the builder did. Minted `shekyl_types::SigningPayloadHash` (its doc separates it from `PqcAuthHash`, the txid component over the auths that sign it), flowed through `phase1_payload_hashes` / `sign_pqc_auths` / K13's slot verifier / I17 to the primitives' `as_bytes()`; the allowlist entry deleted rather than re-keyed (`RTN_7_WIRE_HASH_TYPES.md` §4 UPDATE). **Owed to commit 10:** the census I17 site cell (`tx_pqc_verify.cpp:62–158 — assembled in C++`) now names deleted lines; `FCMP_SPEND_SIGNING_PREIMAGE.md` §1's C++ line anchors likewise — both re-pinned in the 4.I sweep | commit 1 — every shape came from the vectors; `shekyl-daemon-rpc`'s submit verifier (K13) already read the wire's hashes, so the daemon's two verification paths now share one derivation |
| 8 | **I18 LANDED 2026-09-25 (three commits, cost 3 against 1–2 — over, and the falsifier fires for the reason below); I15 and H19-verify DEFERRED, one FOLLOWUPS row, blocked on the driver producing a spend.** **(i)** `shekyl_crypto_pq::signature::verify_pqc_auth` — the scheme dispatch existed twice (the FFI's `shekyl_pqc_verify`, daemon-rpc's K13) and I18 would have been the third; now one body all three call, the FFI's discriminants unchanged. **(ii)** the fixture commit, and **the ruling deviation this row must own:** §5.3 ruled *"I15/I18/H19-verify wait on that migration either way"* — the ~fifty synthetic-spend tests moving to scenarios. I18 did not wait. `fixture::signed`, at `anchored_at`, signs every slot of every anchored fixture with a deterministically derived hybrid key over the body's real §1.1 hash, through the production signer, memoized because ML-DSA is hedged and a fixture must be a pure function of what built it. The argument for it: nothing about I18's *subject* is constructed — the message is the derivation of record and the signature is a real one; only the key is a fixture's, exactly as the wallet's arrangement with a fixture secret in place of the output's. That is not one of §5.3's four refused paths (loader, replay, knob, wait) — but it is the constructed-fixture premise §5.3 rejected, and the scenario driver will exercise I18 on wallet-built spends when it can build them. The reviewer accepts or reverses; the commit is isolated. **RULED 2026-09-25 — kept, on the discriminator that sent I10 and I12 to the driver: whether the chain is the subject.** I18's subject is the transaction — a signature over its own bytes; no chain state enters — so the constructed-input test comes out the other way from I10's. And it is not a fixture constructing the state the rule reads back: `sign_pqc_auth_for_output` is the production signer, I18 the production verifier, independent paths meeting over a constructed message — a KAT's shape; the tautology would be a test computing the expected signature and asserting the rule accepts it. The question had conflated two things: **(a)** the ~fifty fixtures whose subject is another rule (a pop, a sequence, a belt) — for those `fixture::signed` is the right infrastructure and better than migration, the signature being real; **(b)** I18's *own* witness — which `cacc10135`'s four mock tests were not, because a fixture key is the key image doubled, a transaction no chain can hold (I15 binds image to output). The witness already existed and was unasserted: the captured chains' wallet-built spends, real HKDF keys, replayed through `validate` with I18 live — `vectors_tests::hold` now asserts the 4.I rows recorded on them, and says why the admission (`refused: None`) is the witness and the bit only the weaker half. **The line, so the fifth path is a test and not a precedent** (now in `50-testing.mdc`): a rule's own witness comes from the driver or the captured chains; fixtures whose subject is another rule may be constructed, including signed, provided the construction uses production paths — which admits `fixture::signed` and still excludes the four refused paths, each of which made the rule's own subject synthetic. **The next cascade, scheduled:** the fixture key is derived from the image, the reverse of production; I15 binds each image to its spent output and will refuse every signed fixture when it lands — the third fixture relationship this slice predicted a rule ahead (`[0x80; 32]` output keys, `[fill; 32]` images, now this). I15 is the collector; the §5.3 migration happens there, at the rule whose subject requires it. **(iii)** I18 itself (`judge_signatures`, the I17→I18 D4 pair, last in `tx_against` as `:4277` is last in `check_tx_inputs`); the mock holds the forged-byte, the changed-body and the serve-credit-boundary cases; the driver gains `ForgedSignature` and replays the captured spends' wallet-made signatures. **I15 and H19-verify:** the four captured chains carry **one** non-coinbase transaction per block (measured over `corpus.e2`), so Q9's condition fails and the fold is the successor Q9 named; and I15 needs a proof made in the tree it is judged against, which no fixture has and the driver cannot yet build (`placeholder_root_after` — S-CURVE grows the tree). One FOLLOWUPS row carries both, with the falsifier. Coverage 72 → 73. **Owed to commit 10:** census I18 cell (the "on the vin" phrase, already listed) and its site (`tx_pqc_verify.cpp:231–243` now names lines the cutover moved). *(The paragraph below is the charter this commit was written against, kept as the record of what was asked.)* I15 over `shekyl_fcmp::proof::verify`; I18 over `shekyl_crypto_pq` through I17's payloads; H19's verification half as a **`validate` fold** over the block's BP+ proofs (Q9), if the captured contexts carry ≥ 2 spends per block — else a named successor, disclosed at commit 1. **I18's boundary, pinned before it starts (from row 7's source check):** I18 verifies `pqc_auths[i]` over I17's `signed_hash(i)` and is therefore **vacuous on the serve-credit-only class by construction** — I17 yields zero hashes there because CEN-H20 forbids the form `pqc_auths`; the form's own countersignature is **CEN-J10's** (over the pass record, by the bond's key), a different row on a different object, and I18 must neither reach for it nor record itself as having judged it. **The test shape, pinned on I17 first (`i17_reads_none_of_a_serve_credits_pass_record_and_yields_only_where_pqc_auths_are`) and owed on I18 in the same form:** (1) *does not reach for J10's object* — corrupt every payload byte of the pass record and the verdict does not move (for I18 this is the real falsifier: a verifier that read the countersignature would refuse a corrupt one); (2) *zero-yield is exactly the classes with no `pqc_auths` by construction* — the serve credit and the coinbase — and a spend yields one per input, so the vacuous record follows from the class the test holds fixed, never from an in-scope class that produced nothing. **What is not asserted, and why:** that the coverage row is *absent*. The crate records an out-of-scope row **vacuous, not skipped** (slice 5 Q2, `rules/mod.rs:57–61`; `judge_reference` does it for I11/I12 on a non-spend) so a block of serve credits can be `is_complete_for` and the pool cannot mis-declare a kind; the bitset has one bit per row by design, and absence would break completeness on every block carrying the form. The quiet failure the review named — coverage over a class the rule cannot judge — is therefore caught by (2), the class binding, not by the bit. The census I18 cell's *"signature lives on the vin, CEN-J10"* is the same half-truth row 7 corrected (the ML-DSA leg is in the pruned record) — **owed to commit 10's** census sweep with the I17 site cell | commits 1, 6, 7 |
| 9 | **LANDED 2026-09-25 (cost 1).** **The dead arm.** Its note named two orderings inside the wire's `Transaction::validate` — (1) `validate_context_free_pruned` first, forcing `n_out ≥ 2` for any key-imaged body; (2) `must have no outputs` before the key-image check inside the fee-only arm — and said a rule inserted between them makes the arm live. The re-check is not a confirmation but the answer to *where the verification rows went relative to those two*: I17 and I18 are `tx_against`'s, run after `tx_form`, which is downstream of the whole twin — **after both orderings, between neither** — and `validate`'s body is unchanged since the slice's base (`eebe9a8fd`: the slice touched `PqcAuth`'s header, `Ct`'s doc, `write_pruned`, the re-export, and deleted the old preimage body; no hunk in `:1600–2070`). I15 and H19-verify will land in the same place. Still dead, by the same two orderings; the note now says so at the arm. **I19's arm.** The one site keyed by a source fragment (`check_tx_extra_shape(&fields, n_out, subject)…`) has **three** renderings: I19's missing-field, I20's coinbase grammar, and `NonceOutsideCoinbase` under `ExtraSubject::General` — the clause commit 3 ruled is I19's. The table is one row per site, so that third rendering was asserted at the crate (`tx_extra_tests`) and nowhere against the twin. `the_i19_sites_nonce_rendering_is_i19_at_both_sites` holds it: the twin refuses a nonce on a spend with the nonce message, `tx_form` refuses the same body on I19 at both listed slots. The register and the coverage gate are unmoved (133 rows; implemented 75) | commit 8 |
| 10 | **LANDED 2026-09-26 (cost 1).** Two classes, swept differently. **Cells with a known wrong phrase, by grep and edit:** I17's site (now `PqcSigningPreimage`, the derivation of record, with the deleted C++ assembly as records-was); I18's *"exempt (signature lives on the vin)"* (the form carries no `pqc_auths` by H20 and is signed over the pass record, J10 — vacuous by construction, not exempt); J10's *"vin-borne"* (the two legs named, RF-D2); the I19/I20 nonce clause moved to I19 with `TXE-Q6′` cited from both; §3.4's FAKECHAIN corrections on I2 and I3 (*not* exempt — the gate chooses which refusal fires, behaviour does not vary) and §10 R8's mis-attribution (the carve-out block is I4's). **The belt-versus-rule pattern, as a query:** every consensus-bucket cell naming a belt (`SI-n`, `MDB_NODUPDATA`, `MDB_APPEND`, *re-enforces*, *belt*) — twelve rows. Eleven use the word correctly: a bucket-3 belt named beneath a validator row (A1, G1 → L3; K3 → `MDB_NODUPDATA`), a marshaling or display shim (D2, D6, K1a), a rule re-enforced by another *rule* (H2 → I3; H10 ↔ I5), narrative (J13, H15, H9). **One** names a store belt as a validator row's enforcement — **CEN-I7**, *"the storage layer independently re-enforces at connect (CEN-L1)"* — and the register's I7 row went further: its REWRITE-NOTE said *"the rewrite carries one site"*, which is the instruction that left L1 unbuilt. Both corrected in place (the register row's status stands as the C++ read at its era; its account of what the rewrite owes is refuted); §6 of this plan had been corrected at commit 4. Three surfaces, one origin, one instance — bounded by C2-R8's date as predicted, and shorter than it felt. **The variant the corollary did not yet name, and its own sweep (review of 2026-09-26):** the register's I7 note was not a stale claim or a wrong attribution but *an instruction living in a register* — prescriptive language in a document whose job is to record what the C++ does, and someone followed it. The population: 22 REWRITE-NOTE rows, **eight prescriptive** ("the rewrite carries / must / should"). Read: four described work E6 had since landed (H5 one site, F4 the exemption and the caller-derived operand, F19 the structural read-point, H12 the one live arm) and are rewritten as records with their era; three name owed work and now point at the plan that owns it (J3 → `ARCHIVAL_CHALLENGE_MECHANISM.md` §9.5.1, J26 → `DRS_E1_SPOOL.md`'s typed cache, J17 → a new FOLLOWUPS row, its only home); I7 refuted in place. The rule, at the marker's definition in the register: *the register records what is; what the rewrite owes belongs in a plan.* Also: `CHAIN_RULES_CRATE.md` §4.3 (`height_of`), §4.6 (`tx_against(tx, slot, view, rule_set) -> ViewRead`, `TxAgainstRule`, the two D4 sequences, `TxScope::Coinbase`), `:1456`; the index row; CHANGELOG (the L1 finding as security, the preimage cutover and `SigningPayloadHash` as API); the FOLLOWUPS capture row removed — its subject landed as E2's vectors and its residue is the I15/H19-verify row. *(Charter, as written:)* Docs: census 4.I re-pinned at the landing tree (read, not diffed); **the I19/I20 nonce clause moved to I19's cell with `TXE-Q6′` cited from both cells** (row 3 (a), ruled 2026-09-25 — the enforcement's home decides the cell; the citation keeps the two halves one ruling); the FAKECHAIN corrections from §3.4's sweep (I2, I3, §10 R8); `CHAIN_RULES_CRATE.md` §4.3 (`height_of`), §4.6 (`TxAgainstRule`, `TxScope::Coinbase`); index; FOLLOWUPS (the capture row closed; the I17 shim finding); CHANGELOG | — |
...
| 7 | I17 (c): the payload derivation as a production body in `shekyl-wire`, the FFI shim, the byte-identity gate against the C++ on real multi-input and serve-credit shapes — before `tx_pqc_verify.cpp:62–158` can go. **Landed at 3 (2026-09-25): the KAT capture, the derivation rewritten, the cutover** | 2, may run to 3–4 | **no — exempt.** This is a cross-crate cutover of TXE's coarse-call shape, which took its own PR; it was always sized like one. An overrun here says a cutover took what a cutover takes, not that the substrate was unfinished. Named so the signal, if it fires, fires for the right reason |
| 8 | I15 over `proof::verify`; I18 through I17's payloads; H19-verify as a `validate` fold — witnessed by the captured chains' real proofs, gated on 5 and 7 only. **Landed at 3 for I18 alone (2026-09-25); I15 and H19-verify deferred.** The falsifier fires, and for the reason it exists: the estimate assumed the captured chains were the witness for all three, and they are for I18 only — one spend per block, and no fixture holds a proof. The third commit was the shared verify body the estimate did not see | 1–2 | yes — **fired** |
| 9 | conformance re-check (the dead arm's ordering; I19's arm). **Landed at 1 (2026-09-25)** | 1 | yes |
| 10 | docs: 4.I re-pinned to function anchors with eras — into a census that is now in the citation gate's `DEFAULT_DOCS` (#854), so the re-pin is checked the moment it lands; index; CHANGELOG. **Landed at 1 (2026-09-26)** | 1 | yes |
...
SHA (six pins died in the last rebase — the rule at this file's head).
...
- **Q9 — H19's verification half (§3.6). RULED 2026-09-24 on a structural argument: a batch verify is inherently multi-transaction and `tx_form` is per-transaction, so batching cannot live in `tx_form` without abandoning the batch or smuggling block scope into a per-tx rule — H19-verify's home is block-level. The reviewer asked whether that misses the second option. It does not miss it, because the second option was never about placement: both options placed the fold in `validate`; they differed on SEQUENCING — this slice (commit 7, if the vectors carry ≥ 2 spends per block context) versus a named successor after the vectors, owning the one-bad-proof-among-good fixture. The structural argument settles the home; the sequencing follows Q1's (ii): the vectors land first, so the ≥ 2-spends condition is decidable at commit 1, and the fold lands in this slice if it holds. If the captured contexts carry one spend each, the fold is a named successor, disclosed here, not a deferral found later.** In this slice as a `validate`
  ...
  context; else a named successor. **UPDATE 2026-09-25 — the condition
  measured, and false:** every captured block carries at most one
  non-coinbase transaction (`corpus.e2`, all four chains). The fold is the
  named successor, on one FOLLOWUPS row with I15 (§5 row 8), blocked on the
  scenario driver listing two tx-builder spends in one block. The
  condition should have been decided at commit 1 as this text says; it was
  decided at commit 8 — the same class as the I13 skip, a condition nobody
  re-read.
