...
constant (`GENESIS_PINS_ARE_PLACEHOLDERS`) rather than a comment, so the arm
that skips the comparison is visible to a grep and cannot be mistaken for a
live check. A pin whose value is not derived from the chain it names is a
...
becomes `false`, and the axis arms with no other change.
