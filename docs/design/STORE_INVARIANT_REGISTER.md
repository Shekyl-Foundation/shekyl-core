...
| SI-10 | The recorded difficulty accumulator is coherent as the validator reads it: cumulative difficulty is monotone across recorded heights, the parent's plus the connecting target fits the type, and a target derived from the recorded window is nonzero. **The one row whose observer is the validator, not a write site:** D4's window walk and D6 read the record through the batch's branded view and return `Fault::Corrupt` — an `InvariantViolated` the store did not see itself. The pipeline hands the observation back through `WriteBatch::refuse_corrupt(Corrupt)` (DRS-E2 RD-Q4, built 2026-09-20), which arms the batch's poison as a belt would: nothing the batch wrote lands, the writer halts at the connecting height, later writes are refused. The validator's own `Corrupt` value is carried, not re-described. `CumulativeDifficultyOverflow` is unreachable at the ratified `(N, T)` (pinned by a `u128::MAX` fixture in the rules crate) and `ZeroTarget` at the mint; both arms are mapped rather than dropped so the refusal has a name if a producer ever appears. | `block_info` (cumulative difficulty), the D4 window | CEN-D4 / CEN-D6 | DRS-E2 RD-Q4; slice 2 §4.3's deferral | built | `StoreInvariant::DifficultyRecordIncoherent` |
