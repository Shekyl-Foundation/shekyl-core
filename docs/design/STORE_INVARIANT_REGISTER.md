...
| SI-16 | **A pool entry is one entry.** `pool_meta[h]` exists ⇔ `pool_blob[h]` exists — in the **pool file** (`crate::pool`, DRS-E1 S-POOL): `insert` writes both rows in one batch, `remove` deletes both, `update` touches only the meta row, and no other writer exists, so a key present in one table and absent from the other is a write that bypassed the store. The C++ enforced the meta ⇒ blob half at `for_all_txpool_txes` (`db_lmdb.cpp:2304`, `DB_ERROR("Failed to find txpool tx blob to match metadata")`) and never checked the other. Observed by the enumeration's blob read (`PoolEntry::blob`, meta without blob) and by `insert` (a key in exactly one table, either direction); named by the read that found it, not a string — built S-POOL commit 1 (`pool/mod.rs`) | `pool_meta` ↔ `pool_blob` (the pool file) | none — the pool is not chain state (§11.2); no census row reads it | DRS-E1 S-POOL (`DRS_E1_SPOOL.md` §5), 2026-09-24 | built | `StoreInvariant::PoolEntryUnpaired` |
