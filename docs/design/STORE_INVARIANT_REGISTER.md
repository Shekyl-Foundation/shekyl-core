...
| SI-3 | `txs` is keyed by tx hash: inserting a hash already present is fatal — built S-CHAIN-W commit 6 (`connect` → `tx_indices` insert) | `txs` | CEN-G1 (listed txs); CEN-F5 corollary (miner tx — reversion clause in C2-R8 §7.1) | CEN-L3 | built | `StoreInvariant::TxHashNotFresh` |
...
| SI-9 | Store-derived ids are dense and fresh: `tx_id`, `output_id` and the per-amount `amount_index` are the owning table's entry count at write time, and the slot an `insert` targets under one is absent — built S-CHAIN-W commit 6 (`connect` → `txs_*[tx_id]`, `tx_outputs[tx_id]`, `output_txs[output_id]`, `output_amounts` member inserts) | `txs_pruned` / `txs_pqc_auths` / `txs_prunable` / `txs_prunable_hash` / `tx_outputs` (`tx_id`); `output_txs` (`output_id`); `output_amounts` (`amount_index`) | — (pure storage integrity; deliberately **not** folded into SI-3, whose twin is CEN-G1 — R8-Q1's three-arm test needs rule-twinned belts and pure invariants to stay distinguishable) | S-CHAIN-W pre-flight SCW-4 ([`DRS_E1_SCHAIN_W.md`](DRS_E1_SCHAIN_W.md) §6), ruled 2026-09-15 | built | `StoreInvariant::IdNotFresh` |
