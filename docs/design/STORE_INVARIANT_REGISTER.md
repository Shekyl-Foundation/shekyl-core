...
| SI-13 | A recorded **fold never decreases** along the chain: `block_info.cumulative_tx_count` (S-CHAIN-R; the parent's row plus this block's listed transactions) is a prefix sum the store only adds to, so a later height's value is never below an earlier one's. SI-8's wrap rule guards the write; this row is the *read-side* form — **armed by the validator reading the store, not by a write site**, as SI-10 is: CEN-F20 forms the volume window as the difference of two prefix sums (E6 slice 4), a negative difference is `Corrupt::TxCountNotMonotone { at }`, and the ingest pipeline hands the `Fault::Corrupt` to `WriteBatch::refuse_corrupt`, which arms this row and halts the writer at the connecting height. Never a saturated zero: a zero window would price the chain as dormant, a wrong answer that looks valid — built E6 slice 4 (`rules/miner.rs` `F20::window`; `store/write.rs` `refuse_corrupt`) | `block_info.cumulative_tx_count` | CEN-F20 (the volume operand) | E6 slice 4 landing (`CHAIN_RULES_SLICE_4.md` §5), 2026-09-22 | built | `StoreInvariant::FoldNotMonotone` |
| SI-14 | **A bond record's holdings are one shape.** `archival_bond[p]`'s compact holdings carry exactly one add-epoch per held shard, ids distinct, insertion order preserved — the C++ record kept `held_shard_ids` and `shard_add_epochs` as two vectors coupled by one count in the codec and by call-site discipline in memory, with a FATAL on desync (`db_lmdb.cpp:4943`). The redb record makes the desync **unrepresentable**: `Holdings::ShardSet(Vec<HeldShard>)` is one list of `(shard, add_epoch)` pairs, built only through `Holdings::shard_set`, which refuses an over-cap or duplicate-bearing list (S-ARCH commit 1, `codec::archival`). **Armed by the codec, not by a read**: a persisted row that names a shard twice fails `BondRecord::decode` and surfaces as SI-7 (`CellCorrupt { Undecodable }`) at A1 — there is no separate observation to name, so this row is `ruled` with SI-7's variant as its arm. Reopens to `built` with its own variant only if a distinct read-side observation appears (rule 21) | `archival_bond` | CEN-L16 (the holds-shard fold reads it); CEN-J8 | DRS-E1 S-ARCH (`DRS_E1_SARCH.md` §5), 2026-09-23 | ruled | |
| SI-15 | **Serve-credit rows are keyed by a persona with a record.** Every `archival_serve_credit` key's `PCanonicalId` component has an `archival_bond` row: the connect hook that writes a pass bit refuses one for an unknown persona (CEN-L7's fatal backstop), so rows for a record-less persona are a write that bypassed the writer. **Armed by the reads, not by a write site** (E4's writer is not built): the three serve-credit walks — A3 `last_served_epoch`, A4 `served_shards`, A5 `pass_count` — have the persona in hand and, having found at least one row under its prefix, check the record exists; a walk that found nothing asserts nothing, so a query about a stranger is an answer. When E4's hook lands, its refusal is the first check and this becomes the second, as SI-9's read-side belt is for `output_txs` — built S-ARCH commit 2 (`store/archival_reads.rs`) | `archival_serve_credit` → `archival_bond` | CEN-L7 (the hook's refusal); CEN-J8 / CEN-L16 read the rows | DRS-E1 S-ARCH (`DRS_E1_SARCH.md` §5), 2026-09-23 | built | `StoreInvariant::ServeCreditWithoutBond` |
| SI-16 | **A pool entry is one entry.** `pool_meta[h]` exists ⇔ `pool_blob[h]` exists — in the **pool file** (`crate::pool`, DRS-E1 S-POOL): `insert` writes both rows in one batch, `remove` deletes both, `update` touches only the meta row, and no other writer exists, so a key present in one table and absent from the other is a write that bypassed the store. The C++ enforced the meta ⇒ blob half at `for_all_txpool_txes` (`db_lmdb.cpp:2304`, `DB_ERROR("Failed to find txpool tx blob to match metadata")`) and never checked the other. Observed by the enumeration's blob read (`PoolEntry::blob`, meta without blob) and by `insert` (a key in exactly one table, either direction); named by the read that found it, not a string — built S-POOL commit 1 (`pool/read.rs`, `pool/write.rs`; one `RowPair` classifies both) | `pool_meta` ↔ `pool_blob` (the pool file) | none — the pool is not chain state (§11.2); no census row reads it | DRS-E1 S-POOL (`DRS_E1_SPOOL.md` §5), 2026-09-24 | built | `StoreInvariant::PoolEntryUnpaired` |
