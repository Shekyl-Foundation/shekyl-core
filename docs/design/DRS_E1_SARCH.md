...
**Round 1 questions posed** (§9, defaults stated), awaiting the maintainer's
rulings before the increment is cut. No implementation commit lands on the
branch this document opens until §9 is RULED (rule 26, the halt condition).
...
| 5 | `archival_bond_holds_shard(p_id, shard, at_height)` → `bool` (`:2188`) | `archival_bond[p_id]` **and `archival_slash_log`** — a C++ as-of-height fold (`archival_bond_holds_shard_of`, `db_lmdb.cpp:4889–4950`) that, for a shard not held at tip, range-scans the slash log strictly above `at_height` for a row that removed it (`archival_slash_removed_holding_after`, `:4804–4870`) | `:4874` `check_archival_serve_credit_input`; **and E4's slash writer** (`:5381`, slash eligibility inside `process_archival_slashes`) | validator; E4 | **A1 + A2** (`slash_log_after`) + the fold ported to `shekyl-archival-retention` — **or deferred to E4 with the journal question** (`SAR-Q7`; SAR-2, SAR-7) |
...
(`consensus_state.rs:105`); `holds_shard_at` is ported there (or deferred,
`SAR-Q7`), taking a `&BondRecord` and the slash-log rows A2 returns.
...
- `CONSENSUS_RULE_CENSUS.md`: CEN-L10's row notes `PDM-Q12` and the E4
  deletion (SAR-5); no bucket move until E4 deletes the code.
