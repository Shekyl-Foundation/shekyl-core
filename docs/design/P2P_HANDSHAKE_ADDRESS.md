...
its lane; contains dev `fbd80393a`). Branch: `p2p/basic-node-data-address`. **This unit executes an
...
re-decide it. Where this note and that doc disagree, the doc wins.
...
| 5 | Handshake-complete sentinel (`context.peer_id != 0`) | **Explicit local session-state flag, never on the wire** (ruled in the amendment; `connection_context` already carries typed state — the flag reads `session_established()`, which is **session established** in `SHEKYL_P2P_PROTOCOL.md` §3.5 *Two handshakes*) | Executed here; `for_each_connection`/`for_connection` drop the `peerid_type` parameter |
...
- **Anonymity zones, serving:** `zone.m_our_address` (self-declared, dialable
  — the only verification an overlay address admits). It reaches a peer's
  peerlist through the timed-sync self-announcement, not this socket-derived
  path: an overlay remote carries no host to combine an advertised port with.
...
## 4. The two questions this note opened, and how they were ruled

- **Q1 — address type discipline. RULED YES, implemented.** The field stays
  `network_address` (no new codec, wire-compatible with `peerlist_entry.adr`)
  and never-read-the-advertised-host is **structural, not prose**:
  `derive_advertised_endpoint(observed, advertised_port)` takes the two as
  separate inputs, so a caller cannot pass the advertised host where the
  observed one belongs — there is nowhere to put it. Pinned by
  `node_server.an_advert_contributes_its_port_and_never_its_host`.
- **Q3 — RULED: the nonce rides now, and does.** Recorded as it was argued,
  because the reasoning is what a later reviewer needs. This unit
