...
template). At Round 0 the **block template / coinbase writer had no Rust
owner** (TXE-F8): E6 slice 4 was landing the coinbase *judge* (F1–F10, F13,
F20 — `CHAIN_RULES_SLICE_4.md` §4), not the builder. UPDATE 2026-09-24: the
owner is `shekyl-block-template` (#852, E6 slice 6 §5.3); `construct_miner_tx`
and these callers are the C++ that crate's daemon consumers retire (E3/E4).
...
| **TXE-F8** | **The block template's Rust owner is `shekyl-block-template` (LANDED 2026-09-24, #852; was: "no Rust owner").** As found at Round 0: E6 slice 4 lands the coinbase judge (`CHAIN_RULES_SLICE_4.md` §4: F1–F10 as a miner-transaction rule class); no DRS plan, index row or FOLLOWUPS row owns the builder. This cutover moves the coinbase *extra* into Rust and leaves the template where it is. **CLAIMED 2026-09-24 by E6 slice 6 (`CHAIN_RULES_SLICE_6.md` §5.3): the owner is the new crate `shekyl-block-template` — `create_block_template`'s successor, store-free, pure over a caller-supplied context — whose first consumer is the slice's scenario driver; `get_block_template`'s Rust handler and the built-in miner are the next two. This row is that crate's row. UPDATE 2026-09-24: crate LANDED at `rust/shekyl-block-template` (`build(&TemplateContext) → Template`; the coinbase by `construct_output` + `build_coinbase_extra`, the amount by `paid_block_reward` → `compute_emission_split` + `compute_fee_burn`; header timestamp `max(now, median + 1)` **checked against CEN-C1 with `is_timestamp_below_ftl`** — reachable when a near-limit-stamped window's median runs more than FTL ahead of a behind clock; refused by the builder, and the test shows the validator refuses the same header on C1); STAGED — its named consumer is the slice-6 scenario driver, next commit. Held under `check_chain_rules_no_store.sh` alongside the rules crate. Re-pricing budget pinned to the C++ `try_count` of ten (`blockchain.cpp:1830`); the one non-convergence is a varint-boundary two-cycle in the penalty zone, **constructed** in `a_reward_exactly_on_a_varint_boundary…` (at `2^35` with a ~317 kB block the band is ≈`4.0e10` atomic units of `already_generated`, about one block's emission wide) and refused by both producers — never produced by one and declined by the other. Not a wall: at a supply inside the band 3 of 300 000 body weights cycle and a settling one is a byte away; the producer builds a different body.** | `rg -i 'block template\|construct_miner_tx' docs/design` → no owner *(records-was; the owner is now `CHAIN_RULES_SLICE_6.md` §5.3)*; `cargo tree -p shekyl-block-template --depth 0` resolves |
