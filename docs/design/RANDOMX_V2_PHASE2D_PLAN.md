...
| Implementation | 2026-05-22 | §8 commits 1–4 + T16 (5b's real-hash flip via T16 sibling) landed on `feat/randomx-v2-phase2d` (PR #66 / `26fc49d6c`). §8 commit 5a landed in follow-up: `phase2d/` C generator (`tests/vectors/reference/_generator/phase2d/{gen.cpp,Makefile,README.md}`) emits the T9–T15 binaries and matching `.meta.txt` provenance under `tests/vectors/reference/vm/`. Rust spec-vector tests `{t9,t10,t11,t12,t13,t14,t15}_vm_..._matches_fork_reference` in `src/vm.rs#mod tests` drive `dispatch_instruction` against the canonical Phase 2d fixture (matching `phase2d/gen.cpp`'s `CANONICAL_*` tables) and assert byte-equality against the pinned fork's `executeInstruction` output. All 7 vectors pass on x86_64-linux; clippy `-D warnings` clean; non-determinism-sensitive determinism tests (`t3'`/`t4'`/`t5'`) continue to pass. Phase 2d byte-equality surface against fork pin `aaafe71` is now exhausted (per-opcode T9–T15 + end-to-end T16). §8 commit 5b's separate T8 NOP→real-hash expectation flip remains scoped via T16's superseding real-hash vector (Phase 2c T8 stub-NOP and T6/T7 tagged `#[ignore]` with explanatory comment). |
