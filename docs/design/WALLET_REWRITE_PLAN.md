...
todos:
  ...
  - id: phase2_ops_refresh_send
    content: "Phase 2a: Wallet::refresh() over shekyl-scanner using typed ScanResult merge via Wallet::apply_scan_result (additive, slice-scoped, snapshot-consistency-checked); three-method send lifecycle build_pending_tx / submit_pending_tx / discard_pending_tx with reservation semantics on input outputs; PendingTx anchored on (built_at_height, built_at_tip_hash, fee_atomic_units); fee priority resolved via daemon get_fee_estimates with sanity ceiling (TxError::DaemonFeeUnreasonable); TxRequest has no payment_id field; depends on shekyld get_fee_estimates audit (PR 0.3). Engine send substrate (2a-1…2a-4: LocalPendingTx daemon fee/build/sign/submit + TestDaemon integration) landed; orchestrator methods (Engine::refresh / start_refresh / apply_scan_result / build_pending_tx / submit_pending_tx / discard_pending_tx) landed with the Phase 1 closeout (2026-06-10) — remaining 2a residue is tracked in PHASE_2A_SEND_PATH.md §10, not Phase 1. UPDATE 2026-06-30: 2a-the-phase (synthetic-path build/sign/submit) is DONE per its own DoD; real-root MAINNET validity is gated on the separately-scoped curve-tree client prerequisite (PHASE_2A_SEND_PATH.md §3.0.4 — buildable now, independent of 2b) + Phase 6; open_view_only / open_hardware_offload remain CapabilityNotYetImplemented stubs; ProofStale detection deferred to Phase 6."
    status: completed
...
- `archival_p` registration crypto — **not yet in production Rust**: the `ARCHIVAL_P_DERIVE_V1` KAT + `shekyl-crypto-pq::archival_p` module are still open, hard-gated on the C-1 vin-layer ML-DSA equality check ([`ARCHIVAL_FIREWALL_GATE6.md`](ARCHIVAL_FIREWALL_GATE6.md) §9.8).
...
- **Reward-emission leg + principal stake/unstake/drain lifecycle** — **not yet homed in any plan doc.** Hard-gated on C-1 (vin-layer ML-DSA equality check, [`ARCHIVAL_FIREWALL_GATE6.md`](ARCHIVAL_FIREWALL_GATE6.md) §9.8) + Gate 6 R4. These are the biggest unbuilt 2b chunk and need their own PR-decomposition round.
