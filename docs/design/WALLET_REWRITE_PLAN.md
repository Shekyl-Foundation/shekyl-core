...
todos:
  ...
  - id: phase5_cpp_delete
    content: "Phase 5: single commit deleting wallet2.{h,cpp}, wallet2_ffi.{h,cpp}, wallet/api/, simplewallet/, the remaining wallet_rpc_server*.h headers (**the wallet_rpc_server executable + wallet_rpc_server.{h,cpp} landed early, 2026-08-18, with the C1/C2 install cutover — the two *_defs.h / *_error_codes.h headers stay only because wallet2_ffi.cpp includes them**), related unit tests, account_base::load_from_shkw1 + forget_master_seed (transitional helpers), shekyl_wallet_* C-ABI exports + wallet_ledger_ffi.rs (FFI surface that existed only for wallet2.cpp), wallet sections of shekyl_ffi.h, shekyl_ffi_*.cpp tests; CMake cleanup so shekyld still builds; long commit message naming every deleted path"
    status: pending
  - id: phase6_tests
...
- **In scope:** `shekyl-core` only. The Rust wallet stack (`rust/shekyl-engine-*`, `shekyl-tx-builder`, `shekyl-scanner`, `shekyl-proofs`, `shekyl-cli`, `shekyl-wallet-rpc`, `shekyl-daemon-rpc`) is completed to feature parity with the daily-use surface of `simplewallet` + `wallet_rpc_server`. The C++ `wallet2` stack is deleted.
...
- **Multisig:** modified FROST scaffold lives behind `shekyl-engine-core` (historical protocol name V3.1); full PQC-FROST ship-readiness is a separate plan. The Rust wallet API is shaped FROST-aware from day 1 so that work is a feature flip, not a refactor.
...
- **C++ deletion:** **Landed 2026-08-19 (#507).** `src/wallet/` (including `wallet2.{h,cpp}`, `wallet2_ffi`, `wallet_rpc_server*`) and `src/simplewallet/` are gone. The live wallet is `shekyl-cli` + `shekyl-wallet-rpc` on `shekyl-engine-core`. Residual C++ `construct_tx*` in `cryptonote_tx_utils.cpp` is consensus-oracle / miner-tx, not a product spend path.
...
flowchart TB
    ...
    subgraph Api[Domain]
      WalletCore["shekyl-engine-core::Engine (orchestrator)"]
    end
...
rounds
([`docs/design/STAGE_1_PR_3_KEY_ENGINE.md`](../completed/STAGE_1_PR_3_KEY_ENGINE.md),
...
- **`Network` — closed enum, no feature flags, mismatch is a typed error.** `Network = { Mainnet, Testnet, Stagenet, Fakechain }`. No Cargo feature flags excluding networks at compile time (they create matrix complexity for no real benefit; runtime enforcement is sufficient). The wallet file authoritatively declares its network in region 1 (capability + network field). `Wallet::open` requires a `DaemonRpcClient` whose declared network matches; mismatch is `OpenError::NetworkMismatch`, never a warning. The daemon URL's network is **verified before any wallet operation** — defends against DNS hijack pointing a testnet wallet at a mainnet daemon. **Carrier amended 2026-09-09 (`VC-R20`): the check reads `get_version`, not `get_info`.** This lock named `get_info` from Phase 1 and nothing implemented it; when `VC-4` built it, `get_info` was the wrong carrier twice over — it is a bridged C++ leg that `RK-5c` retires, and reading the network from it would put the network axis on a *second* round trip, so a restart or a proxy between the two calls could pair a version from one daemon with a network from another. The requirement is unchanged and now honoured: `DaemonClient::verifying` compares wire version, consensus-constant digest, network and genesis hash from **one** `get_version` reply, before the first request ([`CLIENT_VERSION_CONSTANTS_VALIDATION.md`](CLIENT_VERSION_CONSTANTS_VALIDATION.md) §2, `VC-4`).
...
3. **Three timeframes:** does the design work today, at mining-era end, and into the V4 lattice-only transition? (See [.cursor/rules/05-system-thinking.mdc](../../.cursor/rules/05-system-thinking.mdc).)
...
- Delete [docs/benchmarks/MID_REWIRE_WARNING_WINDOW.active](shekyl-core/docs/benchmarks/MID_REWIRE_WARNING_WINDOW.active); rotate bench baseline against post-2k.c steady state; update [docs/MID_REWIRE_HARDENING.md](../MID_REWIRE_HARDENING.md) §3.3.1 closure note to point at this plan instead.
- Add **"V3.1+ — Legacy C++ → Rust rewrite scope"** section to [docs/FOLLOWUPS.md](../FOLLOWUPS.md). Sub-sections: "wallet2.cpp" (capturing 2l/2m/2n absorption), "tx_pool / blockchain_db LMDB transactional wrapper" (capturing the Dandelion-class lesson — typed `LockedTXN` where commit-on-success is enforced by the type system, not by caller discipline).
...
**PR 0.4 outcome (landed 2026-04-25):** see [`docs/MONERO_OXIDE_VENDOR_STATUS.md`](../MONERO_OXIDE_VENDOR_STATUS.md). Three findings feed PR 0.6, the half-day review gate, and the future un-pin plan:
...
After PR 0.6 lands, append a dated section to [`docs/MONERO_OXIDE_VENDOR_STATUS.md`](../MONERO_OXIDE_VENDOR_STATUS.md) per its append-only lifecycle policy: new vendored commit, what changed, what verifications passed.
...
1. **Read PR 0.4 vendor-status findings** ([`docs/MONERO_OXIDE_VENDOR_STATUS.md`](../MONERO_OXIDE_VENDOR_STATUS.md)). Original gate scope.
2. **Read PR 0.3 daemon-side findings** ([`docs/SHEKYLD_PREREQUISITES.md`](../SHEKYLD_PREREQUISITES.md)). Confirm regtest mode is sufficient for Phase 6, `get_fee_estimate(s)` shape is what Phase 2a binds to, and the `fee_policy_version` absence is the daemon-side V3.1 follow-up the wallet plans for via `Option<u32>`.
3. **Read FOLLOWUPS V3.1+ section** ([`docs/FOLLOWUPS.md`](../FOLLOWUPS.md) → "V3.1+ — Legacy C++ → Rust rewrite scope"). Confirm every item that touches the wallet rewrite is **either absorbed** (`wallet2.cpp`, `WalletPrefs` round-trip, daemon-side tracing init), **cross-linked** (`fee_policy_version`, `tx_pool` LMDB wrapper, `monero-oxide` un-pin), **or explicitly closed by Phase 5** (`shekyl-cli` key image binary format, `wallet_tools.cpp` mixin/decoy). Items independent of the rewrite (Chore #4 platform-gate audit, `rand 0.9` migration, libunwind diagnostics, etc.) are noted but not absorbed.
4. **Confirm cross-cutting locks** (the section above) before any Phase 1 method signature lands. Each binding decision either still stands or gets a dated supersession entry in [`docs/V3_WALLET_DECISION_LOG.md`](../V3_WALLET_DECISION_LOG.md). No silent reinterpretation.
...
> Open residue (View/HW lifecycle bodies) was tracked in
> [`docs/FOLLOWUPS.md`](../FOLLOWUPS.md) with a reversion clause —
> closed 2026-09-07 by rejection: ViewOnly REJECTED, HardwareOffload
> DEFERRED with zero symbols (decision log 2026-09-07; rule 23).
...
- **`tx_keys` storage:** *Why now?* `get_tx_proof`. *Can we do better?* Re-derive per-call from view secret + tx pubkey when we have it; only persist when the user has requested an explicit proof artifact. **Answered empirically (2026-07-24, WI-RPC-3): re-derivation is impossible for OUTBOUND proofs.** The verifier re-derives the hybrid combined secret via KEM re-encapsulation, and the ML-KEM encapsulation coins derive from `tx_key_secret` itself — the view secret cannot reconstruct it. Persist-at-submit is the only shape: the retention writer stores `TxMetaBlock.tx_keys` at dispatch (persisted with the in-flight flip, before the bytes can reach the daemon; review-hardened 2026-07-24 from the original submit-ACCEPTED form — the verdict observes exposure, it does not start it), with the I-2 no-orphans invariant bounding retention to txs the ledger still references (lifecycle pinned in `wallet_rpc.yaml` §OUTBOUND PREREQUISITE).
...
- `PendingTx` type with `(built_at_height, built_at_tip_hash, fee_atomic_units)` anchoring and reservation ledger entry on input outputs. **The engine-actor-bearing implementation of the `PendingTx` lifetime + reservation surface lives in `PendingTxEngine` per [`STAGE_1_PR_5_PENDING_TX_ENGINE.md`](../completed/STAGE_1_PR_5_PENDING_TX_ENGINE.md)** — the cross-cutting decision pinned the trait surface and the (γ) lean three-collection state shape (`output_locks` + `consumer_held` + `in_flight`); the design doc holds the eight-commit C0–C8 decomposition that ships this surface under PR 5 of Stage 1. Snapshot identity is content-derived (`SnapshotId` via cSHAKE256 with customization `shekyl/snapshot-id-v1` over `LedgerSnapshot` deterministic fields, 16-byte truncation — SA-3c), and chain-state-pinning lives there end-to-end. Reservations and submit-side staleness checks land in `LocalPendingTx` (the Stage 1 implementor); Stage 4's `PendingTxActor` swap is invariant per the trait surface.
...
**Do not** implement the entitlement claim path then replace it. **`design/PHASE_2B_FSM_RETOOL.md`
...
|------------|-----------|----------------------|
| **Phase 2b — stake + `P` + rebased reward** | [`design/PHASE_2B_FSM_RETOOL.md`](PHASE_2B_FSM_RETOOL.md) | **No** (2a send independent) |
...
substrate. **Authoritative scope:** [`design/PHASE_2B_FSM_RETOOL.md`](PHASE_2B_FSM_RETOOL.md)
...
- **Principal stake/unstake/drain lifecycle** — **now scoped** (Round 0) in [`PRINCIPAL_STAKE_LIFECYCLE.md`](PRINCIPAL_STAKE_LIFECYCLE.md): the user-facing economic surface (`StakeEngine` is persona/bond-only today). Genesis-seal-blocked on the reinstate/release FSM pin + Gate 6 R4 (GF-4/GF-7) + the emission leg; the scoping doc + its Round-1 ratification are the buildable-now slice.
...
- **Why are confirmation prompts useful NOW?** Real money. *Can we do better?* Show fees + recipient + change destination + dry-run merkle roots before commit; require explicit `yes` for non-trivial sends. Document the "what gets confirmed" surface in [USER_GUIDE.md](../USER_GUIDE.md). Confirmation lives in the CLI layer, not in the RPC — the RPC accepts already-confirmed `PendingTx` handles.
...
- **Capability mode:** `"FULL"` on every wallet handle response — never inferred. (Originally enumerated `VIEW_ONLY` / `HARDWARE_OFFLOAD` too; both collapsed 2026-09-07 — ViewOnly REJECTED, HardwareOffload DEFERRED with zero symbols. The field stays on the wire so a future capability is an additive value, not a schema change.)
...
OpenAPI 3.1 spec at [docs/api/wallet_rpc.yaml](../api/wallet_rpc.yaml) lands **before or alongside** the first method (4b sub-PR). The spec is the contract; the implementation conforms to it. The reverse — writing the implementation first and back-filling the spec — produces "the API is whatever the implementation does," which is exactly what wallet_rpc_server became and exactly what we are rejecting.
...
- **Why is HTTP basic auth useful NOW?** Tooling compatibility. *Can we do better?* Keep HTTP basic available but recommend UDS+filesystem-perm auth as the default deployment mode in [docs/WALLET_RPC_README.md](../WALLET_RPC_README.md). UDS is the path Shape B (Phase 3) drives by default.
...
- **Layered config** via `figment` (or equivalent): TOML at `~/.config/shekyl/wallet-rpc.toml`, env vars with `SHEKYL_WALLET_RPC_*` prefix, CLI args. Precedence: CLI > env > file > built-in default. Documented in [docs/WALLET_RPC_README.md](../WALLET_RPC_README.md).
...
a release producer would have kept 13,360 lines compiling to serve a
...
The Phase 5 commit message names every closure so reviewers see them explicitly. Two from [`docs/FOLLOWUPS.md`](../FOLLOWUPS.md) §V3.2:
...
- **KAT carry-over:** the wallet-file-format Tier-3 KATs in [docs/test_vectors/WALLET_FILE_FORMAT_V1/](../test_vectors/WALLET_FILE_FORMAT_V1) and the seed-derivation Tier-1/2/3/4 KATs continue to gate; no Rust change can drift them without two-of-two maintainer review.
...
- **Docs sweep (per [.cursor/rules/91-documentation-after-plans.mdc](../../.cursor/rules/91-documentation-after-plans.mdc)):**
  - [docs/USER_GUIDE.md](../USER_GUIDE.md): rewritten for `shekyl-cli`.
  - [docs/WALLET_RPC_README.md](../WALLET_RPC_README.md): rewritten for `shekyl-wallet-rpc`.
  - [docs/POST_QUANTUM_CRYPTOGRAPHY.md](../POST_QUANTUM_CRYPTOGRAPHY.md): hardware-offload section update.
  - [docs/FCMP_PLUS_PLUS.md](../FCMP_PLUS_PLUS.md): wallet integration section.
...
Per [.cursor/rules/06-branching.mdc](../../.cursor/rules/06-branching.mdc), this work cannot live on a long-lived feature branch. Land it on `dev` in phase-sized PRs:
...
- Does not address `tx_pool.cpp` / `blockchain_db.cpp` LMDB transactional wrapper redesign (the Dandelion-class lesson). That's filed in [docs/FOLLOWUPS.md](../FOLLOWUPS.md) under "Legacy C++ → Rust rewrite scope" as its own future plan.
