...
| **2** | **The covert executor.** Per-channel pending, in-flight remainder, bind, CV-1 discard — today's `send_noise` / `queue_covert_notify` / `clear_channel`. | Skeleton only (`CovertQueues`). Must become a real port or stop claiming CV-1. | **This PR (`#508`).** Window is `dummy.len()`; `CovertSend` is epoch-bound; enqueue refuses a non-multiple; CV-4 still hands distinct queues to `covert_cadence`. `Driver::poll` still takes no queue. No production caller — step 4 wires notify. **Corrected 2026-08-25: step 4 did not, and was never going to — see §3's status table, the row headed "§2.9 step 2 — covert executor", which is the authority on what the missing caller actually is. Plan text kept verbatim as the record of what was planned.** |
...
| **§2.9 step 2 — covert executor** | **landed (type)**; **caller now owed in Rust** | `NoiseQueues` is a real port: constant window, CV-1 restart, epoch-bound `CovertSend`, enqueue refuses a non-multiple. CV-4 still threads distinct queues through the cadence. **Corrected 2026-08-25:** the "no production caller" line attributed the gap to an in-process path step 5 must provide. That inference was wrong. The missing join is `Driver::poll`'s `Effect::NoiseSend { channel, peer }` to `NoiseQueues::take_for_send(channel, peer)` — both sides are Rust, `NoiseQueues` is simply held by neither `Zone` nor `Driver`. C++ performs the transport for stem and fluff today and can do the same here. |
