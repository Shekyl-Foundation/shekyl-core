...
The carrier caller is built, and it is reachable only under
`SHEKYL_CARRIER_DEVELOPMENT`. That is a ruling with reopening criteria, not
caution, and the reason is that an operator switch would ship with an
undefined meaning.
...
| --- | --- | --- | --- | --- |
| modal (1-in/2-out, max depth) | 13,042 | 1 | 6.25 s | 1387 s (23.1 min) |
