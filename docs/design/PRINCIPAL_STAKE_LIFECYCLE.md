...
`Reinstate` (né `Rebond`) is the **sole in-place record operation**: zero-money,
post-slash, re-arming slashability on a record whose holdings it is forbidden to
change. Post-holdings must equal current.
