...
| **2** | **The C1 oracle, height-keyed.** Property tests: snapshot-derived root vs `build_layers` across every layer-finalization edge. **Red-bite first** — it must be able to fail before anything is built to pass it. **Second red-bite, from Q2 — ARMED at this increment, graded at 4 (amended 2026-09-23):** the two tiers are **total, and identical where both answer**, across the moving freeze boundary — a height no tier answers is the defect, and a height both answer disagreeing on is the other one. **The examiner lands here, before the tier it will grade.** Two shapes were considered and both refused: a placeholder increment 4 must satisfy is *green by construction* — the tautology gate scheduled in advance — and deferring the bite to increment 4 would have the subject's author write its own examiner in the same commit, the authorship pattern that produced the verify-not-on-the-graded-path defect. Instead the property is written **generic over the one seam C1 pins (height in, root out)** and instantiated now against a **deliberately disagreeing shim**, with the disagreement and the uncovered-height fault each **proven to red**; blinding the examiner reds both. Increment 4 arrives to an examiner it did not write and plugs in as the real instantiation. If the real tier will not fit the seam, reshaping an examiner under review is a better failure than birthing subject and examiner in one commit | §6.3.4 row 4; Q2 | Q2 (**ruled**); Q1's *shape* only — its constants are not inputs to the oracle |
...
| 2026-09-23 | **`Q2`'s red-bite is armed at increment 2 against an injected subject, not deferred and not placeholdered** | Both offered shapes were defective: a placeholder is green by construction, and deferring it makes the subject's author write its own examiner. C1 already pins the only seam a tier can have, so the examiner is generic over it, instantiated against a disagreeing shim, and proven to red — the injected-wrong-subject pattern |
