...
**Status (as of 2026-05-25).** All Phase 2 sub-PRs have landed on `dev`: 2a (Argon2d), 2b (AES + SuperscalarHash), 2c (`Cache` + `compute_hash` + T1–T8 spec parity), 2d (v2 bytecode dispatch + FPU rounding), 2f (`CacheStore` + crate-level invariants + A/B bench), and 2g (differential-test harness + R5-D1 `test-internals` feature gate). Track A end state is reached for everything not deferred by R7 (the post-2g adversarial-corpus methodology + `mode_worst_case` + the `compute_hash` divergence at large data inputs are V3.0 pre-genesis-queue work tracked in [docs/FOLLOWUPS.md](../FOLLOWUPS.md), not Track A re-entry). Track B (Phase 3+) remains gated on the wallet V3.2 cutover per the gating signal below.
