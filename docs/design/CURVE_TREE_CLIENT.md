...
6. **Block-derived leaves (§6). CLOSED (CT-3 Round 1, 2026-06-13).** The
   ...
   [`CT3_SYNC.md`](../completed/CT3_SYNC.md) §3 R1-Q1; closeout in
   [`CT3_ROUND1_CLOSEOUT.md`](../completed/CT3_ROUND1_CLOSEOUT.md) §4.
...
| **CT-1** | **Round 1 closed** — see [`CT1_ROUND1_CLOSEOUT.md`](../completed/CT1_ROUND1_CLOSEOUT.md). `LeafStore` on **redb** (`CT1_ROUND1_PINS.md`): subtree-aligned segment cache keyed by `R_k` (pin/prune seam, §7.6), height-gated freeze (`end_block_height`), canonical 4×32 Selene-scalar leaf encoding, `build_upper_layers` mixed-composition hot path + `upper_layers_kat`, ACID reorg truncate, `store_kat` Tier A. `CurveTreeClient` mirrors drained leaves into the store on ingest. Remaining: on-disk path wiring (CT-3/CT-5), full-segment freeze KAT at `j=2` scale. | `shekyl-curve-tree/store`, `tests/store_kat.rs`, `tests/upper_layers_kat.rs` |
| **CT-2** | **Round 1 closed** — see [`CT2_ROUND1_CLOSEOUT.md`](../completed/CT2_ROUND1_CLOSEOUT.md). Reconstruct-root KAT (`recon_kat.rs`) + production client path (`client.rs`) over `ct2_tier_a.json`: Rust root == C++ header root at every height on `main`, `reorg_deep`, `reorg_shallow`. Tier A pins empty-window boundary (`last_empty=60`, `first_drain=61`), coinbase `+60`, undeepen via `reorg_deep` (fork 140), freeze-lag via shallow reorg. Tier B `#[ignore]` scaffold in `recon_tier_b.rs`. CT-4 `assemble` and CT-1 `store` landed ahead of schedule. Remaining: CT-3 sync, CT-5 engine wiring. | `recon`, `client`, `assemble`, `tests/recon_kat.rs`, `tests/recon_tier_b.rs` |
| **CT-3** | **Round 1 closed** — see [`CT3_ROUND1_CLOSEOUT.md`](../completed/CT3_ROUND1_CLOSEOUT.md). Persistent client lifecycle landed across CT-3a (store schema, PR #128), CT-3b (`open(path)` + resume + delta ingest, `SCHEMA_VERSION` 2→3), CT-3c (`rollback_to_fork`, drain-cutoff partition, frozen-tail `R_k` recheck, PR #132). Block-derived forward sync is the confirmed default (R1-Q1 — the §6 reversion criterion fired: CT-2 landed the replication, KAT-verified); the bulk-leaf RPC (R1-Q1) and `SegmentSource` seam (R1-Q5) are deferred-with-recorded-shape, landing with the post-prune refetch path. Remaining: CT-5 engine wiring consumes the resume+ingest+rollback API. | `client`, `store`, `tests/recon_kat.rs` |
| **CT-4** | **Round 1 closed** — see [`CT4_ROUND1_CLOSEOUT.md`](../completed/CT4_ROUND1_CLOSEOUT.md). Landed ahead of schedule (assembly with CT-1/CT-2). Membership-path assembly (`assemble::assemble_path` → `AssembledPath`, gated on the §3.3 root match) + reference-block selection / validity-horizon arithmetic (`reference`: `select_reference_height`, `reference_block_age`, `proof_submittable`, `proof_expired`, `should_reanchor`, §5). Selection is pure height arithmetic, **not** a `ReferenceBlock` constructor — the client stores no header roots, so the caller binds height→root→hash and supplies the `ReferenceBlock` anchor (`{height, curve_tree_root, block_hash}`, hardened by the CT-3↔CT-4 audit). Consumed by CT-5. | `assemble`, `reference`, `tests/assemble_kat.rs` |
| **CT-5** | **Round 1 closed** — see [`CT5_ROUND1_CLOSEOUT.md`](../completed/CT5_ROUND1_CLOSEOUT.md). Production `CurveTreeClient` wired into the 2A send path behind a single-writer `CurveTreeActor` (synthetic vectors retired): forward ingest + reorg rollback + `Poisoned` drop-and-reopen (CT-5a/b), §3.3 verify-at-ingest (derive-not-hold), `assemble_path` cutover (CT-5c), proactive re-anchor horizon (CT-5d, PR #159), and the real-tree FCMP++ prove→verify roundtrip (PR #162 — partial-branch-chunk zero-pad). Engine-path reorg root matches the CT-2 Tier-A oracle at every height. The CT-5-tagged FOLLOWUPS residuals (poison reaction, client→engine `BlockHeight` typing) audited **delivered** and closed; per-input reconstruction-reuse perf deferred. Remaining is **not CT scope** — end-to-end daemon parity + Tier-B ride Track 2 (FAKECHAIN regtest). | `shekyl-engine-core`, `shekyl-fcmp`, `PHASE_2A_SEND_PATH.md` |
