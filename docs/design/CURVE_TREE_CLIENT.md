...
CT-1) is in progress on `redb` per [`CT1_ROUND1_PINS.md`](./CT1_ROUND1_PINS.md).
...
| **CT-1** | **In progress.** `LeafStore` on **redb** (`CT1_ROUND1_PINS.md`): subtree-aligned segment cache keyed by `R_k` (pin/prune seam, §7.6), height-gated freeze (`end_block_height`), canonical Selene-scalar leaf encoding, `build_upper_layers` mixed-composition hot path + KAT, ACID reorg truncate, `store_kat` Tier A. Types + `assemble` landed ahead of schedule. | `shekyl-curve-tree`, `shekyl-fcmp::tree` |
