...
| --- | --- | --- | --- | --- | --- | --- | --- |
| CEN-A1 | A block whose hash is already known (main, alt, or invalid set) is rejected with `m_already_exists` | 3060–3095 (`have_block_unlocked` three-store membership; `m_invalid_blocks` is process-lifetime memory); consumer 6626 (`m_already_exists`) | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q3a (ratified/signed 2026-09-04) | RC-4 ⇒ merged. DB belt is CEN-L3. Ratified orchestration-with-belts; the invalid set consulted for dedup only (Q3a falsifier) |
...
| CEN-G2 | Every listed tx hash must resolve from the mempool or the block's pool supplement; otherwise reject with `m_missing_txs` (taken pool txs are returned on any failure) | 5992–6028 | C | 4 | none | — | tx content ↔ hash agreement is established here by lookup-under-computed-hash; the DB layer trusts it (§6, CEN-L4) |
...
18. **CEN-B5's S1 is fixed — the check runs at admission, before the add,
    ...
    `m_verifivation_failed` and returns before any write; the post-add
