...
| id | rule | site(s) | C/P | b | class | evidence | notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| CEN-E1 | A block connecting at an anchored height must carry the release-carried anchor's hash — the anchor's own rule (`PDM-Q11`), the `assumevalid` argument as a predicate. *[Re-keyed 2026-09-20 (E6 slice 3, `PDM-Q-F30`'s trigger: checkpoint state moved into Rust): the statement had read "hardcoded/**JSON-loaded** checkpoint hash" — the JSON path was deleted under `PDM-Q-F23` (#733); the C2-R0 HELD existence question is answered by `PDM-Q5` (a release-carried anchor `C`; runtime pins REJECTED). Every network's table is empty at this re-key (`ReleaseAnchors::for_network`, as `init_default_checkpoints` is), so the row is vacuous on every existing chain and its parity evidence is fixture-only.]* | **Rust:** `rust/shekyl-chain-rules/src/rules/anchors.rs` (`E1`, view-bound, in `validate`'s block list; recorded as evaluated at unanchored heights, which the C++ never records); anchors from `Trust` (`trust.rs`), table `anchors.rs`. **C++ (until cutover):** main `blockchain.cpp:5545`–`:5552` (`is_in_checkpoint_zone` → `check_block`, a lookup short-cut not a rule); alt `:2186`–`:2192` (a checkpoint match forces the reorg, `:2446` — an alt-chain consequence, homed with CEN-E2 in slice 9); `src/checkpoints/checkpoints.cpp:73` | C | 2 | ratified | **RULED C2-R1b (ratified 2026-09-03)** — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b; existence and the anchor model: [`ARCHIVAL_PRUNED_DAEMON_MODE.md`](ARCHIVAL_PRUNED_DAEMON_MODE.md) `PDM-Q5` (RULED 2026-09-18), `PDM-Q11`; Rust home: [`CHAIN_RULES_SLICE_3.md`](CHAIN_RULES_SLICE_3.md) | Q2 crossing **fired** by the re-key: the table is Rust data (`const` per network, `PDM-Q5`'s "trusted with the binary" — never an operator-editable carrier) and the equality is a Rust predicate over the token's B6 identity. Reopening: an anchor entry ships (a data edit in `anchors.rs` that fails `no_release_has_shipped_an_anchor_yet` deliberately; `PDM-Q5`'s launch-window item). Prior: RC-16 merged |
