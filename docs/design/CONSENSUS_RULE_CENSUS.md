...
95 CHECKED-CONFORMANT, 5 DIVERGENT (CEN-L11 with CEN-L12 coupled; **CEN-D2 at
S1** with CEN-D1 coupled; CEN-B5's rule-71 FAKECHAIN skip), 2 failed closed
(CEN-L8; CEN-I12, split anchor source). **CEN-M8's S0 is fixed** (PR #602
merged) and M8/G4/J26 are promoted (round 6); every other row is UNREVIEWED. Bucket-4 rows record
...
| CEN-D2 | The longhash function is RandomX v2 via Rust FFI unconditionally (height/version ignored); on FFI failure the hash is forced to `0xff…` so it can never meet a target (fail closed; alt path pre-seeds `0xff…`) | cryptonote_tx_utils.cpp:748/:766; src/crypto/pow_registry.cpp:6; fail-closed :784–794; alt pre-seed blockchain.cpp:2325–2326 | C | 1 | spec | [`RANDOMX_V2_PLAN.md`](RANDOMX_V2_PLAN.md) ("C JIT for mining, Rust interpreter for verification — permanent") | RC-22 (hash half), RC-23 ⇒ merged. **P0f correction (round 4, 2026-09-03): the fail-closed claim is overstated** — `check_hash(0xff…, 1)` passes, both validation paths ignore the verifier's bool, and difficulty 1 is reachable (`--fixed-difficulty` on any nettype; no organic floor above CEN-D6's zero-guard), so the sentinel fails **open** at difficulty 1; CSR §5.4.1 CEN-D2 carries the walk (DIVERGENT, S1) |
