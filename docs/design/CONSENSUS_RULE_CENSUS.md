...
| F miner tx / emission | 22 | 13 | 4 | 1 | 4 | 0 |
| G block body (connect) | 14 | 7 | 2 | 1 | 4 | 0 |
| H tx non-input consensus | 24 | 9 | 7 | 1 | 7 | 0 |
...
deeper on `src/checkpoints/checkpoints.cpp` internals, `HardFork` internals, and the Rust
...
| `shekyl-economics` (reward, split, burn, release) | CEN-F13–F17, F20 | **Yes** (CEN read `rust/shekyl-economics/src/emission.rs`, `rust/shekyl-economics/src/release.rs`; 81-vector cross-language KAT) | `config/economics_params.json`; [`ECONOMY_EXPLAINED.md`](../ECONOMY_EXPLAINED.md) |
...
| `shekyl-pow-randomx` (hash); `shekyl-difficulty` (seed epoch — the schedule moved there 2026-09-19, E6 slice 2, so the validator adopts it without the engine) | CEN-D2, D3 | **Yes** (`shekyl-difficulty/src/seed_epoch.rs` read) | [`RANDOMX_V2_RUST.md`](RANDOMX_V2_RUST.md) :393, :790–823; [`RANDOMX_V2_PLAN.md`](RANDOMX_V2_PLAN.md) |
...
| `shekyl_tx_extra_shape_of` (over `shekyl-wire`'s `check_tx_extra_shape`) | CEN-I19, CEN-I20 | **Yes** (the rule is `check_one` over the two field-length lists, plus the coinbase grammar; every arm has a KAT, and the daemon logs the error type's own `Display` across the boundary rather than re-formatting it). The lengths-taking `shekyl_tx_extra_pqc_field_shape` and the C++ parser that fed it were deleted 2026-09-23 (`TX_EXTRA_RUST_CUTOVER.md`); this row records the walk, which stopped at that form. | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §9.6a |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| CEN-A1 | A block whose hash is already known (main, alt, or invalid set) is recorded as `ALREADY_EXISTS` | 3060–3095 (`have_block_unlocked` three-store membership; `m_invalid_blocks` is process-lifetime memory); consumer 6626 (`block_already_exists`) | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q3a (ratified/signed 2026-09-04) | RC-4 ⇒ merged. DB belt is CEN-L3. Ratified orchestration-with-belts; the invalid set consulted for dedup only (Q3a falsifier). Outcome byte, not the inherited bool, as of the PWD-B7 block twin |
| CEN-A2 | A block whose `prev_id` equals the current tail goes to the main-chain path; any other goes to the alternative path; main-chain connect re-checks `prev_id == top_hash` and fails closed | 6631 routing; 5741–5745 fail-closed re-check | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q3a (ratified/signed 2026-09-04) | RC-5 ⇒ merged (fail-closed re-check site added from RC). Ratified orchestration |
...
| CEN-A4 | A block whose parent is in neither main nor alt storage is marked orphaned and **not stored**; the p2p layer must re-request ancestry | 2509–2517 (orphan marking, :2512); sync-arm consumer protocol_handler.inl (Q3b fix); live arm :695 | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q3b (ratified/signed 2026-09-04) | RC-6 ⇒ merged. **Q3b: the sync-path punish consumer was RULED A DEFECT and FIXED in-round** (orphan = our-store state; re-walk via `request_chain_history`; the `:1407` bookkeeping-mismatch drop keeps its teeth). One flag no longer means two things |
...
| CEN-B4 | The block's `attestation_root` must equal the root recomputed from the (possibly empty) credit-wire witness, and every pass record's P-countersignature must verify against the bond's committed hybrid pubkey over `nonce[32] ‖ anchor_height_le[8] ‖ block_hash(anchor_height)[32] ‖ shard_id_le[8]` under `shekyl/archival-attestation-scheme-v2`, where `nonce`/`anchor_height` are carried on the record, the hash is read from the **connecting** chain (alt chain above the fork point), and `anchor_height ∈ [h − 720 − L, h − 720]` for validated predecessor `h` (`L = archival_attestation_anchor_lag_blocks`, 4 PROVISIONAL); no record is admissible below `720 + L` *(rule amended 2026-09-13 with PR #734, `SF-D8` v2: the row previously read "nonce binds coinbase `vout[0]` key and the validated `prev_id` (all-zero prev hash refused by the FFI)" — that v1 derivation and its FFI inputs are deleted, and the pinned line numbers predate the change)* | 5738 (main) / 2253 (alt) → `verify_block_attestation` 5565–5662; verdict Rust `shekyl_archival_verify_attestation` | C | 1 | spec | [`ARCHIVAL_CREDIT_WIRE.md`](ARCHIVAL_CREDIT_WIRE.md) §3–§4; RF-D3; [`ARCHIVAL_SHARD_FETCH.md`](ARCHIVAL_SHARD_FETCH.md) `SF-D8` | RC-12, RC-13 ⇒ merged. Enforcement status: the mechanism is the full recompute + countersignature verify; pre-population the only valid header value is the empty-set root (`ARCHIVAL_CREDIT_WIRE.md` :214–223), so the ratified-in-full admission recompute is *exercised* only on its empty-root slice until the cutover. The doc's §3 enforcement-status paragraph is stale (names the deleted `check_attestation_root`); spec content stands |
| CEN-B5 | At admission, before the block is added, the header's `curve_tree_root` must equal the tip root — the tree state at the block's own height, grown with every leaf that matured through its parent; mismatch rejects the block without connecting it *(wording corrected 2026-09-05 with the fix, §7 #18: the row previously said "after connect … grown with this block's outputs; mismatch pops", which was the defect, not the rule)* | 5885–5897 (at `a5c266125`) | C | 1 | spec | [`FCMP_PLUS_PLUS.md`](../FCMP_PLUS_PLUS.md) :264, :281; [`CURVE_TREE_CLIENT.md`](CURVE_TREE_CLIENT.md) | RC-14 ⇒ merged. FAKECHAIN skip retired 2026-09-05 (PR #623, §7 #18: the test generator computes real roots through the Rust client, so the check runs on every nettype); alt path defers to promotion (CEN-K5). **Second divergence found 2026-09-04 (§7 #17), graded S1:** the check compares the header against the root read *after* the add (`:6413`), but the template fills the header from the root *before* it (`:2006`) — the operands differ at every block where a leaf drains (observed on real LMDB: `archival_substrate_lmdb.cpp` `cen_b5_header_root_is_the_tip_root_before_add_block_not_after (renamed on PR #623 from `cen_b5_post_add_root_is_not_the_state_the_header_was_filled_from`, the name it carried when found)`), so on every non-FAKECHAIN nettype block index 60 is rejected and the chain halts; masked by the FAKECHAIN skip on the only chain type ever run past 59. Fixed on PR #623 (§7 #18). The rule's own wording is also loose — the tree grows with the leaves that *mature* at the block, never with the block's outputs, which are deferred |
...
| CEN-D2 | The longhash function is RandomX v2 via Rust FFI unconditionally (height/version ignored); on FFI failure the verifier's returned bool is the fail-closed gate — every consumer (main connect, alt path, longhash precompute worker) rejects on it, at **every** difficulty. The `0xff…` hash written on failure is a **belt** for display-only callers that ignore the bool, not the gate: `check_hash(0xff…, 1)` **passes**, so at difficulty 1 the sentinel alone fails open (corrected 2026-09-03; see the note) | src/cryptonote_core/cryptonote_tx_utils.cpp:257/:282 (were `:755/:791` before the 2026-09-23 tx_extra cutover deleted `construct_tx*` above them); src/crypto/pow_randomx.cpp:45; fail-closed :267–278; alt pre-seed blockchain.cpp:2201 | C | 1 | spec | [`RANDOMX_V2_PLAN.md`](RANDOMX_V2_PLAN.md) ("C JIT for mining, Rust interpreter for verification — permanent") | RC-22 (hash half), RC-23 ⇒ merged. **P0f correction (round 4, 2026-09-03) — the fail-closed claim as written in this row was overstated, and the code has since been FIXED (PR #604).** `check_hash(0xff…, 1)` passes, and the validation paths *used to* ignore the verifier's returned bool, so with difficulty 1 reachable (`--fixed-difficulty` on any nettype; no organic floor above CEN-D6's zero-guard) the sentinel failed **open** there. The verifier's bool now gates all three consumers, and CSR §5.4.1 CEN-D2 — **CHECKED-CONFORMANT** as of the 2026-09-03 re-review — carries both the original divergence walk (recorded there as history, graded S1) and the post-fix verification. **UPDATE 2026-09-15:** `pow_registry` deleted; dispatch is `hash_pow_randomx`. |
| CEN-D3 | RandomX seed block: epoch 2048 with lag 64 — `seedheight(h) = 0 for h ≤ 2112, else (h−65) & ~2047`; seed hash is the block id at that height (alt path: resolved along the alt chain when the seed height is on it). The `SEEDHASH_EPOCH_*` env override is refused on public networks (fakechain-only lever, init-time) | rust/shekyl-difficulty/src/seed_epoch.rs:49 (`seedheight`; constants `:37`/`:40` — moved from `shekyl-pow-randomx` 2026-09-19, E6 slice 2); env clamps at the FFI, rust/shekyl-ffi/src/pow_randomx_ffi.rs:299/:315; blockchain.cpp:6493, 2327–2345; env refusal 640–643 | C | 1 | spec | [`RANDOMX_V2_RUST.md`](RANDOMX_V2_RUST.md) :393, :790–823 (seed-epoch schedule spec — note it lives here, not in `RANDOMX_V2_PLAN.md` or `SPEC_ANCHORS.md`); [`RANDOMX_V2_PLAN.md`](RANDOMX_V2_PLAN.md) :171, :201 (non-tunable) | RC-22 (seed half), RC-25, RC-27 ⇒ merged |
| CEN-D4 | Next-block difficulty is LWMA-1 over the last N+1 (=91) blocks (Rust `lwma1_next_difficulty`); below N blocks of history the genesis difficulty constant applies — **`daa_genesis_difficulty` in `config/consensus_constants.json`, 400 since the 2026-09-11 testnet calibration** *[amended 2026-09-19, E6 slice 2 F13: this row read "(100)", the zawy12 historical pin `DAA_LWMA1.md` §2.6 records; the numeric value lives only in the JSON key and the row no longer repeats it. Distinct subject, already gated: the genesis **block's** own PoW difficulty is 1 (`shekyl-difficulty/src/consts.rs:94`–`:102` const-asserts `GENESIS_DIFFICULTY > 1` to keep the two apart; the validator judges block 0 at 1, `rules/difficulty.rs` `Target::GENESIS_BLOCK`)]*; target block time T = `SHEKYL_DAA_TARGET_SECONDS` (120) | marshal 130–200; callers 1190 (main) / 1512 (alt); rust/shekyl-difficulty/src/lwma1.rs | C | 1 | spec | [`DAA_LWMA1.md`](../completed/DAA_LWMA1.md) (ratified 2026-05-18); `config/consensus_constants.json` `daa_*` | RC-19, RC-20, RC-21 ⇒ merged. Bias 99/200, clamp 6, min-L 1/20 are deliberate bare literals per the Round-3 disposition |
| CEN-D5 | Alt-chain difficulty: the same LWMA-1 fed by a window stitched from main-chain prefix + alt-chain suffix, window ending at `bei.height−1`; a height-0 alt candidate sentinel-returns difficulty 0 (`CHECK_AND_ASSERT_MES` log-and-return at 1559–1563), which CEN-D6's zero guard then rejects | 1544–1638 | C | 2 | ratified | **RULED C2-R1b (ratified 2026-09-03)** — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b | Q1b: the window SELECTION crosses — `shekyl-difficulty::alt_window_plan` behind `shekyl_difficulty_alt_window_plan` (both regimes, one function; contiguity precondition fails closed to the D6 sentinel); C++ performs the fetches the plan names; vectors `FORK_CHOICE_V1.json`. Prior: RC-26 merged w/ recorded dissent |
| CEN-D6 | A zero next-block difficulty **rejects the block** — `CHECK_AND_ASSERT_MES` is log-and-return-false (`misc_log_ex.h:396`), not a process abort. *[amended 2026-09-20, DRS-E2 RD-F17: this row said the value "can only be 0 via a difficulty-function sentinel return (e.g. CEN-D5's height-0 arm)" — **false of the implementation**: LWMA-1 has no output floor (`shekyl-difficulty/src/lwma1.rs:176`–`:206`), and with every solvetime at the `+6T` clamp the formula is zero for `avg_D ≤ 6`, so a conforming slow chain derives it; once it does, every successor block is refused by this row. The Rust validator reproduces the refusal as the CEN-D6 verdict (`D6::mint`); whether the rule becomes a floor is the DAA owner's question — FOLLOWUPS]* | 5766 / 2324 | C | 2 | ratified | **RULED C2-R1b (ratified 2026-09-03)** — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b | Q1b: ratified as the marshaling belt on the FFI result at both call sites — a sentinel-eating shim would convert a difficulty-function failure into a silently accepted block; prior: C1 §7.14 correction |
...
| id | rule | site(s) | C/P | b | class | evidence | notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| CEN-E1 | A block connecting at an anchored height must carry the release-carried anchor's hash — the anchor's own rule (`PDM-Q11`), the `assumevalid` argument as a predicate. *[Re-keyed 2026-09-20 (E6 slice 3, `PDM-Q-F30`'s trigger: checkpoint state moved into Rust): the statement had read "hardcoded/**JSON-loaded** checkpoint hash" — the JSON path was deleted under `PDM-Q-F23` (#733); the C2-R0 HELD existence question is answered by `PDM-Q5` (a release-carried anchor `C`; runtime pins REJECTED). Every network's table is empty at this re-key (`ReleaseAnchors::for_network`, as `init_default_checkpoints` is), so the row is vacuous on every existing chain and its parity evidence is fixture-only.]* | **Rust:** `rust/shekyl-chain-rules/src/rules/anchors.rs` (`E1`, view-bound, in `validate`'s block list; recorded as evaluated at unanchored heights, which the C++ never records); anchors from `Trust` (`trust.rs`), table `rust/shekyl-chain-rules/src/anchors.rs` (the rule is `rules/anchors.rs`; the data is this one). **C++ (until cutover):** main `blockchain.cpp:5545`–`:5552` (`is_in_checkpoint_zone` → `check_block`, a lookup short-cut not a rule); alt `:2186`–`:2192` (a checkpoint match forces the reorg, `:2446` — an alt-chain consequence, homed with CEN-E2 in slice 9); `src/checkpoints/checkpoints.cpp:73` | C | 2 | ratified | **RULED C2-R1b (ratified 2026-09-03)** — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b; existence and the anchor model: [`ARCHIVAL_PRUNED_DAEMON_MODE.md`](ARCHIVAL_PRUNED_DAEMON_MODE.md) `PDM-Q5` (RULED 2026-09-18), `PDM-Q11`; Rust home: [`CHAIN_RULES_SLICE_3.md`](CHAIN_RULES_SLICE_3.md) | Q2 crossing **fired** by the re-key: the table is Rust data (`const` per network, `PDM-Q5`'s "trusted with the binary" — never an operator-editable carrier) and the equality is a Rust predicate over the token's B6 identity. Reopening: an anchor entry ships (a data edit in `rust/shekyl-chain-rules/src/anchors.rs` that fails `rust/shekyl-chain-rules/src/anchors_tests.rs`' `no_release_has_shipped_an_anchor_yet` deliberately; `PDM-Q5`'s launch-window item). Prior: RC-16 merged |
| CEN-E2 | An alternative block at or below the last anchor preceding the current height is refused (`is_alternative_block_allowed`); **home of `D_max`** — the same function grows a second band, the rolling reorg cap above the anchor floor (`PDM-Q11`, RULED 2026-09-17, [`ARCHIVAL_PRUNED_DAEMON_MODE.md`](ARCHIVAL_PRUNED_DAEMON_MODE.md) §2). *[Re-keyed 2026-09-20 (E6 slice 3, `PDM-Q-F30`): **no Rust site yet** — the Rust store admits no alternative block, and on the main chain the floor holds by construction (a candidate at `tip + 1` is above every anchor `≤ tip`), so the only refusing arm is alt admission.]* | **C++ (until cutover):** `blockchain.cpp:2105`–`:2110` → `src/checkpoints/checkpoints.cpp:97`–`:109`. **Rust:** none — `pending`, **subsumed behind two things**: the alt `ChainView` (slice 9; D5 waits on the same view) **and** `D_max`'s numeric (`PDM-Q11`: shape frozen, 720 **provisional**, re-pinned at the Round-2 testnet gate). The longer wait governs; "pending the alt view" alone would understate it. **Pinned 2026-09-25 (S-PRUNE, `DRS_E1_SPRUNE.md` SPR-8): when this arm is built it reads the cap through `RuleSet::reorg_cap()` — the in-force set's, `GENESIS` carrying `D_MAX`, a Fakechain set its own — never `shekyl_chain_rules::D_MAX` directly and never a store field; the store's undo retention is constrained by that same cap (`Horizons::new`, `connect`), which is what keeps a legal reorg clear of `PopBelowFloor` on every nettype.** | C | 2 | ratified | **RULED C2-R1b (ratified 2026-09-03)** — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b; `D_max` home: `PDM-Q11`; Rust disposition: [`CHAIN_RULES_SLICE_3.md`](CHAIN_RULES_SLICE_3.md) §2 | The C2-R0 HELD existence clause is answered by `PDM-Q5` (see CEN-E1). Q2 crossing for the Rust-held table fired with E1's re-key; this row's own crossing (the alt-height floor over a Rust alt view, `D_max`'s band) lands in slice 9. The alt-height floor stands as the checkpoint arm's admission fence; prior: RC-135 merged |
...
| CEN-E5 | **At writer open, the recorded chain must agree with the release-carried anchors at every anchored height at or below its tip**; a conflict below the tip pops until the chain's block count is `max(h − 2, 1)` (a `ChainCount`; the tip it leaves is `count.tip()`, so a conflict at height 1, 2, or 3 leaves genesis alone), a conflict **at genesis** refuses to run (the file is on the wrong network for the binary — no rollback can resolve it), and a rollback the store refuses (the prune watermark, `StoreCannot::PopBelowFloor`) refuses to run (C2-R1b F-1(b)). *[Heading re-keyed 2026-09-20 (E6 slice 3 F4, rule 91): the row led with "Removed" while sitting in bucket 2 — the **mechanism** (runtime `<data-dir>/checkpoints.json` pins, `--enforce-dns-checkpointing`-era) was removed under `PDM-Q-F23` (#733, 2026-09-13: under `PDM-Q5` a runtime-loadable pin is a trust channel that bypasses the release-carried anchor; the C2-R0 HELD existence question is answered *no*), and this **rule** is what survived — C2-R1b clause (3), run once at init over the compiled-in set. Not a denominator change; the survivor was ratified inside the removed shape, so the status column stays `—`.]* | **Rust:** `rust/shekyl-chain-rules/src/rules/anchors.rs` (`E5::conflict_with`, public as `ReleaseAnchors::conflict_with`; `AnchorConflict::remedy` → `Remedy::{RefuseToRun, PopTo}`); registry status `enforced_at` — enforced once, by the writer, not per block, so outside per-block coverage. **Remedy execution:** the writer's (the DRS-E2 ingest driver at open — FOLLOWUPS, owner DRS-E2). **C++ (until cutover):** `Blockchain::enforce_checkpoints` (`blockchain.cpp:6309`) → `check_against_checkpoints` (`:6225`), fail-stopping `core::init` (`cryptonote_core.cpp:660`); deleted with the mechanism: `load_checkpoints_from_json`, `core::update_checkpoints`, the ten-minute reload, `JSON_HASH_FILE_NAME`, the JSON-path tests and stubs | C | 2 | — | **RULED C2-R1b (ratified 2026-09-03)** while the mechanism existed — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b; existence ruled by [`ARCHIVAL_PRUNED_DAEMON_MODE.md`](ARCHIVAL_PRUNED_DAEMON_MODE.md) `PDM-Q5` item 3 / `PDM-Q-F23`; Rust home: [`CHAIN_RULES_SLICE_3.md`](CHAIN_RULES_SLICE_3.md) §2, Q4 | Q2a/Q2b's uniformity and JSON-conflict clauses are moot with the surface; Q2c's deletions stand; the rollback floor is a chain count of **1** (genesis alone — not tip height 1, and not `saturating_sub(pt.first, 2)` → 0; see `CONSENSUS_STORE_RECONCILIATION.md` CEN-E5 row-text correction), pinned in Rust by `the_remedy_refuses_at_genesis_and_pops_to_a_chain_count` |

### 4.F Miner transaction (structure and emission)

| id | rule | site(s) | C/P | b | class | evidence | notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| CEN-F1 | Coinbase has exactly one input, of type `txin_gen` | `prevalidate_miner_transaction` `blockchain.cpp:1393`–`:1394` (was `1642–1643`) | C | 4 | none | — | RC-33, RC-34 ⇒ merged. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: `rules::miner::F1` (form; slice 4) |
| CEN-F2 | Coinbase tx version ≥ 3 | `blockchain.cpp:1395` (was `1644`) | C | 2 | ratified | rule 60 (v3-from-genesis); genesis blob is v3 (`033c…`, cryptonote_config.h:368) | RC-35 ⇒ merged. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: **by construction** — `shekyl_wire::Transaction::read` admits `TX_VERSION` only (`RowStatus::ByConstruction`, falsifier `f2_the_wire_admits_one_transaction_version`; slice 4 Q4) |
| CEN-F3 | Coinbase CT type is `CTTypeNull` (no FCMP++ signature material) | `blockchain.cpp:1396` (was `1529`) | C | 2 | ratified | rule 60: "`CTTypeNull` for coinbase" | RC-36 ⇒ merged. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: `rules::miner::F3` (form) |
| CEN-F4 | Coinbase has exactly 1 output; genesis (caller-derived height 0) exempt; the height operand is caller-derived, not `txin_gen.height` (spoof closed by CEN-F5) | `blockchain.cpp:1424`–`:1426` (was `1546–1560`) | C | 1 | spec | [`ARCHIVAL_WORK_PRECISION_AND_ESCALATION.md`](ARCHIVAL_WORK_PRECISION_AND_ESCALATION.md) §12.3 (F-H); FOLLOWUPS "GENESIS-FREEZE: cap the coinbase output count" | RC-37 ⇒ merged. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: `rules::miner::F4` (validate; the height is `cx.connecting`) |
| CEN-F5 | `txin_gen.height` must equal the block's chain position | `blockchain.cpp:1428`–`:1433` (was `1561–1566`) | C | 4 | none | — | RC-38 ⇒ merged. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: `rules::miner::F5` (validate) |
| CEN-F6 | Coinbase `unlock_time` must equal height + `CRYPTONOTE_MINED_MONEY_UNLOCK_WINDOW` (60) | `blockchain.cpp:1434` (was `1567`); constant `cryptonote_config.h:45` (not `config/`) | C | 4 | pinned-not-re-derived | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §13 :907 (frozen domain constraint); [`CT2_DRAIN_ORDER.md`](CT2_DRAIN_ORDER.md) :248 ("60, not 10"); [`MERKLE_TREE.md`](../MERKLE_TREE.md) :201 acknowledges the CryptoNote inheritance | RC-39 ⇒ merged. Demoted from CEN's bucket 2 under the §2 bar: the 60 is frozen and documented, never re-derived. The same 60 drives coinbase tree maturity (CEN-L12). §10 R5. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: `rules::miner::F6` over `RuleSet::mined_money_unlock_window` (60; pinned to the `#define` by `the_unlock_window_is_the_cxx_define`, slice 4 Q5) |
| CEN-F7 | Coinbase output amounts must not overflow when summed | `blockchain.cpp:1437` (was `1686`) → `check_outs_overflow` `cryptonote_format_utils.cpp:885` (was `:899`) | C | 4 | none | — | RC-40 ⇒ merged. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: `rules::miner::F7` (form) |
| CEN-F8 | Coinbase outputs must be `txout_to_tagged_key` (view-tagged; sole output type) — same commitment as CEN-H12, enforced separately at the miner-tx surface (the coinbase never passes the H path) | `blockchain.cpp:1443` (was `1692`) → `check_output_types` `cryptonote_format_utils.cpp:999` (was `:971`; the dead hard-fork arms deleted by the slice-4 precursor) | C | 2 | ratified | as CEN-H12 | RC-41 ⇒ merged. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: **by construction** — `shekyl_wire::Output::read` admits the tagged-key tag only (falsifier `f8_the_wire_admits_one_output_tag`) |
| CEN-F9 | Coinbase output keys must be canonical, prime-order, non-identity points (Rust batch check) | `blockchain.cpp:1456` (was `1697`) → `check_outs_valid` `cryptonote_format_utils.cpp:816` (was `:837`) → `shekyl_check_output_keys` | C | 1 | spec | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §2.3 | RC-42 ⇒ merged. Miner tx never passes `check_tx_semantic`; this is the only site. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: `rules::miner::F9` (form; adopted `shekyl_ct_balance::check_output_keys`) |
| CEN-F10 | Coinbase commitment masks: **one mask per output** (`outPk.size() == vout.size()`), canonical prime-order, non-trivial (≠ identity, ≠ G), and ≠ `zeroCommit(amount)` (the amount-leaking fingerprint). *[Amended 2026-09-21 (E6 slice 4 §3.1 S25/S27): the arity clause was enforced at `check_commitment_mask_valid` but never stated here — and `src/blockchain_db/blockchain_db.cpp:573` cites the "four `outPk.size() != vout.size()` gates" as the ground for CEN-L11's unreachable grading, so an uncensused clause was load-bearing for a censused row. Also: whether the fingerprint gate applies was decided in C++ by `rv.type == CTTypeNull`; the selection is now Rust's (`shekyl_ct_balance::MaskSubject`), fed the type byte as a fact.]* | `prevalidate_miner_transaction` `blockchain.cpp:1421` → `check_commitment_mask_valid` `:3240`–`:3295` (facts-only marshal); Rust `shekyl_ct_balance::check_commitment_masks_for` via `shekyl_check_commitment_masks(ct_type, n_outputs, masks, n_masks, amounts)` | C | 1 | spec | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §2.3; arity + selection: [`CHAIN_RULES_SLICE_4.md`](CHAIN_RULES_SLICE_4.md) §3.1 | RC-43 ⇒ merged. Sole mask gate for `CTTypeNull`. Pins were `1699 → 3287–3341` at `02c086f4b` |
| CEN-F11 | Genesis (height 0) emission is accepted as configured (`GENESIS_TX` blob per nettype); structure validated, amount not recomputed | `validate_miner_transaction` `blockchain.cpp:1513`–`:1520` (was `1754–1761`); init `:381` (was `508–517`) | C | 1 | spec | genesis pipeline ([`GENESIS_TRANSPARENCY.md`](../GENESIS_TRANSPARENCY.md), [`GENESIS_ALLOCATIONS.md`](../GENESIS_ALLOCATIONS.md)) | RC-44 ⇒ merged. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: `rules::miner::F11` (definition — `Subsidy::Configured` at height 0) for the amount; the identity half is CEN-E1 at the height-0 anchor (`ReleaseAnchors` carries each network's genesis, slice 4 Q3) |
| CEN-F12 | **Deleted (E6 slice 4 Q2, ruled (a) 2026-09-21).** Was: decomposed-denomination check on coinbase outputs — **dead code**: the gate was `if (version == 3)` where `version` is the *hard-fork* version parameter, always 1, so `is_valid_decomposed_amount` and its CryptoNote denomination table `valid_decomposed_outputs[]` never ran. Rule 60's instruction (*"when you encounter `if (version < N)` … delete the dead branch"*): the branch, the predicate, the table, its header declaration and the test that exercised only them (at `ac5bad744`, the deleting commit's parent: `tests/unit_tests/canonical_amounts.cpp`) are deleted in the same PR; the census amendment and the deletion are one change, so no reader can find a bucket-3 row with a live symbol behind it. Load-bearing dead: with the 1-output cap (CEN-F4) and exact payout (CEN-F18) the single output is `emission+fees`, essentially never a table denomination — a Rust port implementing this arm as live would reject nearly every block (which is why (c), a vacuous `implemented`, was refused: a fixture that cannot fire). | deleted with its selfsame PR (`92a1a5787`) — was, at `ac5bad744` (that commit's parent, the last tree that had them): `validate_miner_transaction` `blockchain.cpp:1523`–`:1530` (gate); `is_valid_decomposed_amount` in `src/cryptonote_basic/cryptonote_format_utils.cpp` (line 1588 there; `:1547` at the P0f pin — the function moved before it was deleted. Path and line are written apart on purpose: `check_doc_claims.py` reads a rooted `path:line` at HEAD only and would count a correctly era-pinned records-was cite as rot); table `:55`–`:76` | C | 3 | — | [`CHAIN_RULES_SLICE_4.md`](CHAIN_RULES_SLICE_4.md) §8 Q2 | RC-45, RC-46, CEN §6.8/§6.10 ⇒ merged; adjudicated by C1 steering: **CEN-F12's demotion stands**, and #583 corrected RC-45's first-draft "always-on" reading in-PR ("keys off HF version"). Examined-and-mooted trail: `GENESIS_TX_WIRE_FORMAT.md` :454 (Q2) sheds the *chunking*, not this gate; `ARCHIVAL_WORK_PRECISION_AND_ESCALATION.md` :973 + §12.3 F-H recommends the 1-output cap. Was bucket 4 / class `none` from the merge until the deletion |
| CEN-F13 | Base subsidy: `(MONEY_SUPPLY − already_generated) >> 21`, floored at tail subsidy 600 000 000 atomic/block; computed in Rust (`shekyl-economics::block_reward_with_penalty`), C++ is a marshaling shim | `blockchain.cpp:1529` (was `1776`) → `cryptonote_basic_impl.cpp` → `rust/shekyl-economics/src/emission.rs:79` (`base_block_reward`; was `:142`) | C | 1 | spec | `config/economics_params.json`; [`ECONOMY_EXPLAINED.md`](../ECONOMY_EXPLAINED.md); 81-vector cross-language KAT | RC-48 ⇒ merged (curve legs split out as RC-169–172 map to F14/F15/F20). **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: `rules::miner::F13` (definition, `Emission::derive`; recorded in coverage, off the verdict until F14b's paid reward) |
...
| CEN-F14b | The subsidy penalty curve: median soft-raised to the 300 000-byte free zone; weight ≤ median ⇒ full subsidy; median < weight ≤ 2·median ⇒ quadratic penalty `base·(2m−w)·w/m²` in u128 (fail-closed on overflow) — the inherited ArticMine/CryptoNote curve shape, ported to Rust under KAT | emission.rs:150–176 | C | 2 | ratified | `docs/CHANGELOG.md` :1178 (81-vector KAT asserted from both languages); [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md) Q4 (shape derived; identity to shipped formula shown) | Split from CEN-F14 in C1. Survey A claimed the penalty was "examined in the A3 fee round" — **no locatable A3-round record exists** (searched docs/completed/, docs/design/, decision log); the locatable trail is the KAT plus the submit-path fee exam (CEN-M3), so the class is KAT-port, not ratified. §10 R2. **R2 SIGNED** — [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md); GAP-7 verification-cost condition DISCHARGED passing: zone-point worst cold block 7.23 s ≈ 6.0 % of T, measured on the Pi 4 floor (the floor figure, per the signature's condition) |
| CEN-F15 | Release-rate multiplier: subsidy scaled by `clamp(tx_count_sum·10⁶/(baseline·blocks), 0.8·10⁶, 1.3·10⁶)/10⁶` — one division, exact window mean (FL-R24) — then capped to remaining supply (saturating; defends uint64 underflow past the cap) | cryptonote_basic_impl.cpp (`get_block_reward` 6-arg shim); rust/shekyl-economics/src/release.rs; rust/shekyl-economics/src/volume.rs (`TxVolume::ratio_scaled`) | C | 1 | spec | `config/economics_params.json` (`shekyl_release_min/max`, `shekyl_tx_volume_baseline` 50); [`ECONOMY_EXPLAINED.md`](../ECONOMY_EXPLAINED.md); [`FEE_LADDER_DERIVATION.md`](FEE_LADDER_DERIVATION.md) §11.6 PR A, §11.7 (FL-R24 evidence) | RC-170, RC-171 ⇒ merged. **UPDATE 2026-09-10 (FL-R24, PR A):** operand was `tx_volume_avg·10⁶/baseline` with `tx_volume_avg` the *floored* window mean; the floor was a quantizer on `M_r` (one tick = 1/V) and is removed — mid-curve at mean 40.5 the reward moves 819 200 000 000 → 829 440 000 000 (KAT `fl_r24_exact_window_mean_moves_the_reward_off_the_truncated_value`). Rust: `rules::miner::F15` (definition — `effective_emission` over F20's window; slice 4) |
| CEN-F16 | Emission split: subsidy divides into miner and staker legs (`shekyl-economics::compute_emission_split`, **the one owner** — zero-emission arm, the effective share at the height from CEN-F21's epoch, and the split); the coinbase may pay only the miner leg. *[Amended 2026-09-21 (E6 slice 4 §3.1 S4/S6): the row said "Rust", but the zero arm and the share→split composition lived in the C++ shim `economics.h`, with the three constants marshaled from `cryptonote_config.h`; the shim decided the zero case before Rust ran. All three are the Rust function's now; the shim marshals one call.]* | `validate_miner_transaction` `blockchain.cpp:1544` (was `1668–1674`); accrual `:5913`; template via `construct_miner_tx`; all through `shekyl::compute_emission_split` (`economics.h`) → `shekyl_compute_emission_split` | C | 1 | spec | [`ARCHIVAL_BUDGET_SCHEDULE.md`](ARCHIVAL_BUDGET_SCHEDULE.md) §2.2 (F-B1c operand discipline); [`CHAIN_RULES_SLICE_4.md`](CHAIN_RULES_SLICE_4.md) §3.1 | RC-49 ⇒ merged |
| CEN-F17 | Fee burn split: fees divide into `miner_fee_income` / staker pool / destroyed (`shekyl-economics::compute_fee_burn`, **the one owner** — zero-fee arm, percentage and escalated split; operands: fee sum, the exact volume window, **circulating supply = `coins_generated − total_burned`** (checked; `total_burned > coins_generated` is a store-invariant violation, never a zero), frozen segment count at parent state); the coinbase may pay only `miner_fee_income`. *[Amended 2026-09-21 (E6 slice 4 §3.1 S1/S3/S15): (i) the row said "circulating supply" without its definition, and the C++ defined it at two call sites as `already_generated_coins` — **gross** emission ignoring burn. That is **FL-R16c's ruled definitional defect** (`FEE_LADDER_DERIVATION.md` §8, review round 4, "binds the implementing PR"), rediscovered by the slice-4 sweep as a fresh finding because the ruling's line pins (`:1787`, `:2074`) had drifted to `:1534`, `:1821`. Built by the precursor: the definition is Rust's, derived once from two store facts. (ii) The zero-fee arm and the pct→split composition lived in the C++ shim `economics.h`, deciding the outcome before Rust ran; both are `compute_fee_burn`'s.]* | `validate_miner_transaction` `blockchain.cpp:1551` (was `1675–1681`); accrual `:5916`; template via `construct_miner_tx`; all through `shekyl::compute_fee_burn` (`economics.h`, facts-only marshal) → `shekyl_compute_fee_burn` | C | 1 | spec | `config/economics_params.json` `shekyl_burn_*`; supply definition **RULED** FL-R16c ([`FEE_LADDER_DERIVATION.md`](FEE_LADDER_DERIVATION.md) §8); [`ECONOMY_EXPLAINED.md`](../ECONOMY_EXPLAINED.md); [`ARCHIVAL_WORK_PRECISION_AND_ESCALATION.md`](ARCHIVAL_WORK_PRECISION_AND_ESCALATION.md) §12.11.1 | RC-50 ⇒ merged. The relay-floor ring and fee estimate still form `C` over the gross operand (no per-height cumulative-burn fact exists) — recorded on FL-R16c for the fee-ladder lane |
...
| CEN-F19 | `frozen_segment_count` (the D2 escalation operand) must be read at parent state: `m_db->height()` must equal the block's height at the read point or the node halts; verify's operand IS the accrual's operand (single-read discipline) | `parent_frozen_segment_count` `blockchain.cpp:1491`–`:1503` (was `1732–1743`); read once at `:5814` (was `6271`), shared by verify and accrual | C | 1 | spec | [`ARCHIVAL_WORK_PRECISION_AND_ESCALATION.md`](ARCHIVAL_WORK_PRECISION_AND_ESCALATION.md) §12.12 :2121 (the read-point ruling — the "single asserting reader"; note it is §12.12, not §6.2) | RC-52 ⇒ merged. **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: **by construction** — `validate` runs inside the write transaction over a `ChainView<'id>` that *is* parent state; falsified by the `compile_fail` doctests on `validate` (`doctest:validate`) |
| CEN-F20 | The volume operand is the exact window `(tx_count_sum, blocks)`: `tx_count_sum` = Σ `tx_hashes.size()` over the prior `min(height, SHEKYL_TX_VOLUME_WINDOW)` (720) blocks, `blocks` that count; height 0 → `(0, 0)`; the pair crosses the FFI undivided and Rust forms `sum/(baseline·blocks)` once (FL-R24) | blockchain.cpp (`get_tx_volume_window`; callers `validate_miner_transaction`, `create_block_template`, fee estimate, accrual); src/shekyl/tx_volume_window.h; rust/shekyl-economics/src/volume.rs | C | 1 | spec | `shekyl-economics` activity spec; [`FEE_LADDER_DERIVATION.md`](FEE_LADDER_DERIVATION.md) §11.6 PR A; window 720 is a consensus-visible constant **not** in `config/` (RC decision-log #10 class) | [m] RC-172 ⇒ minted. **UPDATE 2026-09-10 (FL-R24, PR A):** was "`tx_volume_avg` is the integer mean … `tx_count_sum / blocks`"; the function is renamed `get_tx_volume_window` because it no longer returns a mean, and the C++ side is forbidden to divide the pair (comment at the scan). Rust: `rules::miner::F20` (definition — two `RecordedBlock.cumulative_tx_count` reads, never `W`; a decreasing prefix sum is `Corrupt::TxCountNotMonotone`, slice 4) |
| CEN-F21 | `genesis_ng_height` at connect is **1**: `get_earliest_ideal_height_for_version(HF_VERSION_SHEKYL_NG)` returns the table row's height 1, not 0; genesis (height 0) is `original_version` 1 but not that lookup — the emission-split operand is therefore 1 | `src/cryptonote_basic/hardfork.cpp:383–394`; used `blockchain.cpp:1536`, `:1817`, `:5893` (were `1783`, `6345`) | C | 1 | spec | v3-from-genesis table; economics.h deleted-gate comment (RC-182's evidence) | [m] RC-182 ⇒ minted. Independently ratifiable from "NG from genesis". **Pins re-resolved 2026-09-22 (E6 slice 4 landing; every 4.F pin had drifted from `02c086f4b`).** Rust: `rules::miner::EMISSION_SPLIT_EPOCH` (1), **by construction**, pinned to the hardfork tables by `the_emission_split_epoch_is_the_hardfork_tables_first_row` (slice 4 Q5; amended 2026-09-23 — a `RuleSet` field when a schedule step names a different epoch, not before) |
...
| CEN-G2 | Every listed tx hash must resolve from the mempool or the block's pool supplement; otherwise record `MISSING_TXS` and do not connect (taken pool txs are returned on any failure). Not a rejection and not a drop | 5992–6028 | C | 4 | none | — | tx content ↔ hash agreement is established here by lookup-under-computed-hash; the DB layer trusts it (§6, CEN-L4). Outcome byte, not the inherited bool, as of the PWD-B7 block twin |
...
| CEN-G6 | Weight-limit frozen bounds: long-term window 100 000 blocks; short-term surge factor **ratified S = 4** (Q3 re-derivation, d24 denomination — the shipped C++ constant is the refuted ×50, a known divergence until the store port implements the signed value) | 6580, 6593; constants cryptonote_config.h | C | 2 | ratified | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §10 :803 (block-level bounds table — a freeze, not a derivation); [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md) Q2 (window) + Q3 (surge struck) | Demoted from CEN's bucket 2 under the §2 bar: the freeze record exists; no examination of the ×50 or the window value does. §10 R2. **R2 SIGNED** — [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md); GAP-7 verification-cost condition DISCHARGED passing: zone-point worst cold block 7.23 s ≈ 6.0 % of T, measured on the Pi 4 floor (the floor figure, per the signature's condition). **This row's ×50 surge factor is the value Q3 STRUCK — refuted by the GAP-7 floor measurement, re-derived and SIGNED at ×4 (d24 denomination); the 100 000 window is ratified, the ×50 is not ratified-as-was, it is REPLACED** |
| CEN-G6b | Weight-limit remainder: effective median = clamp(short-term median, long-term effective median, S×long-term) with **ratified S = 4** (shipped C++: the refuted 50 — divergent until the port), floored at the 300 000-byte penalty-free zone (`…FULL_REWARD_ZONE_V5`); **limit = 2 × median**; long-term weight = weight clamped to [median·10/17, median·1.7]. `get_min_block_weight(version)` ignores its version argument and always returns the V5 zone | 6440, 6568–6607, 6548–6566, 1809–1870; vestigial version arg cryptonote_basic_impl.cpp:81–85 | C | 2 | ratified | —; [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md) Q1 (zone; GAP-7 discharged on the floor) + Q2 (clamps) | RC-133, RC-181 ⇒ merged. The V5 reward-zone value's arbitration was explicitly punted "to the economics doc" (`GENESIS_TX_WIRE_FORMAT.md` :806–811, the "fossil flag") and never landed anywhere — no economics doc, decision-log entry, or (pre-#584) FOLLOWUPS row received it; 300 000 is consumed as given at [`ARCHIVAL_SETTLEMENT_WRITER.md`](ARCHIVAL_SETTLEMENT_WRITER.md) :130 and decision log :4686. The 1.7× clamps ride along unexamined. FOLLOWUPS row (owner: this doc). §10 R2. The reward-side 2·median rejection is CEN-F14. **R2 SIGNED** — [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md); GAP-7 verification-cost condition DISCHARGED passing: zone-point worst cold block 7.23 s ≈ 6.0 % of T, measured on the Pi 4 floor (the floor figure, per the signature's condition) |
...
| id | rule | site(s) | C/P | b | class | evidence | notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| CEN-H1 | Serialized tx size ≤ `CRYPTONOTE_MAX_TX_SIZE` (1 000 000 bytes) | tx_verification_utils.cpp:66; cryptonote_core.cpp:711; cryptonote_basic_impl.cpp:88–90 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 2 | ratified | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §10 :790 (frozen limit); [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md) Q6 (parse-bound job stated) | RC-53 ⇒ merged. Demoted from CEN's bucket 2 under the §2 bar: frozen, never derived. Constant hand-maintained (cryptonote_config.h:42), not `config/`. **R2 SIGNED** — [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md); GAP-7 verification-cost condition DISCHARGED passing: zone-point worst cold block 7.23 s ≈ 6.0 % of T, measured on the Pi 4 floor (the floor figure, per the signature's condition) |
...
| CEN-H3 | Tx weight ≤ half the minimum block weight minus coinbase reserve (= 149 400) | tx_verification_utils.cpp:82, 203–210 | C | 2 | ratified | [`DAEMON_SUBMIT_VERDICT.md`](DAEMON_SUBMIT_VERDICT.md) §8 row N3 :1271 — examined on the **submit path** only; [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md) Q7 (formula + both paths on record) | RC-69 ⇒ merged. Demoted from CEN's bucket 2 under the §2 bar; the block-connect side of the same limit was never separately examined, and the limit formula is inherited scaling. §10 R2. **R2 SIGNED** — [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md); GAP-7 verification-cost condition DISCHARGED passing: zone-point worst cold block 7.23 s ≈ 6.0 % of T, measured on the Pi 4 floor (the floor figure, per the signature's condition) |
| CEN-H4 | Non-coinbase tx must have ≥ 1 input | cryptonote_core.cpp:757–763 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 4 | none | — | RC-57 ⇒ merged |
| CEN-H5 | Input-variant whitelist: `txin_gen` forbidden outside coinbase; only `txin_to_key` and the three archival variants (`serve_credit_response`, `bond_post`, `reward_emission`) are accepted; `txin_to_script`/`txin_to_scripthash` rejected everywhere (incl. the double-spend visitor and the DB whitelist) | cryptonote_format_utils.cpp:513–574 (`check_inputs_types_supported` :490); blockchain.cpp:3379–3384 (classification), 3539–3544 (the non-FCMP reject); blockchain_db.cpp:398–402 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 1 | spec | archival vin taxonomy: [`REWARD_EMISSION_E3_GATING_ROUND.md`](../completed/REWARD_EMISSION_E3_GATING_ROUND.md) Q3/Q11; rule 60 "Accepted transaction types" | RC-58, RC-59 ⇒ merged. **Promoted from CEN's bucket 4**: the whitelist is the E3-ruled vin taxonomy (RC's reading, evidence located). The script-variant *type fossils* remain rule-60 deletion residue — §10 R5, and the double-spend visitor joins them. **P0f correction (2026-09-03):** the visitor cited here is **dead** (only call commented out at `blockchain.cpp:6106` — as this census's own §5 item 3 and CEN-L1's row already record) and its body accepts `txin_gen`; live enforcement is site 1 (relay admission + pool-supplement connect via `ver_non_input_consensus`), `check_tx_inputs`' typed dispatch at connect, and the DB backstop for script variants — CSR §5.4.1 CEN-H5 carries the corrected walk |
| CEN-H6 | Archival vin mixing: serve-credit vins mix with nothing; ≤ 1 bond post; ≤ 1 emission; emission and bond-post never co-reside; key-imaged `txin_to_key` are the only permitted co-residents of a bond post or emission (single-sourced in `classify_archival_tx`) | cryptonote_format_utils.cpp:542–573; cryptonote_basic.h:353 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 1 | spec | E3 §2.1 (Q3 arity), §2.2 (Q11 mixing), §9.5 item 4 | RC-60, RC-61 ⇒ merged |
| CEN-H7 | Every output must yield a canonical, prime-order, non-identity output key (Rust batch `shekyl_check_output_keys`) | cryptonote_format_utils.cpp:577–606 (`check_outs_valid` :554); via cryptonote_core.cpp:773 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 1 | spec | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §2.3; output-point validity ratified 2026-07-22 (:404–408) | RC-62 ⇒ merged. Stricter than the inherited `check_key` |
| CEN-H8 | For v>1: `outPk` count == vout count (re-checked locally by the mask gate, and at parse/expand) | cryptonote_core.cpp:780–789; blockchain.cpp:3962 (emission arm); cryptonote_format_utils.cpp:137–140 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 4 | none | — | RC-55, RC-63 ⇒ merged |
| CEN-H9 | Output amounts must not overflow on summation; input-side sum covers only `txin_to_key.amount` — archival vins exempt (their value is CT-side/opaque) | cryptonote_core.cpp:808–814; cryptonote_format_utils.cpp:609–660 (`check_money_overflow` :593, `check_outs_overflow` :623) (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 4 | none | — | RC-64 ⇒ merged. Amounts on `txin_to_key` are 0 under CT; a leftover overflow belt |
...
| CEN-H12 | Every output must be `txout_to_tagged_key` (view tag required; sole output type from genesis) | cryptonote_format_utils.cpp:717–747 (`check_output_types`); sites cryptonote_core.cpp:843, blockchain.cpp:3317, 1443 (miner) (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 2 | ratified | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §2.2 :308–309 ("`view_tag` becomes mandatory"; "The **sole** genesis output type"); [`FA-6_VIEW_TAG_ML_KEM.md`](FA-6_VIEW_TAG_ML_KEM.md) (S1–S9 closed: S1–S4/S6–S9 signed 2026-06-02, S5 ratified same date); DSV matrix M9/O5 | RC-78 ⇒ merged. Dead `hf > / < VIEW_TAGS` grace arms (cryptonote_format_utils.cpp:985–1001) are rule-60 deletion residue (RC-167 ⇒ merged here). The ML-KEM tag *derivation* is FA-6's wallet-side surface |
| CEN-H13 | Non-coinbase tx version < 3 rejected at output check (dup of CEN-H2, third site) | blockchain.cpp:3272–3277 (`check_tx_outputs` :3268) (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 2 | ratified | as CEN-H2 | RC-73 ⇒ merged |
| CEN-H14 | All vout amounts must be 0 (confidential) — except a well-formed archival **emission** tx, whose non-zero ("loud") vouts are the reward commit set | blockchain.cpp:3286–3298 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 1 | spec | zero-amount base: [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §9.6 :769; loud-emission exemption: [`REWARD_EMISSION_LEG.md`](REWARD_EMISSION_LEG.md) §5.5 :494–503 ("emission mint is not confidential") — NOT `REWARD_EMISSION_VIN_PLAN.md` §4 (PR sequencing; the in-code pointer is broken, §7) | RC-74 ⇒ merged. Uses `classify_archival_tx`, not a bare vin count |
| CEN-H15 | CT type must be `CTTypeNull` or `CTTypeFcmpPlusPlusPqc`; anything else rejected (again in the batch verifier; `CTTypeNull` re-rejected for non-coinbase at input check) | blockchain.cpp:3300–3306; tx_verification_utils.cpp:221–239 (`case CTTypeNull` :223, unconditional); blockchain.cpp:3550–3554 (`case CTTypeNull`, unconditional — the `m_nettype != FAKECHAIN` gate at :3396–3402 is CEN-I2's earlier refusal, and the `else` at :3539–3544 closes it for `Null` on Fakechain too) (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 2 | ratified | rule 60 ("Accepted transaction types … Anything else is a consensus error") | RC-75, RC-70 (type half) ⇒ merged. **E6 slice 5 Q9 (RULED 2026-09-23): the Rust rule (at `4dc5194de`, the slice-5 merge that landed the file — the row's `d8ebfd18c` era predates it: `rust/shekyl-chain-rules/src/rules/tx.rs` `H15`) is unconditional. A same-day DIVERGENT grading was REFUTED at `d8ebfd18c`:** the `m_nettype != FAKECHAIN` gate at `blockchain.cpp:3396–3402` is CEN-I2's earlier refusal, not this row's; this row's pinned site `ver_non_input_consensus` (`tx_verification_utils.cpp:223`) and `check_tx_inputs`'s `switch` (`blockchain.cpp:3550–3554`) refuse `CTTypeNull` for a non-coinbase **unconditionally**, and the `else` at `:3539–3544` refuses a `Null` spend on Fakechain before either. Rust and C++ agree on every nettype — CSR-3a `CHECKED-CONFORMANT` (the gate was read, the belt below it was not; rule 16's corollary). The Fakechain gate itself is still rule 71's prohibition verbatim, owned by CEN-I2/I3/I4 ("FAKECHAIN exempt"), and TXE-Q1's builder deletion is what removes its reason to exist. The type-set half holds `by_construction` on `shekyl_wire::Ct` |
| CEN-H16 | `unlock_time` must be < `CRYPTONOTE_MAX_BLOCK_HEIGHT_SENTINEL` (500 000 000): timestamp-based unlocks are consensus-rejected; height form otherwise unconstrained for non-coinbase | blockchain.cpp:3308–3313 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 2 | ratified | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) :888–891 (ratified creation cut); Decision 13 (`docs/CHANGELOG.md` :26409–26411); decision-log :4325–4327 (height-not-manipulable-time rationale) | RC-77 ⇒ merged. The unlock_time triple-divergence (consensus-legal / relay-illegal CEN-M5 / tree-inert CEN-L12) is a §6 finding |
| CEN-H17 | Output commitment masks (all txs): canonical prime-order, non-trivial; coinbase additionally ≠ `zeroCommit(amount)` | blockchain.cpp:3215–3266 (`check_commitment_mask_valid`; sites 3325, 1458 miner) (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 1 | spec | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §2.3; Rust `shekyl-ct-balance` | RC-79 ⇒ merged |
| CEN-H18 | CT cleartext balance: `sum(pseudoOuts) = sum(outPk masks) + fee·H`, canonical points, verified in Rust (`shekyl_verify_ct_balance`), single-sourced with the builder | src/fcmp/ct_semantics.cpp:160–186 (`verCtSemanticsSimple`; `shekyl_verify_ct_balance` :176) (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 1 | spec | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §2.3; site comment | RC-70 (balance half) ⇒ merged |
| CEN-H19 | Bulletproof+ layout must be canonical (`nbp == 1`, length exact by `nout`, `L.size() ≥ 6`, max amounts ≥ vout count, V reconstructed as `mask·INV_EIGHT`) and the aggregate range proof must verify (batched across the block's txs) | ct_semantics.cpp:187–194 (`verBulletproofPlus` over the batch), 230–235 (the archival helper's layout check); ct_types.cpp:240 (`is_canonical_bulletproof_plus_layout`); src/fcmp/bulletproofs_plus.cc; expand-time structural half cryptonote_format_utils.cpp:170–192 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 4 | none | layout bound frozen: [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §10 (canonical-form corollary rows) | RC-56, RC-97, RC-70 (BP+ half) ⇒ merged. The verifier is the largest inherited-crypto implementation still on the acceptance path; [`CPP_INHERITANCE_INVENTORY.md`](../CPP_INHERITANCE_INVENTORY.md) :190 carries only a **tentative categorization pending re-verification** — no disposition. Serve-credit exempt (RF-D9). §10 R6 |
| CEN-H20 | Serve-credit-only tx CT shape: no `pqc_auths`, no vouts, zero fee, no outPk/BP+/proof/pseudoOuts, type `CTTypeFcmpPlusPlusPqc`, and the fee-only balance `sum(masks) + fee·H = identity` holds vacuously | tx_verification_utils.cpp:113–131; ct_semantics.cpp:326–355 (`verCtSemanticsFeeOnly`); re-checked blockchain.cpp:3580–3669 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 1 | spec | gate-2 §5 ([`ARCHIVAL_RETENTION_GATE2.md`](../completed/ARCHIVAL_RETENTION_GATE2.md)); [`ARCHIVAL_RESPONSE_FORMAT.md`](ARCHIVAL_RESPONSE_FORMAT.md) | RC-109 ⇒ merged |
| CEN-H21 | Bond-post tx CT shape: `pqc_auths` == vin count, ≥ 1 spend (funding) input, pseudoOuts == spend count, non-empty FCMP++ proof, CT balance with (credit, debit) = (`bond_credit`, `bond_debit`); funding inputs: empty offsets, unspent KIs, FCMP++ over the spend subset (same ref-age/depth as CEN-I10–I15) | tx_verification_utils.cpp:132–148; ct_semantics.cpp:269–290 (`verCtSemanticsBondPost`; shared tail :221–267); funding blockchain.cpp:3465–3485, 3670–3797 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 1 | spec | [`ARCHIVAL_BOND_GATE4.md`](ARCHIVAL_BOND_GATE4.md) | RC-119, RC-120 ⇒ merged. The funding-floor C++ copy is disclosed as pending a Rust single-source (:3772–3779) |
| CEN-H22 | Emission tx CT shape: reward total = checked sum of vout amounts, must be > 0; proof presence ⇔ fee inputs present; pseudoOuts == fee-input count; balance `sum(pseudoOuts) + total_reward·H = sum(masks) + fee·H` (mint rides the debit slot) | tx_verification_utils.cpp:149–181; ct_semantics.cpp:292–324 (`verCtSemanticsEmission`) (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 1 | spec | E3 §2.2 (`verCtSemanticsEmission` KATs); [`DAEMON_SUBMIT_VERDICT.md`](DAEMON_SUBMIT_VERDICT.md) :1445 row EV4 | RC-127 ⇒ merged. Rust checked-sum shared with CEN-J24 |
| CEN-H23 | A tx must deserialize via `binary_archive` and `expand_transaction_1` must succeed; failure rejects | cryptonote_format_utils.cpp:178–200 (`parse_and_validate_tx_from_blob`), 202–209 (`…_base_from_blob`), 109–196 (`expand_transaction_1`) (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 4 | none | — | [m] RC-54 ⇒ minted. Wire-format boundary twin of CEN-A6; the format itself is [`BLOCK_TX_WIRE_FORMAT_PORT.md`](BLOCK_TX_WIRE_FORMAT_PORT.md)'s subject |
| CEN-H24 | Ring-members residue check: for each `txin_to_key`, no relative `key_offset` after the first may be 0 ("duplicate ring members"); archival vins skipped; the unused `hf_version` parameter is ring-era residue | cryptonote_core.cpp:945–961 (`check_tx_inputs_ring_members_diff`); semantic caller :827 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 3 | — | rule 60 **names mixin enforcement** in its deleted-from-construction list; FCMP++ requires empty `key_offsets` (CEN-I6), so this can only fire before CEN-I6 rejects the same tx | [m] RC-66 ⇒ minted at RC's bucket: a live-but-redundant mixin-era check with a named deletion disposition. §10 R5 executes. **Falsifier (E6 slice 5 Q7, amended 2026-09-23):** `h24_cannot_fire_on_an_input_i6_admits` (at `4dc5194de`, the slice-5 merge that landed the file: `rust/shekyl-chain-rules/src/rules/tx_tests.rs`) — the residue predicate is vacuous on the empty offset list and fires on a zero relative offset, and `tx_form` still accepts that fixture. The acceptance is the gap; the `expect` fails on its own the day a row refuses it. Bucket 3 keeps this row out of the registry and every count; that test and this cell are the only two things that index it |
...
| CEN-I2 | Non-coinbase txs must be `CTTypeFcmpPlusPlusPqc` (ring-based inputs unrepresentable; non-FCMP/non-archival classified txs reject). **Not FAKECHAIN-exempt** — corrected 2026-09-26 (E6 slice 6 §3.4, read at `4dc5194de`): the gate at the carve-out only chooses *which* refusal fires; the `else` beneath it refuses a non-FCMP transaction on every nettype, so behaviour does not vary. The cell read "the FAKECHAIN carve-out at 3476 exempts this row **and** CEN-I3/I4"; of the three, only I4's behaviour varies by nettype (see its cell) — the guard variant of rule 16's corollary, a conditional check read as evidence of conditional behaviour | 3476–3483, 3615–3620 | C | 2 | ratified | rule 60; [`FCMP_PLUS_PLUS.md`](../FCMP_PLUS_PLUS.md) | RC-76, RC-86 ⇒ merged |
| CEN-I3 | Tx version exactly 3 (min == max == 3, hardcoded independently of the HF dispatch). **Not FAKECHAIN-exempt** — corrected 2026-09-26 (E6 slice 6 §3.4): `ver_non_input_consensus` bounds the version unconditionally at both of its sites; the cell read "FAKECHAIN exempt" | 3493–3506 | C | 2 | ratified | as CEN-H2 (Q12) | RC-82 ⇒ merged. Drift pair with CEN-H2's table form — FOLLOWUPS row repointed here |
| CEN-I4 | ≤ `FCMP_MAX_INPUTS_PER_TX` (8) inputs — the cap covers total `vin.size()`, not just the key-image subset. **C++: FAKECHAIN exempt** (the check sits inside `if (m_nettype != FAKECHAIN)`); **Rust: unconditional** (`rules/tx_inputs.rs::I4`, E6 slice 6 commit 2, rule 71) — the one deliberate nettype divergence on the transaction path, graded DIVERGENT in `CONSENSUS_STORE_RECONCILIATION.md` with identity-on-Fakechain as the failure. The number is inherited from RingCT's era with *"bounds proof generation time and tx size"* as its rationale; measured 2026-09-24 (`CHAIN_RULES_SLICE_6.md` §5.4): binding at 8 against CEN-H3's weight-implied 22 (two-output shape), verifier work linear per input — derivation owed (FOLLOWUPS) | `check_tx_inputs`, the `FCMP_MAX_INPUTS_PER_TX` compare inside the FAKECHAIN gate (was `3485–3491` at the P0f pin; the function anchor is the durable cite); `cryptonote_config.h` `FCMP_MAX_INPUTS_PER_TX` (`:312` at `dev@a1159f1a2`, 2026-09-24; `:313` one rebase earlier, `:311` at P0f — the symbol is the cite) | C | 1 | spec | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §10 :788 (frozen), :165 (cap widened in the oracle pass); [`FCMP_PLUS_PLUS.md`](../FCMP_PLUS_PLUS.md) :1155; embargo coupling [`DAEMON_RELAY_PRIVACY.md`](DAEMON_RELAY_PRIVACY.md) | RC-81 ⇒ merged. Hand-maintained constant, not `config/` |
...
| CEN-I7 | No input's key image may already be spent on-chain (per-input DB lookup — the chain-wide half; the **intra-block** half, one image twice among a block's own inputs, is CEN-L1's, the validator's, C2-R8 Q6). *Corrected 2026-09-26 (E6 slice 6 commit 10): this cell read "the storage layer independently re-enforces at connect (CEN-L1)" — a belt named as a validator row's enforcement. The store's `spent_keys` set-ness is SI-1, a belt whose breach is a fault, never a verdict; naming it as L1's enforcement is how L1 went unbuilt while three documents agreed, and a block spending one image twice across two listed transactions passed `validate` into a fatal store halt (`CHAIN_RULES_SLICE_6.md` §5 row 4 (a)). Rust: I7 at `rules/tx_against.rs` via `ChainView::has_key_image`, L1 a `BlockRule` beside it, both E6 slice 6 commit 4* | 3607–3612 (also 3558, 3582) | C | 1 | spec | [`FCMP_PLUS_PLUS.md`](../FCMP_PLUS_PLUS.md) §7 step 0 | RC-85 ⇒ merged. Inverse spot-check row. Pool twin is CEN-M6 |
...
| CEN-I17 | The PQC-signed payload binds, per input: full tx prefix ‖ CtSig base ‖ keccak256(prunable) ‖ that input's PQC header ‖ keccak256 of **every** input's hybrid pubkey — neither the FCMP++ proof nor any input's key can be swapped without invalidating signatures. Cross-input key hashes use Keccak, **not** `shekyl_fcmp_pqc_leaf_hash` (Blake2b-512 domain `shekyl-pqc-leaf`) | **Rust, the derivation of record since 2026-09-25 (E6 slice 6 commit 7):** `shekyl-wire/src/transaction/signing_preimage.rs` `PqcSigningPreimage` — `payload(i) = pruned ‖ prunable_hash ‖ header(i) ‖ key_hashes`, `signed_hash(i)` typed `SigningPayloadHash`; the daemon reads it through `shekyl_tx_pqc_signing_payload_hashes` (`verify_transaction_pqc_auth`, tx_pqc_verify.cpp:80) and daemon-rpc's K13 and the validator's I17 (`rules/tx_against.rs`) read the same body. Held to the specification's output by `pqc_signing_preimage_v1.json` (eight daemon-accepted shapes, 21 payloads, captured from the C++ before its deletion). *Records-was:* the C++ assembly `get_transaction_signed_payload` at tx_pqc_verify.cpp:62–158 (prunable bind :92–95; cross-input bind :133–143), deleted 2026-09-25 | C | 1 | spec | [`FCMP_SPEND_SIGNING_PREIMAGE.md`](FCMP_SPEND_SIGNING_PREIMAGE.md) §1.1 (exact formula) | RC-105, RC-107 ⇒ merged |
| CEN-I18 | The hybrid signature (Ed25519 **and** ML-DSA, or M-of-N multisig) over keccak256(payload) must verify in Rust, per input. The serve-credit form carries **no** `pqc_auths` (CEN-H20) — its hybrid countersignature is over the **pass record**, Ed25519 leg on the vin and ML-DSA leg in the pruned record (CEN-J10) — so the row is vacuous on it by construction, not by exemption. *Corrected 2026-09-26: this cell read "exempt (signature lives on the vin)" — half-true since RF-D2 split the legs, and "exempt" reads as unsigned* | `verify_transaction_pqc_auth` (tx_pqc_verify.cpp:60–: hashes from `shekyl_tx_pqc_signing_payload_hashes` :80, each slot through `shekyl_pqc_verify` :148); gate blockchain.cpp:4277–4285 (routes the serve credit around it); Rust: `rules/tx_against.rs` I18 (E6 slice 6 commit 8) — one body, `shekyl_crypto_pq::signature::verify_pqc_auth`, shared by the FFI, daemon-rpc K13 and the validator | C | 1 | spec | [`POST_QUANTUM_CRYPTOGRAPHY.md`](../POST_QUANTUM_CRYPTOGRAPHY.md); [`PQC_MULTISIG.md`](../PQC_MULTISIG.md) | RC-106, RC-108, RC-100 (gate half) ⇒ merged. **Inverse spot-check row** |
| CEN-I19 | `tx_extra` PQC field shape: with `n = vout.size()`, a transaction carries **exactly one** `0x06` KEM-ciphertext field of `1120·n` bytes and **exactly one** `0x07` leaf-entry field of `64·n` bytes when `n > 0`, and **neither** when `n == 0`; `tx_extra` must parse; and (content, `PL-D3` 2026-09-14) every `0x07` entry's leading 32 bytes must be a canonical prime-order non-identity Ed25519 point (`check_pqc_leaf_entries`); and **no `0x02` nonce field off the coinbase** (`TXE-Q6′`, ruled 2026-09-23 — the clause sat under CEN-I20 until 2026-09-25; I20 is `Coinbase`-scoped and never sees a listed transaction, and the C++ and the crate both refuse it from this row's site, `check_tx_extra_shape(General)`). Enforced at relay and block (`core::check_tx_semantic`, no `kept_by_block` exemption) and on the coinbase (`prevalidate_miner_transaction`); the DB collector aborts on the same shape | `cryptonote_format_utils.cpp` `check_tx_extra_shape` (adapter → `shekyl_tx_extra_shape_of` over the codec's own parse; the lengths-taking `shekyl_tx_extra_pqc_field_shape` and the C++ parser it fed were deleted 2026-09-23, `TX_EXTRA_RUST_CUTOVER.md`); `cryptonote_core.cpp` relay; `blockchain.cpp` coinbase; `src/blockchain_db/blockchain_db.cpp` DB add (`shekyl_tx_extra_leaf_entries`, abort-unreachable) | C | 1 | spec | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §9.6a (both sentences corrected 2026-09-05); [`CT2_DRAIN_ORDER.md`](CT2_DRAIN_ORDER.md) §3.1 | **Born 2026-09-06 with its implementation (PR #630; §7 #19).** Ruled by Rick 2026-09-05: exactly one field per tx (first-match "is a choice among valid parses … the wire admits a transaction whose meaning depends on which implementation reads it"), and the `0x06` length is consensus, not policy (symmetry with `0x07`; a relay-only reject "leaves a miner free to include a malformed `0x06` … the funds are gone at connect"). Before: no admission-side check existed and `src/blockchain_db/blockchain_db.cpp` zero-filled a missing/short `0x07` into the leaf (anchors as found: `:519-521`, `:533-535`). The rule lives in `shekyl-wire` (`check_pqc_field_shape`, one home for daemon and port); the daemon passes its own parser's field lengths. Zero-output txs (serve-credit) carry neither field — the must-accept control |
| CEN-I20 | **Coinbase extra grammar** (`TXE-Q6′`, ruled 2026-09-23): a coinbase's `tx_extra` is **exactly** `[0x01 pubkey(32), 0x02 nonce(8), 0x06 KEM(1120·n), 0x07 leaf(64·n)]` in that order for `n = vout.size() > 0`, exactly the first two when `n == 0`, and nothing else — no padding, no `0x04`, no staged `0x09`/`0x0A`, no `0x0B` until its producer amends this row. (The ban on `0x02` **off** the coinbase is CEN-I19's clause — `TXE-Q6′`'s ruling is one, enforced at one adapter, recorded on the row whose scope sees a listed transaction; moved 2026-09-25, E6 slice 6 commit 3.) The whitelist is the rule, so an addition is a consensus amendment. The nonce is fixed at 8 bytes: `2^32 / 120 s ≈ 36 MH/s` exhausts the header nonce in one interval, ×256 per byte, 8 never binds and matches the pool convention; fixed so the length carries no signal (capacity 184 KB/day → 5.8 KB/day; the pool fingerprint an 8-byte field still allows is accepted, recorded as such). Enforced wherever CEN-I19 is (`check_tx_extra_shape` → `shekyl_tx_extra_shape_of`) and by the wallet's context-free validator. Genesis regenerated to carry eight zero bytes. | `rust/shekyl-wire/src/tx_extra/coinbase.rs` `check_coinbase_extra_shape` / `check_tx_extra_shape` (the file grew into the `tx_extra/` module under TXE; both live in its `coinbase.rs`); `cryptonote_format_utils.cpp` `check_tx_extra_shape` (relay `cryptonote_core.cpp`, coinbase `blockchain.cpp`, DB add `src/blockchain_db/blockchain_db.cpp`); `shekyl_coinbase_extra` (the one writer) | C | 1 | spec | [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §9.6b; [`TX_EXTRA_RUST_CUTOVER.md`](TX_EXTRA_RUST_CUTOVER.md) §6 TXE-Q6′ | **Born 2026-09-23 with its implementation** (the tx_extra cutover, §7 #22). Closes the consensus-side hole CEN-M4 names: the largest grammar-valid coinbase extra is 18 994 bytes (`vout = 16`), below the 24 576 relay cap. Not yet applied by the Rust judge (`shekyl-chain-rules` applies neither I19 nor I20 — E6 slice 4 wires `check_tx_extra_shape`, not a second rule). |
...
| CEN-J10 | The full response verifies in Rust (`shekyl_archival_verify_serve_credit_vin`): record structure, P's hybrid countersignature over the pass record (Ed25519 leg on the vin, ML-DSA leg in the pruned record — split by RF-D2; *"vin-borne" until 2026-09-26*), retention-proof legs against the registry sub-root and leaf chunk | 5452–5474 | C | 1 | spec | [`ARCHIVAL_RESPONSE_FORMAT.md`](ARCHIVAL_RESPONSE_FORMAT.md); [`ARCHIVAL_CHALLENGE_MECHANISM.md`](ARCHIVAL_CHALLENGE_MECHANISM.md) | RC-116 ⇒ merged |
...
| CEN-J18 | Reinstate (Rust): interval-log preconditions (single open interval, headroom bound), credit against identity key | 5049–5105 | C | 1 | spec | gate-4 §3.4 (P2B-9 pins 4–6) | |
...
| id | rule | site(s) | C/P | b | class | evidence | notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| CEN-K1a | Alt-chain DERIVED height must be ≥ 1 (alternative genesis structurally impossible) | rule: `alt_window_plan` (`rust/shekyl-difficulty/src/alt_window.rs:57`, R1b crossing); C++ belt `blockchain.cpp:1449` ahead of the FFI call | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q1b(i) (ratified 2026-09-04) | Split from CEN-K1: the original row conflated the self-claimed and derived heights (two conditions wearing one row — §5.3). Rust owner + fail-loud belt; falsifier = any second C++ re-spelling of the comparison |
| CEN-K1b | An alt block whose miner-tx CLAIMS height 0 is rejected first (cheapest exit, before checkpoints/attestation/PoW) | blockchain.cpp:2153–2158 | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q1b(ii) (ratified 2026-09-04) | Split from CEN-K1. An ORDERING arm: the full rule is prevalidate's claimed-equals-derived equality; `reject_block_form` is CORRECT (a height-0 claim is about the input, not our state — stated against the mechanism's opposite instances). Falsifier = the arm acquiring any job beyond ordering |
| CEN-K2 | Alt-chain linkage: the stored alt chain must connect to the main chain at the claimed height with matching hashes (asserted while rebuilding) | 2083–2135 (`build_alt_chain` rebuild asserts) | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q3a (ratified/signed 2026-09-04) | Ratified orchestration: parse, height-sanity, connection, parent-in-main asserts, all fail-closed |
| CEN-K3 | An alt block already stored as an alt block is rejected | 2410 add-path assert; A1 routing leg :3075; DB belt `MDB_NODUPDATA` db_lmdb.cpp | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q3a (ratified/signed 2026-09-04) | RC-148 ⇒ merged (DB belt site added). Ratified: triple-layer duplicate rejection, belts named |
| CEN-K4 | Alt blocks are accepted into storage after: version (CEN-B1 at ideal-version-for-height), attestation (CEN-B4), timestamp FTL + strict median over the newest-11 window (CEN-C1/C2 per C2-R3), checkpoint, PoW at alt difficulty (CEN-D1/D5), and **prevalidate**-only miner-tx checks; `validate_miner_transaction`, `check_tx_inputs` and the curve-root check are deferred to promotion | 2152–2427 (the admission ladder in execution order — §5.3) | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q1a (ratified/signed 2026-09-04) | The two-tier admission contract RATIFIED as deliberate defense economics: PoW-at-alt-difficulty inside the cheap tier prices the attacker's staging at the fork point's own difficulty while bounding the victim's per-block cost (§5.1 intervals-not-averages constraint). §3.5 table. Falsifier = a demonstrated cheap-tier cost asymmetry (share-sampling shape) |
| CEN-K5 | Promotion re-validates every block through the full 4-arg main-chain path (all §4.B–§4.J rules); any failure rolls back to the pre-switch chain and discards the failing alt suffix | 1425–1462 | C | 2 | ratified | **RULED C2-R1b (ratified 2026-09-03)** — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b | Q1a: promotion re-validation ratified as the containment for the admission subset; prior notes: RC-138 merged |
...
| CEN-K6 | Reorg trigger: alt cumulative difficulty **strictly greater** than main (equal stays), or an alt block matching a checkpoint forces the switch **Ruled**: strictly greater switches, equality keeps, checkpoint match forces; the discard-vs-readmit asymmetry (K7) is the flip-flop terminator — a checkpoint can promote a LIGHTER chain. | 2481–2504 | C | 2 | ratified | **RULED C2-R1b (ratified 2026-09-03)** — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b | Q1a/Q1b: the comparison's ONE implementation is `shekyl-difficulty::fork_choice` behind `shekyl_difficulty_fork_choice`; C++ marshals and consumes the verdict; shared vectors `FORK_CHOICE_V1.json`. Prior: RC-136/137 merged |
| CEN-K7 | Demoted main-chain blocks re-enter as alt blocks (witness carried); failure there is logged, not a switch failure; on a discarded chain they are dropped | 1404–1419, 1465–1486 | C | 2 | ratified | **RULED C2-R1b (ratified 2026-09-03)** — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b | Q1a: demote-re-entry on difficulty switches, discard on checkpoint-forced switches — ratified as intentional mechanism (the flip-flop terminator); prior: RC-139 merged |
| CEN-K8 | Alt bookkeeping's `already_generated_coins` is a documented approximation (coinbase-paid, not subsidy); nothing consensus-bearing reads it; promotion re-reads the ledger, and the post-reorg miner notification reads DB cumulative supply, not the alt copy | 2277–2303, 1504–1524 | C | 2 | ratified | **RULED C2-R1b (ratified 2026-09-03)** — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b | Q1a: ratified-and-kept — approximation by construction, nothing consensus-bearing reads it, deleting the field would change the persisted `alt_block_data_t` layout (rule 42 fence); prior: RC-140/141 merged |
| CEN-K9 | Alt-block pool-supplement txs must pass NIC and enter the main pool (`relay_method::block`) or the alt block is rejected | 2329–2376 supplement gate; tolerance producers :870 (pop-return), :2347 (supplement), :5932 (return-taken, trusted-prior-verification) | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q1c (ratified/signed 2026-09-04) | Ratified NIC-or-reject, with the tolerance fixed to THREE in-daemon producers each with its own meaning (the amendment: :5932 re-enters on trusted prior verification, dependency armed in Q2a); import feeder named; falsifier = any NEW feed site — and the gate is RUN, not cited |
| CEN-K10 | Pool `add_tx` with `kept_by_block` (block/reorg-sourced txs): input-check failure stores the tx anyway as `verification_impossible` (it may become valid again after further reorg) | tx_pool.cpp:304–357 | C | 2 | ratified | [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5.5 Q2a (ratified/signed 2026-09-04) | RC-158 ⇒ merged (flag disagreement recorded). Ratified AS COMPOSED with CEN-M8's hash gate: the stored-unverified tx pays full verification at connect. DEPENDENCY ARMED: relaxing M8 reopens K10 silently — the invariants gate carries the check |
...
| CEN-M3 | Relay fee floor (FL-R20 / FL-R23, PR B 2026-09-11): admitted at tip `h′` iff `fee ≥ mask_round_up(weight · min{F(h′−k) : 0 ≤ k ≤ G})`, `G` = 5, slack `RELAY_ADMISSION_SLACK_BP` = 0, where `F(h) = R(h)·C(h)·w_ref/M(h)²` floored at 1 with **raw** `C = (1−σ)·M_r/(1−b)` and `M` the un-graced long-term effective median; the 0.95 and the 2 % buffer are **deleted** (the same fudge for the same gap, which the lookback closes by identity); unpayable-reward state refuses; `kept_by_block` exempt — **no consensus fee floor**. *Until PR B:* fee ≥ 98 % of quantized `weight × 0.95·R·w_ref/M²` off the current reward/medians. | tx_pool.cpp `add_tx`; blockchain.cpp `check_fee`, `relay_floor_at`, `rebuild_relay_floor_ring`; shekyl-economics `relay_floor_admits`, `checked_relay_fee_floor` | P | 2 | ratified (policy) | [`DAEMON_SUBMIT_VERDICT.md`](DAEMON_SUBMIT_VERDICT.md) §8 row P2 :1309 (examined on the **submit path**: C + D re-gate F34, KAT against `check_fee`); the 2026-08-16/17 era-max ruling (decision log :4684–4690) derived the **wallet-side** fee ceiling `Fh = 14,000,000` ("KAT-pinned — not a literal"; since superseded twice, basis unchanged — a structural bound at #640, 218,453,333 unrounded at PR B) — it is not a daemon acceptance rule (verified: no such constant in `src/`), so it bounds this row's *machinery* without ratifying it; [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md) Q8 + Q9 (fee floor RULED: none) | RC-152, RC-153 ⇒ merged. Demoted from CEN's bucket 2 under the §2 bar: the 0.95 factor, quantization mask, reference weight and 2% buffer have no examination record as choices. **UPDATE 2026-09-11 (PR B):** two of the four now have one, and it is a deletion — the 0.95 and the 2 % buffer went under FL-R20/FL-R23 (`FEE_LADDER_DERIVATION.md` §11.2); the mask and `w_ref` remain unexamined as choices. §10 R2. **R2 SIGNED** — [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md); GAP-7 verification-cost condition DISCHARGED passing: zone-point worst cold block 7.23 s ≈ 6.0 % of T, measured on the Pi 4 floor (the floor figure, per the signature's condition) |
| CEN-M4 | Relay cap on `tx.extra`: ≤ `MAX_TX_EXTRA_SIZE` (24 576); `kept_by_block` exempt — consensus side had no tx_extra bound beyond CEN-H1 until CEN-I20 (2026-09-23) bounded the coinbase extra by grammar (≤ 18 994 bytes); non-coinbase extras remain relay-capped only | tx_pool.cpp:261–269; cryptonote_config.h:353 | P | 2 | ratified (policy) | bound frozen [`GENESIS_TX_WIRE_FORMAT.md`](GENESIS_TX_WIRE_FORMAT.md) §10 :791; [`DAEMON_SUBMIT_VERDICT.md`](DAEMON_SUBMIT_VERDICT.md) §8 row P3 :1310 (submit path); [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md) Q10 (headroom computed; canonical-form stays credit-wire) | RC-154 ⇒ merged. Demoted from CEN's bucket 2 under the §2 bar. Canonical-form arbitration deferred to the credit-wire lane (per the SA index row) — deferred, not done. **R2 SIGNED** — [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md); GAP-7 verification-cost condition DISCHARGED passing: zone-point worst cold block 7.23 s ≈ 6.0 % of T, measured on the Pi 4 floor (the floor figure, per the signature's condition) |
...
| CEN-M10 | Pool lifetime/eviction: non-`kept_by_block` txs expire at `CRYPTONOTE_MEMPOOL_TX_LIVETIME` (3 days), `kept_by_block` at the longer alt-block lifetime; stale-reference and weight-cap pruning (`m_txpool_max_weight`) evict lowest fee/byte first | tx_pool.cpp:1040–1059, 532 | P | 2 | ratified (policy) | the 3-day livetime is consumed as given by the submit round ([`DAEMON_SUBMIT_VERDICT.md`](DAEMON_SUBMIT_VERDICT.md) :942–943 re-relay bound) — consumption, not examination; [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md) Q11 (couplings stated; cap stored as derivation) | RC-160 ⇒ merged. Pool weight cap and livetime never ratified as policy choices. §10 R2/R7. **R2 SIGNED** — [`CONSENSUS_C2_R2_WEIGHT_FEES.md`](../completed/CONSENSUS_C2_R2_WEIGHT_FEES.md); GAP-7 verification-cost condition DISCHARGED passing: zone-point worst cold block 7.23 s ≈ 6.0 % of T, measured on the Pi 4 floor (the floor figure, per the signature's condition) |
...
8. **FAKECHAIN/regtest levers compiled into consensus paths** (the
   carve-out block, which is **CEN-I4's** — I2's and I3's behaviour does
   not vary under it, E6 slice 6 §3.4, corrected 2026-09-26; CEN-B5's
   root-check skip, retired 2026-09-05; CEN-D7, `RuleSet` data in Rust;
   CEN-D3's env override, a fail-closed lever): enumeration input for the
   rewrite's test-seam design. (§10 R9.)
...
16. **CEN-I12's anchor source reconciled — the §7 step-2 prose was stale
    ...
    written by the parent's connect (`src/blockchain_db/blockchain_db.cpp:636`, post-add
    keying), the wallet's `drained_through = ref_height − 1`
    (`rust/shekyl-curve-tree/src/client.rs:908`), and the CT-2 KAT at height 61. The rejected alternative — the header as
    ...
    R9 was the only thing that made the choice observable (retired 2026-09-05, #18), and only in
    ...
    depth must equal the proof's embedded depth (`rust/shekyl-fcmp/src/proof.rs`,
...
17. **CEN-B5 has a second, live divergence — the header check compares the
    ...
    N. **Observed, not inferred:** `archival_substrate_lmdb.cpp` `cen_b5_header_root_is_the_tip_root_before_add_block_not_after` connects a leaf-creating block on
    ...
    Not observed until the fix's own regression (entry 18) because every Blockchain-level test was FAKECHAIN (the
...
18. **CEN-B5's S1 is fixed — the check runs at admission, before the add,
    ...
    (`blockchain.cpp:5885–5897` at `a5c266125`, the compare block — no nettype
    wrapper; the caption above it is `:5867–5884`): the prev_id check above it guarantees the
    ...
    record stores under key N (entry 16). A mismatch is `reject_block_form`
    and returns before any write; the post-add
    compare and its `pop_block_from_blockchain` / `return_txs_to_pool` arm
    are deleted (net −17 lines in that block, 32 added with the caption). The
    `new_height > 0` condition went with it, and it was never a genesis skip:
    `new_height` is assigned only by a successful `add_block` (1 after
    genesis), so genesis was always checked on real nettypes; the condition
    guarded the failed-verification path where `add_block` never ran, which
    a compare that precedes the add no longer needs. The TESTNET fixture
    exercises genesis through the new check via `Blockchain::init`. *(This
    entry's first wording, and the fix commit's message, called it a genesis
    skip — records-was, corrected in the same PR.)*
    **Observed red → green** by the first non-FAKECHAIN Blockchain fixture in
    the tree, `curve_tree_header_root_check.cpp` (TESTNET, `fixed_difficulty
    = 1`, real LMDB, real `add_new_block` through the RandomX light-cache
    verifier): before the edit blocks 1–59 connected and block 60 was rejected
    with `curve_tree_root mismatch`; after it block 60 connects, the leaf
    count goes 0 → positive, the header of block 60 equals the record under
    key 60, and block 61 connects. **Alt/reorg walk (Rick's condition):**
    every path that adds a block to the main chain is one function —
    `handle_block_to_main_chain` has three callers, connect (`:6653`, via
    the `:3103` wrapper), promotion in `switch_to_alternative_blockchain`
    (`:1326`) and restore in `rollback_blockchain_switching` (`:1251`) — so
    the admission check covers promotion and restore identically: each
    re-added block is checked against the post-pop tip, which the prev_id
    check pins as its parent. `handle_alternative_block` still carries no
    root check of its own; alt blocks defer to promotion (CEN-K5, unchanged).
    **The FAKECHAIN skip is retired in the same PR, and entry 17 misjudged
    what that needed.** Entry 17 expected the fix to retire the skip by
    making the core_tests generator self-consistent with its fake DB; that
    reading was wrong — core_tests replay every generated block into a real
    LMDB-backed `core` (`chaingen.h`, `c.init(vm, test_options_)`), and ten
    of those chains rewind through the 60-block maturity window across five
    files, so placeholder headers would be rejected at every drain once the
    check ran. The generator therefore now computes real roots: it replays
    its own recorded chain through the Rust `CurveTreeClient`, exposed to
    C++ as `shekyl_curve_tree_replica_*` in `shekyl-ffi` (ingest per block,
    rollback to the common prefix on a fork, `next_block_root` — the state
    at `tip + 1`, the one-past-tip read a header *producer* needs, which the
    verifier-shaped `root_at` refuses; added to the client and pinned to the
    recon fixture at every height). The fake-DB test doubles report the
    empty-tree sentinel instead of zeros. The `m_nettype != FAKECHAIN` wrapper
    is deleted and the check runs on every nettype; the client's parity with
    the daemon's store was KAT-pinned at fixed heights, and every generated
    block that connects is now a live cross-implementation check at every
    height. Cost, disclosed: `shekyl-ffi` gains `shekyl-curve-tree`, which
    brings `redb` into the daemon image ahead of the daemon redb store that
    needs it anyway — Rick's ruling (2026-09-05) over a C++-LMDB generator
    oracle that the store rewrite would have deleted. Rule 71's
    counter-example is retired; R9 keeps the D7/I2/D3 levers. The keying pin in
    `archival_substrate_lmdb.cpp` is renamed
    `cen_b5_header_root_is_the_tip_root_before_add_block_not_after` and
...
19. **CEN-I19 minted: the `tx_extra` PQC field shape is a consensus rule, and
    ...
    2026-09-06 landed, PR #630). **Found** (steering, on shekyl-core-12's
    ...
    payment (`rust/shekyl-scanner/src/scan.rs:644-648` skips the output). **Grade S1** on the
    ...
    **Round 3 (PR #630) closed the rule's two remaining gaps and revealed a
    third thing.** (a) `check_pqc_field_shape_of` had NO non-test caller, so
    the port's own validators — `validate_context_free_pruned`, on the daemon
    ...
    check. (c) **Twenty-one wire fixtures encoded transactions that could
    never exist:** outputs with an empty `tx_extra`, a shape no builder has
    ever produced, since every producer emits both fields per output. The rule
    did not invalidate those fixtures; it revealed that the corpus had been
    asserting parser behaviour over transactions the network cannot carry. The
    real-data controls held throughout — a pinned REAL daemon coinbase
    validates unchanged, and the serve-credit vector (zero outputs, empty
    `extra`) is accepted as the must-accept case — so only the synthetic
    fixtures were wrong. The pinned cross-language hash vector
    (`pruned_tx_hash_parity_v1.json`) was regenerated deliberately for the same
    reason: a parity pin that fixes agreement on a transaction consensus
...
20. **C2-R8 ruled — storage-layer enforcement placement is a standing test,
    ...
    curve-tree root CEN-B5 compares against (`src/blockchain_db/blockchain_db.cpp:641–664`),
    ...
    `rust/shekyl-chain-store/src/conformance.rs`, a negative fixture per rule (replay catches
...
21. **CEN-I12's absent-key arm walked — the per-height root record is dense
    ...
    (`src/blockchain_db/lmdb/db_lmdb.cpp:9064`–`:9080`). (c) `ref_height` is
...
22. **CEN-I20 minted with its implementation — the coinbase extra grammar
    (2026-09-23, `TX_EXTRA_RUST_CUTOVER.md` TXE-Q6′).** The cutover that made
    `shekyl-wire` the one `tx_extra` codec first ruled the coinbase `0x02`
    nonce shed (TXE-Q6, 2026-09-22, the fingerprint ground); the maintainer
    reversed it the next day on the arithmetic: the 32-bit header nonce
    exhausts a template at ≈ 36 MH/s, each nonce byte multiplies that by
    256, so a *fixed* 8-byte field costs mining nothing while a fixed width
    removes the length channel and a closed whitelist bounds the coinbase
    extra for the first time (CEN-M4 had named the hole). What the row
    does not claim: bounding closes the pool fingerprint — 8 bytes hold a
    pool id, and the row says so. Genesis was regenerated to carry the
    field as eight zero bytes on all three networks (new ids; the regtest
...
23. **CEN-L16 minted — an R8-class placement row for a store-evaluated
    ...
    ruled on PR #840).** S-ARCH's pre-flight read, at `d8ebfd18c`, `src/blockchain_db/lmdb/db_lmdb.cpp:4889`:
...
| **R1 — Reorg / alt-chain + checkpoints + topology** — **COMPLETE 2026-09-04** ([`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md); three sub-rounds all ruled: R1a 2026-09-02 — CEN-E3/E4 removed-and-executed, CEN-G8 retired, rule 71 minted; R1b 2026-09-03 — merged PR #603; R1c 2026-09-04 — all seven rulings signed, the Q3b sync-orphan defect fixed in-round, K1 split K1a/K1b, nine rows → bucket 2, the line re-anchor executed with a forward-validation instrument) | CEN-K1–K10, E1–E5, A1, A2, A4, D5, D6 (20) | The fork-choice rule (strictly-greater + checkpoint-forced), the prevalidate-only alt-admission subset (§3.5), checkpoint trust surfaces (incl. the PoW/FCMP-skipping compiled-hash path), and reorg semantics across staking-epoch boundaries — none examined on record (§6.5) — freeze as the permanent reorg contract of a chain whose archival state is epoch-scoped |
...
| **R4 — Header identity + hardfork collapse** — **activation mechanism KEPT 2026-09-23** | CEN-B2, B3, B6, B7, A5, A6, A7, H23 (8) | The wire-identity binding (no explicit merkle field — B6) stays open with the rest of the batch. **The V4 activation half is answered:** miners decide when a pre-specified upgrade turns on, by the existing `HardFork` vote window stepping the table (rule 75). Humans still specify the next row. Collapse of the voting machine is rejected. `HardFork::add`'s ignored reject remains the open CEN-B3 defect. |
...
| GAP-4 | Anti-DoS sync: minimum cumulative work and bounded header acceptance (not checkpoints) | C7 | ABSENT — `minimum_chain_work`/`min_chain_work`: 0 hits in `src/`; no `assumevalid` analogue; `BLOCKS_SYNCHRONIZING_MAX_COUNT` (`cryptonote_config.h:95`) is a batch size, not a work gate; R1a deleted the compiled-hash fast path, removing the checkpoint-shaped trust surface without replacing its anti-DoS role | Design-round question; couples to R1c's cheap-deep-fork seeds (`build_alt_chain` re-walks the entire alt ancestry per admission — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §5) |
...
| GAP-7 | A bounded worst-case block **verification cost** — a cost model on the provisioning floor, not only size caps | C13 | PARTIAL — the caps exist as R2 rows (CEN-H1's 1 MB, CEN-H3, CEN-G6/G6b) but nothing binds weight to verification time on the rule-76 floor (Pi 4); the RandomX constraint is already on record (research-lab#144: share-sampling designs disqualified — ~1.5 s verify for 100 shares) and applies to Shekyl identically. **Factual update: the instrument now exists** (`rust/shekyl-ffi/benches/block_connect_verify.rs` + pins + `scripts/bench/gap7_pi4_gate.sh` + `tests/unit_tests/gap7_bp_bench.cpp`) **and floor captures are archived** (`docs/benchmarks/gap7_block_verify_pi4_*.txt`, skl-pi) — measurements, not a binding: **the row's disposition does not move here**; the consensus-side binding is the R2 round's ruling to make | R2 input, rides the R2 deferral (§10): when R2 resumes, the caps' derivation must include the verification-cost leg, not only the economics |
...
| C12 (work-based fork choice + explicit tie-break) | present-and-ruled | R1b Q1a ([`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b): cumulative difficulty only — heights are never compared at the ruled site; equality-keeps-incumbent is the explicit deterministic tie-break; the discard asymmetry is the flip-flop terminator. The measured tie-break-exploitation numbers (GAP-2's evidence) are recorded as R1c wargame input |
