...
| CEN-F20 | `tx_volume_avg` is the integer mean of `tx_hashes.size()` over the prior `SHEKYL_TX_VOLUME_WINDOW` (720) blocks; height 0 → 0 | 2041–2059 (`get_tx_volume_avg`; callers 1657, 1944); rust/shekyl-economics/src/activity.rs | C | 1 | spec | `shekyl-economics` activity spec; window 720 is a consensus-visible constant **not** in `config/` (RC decision-log #10 class) | [m] RC-172 ⇒ minted |
