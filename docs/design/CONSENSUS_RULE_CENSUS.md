...
| **R2 — Block-weight / reward-zone / fee constants** — **OPEN, rulings PROPOSED** ([`CONSENSUS_C2_R2_WEIGHT_FEES.md`](CONSENSUS_C2_R2_WEIGHT_FEES.md), C2-R2-Q1…Q11; three groups by validation surface). The 2026-09-03 deferral's reopen conjuncts are **both discharged, verified at the artifacts** (round-doc header): (a) **FL-R12′ SIGNED** in `FEE_LADDER_DERIVATION.md` §8 at fee-ladder review round 8 (branch `design/fee-ladder-derivation`, commit `ccfb85c72` — **evidence still branch-only**, which is why the R2 PR sequences after #614); (b) `terminal_reward_legs_agree` exists, observed red then green through the new owner (`emission.rs` on the same branch — also branch-only; the earlier "on dev" reading of this row was of the branch artifact). §12 GAP-8's composition finding is consumed by Q4 (penalty-after-floor). Nothing ruled: every Qn awaits the maintainer. | CEN-G6, G6b, F14b, H1, H3, M3, M4, M10 (8) | The 300 000 reward zone (an explicitly punted "fossil flag" arbitration), the 1.7× clamps, ×50 surge, the ArticMine penalty curve, the 1 MB tx cap, and every fee-formula constant fossilize as Shekyl economics without ever having been derived for Shekyl |
