...
| CEN-H24 | Ring-members residue check: for each `txin_to_key`, no relative `key_offset` after the first may be 0 ("duplicate ring members"); archival vins skipped; the unused `hf_version` parameter is ring-era residue | cryptonote_core.cpp:945–961 (`check_tx_inputs_ring_members_diff`); semantic caller :827 (re-pinned at `d8ebfd18c`, E6 slice 5 row 9) | C | 3 | — | rule 60 **names mixin enforcement** in its deleted-from-construction list; FCMP++ requires empty `key_offsets` (CEN-I6), so this can only fire before CEN-I6 rejects the same tx | [m] RC-66 ⇒ minted at RC's bucket: a live-but-redundant mixin-era check with a named deletion disposition. §10 R5 executes. **Falsifier (E6 slice 5 Q7, 2026-09-23):** `h24_cannot_fire_on_an_input_i6_admits` (`rust/shekyl-chain-rules/src/rules/tx_tests.rs`) — the property is CEN-I6's exclusion; today it pins I6 `pending` as a **proxy**, armed to demand the "I6 refuses offsets" assertion when I6 lands. Bucket 3 keeps this row out of the registry and every count; that test and this cell are the only two things that index it |
