...
| CEN-E5 | **Removed (PDM-Q-F23, PR #733, 2026-09-13).** Was: loading `<data-dir>/checkpoints.json` could add checkpoints at runtime (`--enforce-dns-checkpointing`-era mechanism reduced to file load) — operator-supplied consensus pins. Under `PDM-Q5`'s anchor model the compiled-in checkpoint is a release-carried `assumevalid` anchor and a runtime-loadable pin is a trust channel that bypasses it; the C2-R0 HELD existence question is answered *no*. **What survives** is the C2-R1b clause (3) rollback, now run once at init over the compiled-in set only: `Blockchain::enforce_checkpoints` (src/cryptonote_core/blockchain.cpp:6392) → `check_against_checkpoints` (src/cryptonote_core/blockchain.cpp:6308), fail-stopping `core::init` (src/cryptonote_core/cryptonote_core.cpp:660) when the rollback cannot apply | deleted (`load_checkpoints_from_json`, `core::update_checkpoints`, the ten-minute reload, `JSON_HASH_FILE_NAME`, the JSON-path tests and stubs) | C | 2 | — | **RULED C2-R1b (ratified 2026-09-03)** while the mechanism existed — [`CONSENSUS_C2_R1_REORG.md`](../completed/CONSENSUS_C2_R1_REORG.md) §4b; existence ruled by [`ARCHIVAL_PRUNED_DAEMON_MODE.md`](ARCHIVAL_PRUNED_DAEMON_MODE.md) `PDM-Q5` item 3 / `PDM-Q-F23` | Q2a/Q2b's uniformity and JSON-conflict clauses are moot with the surface; Q2c's deletions stand; the rollback floor is **1** (not `saturating_sub(pt.first, 2)` → 0 — see `CONSENSUS_STORE_RECONCILIATION.md` CEN-E5 row-text correction) |
