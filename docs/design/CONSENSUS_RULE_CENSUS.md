...
18. **CEN-B5's S1 is fixed — the check runs at admission, before the add,
    ...
    (`blockchain.cpp:5873–5902`): the prev_id check above it guarantees the
    ...
    `new_height > 0` skip went with it: genesis's header is a source constant
    and the empty-tree root is `selene_hash_init`, so checking it is a gain.
    **Observed red → green** by the first non-FAKECHAIN Blockchain fixture in
    ...
    `handle_block_to_main_chain` has three callers, connect (`:6644`, via
