...
CEN-K1 → K1a/K1b split), CEN-I19 was minted into bucket 1 on 2026-09-06 with its implementation (**re-reviewed at `2ec553a3f` and promoted 2026-09-11**), and CEN-J27 was minted into bucket 1 on 2026-09-12 with its implementation (`EndpointUpdate` B+C1; UNREVIEWED by construction until P0f reviews it), so the live bucket-1/2 count is **131** (counted from the rows, re-counted 2026-09-12: 88 b1 + 43 b2) and, after P0f's slices 9 and 10 (2026-09-11; [`CONSENSUS_STORE_RECONCILIATION.md`](CONSENSUS_STORE_RECONCILIATION.md) §5.4.1), **every one of them carries a register row** — the set difference between the live bucket-1/2 IDs and the IDs carrying a §5.4.1 row is zero, and the two rows still UNREVIEWED each say why: CEN-L8 (failed closed on an unwired clause) and CEN-J27 (born with its implementation, awaiting review). *(Re-counted 2026-09-12 at the J27 mint. The 2026-09-11 reading of this sentence carried slice 9's backlog figure — twenty-seven, a set difference, not a remembered total — which slices 9 and 10 closed that same day.)* **The review's two S-graded findings are fixed and re-verified** — the S0 by
...
| CEN-J12 | `bond_spend_pk` coupling: JoinMarket must commit one (canonical length); every other kind (Release, HoldingsUpdate, Rebond, EndpointUpdate) must not carry one | 5115–5128, 4838, 4906, 5054 | C | 1 | spec | gate-4 §9.11 | RC-118 ⇒ split across CEN-J12/J13. EndpointUpdate arm added 2026-09-12 (B+C1); its site is anchored in the CSR register, not here |
| CEN-J13 | Cold-authority arms (Unbond, HoldingsUpdate-drop, and — from B+C1, 2026-09-12 — EndpointUpdate; the fast-path belt CEN-G8 retired with its mechanism, C2-R1a) authorize only against the record's **committed** `bond_spend_pk`, selected by `requires_cold_authority` in Rust; credit arms (JoinMarket, HU-add, Rebond) authorize with the identity key `P_pubkey` (HU-add auth key must equal `P_pubkey`) | 4850, 4968, 4957–4962, 5099–5104, 5203–5210 | C | 1 | spec | GF-1: [`ARCHIVAL_FIREWALL_GATE6.md`](ARCHIVAL_FIREWALL_GATE6.md) §9.6 :799 (RESOLVED 2026-06-16); [`ARCHIVAL_BOND_GATE4.md`](ARCHIVAL_BOND_GATE4.md) §3.5 step 5 :419 | RC-122 ⇒ merged |
...
| CEN-J27 | EndpointUpdate (bond-post kind 4) semantics: the vin is exactly the endpoint — no `bond_spend_pk`, no holdings, no amount term (`EU-D11`, refused at every serializer and belted at the arm); cold authority is required unconditionally (`EU-D2`: the pin runs before the verify); `shekyl_archival_verify_endpoint_update` requires an existing **Bonded** record (`EU-D7`: zero-bonded refused); the endpoint is not validated as a key (`EU-D9`); CT balance is the no-term equation (`verCtSemanticsEndpointUpdate`); connect replaces the record's `endpoint` and journals the previous one, pop restores it (`EU-D12`) | `blockchain.cpp:5003–5055` (arm, at B+C1 commit); `tx_verification_utils.cpp:139–148`; `blockchain_db.cpp:361`, `:811` | C | 1 | spec | [`ARCHIVAL_ENDPOINT_UPDATE.md`](ARCHIVAL_ENDPOINT_UPDATE.md) `EU-D2`, `EU-D7`, `EU-D9`, `EU-D11`, `EU-D12` | Minted 2026-09-12 with its implementation (B+C1, rule 07); UNREVIEWED by construction in the CSR register |
