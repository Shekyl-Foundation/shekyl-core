...
recorded disposition — **96 CHECKED-CONFORMANT**, **5 DIVERGENT** (CEN-L11 with
...
spec to review against until the census's design rounds rule them. **CEN-L12** is DIVERGENT (coupled to CEN-L11), as are **CEN-G4** and **CEN-J26** (coupled to CEN-M8, round 2); **CEN-L8**
...
Reviewed at **`4b9807c5e`** (2026-09-02). 20 rows CHECKED-CONFORMANT; **CEN-M8
DIVERGENT, graded S0 on the §7.2 ladder** — the gravest finding of the whole
review, presented first — and **CEN-G4 DIVERGENT, coupled to it** (round 2,
2026-09-03: its own clause's precondition is breached by the presence-gated
cache).
...
| CEN-D2 | **CHECKED-CONFORMANT** | `pow_registry.cpp` returns the RandomX schema unconditionally; the fail-closed arm at the cited `cryptonote_tx_utils` sites stands as read |
...
| **CSR-3a** | The **conformance register** (§5.4.1): a bucket says a rule is *ratified*, never that the C++ *implements* it | every parity claim asserting correctness | **ROW COVERAGE COMPLETE 2026-09-02** — all **102** bucket-1/2 rows disposed: **96 CHECKED-CONFORMANT / 5 DIVERGENT / 1 failed closed** (P0f slices 1–8 + round 2): CEN-L11 with CEN-L12 coupled; **CEN-M8 at S0** with CEN-G4/J26 coupled (round 2, 2026-09-03); the examined-and-excluded note on L12 covers its **staked arm only** (a retired claim-era clause), not the row. **Coverage is complete over today's bucket-1/2 set; the set grows** — bucket promotions from design rounds enter UNREVIEWED, and **DRS-P0f** and the census own reviewing them; E2 must consult the register before calling any match correctness |
