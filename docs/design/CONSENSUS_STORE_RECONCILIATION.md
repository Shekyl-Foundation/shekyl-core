...
**DRS-P0f row coverage is COMPLETE as of 2026-09-02** (slices 1–8, all at the
shas pinned per slice): every one of the census's **102 bucket-1/2 rows** has a
recorded disposition — **95 CHECKED-CONFORMANT**, **6 DIVERGENT** (CEN-L11 with
CEN-L12 coupled to it; **CEN-M8 at S0** with CEN-G4 and CEN-J26 coupled to it;
**CEN-D2 at S1**, the round-4 fail-open-at-difficulty-1 finding),
**1 failed closed to UNREVIEWED**
(CEN-L8, whose settlement clause names an unwired writer). UNREVIEWED remains
the default for anything outside bucket 1/2 — bucket-3/4 rows have no ratified
spec to review against until the census's design rounds rule them. **CEN-L12** is DIVERGENT (coupled to CEN-L11), as are **CEN-G4** and **CEN-J26** (coupled to CEN-M8, round 2) and **CEN-D2** (S1, round 4); **CEN-L8**
...
| **CEN-L11** | **DIVERGENT** | Reviewed at `4b9807c5e` (re-verified for P0f, 2026-09-02 — the divergence itself predates the register, recorded in FOLLOWUPS; code re-read and the claim re-confirmed in round 3, 2026-09-03). Leaf-**construct** verdict unchecked: a false return at `blockchain_db.cpp:570–576` (and the `continue` arms :562–565) silently omits an accepted output from the tree — deterministic, permanently unspendable, no verify-time twin. Live FOLLOWUPS row | Identity is **not** the pass condition. Needs a reviewed expected-divergence or a replacement KAT asserting the corrected behavior; **reproducing the omission fails** |
| **CEN-L12** | **DIVERGENT** *(coupled to CEN-L11 — same code path)* | Reviewed at `6bc2de8f2` (P0f, 2026-09-02). The **maturity arithmetic conforms exactly**: coinbase `block_height + CRYPTONOTE_MINED_MONEY_UNLOCK_WINDOW` (=60, `cryptonote_config.h:45`), all others `+ CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE` (=10, `:49`), applied identically to both live target variants (`blockchain_db.cpp:545–560`); `unlock_time` appears **nowhere** in the collect/maturity path, as the rule requires; the spec's **staked** arm is excluded per this row's own census note (claim-era retired — `effective_lock_until` has **zero** live code references, only comments at `shekyl_types.h:135` and `cryptonote_config.h:297`). **What fails is the spec's *universality* clause** — FCMP_PLUS_PLUS.md §7, "**All** outputs (coinbase, regular, and staked) are deferred". Three arms can drop an accepted output before it is ever made pending: `else continue` (`:562`, non-whitelisted target types) and `i >= outPk.size() continue` (`:565`) are **belts** — the `:562` arm switches on *output target* types and is defended upstream by **CEN-H12's output-target whitelist** (sole type `txout_to_tagged_key`; miner-tx leg CEN-F8), and the `:565` arm by four `outPk.size() != vout.size()` checks (`blockchain.cpp:3372`, `:4114`, `cryptonote_core.cpp:799`, `cryptonote_format_utils.cpp:137`) — but the unchecked `shekyl_construct_curve_tree_leaf` verdict at `:570` is **CEN-L11's live defect**, and it breaches L12's own clause. **L12 cannot be promoted while L11 stands.** Scope: connect path only; pop/revert is P0b | Identity is **not** the pass condition. Promotion is gated on L11's divergence closing; a match meanwhile records reproduction of the same omission |
...
| CEN-J26 | **DIVERGENT** *(coupled to CEN-M8 — same skip flag)* | W-EM + the FCMP arm. Absent⇔zero-fee-inputs enforced both directions (`:4186` no-proof-with-inputs reject at `:3889`); the verify-when-run is `shekyl_fcmp_verify` rc-checked reject (`:4351+`). **But this row's own clause is "present ⇒ verifies", and under CEN-M8's presence-gated cache a present fee-input proof that was never verified anywhere can pass this arm with `skip_fcmp_verify` set (`:4221`)** — the M8 shape breaches the clause directly, the CEN-L11→L12 coupling pattern. *(Round 2, 2026-09-03: slice 8's amendment had scoped the breach to M8 alone; review showed this row's clause is itself breached — verdict corrected.)* **REWRITE-NOTE:** `skip_fcmp_verify` is a **pool-admission verification cache**, embargo-load-bearing per the in-code note (`DAEMON_RELAY_PRIVACY.md` §71) — the rewrite must preserve that timing semantics or re-derive the embargo first. **Acceptance:** identity is **not** the pass condition; promotion is gated on CEN-M8's divergence closing (a match meanwhile records reproduction of the same defect) |
...
| Row | State | Evidence (all at `4b9807c5e`) |
| --- | --- | --- |
| **CEN-M8** | **DIVERGENT — §7.2 ladder S0 (fund-safety class); ruled FIX 2026-09-03 (Rick) — PR #602; stays DIVERGENT until re-reviewed at a sha containing the merged fix** | The ratified rule: the pool stores `H(proof ‖ referenceBlock ‖ key images)` and block connect **"skips only the proof re-verify when the hash still matches."** The implementation gates the skip on **presence alone**: `can_skip_fcmp = found_tx_in_pool && is_ct_fcmp_pp_pqc` (`blockchain.cpp:6128–6129`), where `found_tx_in_pool` is a bare txid `take_tx`. The spec's exact check **exists** — `is_fcmp_verification_cached(meta, tx)` (`tx_pool.cpp:1735–1743`, the census's own cited site) verifies `meta.fcmp_verified` **and** recomputes the hash — but its only caller is `is_transaction_ready_to_go` (`:1812`), the **template** path. The connect path never consults the meta. **Why this is S0 and reachable:** `add_tx` with `kept_by_block = true` **tolerates a failed `check_tx_inputs`** ("may become valid again") and inserts with `fcmp_verified = 0`, `fcmp_verification_hash = null` (`tx_pool.cpp:304–330`). Block-carried txs are admitted `kept_by_block` before connect — so a tx whose FCMP++ membership/spend-auth proof **failed (or never ran)** at admission sits in the pool, satisfies `found_tx_in_pool` at connect, and the proof verification is **skipped at the only remaining place it could run**. Structural checks (referenceBlock, ages, depth, key images, PQC auths) still run; the membership/spend-authorization proof does not. An invalid proof can connect. **The fix is small and uses the existing function:** read the pool meta before `take_tx` consumes it and require `is_fcmp_verification_cached(meta, tx)` in `can_skip_fcmp` — a presence-gated skip becomes the hash-gated skip the spec ratified. RECORD-AND-SPECIFY's narrow FIX-IN-CPP class (§6.4) applies: this defect risks fund safety before the rewrite lands, and the fix unblocks this row. **The D++ embargo constraint survives the fix untouched** — pool-admitted-and-verified txs still skip, so `hop` is unchanged; only never-verified txs lose a skip they should never have had | **Regression only.** Identity is not the pass condition; a rewrite reproducing the presence-gated skip **fails**. Promotion requires the hash-gated skip (or the ratified spec re-ruled) |
...
| CEN-B5 | **CHECKED-CONFORMANT** | Post-connect header-vs-computed root compare; mismatch pops the block and returns txs (`:6462–6472`). **REWRITE-NOTE:** connect-then-check-then-compensating-pop is inherited arrangement — in the rewrite the root check belongs **before txn commit**, which a Rust store makes natural |
...
| CEN-D1 | **CHECKED-CONFORMANT** | `check_hash` at main (`:5902`) and alt (`:2416`), backed by `shekyl_difficulty_check_hash` (the crate's u128 comparison per the shim comment; KAT per CEN-D1b's record) |
| CEN-D2 | **DIVERGENT — §7.2 ladder S1 (consensus correctness, connect path); fix-or-accept is Rick's** | Reviewed at `4b9807c5e`; re-walked round 4 (2026-09-03) after review — slice 8's "stands as read" was the tell of a light walk, and the read was wrong. The schema half conforms (`pow_registry.cpp` returns RandomX unconditionally). **The fail-closed clause does not hold at difficulty 1:** the defence is a `0xff…` sentinel written on verifier failure (`cryptonote_tx_utils.cpp:784–794`, alt pre-seed `blockchain.cpp:2325–2326`), and both validation paths ignore the verifier's returned bool and go straight to `check_hash` (`:5899–5902` main, `:2414–2416` alt) — but `check_hash(0xff…, 1)` **passes** ((2²⁵⁶−1)·1 < 2²⁵⁶; the crate's own test documents it, `check_hash.rs:163–165`). Difficulty 1 is reachable: `--fixed-difficulty` is a general daemon arg on any nettype (`cryptonote_core.cpp:92`, `:620–621`), and organic LWMA-1 has **no floor above CEN-D6's zero-guard** — `lwma1_next` clamps L, not the output, so a quiet chain can decay to 1. So under (verifier FFI failure ∧ difficulty 1) a block connects with its PoW **never verified** — the in-tree comment's guarantee ("guarantees check_hash() rejects it") overstates its constant, and the census row ratified the overstatement. **Sibling sweep (same round):** the register's other sentinel-backed claims checked — CEN-D5/K1's difficulty-0 sentinel is caught by CEN-D6's zero-reject; CEN-H16's 500M sentinel is a plain comparison; no second difficulty-1-class hole found. **Fix sketch (routed, not built here):** the validation call sites consume the verifier's bool and reject on failure as a *verification error*, independent of `check_hash` — the sentinel stays as a belt; the false comment is corrected in the same change. §6.4's FIX-IN-CPP class plausibly applies (the defect blocks this row's interim-oracle use) | Identity is **not** the pass condition; a rewrite reproducing the sentinel-only defence **fails**. Promotion requires the checked-verdict rejection (or the ratified spec re-ruled) |
...
| CEN-G4 | **DIVERGENT** *(coupled to CEN-M8 — the skip's admission gate)* | Per-tx `check_tx_inputs` at connect with failure → `add_block_as_invalid` + `bvc.m_verifivation_failed` + txs returned (`:6132–6145`) — the dispatch conforms. **But this row's own clause permits the skip only for "FCMP++ txs that verified at pool admission"** (census §4.G, cache cross-referenced to CEN-M8 in the row itself), **and the presence-gated cache grants it to never-verified `kept_by_block` entries** — the M8 shape breaches the precondition directly. *(Round 2, 2026-09-03: slice 8 recorded this row conformant on the dispatch alone; review showed the clause's precondition is breached — verdict corrected.)* **Acceptance:** identity is **not** the pass condition; promotion is gated on CEN-M8's divergence closing (a match meanwhile records reproduction of the same defect) |
...
| **CSR-3a** | The **conformance register** (§5.4.1): a bucket says a rule is *ratified*, never that the C++ *implements* it | every parity claim asserting correctness | **ROW COVERAGE COMPLETE 2026-09-02** — all **102** bucket-1/2 rows disposed: **95 CHECKED-CONFORMANT / 6 DIVERGENT / 1 failed closed** (P0f slices 1–8 + review rounds): CEN-L11 with CEN-L12 coupled; **CEN-M8 at S0** with CEN-G4/J26 coupled (round 2); **CEN-D2 at S1** (round 4 — sentinel fail-open at difficulty 1); the examined-and-excluded note on L12 covers its **staked arm only** (a retired claim-era clause), not the row. **Coverage is complete over today's bucket-1/2 set; the set grows** — bucket promotions from design rounds enter UNREVIEWED, and **DRS-P0f** and the census own reviewing them; E2 must consult the register before calling any match correctness |
