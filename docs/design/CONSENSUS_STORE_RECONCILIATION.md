...
recorded disposition — **99 CHECKED-CONFORMANT**, **1 DIVERGENT** (CEN-B5, the
...
spec to review against until the census's design rounds rule them. **CEN-B5** (rule-71 skip) is the sole remaining divergence. **Both S-graded findings are closed:** CEN-M8/G4/J26 were promoted at PR #602's merged fix, and **CEN-D2 with CEN-D1 at PR #604's** (2026-09-03) — no S-graded divergence remains. Failed closed for the two distinct reasons already given above: **CEN-L8**,
...
| **CEN-L11** | **CHECKED-CONFORMANT** *(re-reviewed post-fix — history: the register's seed row, DIVERGENT from before P0f)* | Re-reviewed at `920a7d788` and fixed in the same round (2026-09-04). **The original framing was wrong, and the correction matters:** the row (and its FOLLOWUPS twin) described a *live* fund-loss path — a false construct verdict silently omitting an accepted output, deterministically unspendable. Walked at this sha, **all three skip arms are unreachable for any tx that passed validation**: the target-type `else` is foreclosed by CEN-H12/F8 (`txout_to_tagged_key` the sole accepted type), the `i >= outPk.size()` arm by four size-equality gates (`cryptonote_core.cpp:799`, `cryptonote_format_utils.cpp:137`, `blockchain.cpp:3289`, `:4031` — coinbase and non-coinbase, both paths), and `construct_leaf`'s `None` by the two point gates that make its only two attacker-supplied inputs canonical, prime-order and non-identity (`shekyl_check_output_keys` via `check_outs_valid`; `shekyl_check_commitment_masks` via `check_commitment_mask_valid`, `blockchain.cpp:3392`, both coinbase legs in `prevalidate_miner_transaction`). `check_outs_valid`'s own in-tree comment already named this hazard as its reason for existing — the register never connected it back. **So this was a latent silent-failure surface, not an active defect**, and the posed design question (fail-closed vs documented-skip) resolves without a trade: all three arms now **abort** (`blockchain_db.cpp:579` and above), each comment naming the gate that forecloses it. **The foreclosure is now load-bearing, so it is pinned in both directions:** reverse pointers at the three gate definitions warn that relaxing them surfaces as an abort at connect, and a Rust falsifier (`shekyl-ffi/tests/leaf_gate_agreement.rs`) asserts the gates accept only what the leaf builder can encode — red-observed by loosening `check_output_keys`. Empirical: 39 core-test chains connect with zero aborts | Digest identity **required**; a match on this row **is** correctness evidence |
| **CEN-L12** | **CHECKED-CONFORMANT** *(re-reviewed post-fix — history: DIVERGENT coupled to CEN-L11)* | Re-reviewed at `920a7d788` (2026-09-04). The maturity arithmetic always conformed (coinbase `+60`, others `+10`, applied identically to both live target variants; `unlock_time` absent from the path; the spec's staked arm retired). The clause that failed was **universality** — "*all* outputs are deferred" — because an accepted output could be dropped before being made pending. With CEN-L11's three arms now aborting instead of skipping, no accepted output can be dropped: the block is rejected rather than connecting with a hole. Universality holds | Digest identity **required**; a match on this row **is** correctness evidence |
...
| **CSR-3a** | The **conformance register** (§5.4.1): a bucket says a rule is *ratified*, never that the C++ *implements* it | every parity claim asserting correctness | **ROW COVERAGE COMPLETE 2026-09-02** — all **102** bucket-1/2 rows disposed: **99 CHECKED-CONFORMANT / 1 DIVERGENT / 2 failed closed** (P0f slices 1–8 + review rounds + three post-fix re-reviews): CEN-B5 (rule-71 FAKECHAIN skip; R9 owns) is the only divergence left; CEN-L8 and CEN-I12 failed closed. **Both S-grades are closed** — CEN-M8/G4/J26 promoted at PR #602's merged fix, CEN-D2/D1 at PR #604's; the examined-and-excluded note on L12 covers its **staked arm only** (a retired claim-era clause), not the row. **Coverage is complete over today's bucket-1/2 set; the set grows** — bucket promotions from design rounds enter UNREVIEWED, and **DRS-P0f** and the census own reviewing them; E2 must consult the register before calling any match correctness |
