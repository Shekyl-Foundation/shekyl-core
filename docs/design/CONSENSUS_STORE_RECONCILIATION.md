...
| **CEN-L14** (b4) | Five archival uniqueness rules have **no DB constraint at all** — flag-0 overwrites; `blockchain_db/blockchain_db.cpp:775–786` records the bug class that tolerance once masked. |
...
At 2026-09-01 the 10 bucket-4 rows were the load-bearing set: each was a
consensus rule whose *placement* was an accident of the LMDB write path, and
each would have become the new store's implicit specification had the store been
designed before the rule was ruled. **C2-R8 ruled them on 2026-09-14
([`CONSENSUS_C2_R8_STORE_PLACEMENT.md`](../completed/CONSENSUS_C2_R8_STORE_PLACEMENT.md))**
— not row by row but by a standing three-arm category test (rule / invariant /
unspecified) that any surface owner applies without a round — so the premise
this section was written under has been discharged: the store is now designed
*after* the rules it used to carry were ruled, which is the order this document
existed to force.
...
| --- | --- | --- | --- | --- |
| CEN-L1 | 2 | KI uniqueness = `MDB_NODUPDATA` exception, caught 2 frames up; pre-DB check dead | **S-OUT-KI** + S-CHAIN-W | **RULED C2-R8 Q6 (2026-09-14):** *both*, split — the intra-block rule is CEN-L1 (validator, over the block's own inputs), the chain-wide rule is CEN-I7 (validator, `ChainView::is_spent`); `spent_keys` set-ness is **SI-1**, a belt whose breach is `StoreInvariantViolated`, never a verdict |
| CEN-L2 | 3 | `prev_id` exists at height−1; blobs appended at `m_height` (`MDB_APPEND`) | **S-CHAIN-W** (belt landed 2026-09-15: `StoreInvariant::TipMismatch`) | **RULED C2-R8 Q6:** the parent-linkage rule is CEN-A2's (validator); parent-is-the-recorded-block-at-height−1-and-is-the-tip is **SI-2** (store invariant, fatal as `StoreInvariantViolated`). `BLOCK_PARENT_DNE` stops being a consensus outcome |
| CEN-L3 | 3 | `BLOCK_EXISTS` / `TX_EXISTS` reject at write; **only** connect-time uniqueness for the miner tx | **S-CHAIN-W** + S-TX | **RULED C2-R8 Q6:** neither ported nor specified — miner-tx uniqueness is a **corollary of CEN-F5** (coinbase binds height; main-chain heights are unique), verified at the alt case too (alt blocks are stored whole under K3's idempotent put, so `txs` never sees an alt miner tx); listed-tx uniqueness is CEN-G1's; `txs` set-ness is **SI-3**, main-chain block-hash uniqueness follows from **SI-2**. The intra-block twin (no listed tx hash twice in one block) is a validator block-level set pass, named in Q6's intra-block table |
| CEN-L4 | 3 | Block hash recomputed; contained txs stored under **claimed** hashes verbatim | **S-CHAIN-W** (landed 2026-09-15: `connect` writes `ChainValid`'s identities, recomputes nothing) | **RULED C2-R8 Q4/Q6:** the store computes **nothing** consensus-visible — identity is CEN-B6's; the validator derives every hash once and the store writes what `ChainValid` carries. The store-side recompute is **deleted, not moved** |
| CEN-L5 | 3 | Input-type whitelist duplicated at write time | S-CHAIN-W (dissolved 2026-09-15: `connect` matches `shekyl_wire::Input`) | **RULED C2-R8 Q6:** accidental duplicate — and it **dissolves rather than moves**: the store's input is a typed enum, so the whitelist is the type and there is no check to relocate |
| CEN-L6 | 3 | Every stored output carries an outPk commitment; amount-0 indexing | **S-OUT-KI** | **RULED C2-R8 Q6:** commitment presence is CEN-H8/H17's (validator; the store's output type carries a `Commitment` by construction). The amount-0 indexing choice is **arm C — unspecified** — and is routed to **R8b-2** as a spec question, not assigned as a placement |
| CEN-L13 | 3 | Corruption/IO guards abort rather than store inconsistent state | cross-surface | **RULED C2-R8 Q2/Q6:** not chain rules — each guard is classed into the register (**SI-5…SI-8**) or into `StoreCannot` (capability refusals such as pop-below-watermark); the three-class taxonomy `InvalidBlock` / `StoreInvariantViolated` / `StoreCannot` is the ruling, with the conversion ban gated by `check_store_error_conversion_ban.py` |
| CEN-L14 | 4 | Five archival uniqueness rules have **no** DB constraint (flag-0 overwrites) | **S-ARCH** | **RULED in mechanism, ROUTED in semantics (C2-R8 Q6, arm C):** the store separates `insert` (errors on existing) from `upsert` (declared intentional) — no silent overwrite anywhere — so each absence becomes an explicit declaration; *which* of the five is insert and which upsert is unnamed semantics and goes to **R8b-3…7**, one row each |
...
| CEN-L15 | 3 | Dead Monero-v4 RCT accumulation arm on the write path | S-CHAIN-W | delete, do not port — **not ported 2026-09-15** (`connect` counts `cumulative_rct_outputs` per the live rule only) |
...
shas pinned per slice): the **102** bucket-1/2 rows that existed when P0f ran have a recorded disposition. **That denominator has since grown:** C2-R1b promoted **nine** rows into bucket 2 on 2026-09-03 and C2-R1c **ten** more on 2026-09-04 (nine promotions plus the CEN-K1 → K1a/K1b split), CEN-I19 was minted into bucket 1 on 2026-09-06 with its implementation, and C2-R8 promoted CEN-L1 into bucket 2 on 2026-09-14, so the current bucket-1/2 set is **131** (counted from the census bucket column, 2026-09-14: 87 b1 + 44 b2) — and **every one of them carries a record: the set-difference is ZERO** (first reached by slice 10, 2026-09-11, and held through both later mints). CSR-3a's set-growth obligation fired for the first time when that gap opened at twenty-seven, and slices 9 and 10 closed it (eleven `CEN-K` rows, then the sixteen non-K remainder). **Counted from the rows of this register, not carried from the prior figure (2026-09-14): 131 recorded = 126 CHECKED-CONFORMANT + 2 DIVERGENT + 3 UNREVIEWED.** The 2 DIVERGENT are CEN-G6/CEN-G6b (ratified surge factor `S = 4`, shipped `50`); the 3 UNREVIEWED are CEN-L8 (failed closed on an unwired clause), CEN-I19 (re-opened 2026-09-14 by `PL-D3`'s amendment, review owed at its merge sha) and CEN-L1 (born UNREVIEWED with C2-R8, no implementation to review yet) — so **UNREVIEWED by absence is zero, and every UNREVIEWED row is one that says why.** Disposition of the 102 — **100 CHECKED-CONFORMANT**, **1 DIVERGENT** (CEN-B5, the
...
spec to review against until the census's design rounds rule them. **CEN-B5** (rule-71 skip — retired on PR #623 with its S1) was **re-reviewed at `e54e5b983` and PROMOTED 2026-09-11**, leaving **no divergent row at that date** (slice 10 then recorded two — CEN-G6/G6b); **CEN-L11** and **CEN-L12** were re-reviewed and promoted at PR #609's merged fix (2026-09-04); **CEN-I12** was re-reviewed and promoted at `667817d47`, the sha that merged its reconciliation (2026-09-05). **The review's two S-graded findings are closed** — CEN-M8/G4/J26 promoted at PR #602's merged fix, **CEN-D2 with CEN-D1 at PR #604's** (2026-09-03) — **a third was found 2026-09-04 and is **fixed on PR #623 (2026-09-05, census §7 #18)** — the check now runs at admission before the add, on every nettype; re-verified at the merged sha `e54e5b983` on 2026-09-11 and promoted, which **discharges the last gate on DRS-0**:** CEN-B5's header check compared the root *after* the add against a header filled from the root *before* it, so every non-FAKECHAIN chain would have halted at height 60 — **S1**, masked by the rule-71 skip, which the same PR retired (census §7 #17, #18). Failed closed for the reason already given above: **CEN-L8**,
whose settlement clause names behaviour that is not wired — it can be called
neither conformant nor divergent. (CEN-I12 failed closed beside it, its anchor
source disagreeing with itself, until the reconciliation of 2026-09-04 gave it
one spec to review against.)
...
`cryptonote_basic/cryptonote_format_utils.cpp:800–1000`, **W-CT** = `ct_semantics.cpp:206–401`.
...
| CEN-I12 | **CHECKED-CONFORMANT** *(re-reviewed and promoted 2026-09-05 at `667817d47`, the sha that merged the reconciliation; was failed closed round 6, 2026-09-03; **absent-key arm walked 2026-09-15 at `0aeb67619`** — verdict stands)* | **Absent-key arm, walked 2026-09-15 (S-CHAIN-W pre-flight, SCW-19; census §7 #21):** the 2026-09-05 verdict rested on "record key `ref_height` holds the state after blocks `0..ref_height − 1` connected" and did not say what the reader does when the key is absent. Walked: the record is **dense from key 1** — `store_curve_tree_root_at_height(prev_height + 1, …)` at `src/blockchain_db/blockchain_db.cpp:663`–`:664` sits inside the `blk.major_version >= HF_VERSION_FCMP_PLUS_PLUS_PQC` gate (`:493`, always true: the constant is `1`) and **outside** the `if (new_output_count > 0)` block that closes at `:650`, so every connect writes its row whether or not the tree grew. The only absent key is **0** (no connect writes it). For a missing key the reader returns a zero-initialised array silently on `MDB_NOTFOUND` (`src/blockchain_db/lmdb/db_lmdb.cpp:9064`–`:9080`), which `deserialize_tree_root` decodes to the **identity point** (`rust/shekyl-oxide/crypto/helioselene/src/point.rs:373`–`:409`, `x = 0` → *O*). `ref_height = 0` is selectable (age bounds only, `src/cryptonote_core/blockchain.cpp:3748`–`:3763`, while `chain_height ≤ 100`), so a proof anchored at genesis is verified against *O*. **Consequence-free:** the state at height 0 is the empty tree, which has no members, so no honest proof anchors there; a forged proof against *O* needs `Σ cᵢ·Gᵢ = −HASH_INIT` (a discrete-log break); the gap set is identical on every node. The verdict therefore stands; what a reference to genesis *should* resolve to (the empty-tree root, or a refusal of `ref_height = 0`) is a rule question routed to E6 slice 6 with this row as its citation. *Record of the walk:* a pre-flight draft misread the `:639`–`:650` brace scope as enclosing `:664`, concluded the record was sparse over keys `0..60`, and flipped this row DIVERGENT for a few hours on 2026-09-15 before Copilot's re-read of the source corrected it the same day; the flip never merged. **Pass condition for the rewrite's store (CSR-3a):** `root_at(h)` reads key `h`; a recorded height `≥ 1` with no row is store corruption (`Fault`), never a rule outcome; height 0 is the empty tree. **The fail-closed direction is load-bearing, and it is the part of this walk that must survive:** it was established by reading `helioselene`'s `from_bytes` — 32 zero bytes are a *valid* encoding that maps to the identity, so the read succeeds twice over (at the DB layer and at the decode layer) and the question became "what can be proven against *O*?" Had the answer gone the other way — a proof forgeable against the identity root — a C++ node on a short chain would be exploitable during exactly the window testnet runs in, and this row would be a live S0 rather than a conformant one. **The defect shape, named so the review can be repeated:** a silent `MDB_NOTFOUND` arm is fine when the default is distinguishable from a legitimate value (an empty vector, a zero count); it bites when the default lies *inside the valid range of the type*, so absence and a real value are indistinguishable to the caller. **Reviewed 2026-09-15 across `db_lmdb.cpp` for that shape** — functions returning a fixed-width cryptographic value (hash, point, key) with a silent absent arm: `get_curve_tree_root_at_height` is the one member (and its `result == 0 && mv_size != 32` arm also returns zeros, since a wrong-size row passes the `!= MDB_NOTFOUND` test); `top_block_hash` returns `null_hash` only for an empty chain, guarded by the `height()` the caller already reads; `get_output_key` (`:4460`) throws `OUTPUT_DNE` (or returns fewer entries under `allow_partial`, distinguishable by count); `get_block_hash_from_height` throws `BLOCK_DNE`; `get_curve_tree_root` returns `hash_init` for the empty tree, which is that tree's root by definition, not a substitute. No C++ change (rule 20; the implementation is a deletion target). **The general fix is already the house style on the Rust side, and this row is where it is said:** absence is made *unrepresentable as a value* — `AtHeight<T>` has no conversion to `Option` and forces the caller through `AboveTip`; the store's view returns `Fault` for a hole below the tip rather than any default; no codec decodes an absent row into an in-range value. "We made absence a case the caller must handle, not data that reads as a value" is the durable lesson, and it outlives CEN-I12. *History —* **Re-review at `667817d47` (PR #622):** all three `check_tx_inputs` arms — archival bond-post (`blockchain.cpp:3810`), archival emission (`:3959`), general FCMP++ (`:4217`) — resolve `rv.referenceBlock` to its height with `block_exists` (`:3788`, `:3937`, `:4183`), bound it by MIN/MAX age, then read `m_db->get_curve_tree_root_at_height(ref_height)` — the node's own per-height record, never the header. Record key `ref_height` holds the state after blocks `0..ref_height − 1` connected and before the reference block's own drain (post-add keying from the parent's connect; the CEN-B5 keying pin in `archival_substrate_lmdb.cpp` holds `record[N] == pre-add root at chain height N` on real LMDB), which is exactly the ruled state. Conformance rests on that state definition, not on slice 7's placeholder rationale (the in-code comment at `:4215` that stated the placeholder rationale is corrected on PR #623: a consequence, not a reason). The header witness's binding to the record is CEN-B5's row, not this one — the verifier never reads the header, so this verdict does not depend on B5's state. The depth pseudocode split (steps 2b/2c) is CEN-I13's, found-not-ruled. **Spec-level consequence:** the per-height root record is now a consensus witness, not an LMDB detail — `get_curve_tree_root_at_height` is already on DRS's surface map, so nothing is missing today, but any store the rewrite uses must keep a record of the tree state at every height. *As found (round 6) —* The anchor source is **internally split**, so there is no unambiguous spec to affirm against: `FCMP_PLUS_PLUS.md` §7 step 2's normative pseudocode says `get_curve_tree_root_at(ref_height)` (the table read the code performs, `blockchain.cpp:4285`) while its own prose two paragraphs later says `get_block_header(rv.referenceBlock).curve_tree_root` — and the census row records the table-vs-header divergence as an open §7 item. Slice 7 promoted on the in-code rationale (FAKECHAIN headers carry placeholder roots), but **an in-code rationale is not a ratification**, and the FAKECHAIN-shaped reasoning is itself rule-71-adjacent. Same shape as CEN-L8: a clause whose own source disagrees with itself can be neither conformant nor divergent. **Spec reconciled 2026-09-04** (census §7 #16): `git log -S` shows the section was split from its first commit and the prose narrated the header read the code performed until `292c00aff7` (2026-04-13); the anchor is ruled a **state property** — the tree at chain height `ref_height`, after the reference block's parent connects and before its own drain (boundary corrected on review: the first draft said one block later) — with the header field and the per-height record as its two witnesses, the verifier reading its own record. Re-review owed at a sha containing the reconciliation; it must walk all three read sites (`:3810`, `:3959`, `:4217`) and rest on the state definition plus B5, not on the placeholder rationale slice 7 used | Identity **is** the pass condition: the rewrite's verifier must read its own per-height record of the tree state at `ref_height` (a spec-level requirement on its store since 2026-09-04), never the reference block's header. The promotion PR's scope as recorded when this row failed closed — row, the three "split anchor source" status lines, the count re-derived from the column, the spec-level note — is discharged here |
...
| CEN-I19 | **UNREVIEWED** *(amended 2026-09-14 by `PL-D3`, born with its implementation: `0x07` stride `32·n → 64·n` and the leaf-commitment point content rule, `check_pqc_leaf_entries` over the payload bytes the adapter now passes; the 2026-09-11 CHECKED-CONFORMANT re-review at `2ec553a3f`, the sha that merged PR #630, covered the pre-amendment shape rule and is kept below; the row was first UNREVIEWED by construction, born 2026-09-06 with its implementation)* | **Re-review owed at the sha that merges the `PL-D3` implementation PR:** the adapter passes the `0x07` payload (`src/cryptonote_basic/cryptonote_format_utils.cpp` `check_tx_extra_pqc_field_shape`), the FFI gained `leaf_blob`/`leaf_blob_len`, code 10 = `LEAF_POINT` (`FCMP_SPEND_LINKABILITY.md` §6.2). **Re-review at `2ec553a3f` (PR #630), pre-amendment:** every clause checked at the sha. **Adapter** (`cryptonote_basic/cryptonote_format_utils.cpp:1000`) parses with the daemon's own `parse_tx_extra`, collects only the 0x06/0x07 **lengths**, and takes both the verdict and the message from `shekyl_tx_extra_pqc_field_shape` — it applies no rule of its own. **Callers, all four:** relay/pool and block via `core::check_tx_semantic` (`cryptonote_core.cpp:816`), reached from `ver_non_input_consensus_templated` Rule 5 (`tx_verification_utils.cpp:90`) whose wrappers are called on the **block** path at `blockchain.cpp:2337` and `:5919` and the **pool** path at `tx_pool.cpp:236`; the **coinbase** separately at `blockchain.cpp:1582`, because the miner tx never passes `check_tx_semantic`. **The `kept_by_block` negative is structural, not merely absent:** `check_tx_semantic`'s signature is `(tx, tvc, hf_version)` — the parameter does not exist, and `kept_by_block` appears nowhere in `tx_verification_utils.cpp`, so the exemption is unrepresentable rather than omitted. **Fail-open closed, and the abort is reachable:** the collector's `extract_leaf_hashes` (`blockchain_db/blockchain_db.cpp:528–530`) calls the same adapter and **throws `DB_ERROR`** where it once returned `{}` and let `collect_outputs` zero-fill `h_pqc`; `BlockchainDB::add_block` contains no `try`/`catch`, and the caller's `catch (const std::exception&)` at `blockchain.cpp:~6430` sets `m_batch_success = false`, `bvc.m_verifivation_failed = true`, returns txs to the pool and returns false — **fail-closed**, so the throw rejects the block rather than being swallowed. **One home confirmed by sweep:** the only other C++ touching 0x06/0x07 is `cryptonote_tx_utils.cpp:196/198` and `:497/499`, which **construct** the fields; no second parser applies a variant. In Rust, `check_pqc_field_shape_of` (`tx_extra.rs:532`) is an extract-and-delegate adapter over `check_pqc_field_shape` (`:480`), not a second rule. **Coverage:** three Rust tests including `rick_s_vectors_are_refused`, plus thirteen wired C++ tests (`tests/unit_tests/tx_extra_pqc_field_shape.cpp`, CMakeLists `:126`) that cover both the clauses a code walk cannot settle — `coinbase_with_duplicate_leaf_hash_field_is_rejected_before_writing` and `db_collector_refuses_a_short_leaf_hash_field_instead_of_zero_filling`, the named regression for the exact fail-open. **Divergence since:** all four call sites and the single-homed rule are intact on `dev`, tests still wired. **What this re-review did NOT do:** it did not execute either suite, and it did not audit `check_pqc_field_shape`'s *rule content* against a spec — the pass condition checked is identity-with-the-one-rule and the wiring that makes it load-bearing, not whether the rule itself is the ruled shape. **As built — retained:** The rule and its implementation landed together (census §7 #19): admission adapter `cryptonote_basic/cryptonote_format_utils.cpp:1000` over the daemon's own parse, called at `cryptonote_core.cpp:816` (relay + block, no `kept_by_block` gate) and `blockchain.cpp:1582` (coinbase); the rule itself in `shekyl-wire` `check_pqc_field_shape` via `shekyl_tx_extra_pqc_field_shape`; `blockchain_db/blockchain_db.cpp:528` collector aborts instead of zero-filling. Red-first vectors observed accepted before the rule at all three gates; serve-credit must-accept control. A row born with its fix is UNREVIEWED by construction — the register records conformance only from a review at a merged sha. **REWRITE-NOTE:** the port applies the same `shekyl-wire` rule to its own parse — one home, identity with `check_pqc_field_shape` is the pass condition. Round 4 closed the tag-set divergence this note originally named (`0x03`/`0xDE` deleted from C++; `0x0B` modelled in `shekyl-wire`); an unparseable `extra` is now refused. The rewrite's pass condition is identity with that one rule, not with a second C++ parser |
| CEN-I20 | **UNREVIEWED** *(born 2026-09-23 with its implementation — the coinbase extra grammar, `TX_EXTRA_RUST_CUTOVER.md` TXE-Q6′; UNREVIEWED by construction, as CEN-I19 was at birth)* | **Review owed at the sha that merges the tx_extra cutover PR.** What to check there: the grammar is `shekyl_wire::tx_extra::check_coinbase_extra_shape` (exactly `[0x01, 0x02(8), 0x06(1120·n), 0x07(64·n)]`, `[0x01, 0x02]` at `n = 0`, nothing else, in order; off the coinbase no `0x02`) reached through `check_tx_extra_shape(fields, n, ExtraSubject)`; the C++ adapter `check_tx_extra_shape(tx, reason)` derives `is_coinbase` from the tx and is the one call at relay (`cryptonote_core.cpp`), coinbase connect (`blockchain.cpp`) and DB add (`src/blockchain_db/blockchain_db.cpp` via `shekyl_tx_extra_leaf_entries`, abort-unreachable); the writer `shekyl_coinbase_extra` judges what it builds; the wallet's `Transaction::validate_context_free_pruned` applies the same function. KATs: `coinbase_grammar` (8, `src/tx_extra.rs`), `tx_extra_codec_ffi::tests` (9), `mining_parity.coinbase_writer_reproduces_the_genesis_extra_byte_for_byte` (three networks), `validation.rs` `cen_m4_…` / `genesis_coinbase_carries_the_nonce…`. Not applied by `shekyl-chain-rules` (neither is I19) — E6 slice 4's. |
| CEN-L16 | **UNREVIEWED** *(born 2026-09-23 as an R8-class placement row — the as-of-height holds-shard predicate evaluated inside the store, `DRS_E1_SARCH.md` SAR-2 / SAR-7; UNREVIEWED by construction)* | **Review owed when E4 ports the fold** (`SAR-Q7` RULED: the port travels with E4's journal ruling; the row lands now so the R8 gap is visible). What to check there: `holds_shard_at(&BondRecord, ShardId, BlockHeight, &[SlashLogEntry])` in `shekyl-archival-retention` reproduces `archival_bond_holds_shard_of` (`db_lmdb.cpp:4889–4950`) and `archival_slash_removed_holding_after` (`:4804–4870`) on the LMDB tests' as-of-height cases (`archival_substrate_lmdb.cpp:1674–1893`) — add-epoch + 1, complete-tree back-to-join, slash-above-`h` reconstruction, the `u64::MAX` non-wrap; the store returns A1 + A2 typed and folds nothing. Twin: CEN-J8 (the rule). |
...
| CEN-B1 | **CHECKED-CONFORMANT** | `HardFork::do_check` requires `block_version == heights[current].version` **and** `voting_version >=` (`cryptonote_basic/hardfork.cpp:109–113`); with the single `{1,1}` entry (slice 5) major_version must equal 1; called on main (`:5833`) and alt |
| CEN-B4 | **CHECKED-CONFORMANT** *(against the v1 rule; re-review owed at the merged sha of PR #734, which amended the rule to the `SF-D8` v2 anchor-bound transcript on 2026-09-13)* | `verify_block_attestation` on both paths (`:5843` main, `:2312` alt) → Rust verdict, logged per code. The **runs-before-PoW ordering** is the census §6 finding 7's live FOLLOWUPS row — already on record, not re-opened here. **Residue found in #734 review (recorded in FOLLOWUPS, not a v2 regression):** both paths resolve the bond pubkey from main-chain `m_db` state, so an alt block carrying a pass from a bond that joined only on that alt chain is refused `BOND_ABSENT` at alt time |
| CEN-B5 | **CHECKED-CONFORMANT** *(re-reviewed and promoted 2026-09-11 at `e54e5b983`, the sha that merged PR #623; was DIVERGENT from P0f slice 8 with a second, S1 divergence found 2026-09-04)* | **Re-review at `e54e5b983` (PR #623):** the four claims were established independently of the in-code comment, which is the failure mode this row exists to guard. **(i) Admission, before the add:** in `handle_block_to_main_chain`, `check_hash` at `:5840` → the compare at `:5867–5897` → `prevalidate_miner_transaction` at `:5900` → `m_db->add_block` at `:6410`; the compare precedes the write by ~513 lines on the one path all three main-chain adds take. **(ii) Every nettype:** `awk` over the whole function body (`:5722–6507`) returns **no** `m_nettype` token — the sole `FAKECHAIN` occurrence is the word inside the comment. The check is not merely un-skipped, it is structurally unskippable here. **(iii) Skip retired, not bypassed:** `get_curve_tree_root` now has exactly **one** compare site in the file; the post-add compare and its pop arm are gone. Sibling sweep (steering's ask) found twelve surviving `FAKECHAIN` branches — eleven in `Blockchain::init` (bootstrap configuration, not a per-block consensus decision) and one at `:5234` in `regtest_inject_archival_serve_credit`, which is **inverse polarity**: it *refuses* when nettype is not FAKECHAIN, a fence on a test-only entry point rather than a consensus check being skipped. None weakens a per-block check. The `CEN-I2`/`D7`/`D3` family stays census R9's. **(iv) Regression coverage — checked because a fix without one should not imply it:** `tests/unit_tests/curve_tree_header_root_check.cpp` is **wired** (`tests/unit_tests/CMakeLists.txt:124`, unconditional in the main source list — an unwired test file would have been a false green) and carries two TESTNET/real-LMDB tests. `testnet_chain_connects_through_the_first_maturing_block` mines through `CRYPTONOTE_MINED_MONEY_UNLOCK_WINDOW`, asserts block 60 connects, leaf count 0 → positive, and `header(60) == record[60]` — the height-60 halt reappearing fails it. `testnet_chain_rejects_a_header_with_the_wrong_root_before_writing` supplies the limb the row did not claim: a bad header is rejected **and** height, tip and tree root are all unchanged, so connected-then-popped is excluded behaviourally rather than by reading the ordering. The FAKECHAIN enabler holds: `tests/core_tests/chaingen.cpp` computes real roots through `shekyl_curve_tree_replica_*`. **Divergence since:** `dev` has moved the check to `:6050–6069`, still one site, still unwrapped; both tests survive under the same names and still on TESTNET. **What this re-review did NOT do:** it did not execute the suite — no red→green was re-observed here, and the promotion rests on the code walk plus the tests' assertions and their build wiring, the same boundary M8/G4/J26 recorded. **As fixed — retained, this is the record the re-review checked against.** **S1 fixed (2026-09-05, PR #623; census §7 #18):** the compare now runs at admission in `handle_block_to_main_chain`, after proof-of-work and before the miner-tx prevalidation (`blockchain.cpp:5885–5897` at `a5c266125`, the compare block, no nettype wrapper), against `get_curve_tree_root()` read while the tip is still the block's parent — the state at chain height N; mismatch rejects before any write and the post-add compare with its pop arm is deleted. Red → green observed by `curve_tree_header_root_check.cpp`, a TESTNET (non-FAKECHAIN) Blockchain fixture at fixed difficulty 1 on real LMDB: block 60 rejected before, connected after. All three main-chain add paths (connect, promotion, restore) run through that one function, so the alt/reorg path is covered identically. The FAKECHAIN skip is deleted in the same PR: the core_tests generator now computes real roots through the Rust curve-tree client (`shekyl_curve_tree_replica_*`, census §7 #18), the fake-DB test doubles report the empty-tree sentinel, and the check runs on every nettype. The row stays DIVERGENT only until the merged sha is re-reviewed (promotion pins the merged fix, M8/D2/L11 precedent). **Second divergence as found (2026-09-04, PR #618 review; census §7 #17):** the check compares the header against `get_curve_tree_root()` read *after* `add_block` (`:6413`), while `create_block_template` fills the header from the root *before* the add (`:2006`) — the state at chain height N that the per-height record (key N), the wallet client and the CT-2 KAT name. The operands differ at every block where a leaf drains, observed on real LMDB by `archival_substrate_lmdb.cpp` `cen_b5_header_root_is_the_tip_root_before_add_block_not_after (renamed on PR #623 from `cen_b5_post_add_root_is_not_the_state_the_header_was_filled_from`, the name it carried when found)` (pre-add root == record[N]; post-add root == record[N+1]; leaf count 0 → 1). Derived from that observation (no non-FAKECHAIN fixture exists to see it): on mainnet/testnet/stagenet block 60 is rejected and the chain halts — an S1. **Why DIVERGENT rather than failed-closed like CEN-I12:** the template and the check disagree with *each other*, so the code diverges from every reading of what the header commits to; §5's boundary being open does not make the row unreviewable. **The rule-71 skip below is the finding's cause, not its context:** `m_nettype != FAKECHAIN` gating a check is nettype selecting control flow, so every Blockchain-level test and every `--regtest` run (`cryptonote_core.cpp:423–426` sets `FAKECHAIN`) took the arm without the check, and the whole test surface was structurally blind to a defect that halts every real network. The skip did not risk masking something; it masked this. **Fixed on PR #623** (Rick's fix-or-accept ruling was the authorization of that edit); the direction taken — compare against the pre-add root, before any write — is the REWRITE-NOTE this row already carried. **First divergence — re-walked round 6:** slice 8 cited the compare at `:6462–6472` and **missed the guard one line above it** — the whole post-connect root check is wrapped in `m_nettype != FAKECHAIN` (`:6468` at this sha), while the ratified rule says **every** mismatch rejects and rule 71 names this exact skip as "the counter-example this rule exists to retire" (`.cursor/rules/71-network-uniformity.mdc:60–72`, census batch R9 owns the family). Not production-reachable (FAKECHAIN is the test chain type), so no §7.2 S-grade — but a nettype-conditioned consensus check is the divergence class rule 71 bans, and "testnet passed" logic-coverage claims are weakened exactly as the rule predicts. **REWRITE-NOTE** (kept from the prior verdict): connect-then-check-then-compensating-pop is inherited arrangement — the rewrite checks the root **before** txn commit, and uniformly across nettypes (the test generator must produce real roots instead of the check being skipped) | Identity is **not** the pass condition; promotion is gated on **both** the S1 fix-or-accept ruling (which state the header commits to, and the check comparing that state) **and** the R9 uniformity retirement (check runs on every nettype, or the exception is ratified loud per rule 71) |
...
| CEN-D2 | **CHECKED-CONFORMANT** *(re-reviewed post-fix — history: found DIVERGENT at S1 in review round 4; ruled FIX 2026-09-03; fixed by PR #604)* | Re-reviewed at `813d2ed37` (2026-09-03 — the `dev` merge of PR #604). The schema half was always conformant (`pow_registry.cpp` returns RandomX unconditionally). **The fail-closed clause now holds at every difficulty:** the verifier's returned bool is the gate at **all three** consumers — main connect rejects before `check_hash` (`blockchain.cpp:5828–5840`), the alt path rejects on `get_altblock_longhash`'s new bool return (`:2285–2294`), and the precompute worker refuses to cache an uncomputed hash (`:6773–6781`), which mattered because a table hit is trusted downstream without re-checking. The `0xff…` sentinel survives as a belt for display-only callers (RPC `pow_hash` fills), and both comments that overstated it ("guarantees `check_hash()` rejects it") now name the bool as the gate. **Attribution checked, not assumed:** verifier failure is `reject_block_internal`; genuine `check_hash` failure is `reject_block_bad_pow`. P2P asks `shekyl_block_announce_action` / `shekyl_block_sync_action` (PWD-B7 block twin, 2026-09-10): only a severing drop severs, and only `RejectedBadPow` carries the heavier score. Internal does not drop. **The principled reason is that the two verdicts are different claims:** `RejectedBadPow` asserts *this block's PoW is bad*, while a verifier failure asserts nothing about the block at all — we could not compute the hash. Attributing the second as the first would be wrong whoever caused it. **Inducibility, checked against the FFI's actual taxonomy** (`pow_randomx_ffi.rs`, `shekyl_ffi.h` §PoW): the codes that can be *returned* are boundary-contract failures — `ERR_NULL_PTR` (a null seedhash/out buffer, caller-side and never attacker-controlled) and `ERR_DATA_TOO_LARGE` (`data_len > 2 MiB`) — while `ERR_CACHE_DERIVE_FAILED` and `ERR_INTERNAL` are **reserved and never returned**: under `panic = "abort"` a derivation OOM or panic aborts the process rather than returning failure. `ERR_DATA_TOO_LARGE` is the only data-dependent code and is unreachable here, because the hashed input is `get_block_hashing_blob` — a serialized block header plus a 32-byte tree root and a length varint, on the order of a hundred bytes, which does **not** grow with transaction count or block weight. So a peer cannot farm these events; and were that ever to change, the classification would still be InternalFailure, which does not sever. **Evidence is behavioural, not only a walk:** five verdict-contract unit tests (`pow_longhash_gate.cpp`) plus two connect-path regressions (`gen_block_pow_verifier_failure_{main,alt}`) that run at test difficulty **1** — exactly where the belt fails open — and assert rejection *as unproven*; reverting either consumer turns them red (observed). The test seam's containment is itself gated (`scripts/ci/check_pow_test_seam.sh`, three invariants, in the bundled grep job), and it publishes with release/acquire so a worker cannot observe the pointer without an edge to the schema's construction | Digest identity **required**; a match on this row **is** correctness evidence |
| CEN-D3 | **CHECKED-CONFORMANT** *(pass condition for the rewrite added 2026-09-19 — E6 slice 2 pre-flight F5, promoted to this register by ruling; verdict stands)* | `seedheight` matches the rule symbol-for-symbol (`0 for h ≤ blocks+lag, else (h−lag−1) & !(blocks−1)`, 2048/64; re-read at `1d8f9d3b5` — E6 slice 2 commit 6a moved the body from `shekyl-pow-randomx` to `rust/shekyl-difficulty/src/seed_epoch.rs:49`–`:55` so the validator adopts it without the engine crate; the formula and the pinned values are unchanged); one Rust site, C++ calls the FFI (`:669`); the FAKECHAIN schedule override **warns loudly** at startup. **Pass condition for the rewrite's validator (CSR-3a) — a deliberate, port-introduced behavioural difference, the CEN-I12-corrected-read class:** `shekyl-chain-rules` derives the seed height from the two constants (`SEEDHASH_EPOCH_BLOCKS = 2048`, `SEEDHASH_EPOCH_LAG = 64`) at **every** nettype and reads the seed id as `block_at(seedheight(c)).hash` through `ChainView`; **it reads no environment** — the `SEEDHASH_EPOCH_*` lever (`clamp_lag`/`clamp_blocks`, at `1d8f9d3b5` in `rust/shekyl-ffi/src/pow_randomx_ffi.rs:299`/`:315` — the FFI boundary that reads the environment; C++ init refusal on public networks) has no path into the validator, so a FAKECHAIN chain mined under an overridden schedule **fails CEN-D1 in the rewrite by design**. Predicted E2 consequence: a regtest comparison run with `SEEDHASH_EPOCH_*` set diverges at the first block past height 2112 and that divergence is **expected, not a defect** — the comparator harness runs without the lever. Checked 2026-09-19: no harness in `tests/`, `scripts/` or `rust/` sets it (only `tests/unit_tests/seed_epoch.cpp` exercises the clamp), so the lever has no consumer; if a regtest need for a short seed epoch appears it enters as fakechain **rule-set data** (`CHAIN_RULES_SLICE_2.md` §8.1 Q10's shape), never as an environment read. Record: [`CHAIN_RULES_SLICE_2.md`](../completed/CHAIN_RULES_SLICE_2.md) §6 F5. |
...
| CEN-G4 | **CHECKED-CONFORMANT** *(re-reviewed post-fix — history: DIVERGENT coupled to M8, round 2)* | Re-reviewed at `a45916c66` (round 6). Per-tx `check_tx_inputs` at connect with failure → `add_block_as_invalid` + `reject_block_from_tvc` (folds the tx drop verdict onto `bvc`) + txs returned — the dispatch conforms, and the clause's precondition now holds: with the hash-gated cache the skip is granted **only** to admission-verified bytes, exactly "FCMP++ txs that verified at pool admission skip only the FCMP proof re-verify (structural checks re-run)". UPDATE 2026-09-10: the inherited `m_verifivation_failed` bool is gone; the outcome is `BlockIngest::Rejected` | Digest identity **required**; a match on this row **is** correctness evidence |
...
| CEN-K3 | **CHECKED-CONFORMANT** | W-HA + DB belt. Add-path assert `CHECK_AND_ASSERT_MES(!m_db->get_alt_block(id, NULL, NULL), …)` (`src/cryptonote_core/blockchain.cpp:2488`) immediately before `add_alt_block` (`:2495`); DB belt `mdb_cursor_put(m_cur_alt_blocks, &k, &v, MDB_NODUPDATA)` (`src/blockchain_db/lmdb/db_lmdb.cpp:4744`). The **routing leg** (`have_block_unlocked` consults `get_alt_block`, `:3155`; `add_new_block` records `SHEKYL_BLOCK_INGEST_ALREADY_EXISTS` = `BlockIngest::AlreadyExists` and returns `false`, `:6825–6828`) is **CEN-A1's** (UNREVIEWED when this row was written; **CHECKED-CONFORMANT as of slice 10**): this row affirms the assert and the DB belt, cites the leg's presence, and claims nothing for it. Precise word: an already-stored alt block is recorded *already-exists* (not a verification failure) at the leg, and would be *refused* by the assert and the DB if the leg were bypassed. Census `:2410→:2488`, `:3075→:3155` |
| CEN-K4 | **CHECKED-CONFORMANT** | W-HA. Ladder at the pin, in order: height-0 (`:2232`) → checkpoint window (`:2245`) → version (`:2254`; CEN-B1 CHECKED-CONFORMANT) → attestation (`:2264`; CEN-B4 CHECKED-CONFORMANT) → `build_alt_chain` → agc advance (`:2312`) → MTP/FTL `check_block_timestamp(const std::vector<uint64_t>&, const block&, uint64_t&)` (`:2326`; CEN-C1/C2 CHECKED-CONFORMANT) → checkpoint conformance (`:2337`) → alt difficulty (W-AD; CEN-D1 CHECKED-CONFORMANT, **CEN-D5/D6 UNREVIEWED when this row was written; both CHECKED-CONFORMANT as of slice 10**) → PoW (`reject_block_bad_pow`, `:2378`) → `prevalidate_miner_transaction` (`:2382`) → NIC (`:2407`) → supplement `add_tx` (`:2425`) → store (`:2495`) → fork choice (`:2524`). *Negative, untruncated grep over `:2223–2600`:* `validate_miner_transaction`, `check_tx_inputs` (either overload, `:3319`/`:3517`) and the curve-tree verify family are **absent** (comment mentions only) — prevalidate-only, as ratified (Q1a). This row affirms the **ordering and the prevalidate-only tier**; the member rules' bodies are theirs |
...
### 6.2 R8 gains an explicit consumer — RULED 2026-09-14

**Status (2026-09-14): R8 is RULED** —
[`CONSENSUS_C2_R8_STORE_PLACEMENT.md`](../completed/CONSENSUS_C2_R8_STORE_PLACEMENT.md),
dispatched out of §10's proposed order because the consumer this section named
(the Rust store's write path, S-CHAIN-W) had arrived at its blocker: DRS-E1
increment 2 landed every write-path precondition and could go no further
without it. R8 ruled ahead of R4 and R5, which is the direction CSR-5 ratified
and the slot it declined to fix. What R8 handed the store is not a per-row
answer but a **standing three-arm test** (consensus rule / store invariant /
unspecified → R8b), an **error taxonomy with a gated conversion ban**, and the
**transaction-scoping** rule for the validator's `ChainView` — the three things
the store needed before its connect path could be designed. The paragraphs
below record the queue-position argument as it stood; they are records-was.
...
| --- | --- | --- | --- |
| **CSR-1** | R8 is the ruling instrument for store-enforced rules; DRS-C's surface map is its input (§4) | store design | **RATIFIED 2026-09-01; DISCHARGED 2026-09-14** — R8 ruled its 8 rows by a standing category test ([`CONSENSUS_C2_R8_STORE_PLACEMENT.md`](../completed/CONSENSUS_C2_R8_STORE_PLACEMENT.md)): L1 → bucket 2 (minted, UNREVIEWED in §5.4.1), L2/L3/L4/L5/L6/L13 → bucket 3, L14 mechanism ruled + semantics → R8b-3…7; belts re-homed as `SI-1…SI-8` in [`STORE_INVARIANT_REGISTER.md`](STORE_INVARIANT_REGISTER.md) with its bijection gate. B3/K3 were never R8's (R4's; R1c's) and that held |
...
| **CSR-3a** | The **conformance register** (§5.4.1): a bucket says a rule is *ratified*, never that the C++ *implements* it | every parity claim asserting correctness | **ROW COVERAGE COMPLETE OVER THE 2026-09-02 SET** — the **102** bucket-1/2 rows that existed then are disposed: **100 CHECKED-CONFORMANT / 1 DIVERGENT / 1 failed closed** over P0f's **102-row** snapshot (the live bucket-1/2 set is **121** after C2-R1b's nine and C2-R1c's ten, all nineteen UNREVIEWED pending review): CEN-B5 (rule-71 FAKECHAIN skip — retired on PR #623 with its S1) **re-reviewed and promoted 2026-09-11 at `e54e5b983`**; CEN-L8 failed closed; CEN-I12 re-reviewed and promoted at `667817d47` (2026-09-05). **The review's two S-grades are closed** — CEN-M8/G4/J26 promoted at PR #602's merged fix, CEN-D2/D1 at PR #604's — and **CEN-L11/L12 were promoted at PR #609's** (2026-09-04); **a third S-grade was found 2026-09-04 and is FIXED on PR #623 (2026-09-05): CEN-B5's header check compared the post-add root against a pre-add header — S1, every non-FAKECHAIN chain would have halted at 60; now checked at admission on every nettype, **re-verified and promoted 2026-09-11 at `e54e5b983` (census §7 #17, #18)**; the examined-and-excluded note on L12 covers its **staked arm only** (a retired claim-era clause), not the row. **Coverage is complete over that snapshot, and the live set has already outgrown it** — C2-R1b's nine (2026-09-03) and C2-R1c's ten (2026-09-04) promotions are UNREVIEWED, as any future bucket promotion is on arrival; **DRS-P0f** and the census own reviewing them; E2 must consult the register before calling any match correctness |
...
| **CSR-5** | Re-price R8's §10 queue position (§6.2) | R8 dispatch | **DIRECTION RULED 2026-09-01, no slot fixed; DISCHARGED 2026-09-14** — R8 dispatched when its consumer reached the blocker (DRS-E1 increment 2 landed, S-CHAIN-W could not start), ahead of R4 and R5. The slot was set by the consumer arriving, which is what declining to lock a number was for |
...
| **2026-09-23** | **CEN-I20 minted with its implementation — register 126 / 2 / 4.** The coinbase extra grammar (`TX_EXTRA_RUST_CUTOVER.md` TXE-Q6′): joins UNREVIEWED by construction, as CEN-I19 did at birth; its review is owed at the sha that merges the cutover PR, against the checklist in its row. Also recorded: CEN-I19's adapter no longer parses — `check_tx_extra_shape` hands bytes to `shekyl_tx_extra_shape_of` and the C++ parser is deleted; the 2026-09-11 re-review's "adapter parses with the daemon's own `parse_tx_extra`" describes a mechanism that no longer exists, so I19's owed re-review covers that too. |
| **2026-09-23** | **CEN-L16 minted — register 126 / 2 / 5.** An R8-class placement row from S-ARCH's pre-flight (`DRS_E1_SARCH.md` SAR-2 / SAR-7, ruled on PR #840): the as-of-height holds-shard predicate — CEN-J8's operand and the slash writer's — is computed inside the store (`db_lmdb.cpp:4889`), reading the slash log as history. C2-R8's sweep drew its rows from the write paths and did not cover reads. Joins UNREVIEWED by construction; review owed when E4 ports the fold (`SAR-Q7`). |
| **2026-09-14** | **C2-R8 RULED — storage-layer enforcement placement ([`CONSENSUS_C2_R8_STORE_PLACEMENT.md`](../completed/CONSENSUS_C2_R8_STORE_PLACEMENT.md)); CSR-1 and CSR-5 discharged; register 126 / 2 / 3 over a 131-row bucket-1/2 set.** Ruled as a **standing three-arm test** binding any rule on any surface (would it still have to hold if the consensus rules changed? — no: consensus rule, validator; yes: store invariant, register; neither nameable: unspecified, R8b), not as eight dispositions. **Dispositions:** CEN-L1 minted into bucket 2 as the intra-block key-image rule (born UNREVIEWED, §5.4.1); L2/L3/L4/L5/L6/L13 → bucket 3 (absorbed into A2 / F5-corollary / B6 / dissolved into a typed enum / H8-H17 / re-homed as `SI-n`); L14's mechanism ruled (`insert` vs `upsert`, no silent overwrite) and its five semantics routed to R8b-3…7; amount-0 indexing → R8b-2; output-key uniqueness → R8b-1. **Architecture ruled:** a validator crate with no store handle (`ChainView` projected from the *same* write transaction, brand-scoped so cross-batch handoff is unrepresentable — increment 2.5 precondition for S-CHAIN-W); three error classes `InvalidBlock` / `StoreInvariantViolated` / `StoreCannot` with the invariant→verdict conversion **banned and gated** (`check_store_error_conversion_ban.py`); the store computes nothing consensus-visible (the curve-tree root is handed in, recorded, and the pop trim is checked against the recorded root); pop is the mechanical reverse replay of a single LIFO undo log; `RuleSet` is an explicit validation input; `ChainValid` carries rule coverage and only complete coverage is parity evidence; the completeness gate partitions by flag first (consensus 125/159 ratified, policy 5/9) so relay-policy rows cannot count toward consensus coverage. **New artifacts:** [`STORE_INVARIANT_REGISTER.md`](STORE_INVARIANT_REGISTER.md) (`SI-1…SI-8`, LIVING CONTRACT) with `check_store_invariant_register.py` (bijection against the store's `StoreInvariant` enum) — a register born with its gate, per the census-outside-`DEFAULT_DOCS` lesson. **Effect on this document:** §2's bucket table re-derived (the same eighteen rows: 7 / 2 / 7 / 2); §3's eight L rows carry their rulings; §6.2 records-was; **the S-CHAIN-W blocker this document's consumer clause named is lifted** — S-CHAIN-W is gated on increment 2.5 (brand) and on the validator crate's first slice, both scheduled in the ruling's §14. Reversion clause (rule 21) in the ruling's §13: re-anchor if a consensus rule ever *cites* a store constraint as its mechanism, or if one-live-write is relaxed. | Rick + agent, §2, §3, §5.4.1, §6.2, §8 |
| **2026-09-11** | **CEN-B5 and CEN-I19 re-reviewed at their merged shas and PROMOTED — register 102 / 0 / 1.** **Denominator, stated because this figure gets quoted onward and three different populations are in play in this document:** the **103 rows present in §5.4.1 today**, NOT P0f's 102-row 2026-09-02 snapshot (the denominator of the historic 100 / 1 / 1) and NOT the live bucket-1/2 set of 122/130, most of which has no register row at all. The single non-conformant row is **CEN-L8**, still deliberately failed closed; **no DIVERGENT row remains**, and DRS-0's last gate is discharged. CEN-B5 at `e54e5b983` (PR #623): admission-before-add established by tracing the ordering (`check_hash :5840` → compare `:5867` → prevalidate `:5900` → `add_block :6410`), uniformity by the **absence of any `m_nettype` token** in the whole function body rather than by the comment's claim, skip-retirement by there being exactly one compare site left, and regression coverage by `curve_tree_header_root_check.cpp` being **wired** at `tests/unit_tests/CMakeLists.txt:124` — an unwired test file would have been a false green. Sibling sweep: the twelve surviving `FAKECHAIN` branches are eleven `Blockchain::init` bootstrap arms plus one **inverse-polarity fence** at `:5234` that refuses *off* fakechain; none is a consensus check being skipped. CEN-I19 at `2ec553a3f` (PR #630): the `kept_by_block` negative is **structural** — `check_tx_semantic(tx, tvc, hf_version)` has no such parameter and the token appears nowhere in `tx_verification_utils.cpp` — and the collector's `DB_ERROR` throw is reachable and **fail-closed**, the caller's `catch (const std::exception&)` setting `m_verifivation_failed` rather than swallowing. **Both promotions rest on a code walk plus test wiring, not on an executed suite** (M8/G4/J26 boundary); neither audited the Rust rule's *content* against a spec, only identity-with-the-one-rule. **Figure counted from the status column, never derived from the prior figure, and re-runnable:** `awk '/^#+ .*5\.4\.1/{f=1;next} f&&/^## /{exit} f' docs/design/CONSENSUS_STORE_RECONCILIATION.md \| grep -E '^\| \*{0,2}CEN-' \| awk -F'\|' '{print $3}' \| sed -nE 's/^[^A-Z]*(CHECKED-CONFORMANT\|DIVERGENT\|UNREVIEWED).*/\1/p' \| sort \| uniq -c` (the `\|` are GFM table escapes, as in the 2026-09-05 row below). **Two properties this command needs and a simpler one lacks, both learned by reconciling a disagreement with an independent count:** *(1)* the row pattern must admit `**CEN-x**` as well as `CEN-x` — eight rows (H5, L7, L8, L9, L10, L11, L12, M8) carry a **bolded** name cell, and a `$2 ~ /^ *CEN-/` scoping silently drops exactly those, yielding 95 and losing CEN-L8, the only UNREVIEWED row, which makes the register look unanimously conformant. *(2)* the status must be read **anchored to the start of field 3**, because history parentheticals on promoted rows contain the words DIVERGENT and UNREVIEWED — an unanchored `/UNREVIEWED/` match reports CEN-I19 as unreviewed on the very row that promotes it. **Integrity check to run alongside it:** rows matched must equal statuses parsed (103 = 103); an inequality means a row's status cell did not parse and was dropped in silence rather than counted. Register-only PR, no code. | §5.4.1 |
| **2026-09-05** | **CEN-B5's S1 FIXED (PR #623) — the header-root check moved to admission, before `add_block`, against the tip root; the post-add compare and its pop arm deleted.** Rick authorized the scoped edit as entry 17's fix-or-accept ruling. Red → green observed by `curve_tree_header_root_check.cpp`, the first non-FAKECHAIN Blockchain fixture in the tree (TESTNET, fixed difficulty 1, real LMDB): block 60 rejected before the edit, connected after, leaf count 0 → positive, header(60) == record[60]. All three main-chain add paths (connect, promotion, restore) go through `handle_block_to_main_chain`, so alt/reorg promotion is checked identically against the post-pop tip. **The rule-71 FAKECHAIN skip is retired in the same PR** (Rick, 2026-09-05: build it now, on the Rust client, and take `redb` into the daemon image ahead of DRS): the generator computes real roots through `shekyl_curve_tree_replica_*`, the fake-DB test doubles report the empty-tree sentinel, and the check runs on every nettype (census §7 #18). Row stays **DIVERGENT** until re-reviewed at the merged sha, then promotes (M8/D2/L11 precedent). Register figure unchanged by this PR (CEN-B5 stays DIVERGENT until the re-review): 100 / 1 / 1 after the CEN-I12 promotion recorded below, counted from the status column. |
| **2026-09-05** | **CEN-I12 re-reviewed at the merged reconciliation and PROMOTED — register 100 / 1 / 1 over P0f's 102-row snapshot.** Re-reviewed at `667817d47` (the merge of PR #618, which ruled the anchor a state property: the tree at chain height `ref_height`). All three `check_tx_inputs` arms read the node's own per-height record under `ref_height` (`blockchain.cpp:3810`, `:3959`, `:4217`), never the header; record key `ref_height` is the ruled state by the post-add keying the CEN-B5 keying pin holds on real LMDB. Verdict rests on the state definition, not slice 7's placeholder rationale. The header witness's binding is CEN-B5's row (S1 fixed on PR #623, promotion owed there); the depth split is CEN-I13's. Per-height record recorded as a spec-level store requirement. Register-only PR (#622), no code. Figure counted from the status column: `awk '/^#+ .*5\.4\.1/{f=1;next} f&&/^## /{exit} f' docs/design/CONSENSUS_STORE_RECONCILIATION.md \| grep -E '^\| \*{0,2}CEN-' \| awk -F'\|' '{print $3}' \| sed -nE 's/^[^A-Z]*(CHECKED-CONFORMANT\|DIVERGENT\|UNREVIEWED).*/\1/p' \| sort \| uniq -c` → 100 / 1 / 1 (the `\|` are GFM table escapes for pipes inside a code span; the rendered command is copy-pastable, and commit `6eaaa6d15`'s message carries it unescaped). |
| **2026-09-04** | **CEN-L11 and CEN-L12 re-reviewed at the merged fix and PROMOTED — register 99 / 1 / 2 over P0f's 102-row snapshot; CEN-B5 is the sole DIVERGENT row.** Re-reviewed at `ab4693d0e` (contains the `dev` merge `a72e38550` = PR #609; no cited file differs between the two). All three collector arms abort at connect (`blockchain_db/blockchain_db.cpp:561`, `:570`, `:589`), the construct verdict is consumed, the three foreclosing gates carry their reverse pointers, and the evidence is behavioural at this sha: the five `cen_l11_*` LMDB regressions and the Rust falsifier both pass (red-observation recorded in #609). L12's universality clause closes with it. **Register-only PR, by design:** the fix and its promotion were split so the promotion could pin a sha containing the *merged* fix — the rule this register applied to CEN-M8 and CEN-D2/D1 — and so that register churn stops riding fix PRs (#609 spent its last four rounds on doc text over a code half that was stable from round 5). **Two early-promotion leaks recorded, not rewritten:** (1) PR #609 merged under the title *"… — register 99/1/2"*, written before its round 6 withdrew the promotion, so merge commit `a72e38550` carries a figure the tree did not have at that sha — **withdrawn-in-PR, records-was:** 97 / 3 / 2 at that merge, 99 / 1 / 2 only from this re-review — and a `git log --grep` on it lands one PR early. This row is the durable correction, since the commit body cannot be edited; (2) `IMPLEMENTATION_INDEX.md`'s CSR cell already read "CEN-L11 and CEN-L12 were fixed and promoted 2026-09-04" beside a 97 / 3 / 2 count that contradicted it — the withdrawal missed that sentence, and this PR makes it true rather than deleting it. Both are the lesson #602's rounds already drew: **counts do not belong in PR titles**, and a withdrawal must sweep every surface the claim reached. **Not in this PR:** the nineteen C2-R1b/R1c rows (UNREVIEWED; their own P0f round), CEN-I12 (a census ruling), CEN-B5 (R9) | §5.4.1 |
