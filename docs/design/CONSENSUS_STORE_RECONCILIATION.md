...
**As of 2026-09-02 the CHECKED-CONFORMANT set holds two rows: CEN-H5 and
CEN-L10.** DRS-P0f promoted both; CEN-L12 was reviewed and came back
**DIVERGENT**, coupled to CEN-L11. Every other row remains **UNREVIEWED**, so
outside those two the digest is still a regression instrument, and it stays that
way until P0f reviews more rows. **Three of the six reviewable store-enforced
rows are now decided; CEN-L7/L8/L9 — the archival-journal family — remain.** That reads as a strong claim, and it is the honest consequence of the
...
| **CSR-3a** | The **conformance register** (§5.4.1): a bucket says a rule is *ratified*, never that the C++ *implements* it | every parity claim asserting correctness | **OPEN, in progress** — **CEN-H5 and CEN-L10 CHECKED-CONFORMANT** (P0f slices 1–2, 2026-09-02); CEN-L11 and **CEN-L12** DIVERGENT; CEN-L12 (spec's staked arm) examined and excluded as retired. **Not proven complete**; **DRS-P0f** (minted 2026-09-02) and the census own extending it, and E2 must consult it before calling any match correctness |
