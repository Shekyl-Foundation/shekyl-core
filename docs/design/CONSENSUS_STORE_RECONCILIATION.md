...
| CEN-L4 | 3 | Block hash recomputed; contained txs stored under **claimed** hashes verbatim | **S-CHAIN-W** | **RULED C2-R8 Q4/Q6:** the store computes **nothing** consensus-visible — identity is CEN-B6's; the validator derives every hash once and the store writes what `ChainValid` carries. The store-side recompute is **deleted, not moved** |
| CEN-L5 | 3 | Input-type whitelist duplicated at write time | S-CHAIN-W | **RULED C2-R8 Q6:** accidental duplicate — and it **dissolves rather than moves**: the store's input is a typed enum, so the whitelist is the type and there is no check to relocate |
