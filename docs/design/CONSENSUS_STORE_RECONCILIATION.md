...
**As of 2026-09-02 the CHECKED-CONFORMANT set holds exactly one row: CEN-H5.**
DRS-P0f's first slice reviewed the reviewable store-enforced set and promoted
H5; CEN-L12 was reviewed and came back **DIVERGENT**, coupled to CEN-L11. Every
other row remains **UNREVIEWED**, so outside CEN-H5 the digest is still a
regression instrument, and it stays that way until P0f reviews more rows. That reads as a strong claim, and it is the honest consequence of the
...
| **CSR-3a** | The **conformance register** (§5.4.1): a bucket says a rule is *ratified*, never that the C++ *implements* it | every parity claim asserting correctness | **OPEN, in progress** — **CEN-H5 CHECKED-CONFORMANT** (P0f slice 1, 2026-09-02); CEN-L11 and **CEN-L12** DIVERGENT; CEN-L12 (spec's staked arm) examined and excluded as retired. **Not proven complete**; **DRS-P0f** (minted 2026-09-02) and the census own extending it, and E2 must consult it before calling any match correctness |
