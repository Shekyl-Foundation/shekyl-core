...
recorded disposition — **97 CHECKED-CONFORMANT**, **3 DIVERGENT** (CEN-L11 with
CEN-L12 coupled to it; CEN-B5, the rule-71 FAKECHAIN skip),
...
spec to review against until the census's design rounds rule them. **CEN-L12** is DIVERGENT (coupled to CEN-L11), as is **CEN-B5** (rule-71 skip, round 6). **Both S-graded findings are closed:** CEN-M8/G4/J26 were promoted at PR #602's merged fix, and **CEN-D2 with CEN-D1 at PR #604's** (2026-09-03) — no S-graded divergence remains. Failed closed: **CEN-L8** and **CEN-I12**
...
| **CSR-3a** | The **conformance register** (§5.4.1): a bucket says a rule is *ratified*, never that the C++ *implements* it | every parity claim asserting correctness | **ROW COVERAGE COMPLETE 2026-09-02** — all **102** bucket-1/2 rows disposed: **97 CHECKED-CONFORMANT / 3 DIVERGENT / 2 failed closed** (P0f slices 1–8 + review rounds + two post-fix re-reviews): CEN-L11 with CEN-L12 coupled; CEN-B5 (rule-71 FAKECHAIN skip; R9 owns); CEN-L8 and CEN-I12 failed closed. **Both S-grades are closed** — CEN-M8/G4/J26 promoted at PR #602's merged fix, CEN-D2/D1 at PR #604's; the examined-and-excluded note on L12 covers its **staked arm only** (a retired claim-era clause), not the row. **Coverage is complete over today's bucket-1/2 set; the set grows** — bucket promotions from design rounds enter UNREVIEWED, and **DRS-P0f** and the census own reviewing them; E2 must consult the register before calling any match correctness |
