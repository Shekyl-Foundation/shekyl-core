...
spec to review against until the census's design rounds rule them. **CEN-B5** (rule-71 skip) is the sole divergent row; **CEN-L11** and **CEN-L12** were re-reviewed and promoted at PR #609's merged fix (2026-09-04). **The review's two S-graded findings are closed** — CEN-M8/G4/J26 promoted at PR #602's merged fix, **CEN-D2 with CEN-D1 at PR #604's** (2026-09-03) — **a third was found 2026-09-04 and is fixed on PR #623 (2026-09-05), re-verification at the merged sha owed:** CEN-B5's header check compared the root *after* the add against a header filled from the root *before* it, so every non-FAKECHAIN chain would have halted at height 60 — **S1**, masked by the rule-71 skip; the check now runs at admission before the add, red → green observed on a TESTNET Blockchain fixture (census §7 #18). DRS-0 stays gated until that fix is re-verified at its merged sha. Failed closed for the two distinct reasons already given above: **CEN-L8**,
...
| **2026-09-05** | **CEN-B5's S1 FIXED (PR #623) — the header-root check moved to admission, before `add_block`, against the tip root; the post-add compare and its pop arm deleted.** Rick authorized the scoped edit as entry 17's fix-or-accept ruling. Red → green observed by `curve_tree_header_root_check.cpp`, the first non-FAKECHAIN Blockchain fixture in the tree (TESTNET, fixed difficulty 1, real LMDB): block 60 rejected before the edit, connected after, leaf count 0 → positive, header(60) == record[60]. All three main-chain add paths (connect, promotion, restore) go through `handle_block_to_main_chain`, so alt/reorg promotion is checked identically against the post-pop tip. **The rule-71 FAKECHAIN skip survives** — not by choice: core_tests replay placeholder-root headers into real LMDB through the maturity window, so retiring the skip needs the generator to compute real roots (R9's named blocker, census §7 #18). Row stays **DIVERGENT** for that half; the S1 half is promotable at the merged sha per the M8/D2/L11 precedent. Register unchanged at 99 / 1 / 2 (counted from the status column: `grep -c` per verdict over §5.4.1). |
