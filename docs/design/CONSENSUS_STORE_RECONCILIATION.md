...
shas pinned per slice): the **102** bucket-1/2 rows that existed when P0f ran have a recorded disposition. **That denominator has since grown:** C2-R1b promoted **nine** rows into bucket 2 on 2026-09-03 and C2-R1c **ten** more on 2026-09-04 (nine promotions plus the CEN-K1 → K1a/K1b split), and CEN-I19 was minted into bucket 1 on 2026-09-06 with its implementation, and CEN-J27 was minted into bucket 1 on 2026-09-12 with its implementation (`EndpointUpdate` B+C1), so the current bucket-1/2 set is **131** (counted from the census bucket column, 2026-09-12: 88 b1 + 43 b2) — and **as of slice 10 (2026-09-11) every one of them carries a record: the set-difference is ZERO.** CSR-3a's set-growth obligation fired for the first time when that gap opened at twenty-seven, and slices 9 and 10 closed it (eleven `CEN-K` rows, then the sixteen non-K remainder). **Counted from the rows of this register, not carried from the prior figure (re-counted 2026-09-12): 131 recorded = 127 CHECKED-CONFORMANT + 2 DIVERGENT + 2 UNREVIEWED.** The 2 DIVERGENT are CEN-G6/CEN-G6b (ratified surge factor `S = 4`, shipped `50`) and the 2 UNREVIEWED are CEN-L8, failed closed on an unwired clause, and CEN-J27, born 2026-09-12 with its implementation and awaiting P0f's review — so **UNREVIEWED by absence is now zero, and every UNREVIEWED row says why.** Disposition of the 102 — **100 CHECKED-CONFORMANT**, **1 DIVERGENT** (CEN-B5, the
...
| CEN-J12 | **CHECKED-CONFORMANT** | W-BP. JoinMarket requires canonical-length `bond_spend_pk` (`:5201`); Unbond/HoldingsUpdate/Rebond each reject a non-empty one (`:4922`, `:4990`, `:5138`), and a kind-agnostic catch-all rejects the residue (`:5208`) — **all five sites agree**. **Re-anchored 2026-09-12 (`EndpointUpdate` B+C1):** a sixth site, the EndpointUpdate arm's own belt (`:5008`), rejects a non-empty key before the catch-all (the arm returns early); at the B+C1 commit the prior five sit at `:4723`, `:4796`, `:4949`, `:5066`, `:5072`. The endpoint coupling (`EU-D3`) has a single kind-agnostic non-parse belt ahead of every arm (`:4708`) rather than a per-arm twin. Every codec (binary, boost, JSON) also refuses both shapes at parse |
| CEN-J13 | **CHECKED-CONFORMANT** | W-BP. Debit arms authorize via `shekyl_archival_cold_authority_pin` (all non-OK reject; helper `:4789`, Release `:4889`, HoldingsUpdate-drop `:5012`); credit arms check the identity key on every path (add `:5001`, Rebond `:5148`, JoinMarket `:5254`). **Re-anchored 2026-09-12 (`EndpointUpdate` B+C1):** the selector gains `EndpointUpdate` **always** (`EU-D2`), its arm pins before the semantic verify (`:5032`) and has no identity-key path; `check_debit_auth_single_source.sh` asserts the new call by name, and the pin sites at the B+C1 commit are helper `:4622`, Release `:4737`, HoldingsUpdate-drop `:4860`, EndpointUpdate `:5032`. Carried on the rebuilt `archival_*` gtest run at that commit (the J27 row records the arm itself). **Re-anchored 2026-09-11** at the cold-authority refactor, and again when the C++ error-string switch moved to `bond_post_err_cstr`: the pin is unchanged, the selector lives in Rust (`requires_cold_authority`: Release always, HoldingsUpdate iff `bond_debit > 0`), and `NOT_COLD_AUTHORITY_POST` is the arm/predicate cross-check, unreachable through these three sites. Conformance status **carried on an affirmative instrument, not assumed**: the 156/156 `archival_*` gtest run on the rebuilt daemon at the refactor commit. Previous anchors: `:4941` / `:5068` / `:5193` / `:5178` / `:5327` / `:5433` (first re-anchor, helper rename); before that `:4828–4855` / `:5041` / `:5183` / `:5288`, already stale (`:5041` sat above the HoldingsUpdate arm's own start at `:5089`) |
...
| CEN-J27 | **UNREVIEWED** *(by construction — born 2026-09-12 with its implementation, `EndpointUpdate` B+C1 under rule 07; this row IS the arm's CSR-3a record, opened at birth so the set-difference stays zero; review is P0f's on arrival)* | W-BP. The EndpointUpdate arm (`src/cryptonote_core/blockchain.cpp:5003–5055` at the B+C1 commit): `EU-D3`/`EU-D11` shape belts (`:5008`, `:5014`), cold-authority pin before the verify (`:5032`, unconditional in `requires_cold_authority`), `shekyl_archival_verify_endpoint_update` (`:5041`: 32-byte endpoint and record facts; record present and Bonded); CT balance selects the no-term equation by kind (`src/cryptonote_core/tx_verification_utils.cpp:139–148`, `verCtSemanticsEndpointUpdate`); connect replaces the record's `endpoint` and journals the previous one in `archival_bond_endpoint_update_log` (`src/blockchain_db/blockchain_db.cpp:361`), pop restores it (`:811`). Coverage at birth: the `archival_*` gtests (serializer couplings on all three codecs, CT no-term, journal value, connect/pop through the real block path, and the `EU-D9` KAT (b) rotation-inside-the-window run) |
...
- **Walk W-CK** — `src/checkpoints/checkpoints.cpp` (`checkpoints::check_block`,
  `is_alternative_block_allowed`, `add_checkpoint`, `load_checkpoints_from_json`,
  `init_default_checkpoints`) with its fail-stop chain
  `Blockchain::check_against_checkpoints` → `Blockchain::update_checkpoints` →
  `core::update_checkpoints` → `core::graceful_exit`
...
| **2026-09-12** | **CEN-J27 minted with its implementation (`EndpointUpdate` B+C1, rule 07) — register 131 = 127 / 2 / 2.** The census's bucket-1/2 set grows by one (88 b1 + 43 b2) and this register grows with it in the same PR, so the set-difference stays zero; the new row is UNREVIEWED by construction (born with the code, no independent review yet), the second row that says why. CEN-J12 and CEN-J13 re-anchored for the sixth coupling site and the third cold-authority arm | §5.4.1 |
