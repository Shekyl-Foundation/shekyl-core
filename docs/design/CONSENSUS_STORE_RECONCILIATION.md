...
| CEN-I19 (`PL-D3` amendment) | **UNREVIEWED** *(born 2026-09-14 with its implementation: `0x07` stride `32·n → 64·n` and the leaf-commitment point content rule, `check_pqc_leaf_entries` over the payload bytes the adapter now passes)* | Re-review at the sha that merges the `PL-D3` implementation PR: the adapter passes the `0x07` payload (`src/cryptonote_basic/cryptonote_format_utils.cpp` `check_tx_extra_pqc_field_shape`), the FFI gained `leaf_blob`/`leaf_blob_len`, code 10 = `LEAF_POINT`; the promoted row above stands for the shape half at its sha. | `FCMP_SPEND_LINKABILITY.md` §6.2 |
