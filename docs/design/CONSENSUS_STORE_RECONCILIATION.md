...
<!-- conformance-tally: 125 CHECKED-CONFORMANT, 3 DIVERGENT, 3 UNREVIEWED -->
...
shas pinned per slice): the **102** bucket-1/2 rows that existed when P0f ran have a recorded disposition. **That denominator has since grown:** C2-R1b promoted **nine** rows into bucket 2 on 2026-09-03 and C2-R1c **ten** more on 2026-09-04 (nine promotions plus the CEN-K1 → K1a/K1b split), CEN-I19 was minted into bucket 1 on 2026-09-06 with its implementation, and C2-R8 promoted CEN-L1 into bucket 2 on 2026-09-14, so the current bucket-1/2 set is **131** (counted from the census bucket column, 2026-09-14: 87 b1 + 44 b2) — and **every one of them carries a record: the set-difference is ZERO** (first reached by slice 10, 2026-09-11, and held through both later mints). CSR-3a's set-growth obligation fired for the first time when that gap opened at twenty-seven, and slices 9 and 10 closed it (eleven `CEN-K` rows, then the sixteen non-K remainder). **Counted from the rows of this register, not carried from the prior figure (2026-09-15): 131 recorded = 125 CHECKED-CONFORMANT + 3 DIVERGENT + 3 UNREVIEWED.** The 3 DIVERGENT are CEN-G6/CEN-G6b (ratified surge factor `S = 4`, shipped `50`) and CEN-I12 (per-height root record sparse where the tree did not grow, reader substitutes the identity point — found 2026-09-15 by the S-CHAIN-W pre-flight, census §7 #21; was CHECKED-CONFORMANT from 2026-09-05 to 2026-09-15 on a verdict that did not walk the absent-key arm); the 3 UNREVIEWED are CEN-L8 (failed closed on an unwired clause), CEN-I19 (re-opened 2026-09-14 by `PL-D3`'s amendment, review owed at its merge sha) and CEN-L1 (born UNREVIEWED with C2-R8, no implementation to review yet) — so **UNREVIEWED by absence is zero, and every UNREVIEWED row is one that says why.** Disposition of the 102 — **100 CHECKED-CONFORMANT**, **1 DIVERGENT** (CEN-B5, the
