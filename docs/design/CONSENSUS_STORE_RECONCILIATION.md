...
recorded disposition — **98 CHECKED-CONFORMANT**, **3 DIVERGENT** (CEN-L11,
CEN-L12 coupled to it, and **CEN-M8 at S0**), **1 failed closed to UNREVIEWED**
...
spec to review against until the census's design rounds rule them. **CEN-L12** is DIVERGENT (coupled to CEN-L11); **CEN-L8**
...
| CEN-J26 | **CHECKED-CONFORMANT** | W-EM + the FCMP arm. Absent⇔zero-fee-inputs enforced both directions (`:4186` no-proof-with-inputs reject at `:3889`); present ⇒ `shekyl_fcmp_verify` rc-checked reject (`:4351+`). **REWRITE-NOTE:** `skip_fcmp_verify` is a **pool-admission verification cache**, embargo-load-bearing per the in-code note (`DAEMON_RELAY_PRIVACY.md` §71) — the rewrite must preserve that timing semantics or re-derive the embargo first. **Amended by slice 8: the cache's admission gate was found DIVERGENT under CEN-M8 (S0** — presence-gated where the spec ratified hash-gated**)**; this row's presence/verify clauses remain conformant |
...
| CEN-G4 | **CHECKED-CONFORMANT** | Per-tx `check_tx_inputs` at connect; failure → `add_block_as_invalid` + `bvc.m_verifivation_failed` + txs returned (`:6132–6145`) |
...
| **CSR-3a** | The **conformance register** (§5.4.1): a bucket says a rule is *ratified*, never that the C++ *implements* it | every parity claim asserting correctness | **ROW COVERAGE COMPLETE 2026-09-02** — all **102** bucket-1/2 rows disposed: **98 CHECKED-CONFORMANT / 3 DIVERGENT (incl. CEN-M8 at S0) / 1 failed closed** (P0f slices 1–8); CEN-L11 and **CEN-L12** DIVERGENT; the examined-and-excluded note on L12 covers its **staked arm only** (a retired claim-era clause), not the row. **Not proven complete**; **DRS-P0f** (minted 2026-09-02) and the census own extending it, and E2 must consult it before calling any match correctness |
