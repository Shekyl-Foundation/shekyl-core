...
recorded disposition — **95 CHECKED-CONFORMANT**, **5 DIVERGENT** (CEN-L11 with
CEN-L12 coupled to it; **CEN-D2 at S1** with CEN-D1 coupled to it, the
fail-open-at-difficulty-1 finding; CEN-B5, the rule-71 FAKECHAIN skip),
...
spec to review against until the census's design rounds rule them. **CEN-L12** is DIVERGENT (coupled to CEN-L11), as are **CEN-D1** (coupled to D2, round 6) and **CEN-B5** (rule-71 skip, round 6); **CEN-M8, CEN-G4 and CEN-J26 were re-reviewed and PROMOTED in round 6** at the tree containing PR #602's merged fix; **CEN-L8** and **CEN-I12** (round 6)
...
| **CSR-3a** | The **conformance register** (§5.4.1): a bucket says a rule is *ratified*, never that the C++ *implements* it | every parity claim asserting correctness | **ROW COVERAGE COMPLETE 2026-09-02** — all **102** bucket-1/2 rows disposed: **95 CHECKED-CONFORMANT / 5 DIVERGENT / 2 failed closed** (P0f slices 1–8 + review rounds 1–6): CEN-L11 with CEN-L12 coupled; **CEN-D2 at S1** with CEN-D1 coupled (sentinel fail-open at difficulty 1); CEN-B5 (rule-71 FAKECHAIN skip; R9 owns); CEN-L8 and CEN-I12 failed closed; **CEN-M8/G4/J26 promoted in round 6** at the tree containing the merged PR #602 fix; the examined-and-excluded note on L12 covers its **staked arm only** (a retired claim-era clause), not the row. **Coverage is complete over today's bucket-1/2 set; the set grows** — bucket promotions from design rounds enter UNREVIEWED, and **DRS-P0f** and the census own reviewing them; E2 must consult the register before calling any match correctness |
