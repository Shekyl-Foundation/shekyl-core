...
spec to review against until the census's design rounds rule them. **CEN-B5** (rule-71 skip) is divergent, as are **CEN-L11** and **CEN-L12** until their landed fix is re-reviewed at a merged sha. **Both S-graded findings are closed:** CEN-M8/G4/J26 were promoted at PR #602's merged fix, and **CEN-D2 with CEN-D1 at PR #604's** (2026-09-03) — no S-graded divergence remains. Failed closed for the two distinct reasons already given above: **CEN-L8**,
...
| **CEN-L11** | **DIVERGENT** — *fix landed; promotion owed at the merged sha* | The row's original framing was wrong and is corrected here: it recorded a **live** fund-loss path, but all three skip arms are unreachable for any tx that passed validation — CEN-H12/F8 fix the target type, four `outPk.size()` gates the length (`cryptonote_core.cpp:799`, `cryptonote_format_utils.cpp:137`, `blockchain.cpp:3289`, `:4031`), and the canonical/prime-order point gates fix both leaf inputs (`shekyl_check_output_keys` via `check_outs_valid`; `shekyl_check_commitment_masks` via `check_commitment_mask_valid`, `:3392`, coinbase legs in `prevalidate_miner_transaction`). `check_outs_valid`'s own comment had already named this hazard; nothing connected it back. **So this was a latent silent-failure surface, not an active defect**, and fail-closed costs nothing over dead arms. All three arms now abort (`blockchain_db.cpp:586–589` and above), the three gate definitions carry reverse pointers, and `shekyl-ffi/tests/leaf_gate_agreement.rs` asserts gate/leaf-builder agreement (red-observed). **Why still DIVERGENT:** this document's schema requires the pin to name the sha the code was reviewed at, and by the rule applied to CEN-M8 and CEN-D2/D1 that is a sha containing the **merged** fix — which does not exist until this PR lands. Promotion is a follow-up review, not a claim this PR can make about itself | Identity is **not** the pass condition until promotion; a rewrite reproducing the silent omission **fails** |
| **CEN-L12** | **DIVERGENT** *(coupled to CEN-L11 — fix landed; promotion owed with it)* | The maturity arithmetic always conformed (coinbase `+60`, others `+10`, both live target variants, `unlock_time` absent, staked arm retired). The failing clause was **universality** — an accepted output could be dropped before being made pending — which CEN-L11's fix closes: the block is now refused rather than connecting with a hole. Promotion rides CEN-L11's post-merge re-review | Identity is **not** the pass condition until promotion |
...
| **CSR-3a** | The **conformance register** (§5.4.1): a bucket says a rule is *ratified*, never that the C++ *implements* it | every parity claim asserting correctness | **ROW COVERAGE COMPLETE 2026-09-02** — all **102** bucket-1/2 rows disposed: **97 CHECKED-CONFORMANT / 3 DIVERGENT / 2 failed closed** over P0f's **102-row** snapshot (the live bucket-1/2 set is **111** after C2-R1b's nine promotions, which are UNREVIEWED pending review): CEN-B5 (rule-71 FAKECHAIN skip; R9 owns), plus CEN-L11/L12 whose fix has landed and whose promotion is owed at a merged sha; CEN-L8 and CEN-I12 failed closed. **Both S-grades are closed** — CEN-M8/G4/J26 promoted at PR #602's merged fix, CEN-D2/D1 at PR #604's; the examined-and-excluded note on L12 covers its **staked arm only** (a retired claim-era clause), not the row. **Coverage is complete over today's bucket-1/2 set; the set grows** — bucket promotions from design rounds enter UNREVIEWED, and **DRS-P0f** and the census own reviewing them; E2 must consult the register before calling any match correctness |
