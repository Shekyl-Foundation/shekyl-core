...
| --- | --- | --- | --- |
| **CSR-1** | R8 is the ruling instrument for store-enforced rules; DRS-C's surface map is its input (§4) | store design | **RATIFIED 2026-09-01; DISCHARGED 2026-09-14** — R8 ruled its 8 rows by a standing category test ([`CONSENSUS_C2_R8_STORE_PLACEMENT.md`](CONSENSUS_C2_R8_STORE_PLACEMENT.md)): L1 → bucket 2 (minted, UNREVIEWED in §5.4.1), L2/L3/L4/L5/L6/L13 → bucket 3, L14 mechanism ruled + semantics → R8b-3…7; belts re-homed as `SI-1…SI-8` in [`STORE_INVARIANT_REGISTER.md`](STORE_INVARIANT_REGISTER.md) with its bijection gate. B3/K3 were never R8's (R4's; R1c's) and that held |
