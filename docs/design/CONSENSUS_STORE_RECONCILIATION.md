...
| CEN-J27 | **UNREVIEWED** *(by construction — born 2026-09-12 with its implementation, `EndpointUpdate` B+C1 under rule 07; this row IS the arm's CSR-3a record, opened at birth so the set-difference stays zero; review is P0f's on arrival)* | W-BP. The EndpointUpdate arm (`blockchain.cpp:5003–5055` at the B+C1 commit): `EU-D3`/`EU-D11` shape belts (`:5008`, `:5014`), cold-authority pin before the verify (`:5032`, unconditional in `requires_cold_authority`), `shekyl_archival_verify_endpoint_update` (`:5041`: kind, endpoint present, record present and Bonded); CT balance selects the no-term equation by kind (`tx_verification_utils.cpp:139–148`, `verCtSemanticsEndpointUpdate`); connect replaces the record's `endpoint` and journals the previous one in `archival_bond_endpoint_update_log` (`blockchain_db.cpp:361`), pop restores it (`:811`). Coverage at birth: the `archival_*` gtests (serializer couplings on all three codecs, CT no-term, journal value, connect/pop through the real block path, and the `EU-D9` KAT (b) rotation-inside-the-window run) |
