...
| [`ARCHIVAL_FAILURE_CONFIRMATION_PIN.md`](ARCHIVAL_FAILURE_CONFIRMATION_PIN.md) | Escalate-on-failure + randomized recheck (pre-implementation; sim decision gate) |
...
2. **Each epoch `E`:** one **guaranteed baseline** challenge per `(P, s)` — the demand floor so
   honest `P` can earn even with no organic traffic. **Scheduling after a miss** (randomized
   failure-confirmation recheck vs fixed cadence vs sliding-window m-of-n) is pinned in
   [`ARCHIVAL_FAILURE_CONFIRMATION_PIN.md`](ARCHIVAL_FAILURE_CONFIRMATION_PIN.md); genesis
   implements baseline + slash-on-miss; escalation FSM is sim-gated.
