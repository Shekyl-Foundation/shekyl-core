...
| FL-R15 | **Retired-property blast radius (FL-R12′ implementation obligation):** the cap property is encoded beyond the two clamps — `ActivityMetric::new` hard-rejects `circulating_supply > MONEY_SUPPLY` (`activity.rs:124`, an invariant that becomes false once gross issuance passes the asymptote), `base_block_reward` errors on `ag > money_supply` (`emission.rs:61`, unreachable-by-design today, load-bearing wrong under the ruling), and the rename sweep reaches `config/economics_params.json`, the codegen, engine-core consumers, the census rows, and every doc naming `money_supply` — each hit classified asserts-is / records-was / describes-a-closed-hazard before editing (census + CHANGELOG hits are legitimately records-was). Enumerate the constant's JOBS (emission input, burn-ratio denominator, activity invariant, headroom operand), not its name | **record — binds the implementing PR** | minted at the FL-R12′ ruling |
