...
> **Which map is "served", as of #640 review cycle 5.** This section was
> written when the served map was the pow2 ceiling *behind the §7
> hysteresis band*. It no longer is: `blockchain.cpp` passes
> `prev_cq = 0`, so the daemon serves the **un-hysteretic ceiling** —
> the mode this section labels as the comparison arm. The measurements
> below are unchanged and correct **for the modes they name**; what
> changed is which of them the daemon serves. The dwell gate is
> unaffected (the ceiling alone gives a 464-block minimum median dwell,
> above the banded 446), but the FL-C7 residual this section reports for
> the banded mode is NOT what is served: worst tail transitions are
> 1 161 un-banded against 24 banded. See **FL-R3** (§8) for the named
> blocker and the owed ruling; do not read "the served map" in the
> paragraphs below as describing the shipped daemon.
...
| FL-C7 feedback | **pass on the CONSTRUCTED map except one corner, which FL-R18 ACCEPTS AS BOUNDED** (measured round 12 on drift-honest traces; premise refuted and re-dispositioned round 14): un-hysteretic map oscillates at 20 cells (worst 1 177 transitions); the §7 hysteresis (load-bearing, necessary) damps to 14 residual oscillating cells at the boundary-parked high-elasticity corner (`D = 50` — the release curve's own neutral point, not an odd corner; ε ≥ 2; one `C_q`-step amplitude, worst inter-flip dwell ≈ 125 blocks). Per §1.7's chain the residual went to §8 rather than into a smoothed number, and §8 disposes it: the assumed anonymity harm was examined and **refuted** (§4.5b), and the mechanism briefly ruled to close it was measured to make the loop *worse* (§4.5a). The residual is a slow, one-`C_q`-step cycle at the baseline attractor, on record rather than papered over. **RE-OPENED IN PART by #640 review cycle 5:** the band was removed from the SERVED path (FL-R3, named blocker + owed ruling), so what ships is the un-hysteretic arm — worst tail transitions 1 161, not the 24 on which FL-R18 accepted the residual as bounded. This criterion's disposition holds for the mechanism as measured and does NOT currently describe the daemon | §4.5, §4.5a, §4.5b, §8 FL-R18, §8 FL-R3 |
...
| Hysteresis construction requirement | implementation must not flicker at a pow2 boundary: enter a new `C_q` step only when `C` crosses the boundary by a margin. **Measured (rounds 11–12): LOAD-BEARING and NOT SUFFICIENT alone** — without it the served map oscillates broadly (20 cells, worst 1 177 transitions on drift-honest traces); with the 3% band the residual is 14 boundary-parked high-elasticity cells (7–24 transitions, one `C_q` step), which **FL-R18 accepts as bounded at round 14**: the band stays as built (it is what damps boundary noise ~50×), and **nothing is composed on top of it** — the minimum-dwell floor briefly ruled at round 13 is withdrawn on two independent grounds: the anonymity harm it was to prevent does not exist (§4.5b), and it was measured to destabilise the loop it was meant to calm (§4.5a). The mechanism therefore remains exactly the implementing branch's `fee_correction_quantized` as already built — **no rework** *(superseded in part after this row was written: the mechanism is unchanged, but #640 review cycle 5 took the band OFF the served path, so the "as built" mechanism is no longer the one the daemon serves. The measurements in this row stand — they are of the banded map — and FL-R3 carries the named blocker and the owed ruling.)* | No | implementing PR (built on `feat/fee-ladder-impl-1`; the instrument's transliterated copy retires at that merge) |
...
| FL-R3 | The correction enters only pow2-quantized (ceiling) and hysteresis-guarded; per the adopted round-8 amendment the quantized quantity is the WHOLE scalar `C = (1−σ)·M_r/(1−b)` — exactly the round's measured design. Hysteresis implemented Rust-side (`fee_correction_quantized`, 3% band). **The hysteresis half of this row is NOT SERVED, and a ruling is owed on it — the row is recorded as superseded-in-part rather than rewritten to match the code.** What is built and served is the pow2 ceiling snap of the whole scalar; `blockchain.cpp` passes `prev_cq = 0`, so the band is never entered on the daemon path. **Why, both legs measured in #640 review, and the blocker is structural:** (i) a daemon-local remembered value made the served rate depend on the process's query history — a restarted and a long-running node quote differently at the same height, which contradicts the determinism the whole ladder rests on; (ii) seeding the band from the previous block's UNSEEDED snap is deterministic but INVERTS the value, because that is a one-step approximation and not the recurrence — true hysteresis is `C_q(h) = f(C(h), C_q(h−1))`, which has no bounded evaluation without an anchor. **NAMED BLOCKER (rule 22): serving hysteresis requires persisting `C_q` as chain state, which is a consensus and serialization change (rule 42) and is out of this bundle's validation surface.** **The open ruling is which way to close it:** persist `C_q` per block, or retire served hysteresis and accept the residual — noting what each half of the band's job costs, because **FL-R19 covers only one of them**: its fixed deterministic clamp margin is sized to the construction-to-broadcast gap, so it absorbs the ACCEPTANCE consequence of a `C_q` flip inside that window, but it does nothing for the flicker of the quoted rate itself. The mechanism stays in the crate, tested, in the meantime. **What the removal actually costs, measured on this branch and NOT small on one axis:** the FL-C4a dwell gate still passes — the un-hysteretic ceiling's minimum median dwell is **464 blocks**, above the 240-block bar and above the banded map's own 446 — and the rejection-race exposure barely moves (§4.5b: 25 thin cells served vs 22 banded). **But FL-C7 feedback is where the band was load-bearing: worst tail transitions go 24 → 1 161.** The served map is therefore the map §4.5 measured as oscillating broadly, and FL-R18 accepted the residual as bounded at the BANDED figures. **This is a regression against a ratified measurement and is why the ruling is owed rather than assumed.** **BUILT — ceiling served; band built, NOT served, ruling owed** | adopt — **built in part; hysteresis-served disposition OPEN** | |
...
| FL-R9 | Wallet absolute cap. **Re-derived AGAIN in the implementing PR (#640), because the young-chain framing below was wrong in the expensive direction:** it located the era maximum at genesis, where the operand reward peaks, missing that `C_q` GROWS with age (σ decays, burn rises) while `R` decays slowly — so the product peaks in the interior. The reachable maximum is **≈91 000 000 near year 7**, and the 28 000 000 genesis-congested bound this row originally adopted sat **below honest daemon quotes from ≈ year 3**, i.e. it would have refused correct snapshots. The cap is now a **structural bound** with every factor at its own extreme (σ ⇒ 0, `M_r` = release_max, `b` = burn_cap ⇒ `C_q` ≤ 16; `R` ≤ `base_block_reward(0)`; `Mfw` ≥ `Zm`) = **220 000 000**, deliberately loose against the reachable peak because refusing an honest quote dead-letters the wallet while a loose cap only weakens a sanity check. Original framing, retained as the record of the error: *swept maximum of the served top rung over the reachable young-chain grid — anchors 25 000 000 young-congested, 28 000 000 genesis-congested* | adopt — **re-derived as a bound at #640** | |
