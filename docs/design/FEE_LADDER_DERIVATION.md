...
*Operand redefinition (round 8, FL-R12′):* `R` above is redefined to
`R_eff = max(M_r·curve(remaining), TAIL)` and `C_q` to
`C′_q = 2^ceil(log2((1−σ)/(1−b)))` — algebraically identical to the
original form wherever the tail floor does not bind, and immune to the
exhaustion `[0, 0, 0]` defect. The instrument's `C` surface re-derives
under `C′` in the implementing PR (its reachable range narrows: `M_r`'s
[0.8, 1.3] factor moves out of the correction and into `R_eff`).
...
| --- | --- | --- | --- |
| FL-R1 | The ladder derives from the validation-path miner economics — operand redefined at round 8 per FL-R12′: the estimate prices against **`R_eff = max(M_r·curve(remaining), TAIL)`** with **`C′ = (1−σ)/(1−b)`** (identical to the prior `C·R` form wherever the tail floor does not bind) | adopt (operand per FL-R12′) | ⚖ (F14b-adjacent) |
| FL-R2 | Rung values are state-computed daemon-side each estimate | adopt | |
| FL-R3 | The correction enters only pow2-quantized (ceiling) and hysteresis-guarded — since round 8 the quantized quantity is **`C′ = (1−σ)/(1−b)`** (`M_r` lives in `R_eff`, not the correction) | adopt (operand per FL-R12′) | |
...
| FL-R5 | Top rung = `C′_q·2·R_eff/Mfw` unconditional (surge discount removed; operand per FL-R12′) | adopt | |
