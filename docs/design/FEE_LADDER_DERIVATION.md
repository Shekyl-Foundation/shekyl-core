...
Birth stamp carried per the ceil-quantization precedent — this criterion
was **not** in the rule-11 register; it exists because review round 5
found the register's privacy criteria stopped one rung early. Definition:
...
- **At exhaustion, FL-V1's divergence reaches its terminal form:** the
  ...
  at all.** *(Measured against the shipped code; the FL-R12′ accepted direction —
  perpetual tail, signature pending — retires this state once signed and
  implemented: the reward floor becomes
...
rung further is FL-R17's question, argued at W7-revised with §4.7's
measurements.)*
...
| W8 | Miner fee-rank ordering leak (minted at review round 5; **exists TODAY under any multi-rate ladder, independent of FL-R17's outcome**) | Fee-sorted inclusion order: block position reveals fee rank, so miners publish a partial ordering of user urgency every block | Standing finding against the current and proposed ladders alike | Under uniform rates the signal vanishes and deterministic/hash-ordered inclusion becomes cheap to *require* — but that is a relay/consensus-adjacent change (rule-71 surface), not a freebie | Disposition-pointer: rides the FL-R17 round |
...
| FL-R4 | Rung count = 3; `Fm` deleted; FL-C3-vs-C4b conflict resolved for privacy per §5.3 | ~~adopt~~ **CONTESTED at review round 5 — reopened as FL-R17** (`Fm` deletion stands under every candidate) | reopened round 5 |
...
| FL-R12′ | **Terminal emission-state ruling (supersedes malformed FL-R12; FL-V8).** **Direction accepted by the maintainer at review round 4, in-channel: perpetual tail** ("I can live with perpetual tail as the choice"); hard cap rejected; fee-fed burn-recycling declined on the dormancy argument (a maturing monetary asset is mostly high-value/low-volume — exactly when the opportunity-cost attacker is cheapest and a fee-fed pool is empty; a floor that vanishes under the conditions it was built for is not a floor). **Signature: NOT SIGNED.** An earlier revision recorded this row as "signed, relayed via the steering lane"; review round 4 could not confirm any such signature — *unverified, treated as not signed* — and every consumer row is re-scoped accordingly. **F-1 amendment (drafted by the round, awaiting the signature):** the ruling must state which quantity `TAIL` floors — the shipped order floors `base` *before* the release multiplier (`cryptonote_basic_impl.cpp:150-157`), so under dormancy (`M_r = 0.8`) the chain pays **0.48 SKL/block, not 0.6**, in exactly the regime the ruling exists for. Drafted shape: **`paid = max(M_r·curve(remaining), TAIL)` — a floor on the PAID reward** (the pre-multiplier alternative is named and rejected by the ruling's own dormancy argument: it re-creates "vanishes under the conditions it was built for" at 20% scale); `remaining` floors at 0, retiring `cap_reward_to_remaining_supply` entirely; one owner, no flag path (discharges FL-V9); rename per FL-R15. The red oracle asserts the amended contract against the shipped composition (FL-V10). FL-V7's labels stay time-scoped — the finding was that docs and code *disagreed*, and a ruling chooses which side moves: `:35/:49-50/:162/:667` are refuted under the code-as-shipped, ratified once the amended row is signed and built; `:18` ("one fixed supply") then becomes the false line, rewritten to the exactly-true sentence. Record note, per the maintainer: "rejected on the priority-order derivation" is a rule citation, not an argument — the hierarchy does not itself say which option is more secure. **Rule-21 reopen criterion:** the two-leg variance measurement — whether archival serve-credit revenue smooths what fees make lumpy — reopens the floor-source question | **direction accepted; signature pending the F-1 amendment** | provenance corrected at review round 4 |
| FL-R13 | **Fee-floor basis (FL-V11)**: whether the anti-spam floor stays reward-proportional (`Fl ∝ R`, decaying 3 413× to the permanent tail floor of 20·`C_q` atomic/byte) or moves to an absolute/recalibrated basis. New derivation scope with its own pre-registered criteria — not resolved by §5.2, which inherits the decay by construction. **FL-R12′'s perpetual-tail direction (signature pending) retires the genesis-blocking escalation**: the reward-proportional floor now has a permanent nonzero terminal value instead of reaching 0, so this is a calibration round (is 20·`C_q`/byte the right permanent floor for a chain whose stakers store what it admits?), not a security-budget-existence question | **decision required — own round, criteria first; non-blocking pending the amended FL-R12′ signature (reverts to genesis-blocking if the direction reverses to the cap)** | minted at review round 2; recalibrated at review round 4 |
