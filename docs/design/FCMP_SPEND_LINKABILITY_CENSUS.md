...
| S0 | leaf format — **SURFACE REMOVED 2026-09-18** (`SOK-10` Q7 → A) | records-was at `8494f2a27`: `fcmp/src/rpc_path.rs:24-25` (`LEAF_BYTES = 128`), `:53-56` (layout doc), `:72-94` (`append_layer0` — `:94` copied `leaf[96..128]` into `chunk_outputs` served over RPC); the module is deleted | RPC path serving handed the wallet every sibling leaf's 4th scalar in cleartext. WIRE (RPC) — **no longer exists** | If the 4th scalar becomes hiding, this still serves it (fine); if the wallet needs openings it cannot get them here. |
