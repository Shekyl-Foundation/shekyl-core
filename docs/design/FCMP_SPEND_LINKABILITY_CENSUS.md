...
| S0 | wallet-side replica | `rust/shekyl-curve-tree/src/segment.rs:28-38` (`LEAF_BYTES = SCALARS_PER_LEAF*32`, `const _: () = assert!(LEAF_BYTES == 128)`), `:88-95` (`leaf_bytes_to_scalars`) | Segment geometry. LAYOUT | Width. |
...
8. **Three independent `LEAF_BYTES = 128` declarations outside the derived one.**
   ...
   `rust/shekyl-curve-tree/src/segment.rs:36` (`SCALARS_PER_LEAF * 32`, asserted `== 128`) and
