...
count + hash-by-height, PR #540; the block-header projection, PR #541);
design **open for RK-3b**. Census and binding
...
| **RK-3b** | Whole blocks | `get_block` (+ `getblock`) — header + blob + json + tx hashes, and the console's `print_block_by_hash` / `_by_height`, which read **only** this method | 22 | header facts + block blob + tx-hash list + **the epee-rendered `json` string** (RK-D11) | W C K R P |
