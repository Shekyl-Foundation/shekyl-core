...
| **RK-4a** | The `.bin` mechanism, proved small | `/get_o_indexes.bin` | 12 | `get_tx_outputs_gindexs` | W (`shekyl-rpc-client`) |
| **RK-4b** | The remaining live binary endpoint | `/get_blocks_by_height.bin` `/getblocks_by_height.bin` | 15 | `get_blocks_by_height` | E (the engine's timing rig) |
...
| **RK-4x** — **awaiting a ruling** | `/get_blocks.bin` `/getblocks.bin` · `/get_hashes.bin` · `/gethashes.bin` | wallet2's batch sync, and wallet2 is gone: **no in-tree caller of any kind** (see §7, 2026-08-24). Migrate or delete — not this author's call | 33 + 11 | `find_blockchain_supplement`, `get_pool_info` | **none found** |
...
| `build_get_blocks_by_height_req` / the hand-rolled response decode in `shekyl-engine-core/src/engine/daemon_observability.rs` | RK-4b | it hand-builds `{heights:[u64]}` and walks the reply for block blobs, which is the typed command map's job once one exists. Its known-good shape is RK-4a's first cross-language KAT before it is deleted |
