...
| 2071 | `on_relay_tx` | selects `broadcasted` vs `all` for the transaction it will relay | **Not a disclosure — an action**, and **not reachable** on a restricted listener: Rust refuses the method 403 before C++ (`RESTRICTED_METHODS`). Defense in depth; see §7 |
