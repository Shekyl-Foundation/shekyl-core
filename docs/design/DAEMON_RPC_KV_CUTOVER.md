...
| the `rpc_origin()` context and its two test hooks (`core_rpc_ffi.{h,cpp}`) | RK-X | it exists because C++ handlers read a `connection_context*` to learn their own listener's posture. Every migrated handler takes the posture from `state.restricted` instead (RK-D6), so the last bridged handler takes this with it |
...
| Date | Entry |
| --- | --- |
| 2026-08-26 | **The bridge is fixed where it is broken, not only where RK-4c migrates.** The inert `ctx` entry below found the defect while scoping `get_transactions`; sweeping the rest of the dispatch surface found it is worse elsewhere. `on_get_transaction_pool`, `on_get_transaction_pool_hashes` and `on_get_transaction_pool_stats` each compute `allow_sensitive = !request_has_rpc_origin \|\| !restricted` (`core_rpc_server.cpp:889`, `:910`, `:932`), always `true` for the same reason — and those routes take **no txid**. Where `/get_transactions` let an adversary confirm a stem transaction they could already name, `/get_transaction_pool` hands a public listener the whole pool including everything not yet broadcast, and `/get_transaction_pool_hashes` hands over the identifiers to then ask about. Fixing only the two routes this slice migrates would have left the enumeration and called the leak closed. So the fix goes in the bridge: `dispatch_json` / `dispatch_jsonrpc` / `dispatch_jsonrpc_we` now pass a shared `rpc_origin()` context instead of `nullptr`, which restores the intended meaning of `m_restricted && ctx` and `ctx != NULL` for **every** bridged handler at once — the caps that never fired, and every `allow_sensitive`. The plumbing this needs already existed: `core_rpc_ffi_is_restricted` reads `h->rpc->is_restricted()`, so the bridged server has always carried the right per-listener flag; only the argument was missing. Blast radius is bounded by construction — on an unrestricted listener `m_restricted` is false and the expression is false either way, so nothing changes there; only the restricted listener moves, and it moves to the policy it was configured for. The context's address is left default-constructed because `network_address::is_blockable()` is false for one, which keeps `add_host_fail`'s per-host ban scoring the no-op it was: this context asserts "an RPC client asked", never which one, and a shared empty address must not accumulate a score. Two `rpc_bridge_origin` tests hold the contracts (present, and not bannable); returning null again turns the first red. What they cannot prove is that the dispatchers *call* it — that is the live check, and it is why the helper is the single source they all name. |
| 2026-08-26 | **The inert `ctx` is not a bin-table quirk — it is the whole dispatch surface, and on the JSON table it withholds a privacy filter, not just a cap.** RK-4b found `restricted = m_restricted && ctx` false-by-construction because `dispatch_bin` passed `nullptr`. `dispatch_json` and `dispatch_jsonrpc` pass `nullptr` too (`core_rpc_ffi.cpp:64`, `:84`), so the same expression is inert on every JSON route. In `on_get_transactions` that has **two** consequences, and they are not the same kind of thing. The first is RK-4b's: `RESTRICTED_TRANSACTIONS_COUNT` (100), and its twin `RESTRICTED_SPENT_KEY_IMAGES_COUNT` (5000) in `on_is_key_image_spent`, never fire. The second changes *data*: `request_has_rpc_origin = ctx != NULL` is likewise always false, so `get_pool_transactions_info(…, !request_has_rpc_origin \|\| !restricted)` evaluates to `true` unconditionally — and what that flag gates is not detail, it is existence. With it false, `tx_memory_pool::get_transaction_info` **refuses** a transaction that does not match `relay_category::broadcasted` (`tx_pool.cpp:845`), and `broadcasted` means "received via block or fluff" — so the set it withholds is exactly the **stem-phase and locally-submitted** one. As it stands a restricted, public listener returns the full body of a transaction that has not been broadcast yet, to anyone who asks for it by hash. That is not a trimmed field: an adversary who has seen a stem transaction can probe other nodes' public RPC with its id, and the nodes that answer are the stem path, which ends at the origin. Mission #2 — privacy is the product, not a setting, and this is the D++ guarantee RP-1 was built for. Both are fixed in Rust against `state.restricted`, which is real, and both are recorded as deliberate divergences rather than parity. Consequences for the slice's evidence: byte-parity vectors are captured on the **unrestricted** listener, where old and new agree exactly; restricted behaviour gets its own red-able handler tests and a live check, because no captured vector can witness it. The console keeps unrestricted semantics deliberately — in C++ the local console reaches the handler with a null `ctx`, which is the operator's own node and the one caller for whom "unrestricted" was always the right answer, not an accident of the bridge. **Checked and sound, so it is not in this list:** the pool half of `is_key_image_spent` filters on `relay_category::broadcasted` unconditionally (`tx_pool.cpp:1514`), so a stem-phase transaction's key image does not answer `SPENT_IN_POOL` to a public querier — that leak, which would have made a queried node's stem position observable, does not exist and no fix is owed. |
