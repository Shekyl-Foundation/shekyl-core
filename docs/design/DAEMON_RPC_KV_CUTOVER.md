...
`m_restricted && ctx` appears eleven times in `core_rpc_server.cpp`, and the
bridge passing `nullptr` made all eleven dead at once. Fixing the bridge wakes
all eleven at once too, so each one is a behaviour change on a configured
listener and each owes a disposition. Nine are unremarkable and are listed
anyway, because "most of them are fine" is a claim, not a review.
