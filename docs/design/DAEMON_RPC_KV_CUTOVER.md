...
and the resulting race (a tx mined between the blob read and the height read)
is the one the C++ handler already has, named here rather than closed by a
