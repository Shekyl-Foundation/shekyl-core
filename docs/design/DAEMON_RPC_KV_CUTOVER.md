...
| **RK-D8** | **No wire change rides the cutover.** Field sets, names, defaults and aliases of every *reply* are preserved per method, as are the status values (`status`, JSON-RPC error codes) a client branches on; a wire change is its own decision with its own `CORE_RPC_VERSION` bump (3.22 at this commit). RK-1 moves `CORE_RPC_VERSION` itself to `shekyl-rpc-types` — the constant's only reader is `get_version`, which moves with it. **Scope:** the human-readable reason text inside a transport-level failure (the Axum 500 envelope's `error` string, a JSON-RPC `message`) is diagnostic, not contract — it names what failed and may change as the failure modes change; no client or monitor in the tree parses it, and none should. | Cutover and contract change must never share a diff; a diagnostic string is not a contract. |
