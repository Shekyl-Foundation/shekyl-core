...
| [`ARCHIVAL_BOND_WI4_MEASUREMENT.md`](ARCHIVAL_BOND_WI4_MEASUREMENT.md) | WI-4 GF-7 graded genesis gate: a-priori threshold, posture-conditioned correlator spec (§4.1.1/§4.3.1 incl. the pinned in-model ceiling), §13 result (PROVISIONAL-PASS, local-daemon posture only), §14 founder-cover launch posture + partition-adversary arm spec (§14.4 hypothesis class widened pre-implementation), §15 remote-daemon structural-refusal disposition, §16 mechanization addendum (M1 reward-eligibility-by-shard-count consensus rule pre-flight pin + M2–M7; global-and-blind constraint) — §§15–16 proposed, review-gated |
