...
| M1 reward gate (consensus rule) | [`ARCHIVAL_REWARD_GATE_M1.md`](ARCHIVAL_REWARD_GATE_M1.md) (the spec + §9/§10 round records) + `ARCHIVAL_BOND_WI4_MEASUREMENT.md` §16.2 (the pre-flight pin + three obligations) | **Design rounds 1–2 closed (2026-07-06)** — cold-start refusal in consensus form: zero accrual to every `(P, s, E)` for epochs where `shard_count < K_COVER`; uniform/global/blind (§16.1 partition trap), keyed on the monotone-in-height shard count. Spec pins: zero-at-top locus in `epoch_close_compute` (two-field `EpochCloseResult`, no per-`P` widening; **zero through the normal close path, never skip** — close-log revert symmetry, prune advance, stored-zero sigma), **wire-level positive-amount rule** (round-2 resiting of the round-1 positive-share rule: any `reward_amount_plain[i] == 0` rejected at `validate()`; the zero-row beacon unencodable; exactly **one** `K_COVER` comparison site, CI grep tripwire), zero-accrual/no-seniority invariant table + zero-accrual≠zero-accountability (serve credits / bad intervals / slashing live during gated epochs), lagged-`Σwork`-read pinned doubly load-bearing (a primaries-recompute refactor bypasses the gate; reopens the round), activation-boundary KAT (G-1..G-9) + C++ stored-shape and boundary-reorg tests as first-class deliverables, spec/constant split (**`K_COVER` finalization gates on the §14.4 partition run**; provisional sentinel = compile-time refusal in non-test builds per §9.3), reorg argument written (§9.4 — connect/revert pairing suffices, no new KAT case). Genesis-frozen — first item on the WI-4 ranked implementation path; implementation proceeds after the rule-26 pre-flight pass |
