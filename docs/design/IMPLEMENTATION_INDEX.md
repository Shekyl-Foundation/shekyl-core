...
| M1 reward gate (consensus rule) | [`ARCHIVAL_REWARD_GATE_M1.md`](ARCHIVAL_REWARD_GATE_M1.md) (the spec) + `ARCHIVAL_BOND_WI4_MEASUREMENT.md` §16.2 (the pre-flight pin + three obligations) | **Design round 1 open (2026-07-06)** — cold-start refusal in consensus form: zero accrual to every `(P, s, E)` for epochs where `shard_count < K_COVER`; uniform/global/blind (§16.1 partition trap), keyed on the monotone-in-height shard count. Spec pins: zero-at-top locus in `epoch_close_compute`, gated epochs structurally non-claimable on the emission vin, zero-accrual/no-seniority invariant table, activation-boundary KAT (G-1..G-5) as first-class deliverable, spec/constant split (**`K_COVER` finalization gates on the §14.4 partition run**; provisional sentinel + sealing-checklist blocker). Genesis-frozen — first item on the WI-4 ranked implementation path. No code until round 1 closes (rule 05 spec-first; rule 26 pre-flight applies) |
