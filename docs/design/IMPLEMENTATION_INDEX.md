...
| M1 reward gate (consensus rule) | [`ARCHIVAL_REWARD_GATE_M1.md`](ARCHIVAL_REWARD_GATE_M1.md) (the spec + §9 round record) + `ARCHIVAL_BOND_WI4_MEASUREMENT.md` §16.2 (the pre-flight pin + three obligations) | **Design round 1 closed (2026-07-06)** — cold-start refusal in consensus form: zero accrual to every `(P, s, E)` for epochs where `shard_count < K_COVER`; uniform/global/blind (§16.1 partition trap), keyed on the monotone-in-height shard count. Spec pins: zero-at-top locus in `epoch_close_compute`, **uniform positive-share non-claimable rule** (amended at closure — every claimed epoch's recomputed share strictly positive; the vin never consults `K_COVER`; zero-amount claim rows unrepresentable), zero-accrual/no-seniority invariant table (both source checks discharged §9.1), activation-boundary KAT (G-1..G-5) as first-class deliverable, spec/constant split (**`K_COVER` finalization gates on the §14.4 partition run**; provisional sentinel = compile-time refusal in non-test builds per §9.3), reorg argument written (§9.4 — connect/revert pairing suffices, no new KAT case). Genesis-frozen — first item on the WI-4 ranked implementation path; implementation proceeds after the rule-26 pre-flight pass |
