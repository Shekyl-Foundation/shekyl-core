...
| WI-3 block-timed dispatch driver | [`ARCHIVAL_BOND_WI3_DISPATCH.md`](ARCHIVAL_BOND_WI3_DISPATCH.md) (design round 1) + [`ARCHIVAL_BOND_2D2_TRANSPORT_PLAN.md`](ARCHIVAL_BOND_2D2_TRANSPORT_PLAN.md) (SP-T4b/T5 seam) | **Design rounds 1–2 closed (2026-07-05)** — dispositions D-B1..D-B7 (D1 one-per-tick + dispersal draw, seal-before-send with `PENDING_POST_VERSION` 2, observed-not-claimed confirmation, late-is-noise/stale-is-terminal/re-anchor-is-re-assemble, live `BondPostDispatched`); round 2 (doc §7) amended D-B1 to the **sweep-corroborated dispatch clock** `min(claimed_tip, verified_frontier + reorg_depth)` (R2-1: a bare claimed tip lets an inflating daemon collapse the GF-7 jitter budget to zero) and pinned single-blob crash-atomicity / derived-reservation (R2-2/R2-4). Closes the §5 "Block-timed dispatch driver: Missing" gap (`TODO(2d)` in `stake_engine.rs`) when the implementation lands; consumes WI-2's sealed pending posts. **Reconvergence gate (rule 21):** WI-3 may land in parallel with WI-4, but its GF-7 acceptance does **not close** until WI-4's threshold artifact exists — the D1 coordination is only validatable against a defined `P(link \| T_obs)` bound (no armed-gate-without-trigger). **D1 invariant (design-round scope):** the dispatch driver must not co-launch correlated events off a shared sweep tick — the entry-gap draw decorrelates bond-post-vs-funding *ordering* only, not the principal's lifecycle timeline (`stake_engine.rs` `SignBond` handler note), so D1 requires independent submitters/circuits **and** independently-anchored dispatch times (discharged in `ARCHIVAL_BOND_WI3_DISPATCH.md` §3.2); the adversarial check (submission-timestamp observer gains no better-than-prior linkage) is exactly what WI-4 measures |
