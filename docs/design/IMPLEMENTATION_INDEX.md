...
| Reward-emission leg (PR-E1…E3, C-1 verifier) | **PR-E3 verify body + WS-2 write-side landed, C-1 unwired** (PR #269) — the §7.1 steps 1–6 verify module (`emission_verify`, KAT-tested) and the LMDB emission-claim connect / pre-image journal / pop-restore path ship here; the emission numerator FFI (`shekyl_archival_emission_epoch_work`) and the as-of-`E` snapshot gather are live. Still missing: the **C-1 verifier wiring** (FFI seam driving the verify body from the C++ block-connect gather) — the remaining hard Stage-3 blocker | `REWARD_EMISSION_LEG.md`; `shekyl-archival-retention/src/emission_verify.rs` |
