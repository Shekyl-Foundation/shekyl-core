...
| Reward-emission leg (PR-E1…E3, C-1 verifier) | **E0/E1/E2 landed; keystone Q1/Q7 pinned; E3 verify body + C-1 the open implementation** — the hard Stage-3 blocker. Policy trio Q3/Q12 resolved, Q11 proposed-accept; **WS-1 held-sourcing correction proposed (Q10/M-2 = supply-conservation keystone, no backstop under it): M-2 closes under _bits-sourcing ∧ as-of-fire-height acceptance_ — `work_P` sources the per-`(P,s,E)` serve-credit bits (not the tip-mutable `held_shard_ids` descriptor), populated by a `:4307` acceptance gate that must read as-of-fire-height (the M2-1 defect: `blockchain.cpp:4307` → tip accessor `db_lmdb.cpp:4975`). One shared `as_of_E_served_work` both sides; `held_shard_ids` out of the work type; one `at_height` accessor (P2B-7 Pin 4); reorg-symmetry inherited from M1 O-3; three armed KATs**; WS-2 dedup plumbing open (`REWARD_EMISSION_E3_GATING_ROUND.md`, 2026-07-07) | `REWARD_EMISSION_LEG.md`, `REWARD_EMISSION_VIN_PLAN.md`, `REWARD_EMISSION_E3_GATING_ROUND.md` |
