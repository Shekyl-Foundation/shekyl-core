...
| **PR-E0…PR-E3** | Reward-emission implementation PRs; C-1 (ML-DSA equality + membership-only verify) is PR-E3 step 8. **E0/E1/E2 landed; keystone Q1/Q7 pinned; policy trio Q3/Q12 resolved, Q11 proposed-accept; WS-1 held-sourcing correction (Q10/M-2: work_P from per-(P,s,E) serve-credit bits, not the mutable descriptor) proposed; WS-2 dedup plumbing open** | [`REWARD_EMISSION_LEG.md`](REWARD_EMISSION_LEG.md), [`REWARD_EMISSION_VIN_PLAN.md`](REWARD_EMISSION_VIN_PLAN.md), [`REWARD_EMISSION_E3_GATING_ROUND.md`](REWARD_EMISSION_E3_GATING_ROUND.md) | C-1 is the hard Stage-3 emission blocker |
...
| Reward-emission leg (PR-E1…E3, C-1 verifier) | **E0/E1/E2 landed; keystone Q1/Q7 pinned; E3 verify body + C-1 the open implementation** — the hard Stage-3 blocker. **Design round CLOSED 2026-07-07:** policy trio Q3/Q12 resolved, **Q11 ACCEPT ratified** (backing-pseudo-out balance-exclusion KAT reopen); **WS-1 ratified (Q10/M-2 = supply-conservation keystone, no backstop): M-2 closes under _bits-sourcing ∧ as-of-fire-height acceptance_ — `work_P` sources the per-`(P,s,E)` serve-credit bits (not the tip-mutable `held_shard_ids`), populated by a `:4307` gate reading as-of-fire-height (M2-1 defect: `blockchain.cpp:4307` + `db_lmdb.cpp:5270` slash-eligibility both ride tip accessor `db_lmdb.cpp:4975`). One shared `as_of_E_served_work` both sides; `held_shard_ids` out of the work type; one `at_height` accessor, two consumers; reorg-symmetry inherited from M1 O-3 with no-refcount↔dedup (`:4247`/`:4882`) coupling armed; three KATs**. **WS-2 dedup-atomicity ratified (round §6): three dedup layers — wire strictly-increasing (landed) / block-level `(P,E)` uniqueness pass mirroring the `:4882` serve-credit idiom / verify contains-check + connect `check_and_set` single writer — option 1 rejected on Q7 pure-function coherence. Prune-straddle is a double-mint (window floor keys on reorg-non-monotonic `current_settled_epoch`; a pop resurrects a pruned already-claimed epoch): claimed-set reorg-symmetry via journaled revert, row = pre-image delta `{inserted E, evicted members}` one unit (slash-log idiom); prune-straddle KAT asserts post-reorg re-claim rejection; prune-against-finalized recorded as the rule-21 reopen that retires the journal.** Design complete; PR-E3 pre-flight runs against current `dev` (`90e790c9c`) (`REWARD_EMISSION_E3_GATING_ROUND.md`) | `REWARD_EMISSION_LEG.md`, `REWARD_EMISSION_VIN_PLAN.md`, `REWARD_EMISSION_E3_GATING_ROUND.md` |
