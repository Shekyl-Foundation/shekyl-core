...
field the rules crate does not have yet (E6's `Tip` PR carries it), fed by
a `Transaction::pqc_auth_hash` the wire crate does not expose yet
(`hash_with_prunable` computes and discards it; **this lane** lands it and
`hash_with_supplied_pqc_auth` as its own small PR, KAT'd against
`hash()`). Both land before this increment's PR cuts; commit 2 waits on
...
| **Amendment A3:** `txs_pqc_auth_hash: u64 → Hash32` — the txid's **third component**, `keccak256(varint(count) ‖ auths)` exactly as `Transaction::hash()` computes it (**not** `keccak256` of the raw `txs_pqc_auths` segment, which has no count prefix and verifies nothing the chain signed), written by `connect` beside `txs_prunable_hash` from `TxIdentity.pqc_auth_hash: Option<_>`; **sparse** on `txs_pqc_auths`' own predicate (segment present ⇔ row present ⇔ txid 4-part; a split is a store invariant); Rust-only until an LMDB twin exists (`RUST_ONLY_TABLES`, SCW-11); journaled; SI-9-fresh under `tx_id`; sealed by A2 | `schema.rs`, `store/connect.rs` | `PDM-Q-F26` (PR #765) is the finding; DRS **§7.7** the landing plan: `PDM-Q6`'s second occupant is archival subject only if its hash is a persisted per-tx row, and the store had committed the identity to the first occupant alone. **Depends on** `TxIdentity.pqc_auth_hash` (rules crate, E6's `Tip` PR) and `Transaction::pqc_auth_hash` / `hash_with_supplied_pqc_auth` (wire crate, this lane's own small PR) landing first — commit 2 waits on them as it waits on `Tip` (§1) |
