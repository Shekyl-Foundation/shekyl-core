...
`3560b80c2` (the tree that merged PR #757, S-CHAIN-W). §3 is the contract
**proposed for round 1, not ruled**; §9 carries the questions with a default
each. Implements *from*
...
- **FL-R3-STORE** — the two per-block fold fields
  ...
  **connect writes** with a read here; the write half is a one-phase
  ...
  requirement (§3.6, SCR-10; shape is a §9 question).
...
The generic `open_table` / `open_multimap_table` stay for this increment and
narrow to `pub(crate)` once every table S-CHAIN-R reads has a typed read
(SCR-9 / Q3): a raw handle on the read side is the mirror of the raw
`properties` handle the write side already refuses (`PropertiesAreTyped`).
...
| R3 | `block_info(BlockHeight)` | `Result<AtHeight<BlockInfo>, StoreError>` | `get_block_timestamp` (`:2815`), `get_block_weight` (`:2915`), `get_block_cumulative_difficulty` (`:3002`), `get_block_already_generated_coins` (`:3082`), `get_block_long_term_weight` (`:3105`), `get_block_hash_from_height` (`:3128`); `get_block_difficulty` (`:3027`) is two R3 reads and a subtraction **in the caller** (Q1) | `block_info` |
...
| R5 | `block_blob(BlockHeight)` | `Result<AtHeight<Vec<u8>>, StoreError>` — the recorded bytes, **not** verified against `block_info.hash` (Q2) | `get_block_blob_from_height` (`:2789`); `get_block_blob(hash)` = R2 then R5 | `blocks` |
...
`top_block_hash(&h)` writes `*block_height = m_height − 1` **before** the
empty check (`:3185`), so an empty chain reports height `u64::MAX` beside
`null_hash` (SCR-4).
...
reverses — which the journaling verbs give any table for free. **Shape is
Q4.** The default is a Rust-only table `block_fold: u64 → BlockFold { cumulative_tx_count, long_term_effective_median }` (16 B, `InsertTable`
under SI-2, `RUST_ONLY_TABLES` entry per SCW-11), not two more fields on
`BlockInfo`: `BlockInfo`'s contract is "the LMDB struct minus the key"
(`codec/chain.rs` module docs) and its 88 bytes are what E2 diffs against
LMDB's 96; widening it would make the diff's first mismatch a layout choice.
Either shape is a rule-42 layout change: `SCHEMA_VERSION 2 → 3`, snapshot
tests move, `catalogue()` gains a row.
...
| `BlockFold { cumulative_tx_count, long_term_effective_median }` codec, 16 B; `BLOCK_FOLD` table | `codec/chain.rs`, `schema.rs` | §3.6 (if Q4 takes the default) |
...
| --- | --- | --- | --- | --- |
| `block_info` | `u64 → BlockInfo` (88 B) | R1 (last key), R3, R4, R6/R7 (identity) | `connect` phase 6 | dense `0..=tip` (SI-2): hole = SI-7 |
...
| `block_fold` (new, Q4) | `u64 → BlockFold` (16 B) | `cumulative_tx_count`, `long_term_effective_median` | `connect` (amendment A1), journaled like every other row | dense `0..=tip`: hole = SI-7 |
...
| **SCR-2** | `get_block_cumulative_rct_outputs` has exactly one caller, `Blockchain::get_output_distribution` (`blockchain.cpp:2661`); that function's only caller is `core::get_output_distribution` (`cryptonote_core.cpp:1198`), which **nothing calls**: the daemon RPC asserts `/get_output_distribution.bin` is *not* served (`rust/shekyl-daemon-rpc/src/server.rs:727`, `:881`) and `rpc_handler.cpp:29`'s helper has no caller. The method is the ring-decoy distribution rule 60 deleted from the wallet. | `rg get_output_distribution src rust` — no live producer | **Not ported.** The name stays on the DRS §7 row so the surface-map gate's 23 holds (SCW-2 precedent: a row entry can be marked not-a-read without leaving the vocabulary). S-OUT-KI's `get_output_distribution` is dead by the same chain — noted for that lane. The C++ is not deleted here (rule 20, countermand). **→ §9 Q6** only on whether a FOLLOWUPS deletion row is owed. |
| **SCR-3** | `get_block_difficulty(h)` is `cum(h) − cum(h−1)` (`db_lmdb.cpp:3027`–`:3040`): arithmetic over two recorded values, in the store. C2-R8 Q4 is about the store *deriving* consensus-visible values at the write; a read-side subtraction is not that, but it is arithmetic the store would own and could get wrong (a corrupt row with `cum(h) < cum(h−1)` underflows `u128`). | one caller, `Blockchain::block_difficulty` (`blockchain.cpp:2741`), already an unlocked read-only wrapper | **→ §9 Q1.** Default: no store method; the caller does `checked_sub` over two R3 reads and decides what a negative difference means (it is SI-8-shaped, but at a read). |
| **SCR-4** | Empty-chain sentinels: `top_block_hash` returns `null_hash` and writes `*block_height = m_height − 1` before the `m_height != 0` check (`:3185`, `:3191`) — `u64::MAX` beside a null hash; `get_top_block_timestamp` returns `0` (`:2907`); `get_top_block` returns a default-constructed `block` (`:3204`). | as cited | `tip()` returns `Option<RecordedTip>`; there is no sentinel and no field an empty chain can populate. Callers that today compare against `null_hash` (`blockchain.cpp:1168`, `:6641`) match on `None` at cutover. |
...
2. `store: ReadSnapshot::{tip, height_of, block_info, block_infos}` — R1–R4; `RecordedTip` with `connect` (Q5 default); `ReadSnapshot` holds `&'store ChainStore`; tests: empty chain → `None`; tip after `connect`; `AboveTip` above it; a deleted `block_info` row below it → SI-7 **without** halting the writer (`connect_state()` stays `Live` — the read-side half of §3.6.2, pinned).
3. `store: ReadSnapshot::{block_blob, block, blocks}` — R5–R7; `RecordedBlockBody`; tests: body hashes to identity; a rewritten blob → SI-7 on `block`/`blocks`, bytes still returned by `block_blob` (Q2 default); range clamps at the tip.
...
5. `store: block_fold — cumulative_tx_count and long_term_effective_median per block (FL-R3-STORE; S-CHAIN-W amendment A1)` — codec, table, `RUST_ONLY_TABLES` entry, `ConnectFacts` field + `DeletedBy` row, `connect` writes, undo journaling, the two reads, `SCHEMA_VERSION 2 → 3`, snapshot/catalogue moves; the §3.6 gate test (empty blocks, pop, re-connect; `cum(h) − cum(h−1) == |tx_hashes(h)|` at every height).
6. `store: narrow ReadSnapshot::open_table to pub(crate)` (Q3 default) — the in-crate tests keep it; no external caller.
...
Commit 5 is the only one whose shape waits on a ruling (Q4); commits 1–4
and 6 are unconditional and may start once this document merges. Each
commit builds, `fmt`/`clippy` clean, tests green (rule 26 B5).
...
(201 tests at `3560b80c2`); `check_redb_schema_bijection.py` (commit 5 adds
a `RUST_ONLY_TABLES` entry — the gate refuses an unexplained table);
...
change; the property catalogue is unchanged (no new cell).
...
## 9. Round-1 questions — PROPOSED (default stated; not ruled)

| Q | Question | Default | Alternative | Why the default |
| --- | --- | --- | --- | --- |
| **Q1** (SCR-3) | Does the store offer `difficulty(h)`? | **No.** Two R3 reads and a `checked_sub` in the caller. | A `difficulty(h)` read that does the subtraction and returns SI-7-shaped `FoldOverflow` on underflow. | Q4's line is cleanest when the store returns recorded values only; the one caller is already a thin read-only wrapper; a corrupt-file underflow is the caller's to classify (it is not an `SI-` row — nothing was written wrong). Reopener: a second caller that needs per-block difficulty at a rate where two reads matter. |
| **Q2** (SCR-8) | Does `block_blob(h)` verify the bytes against `block_info[h].hash`? | **No.** Bytes are bytes; `block(h)` / `blocks(range)` verify. | Verify on every blob read. | The sync path serves thousands of blobs the peer will validate; one hash each is a cost with no consumer of the assurance. The return type is the boundary: nothing typed `RecordedBlockBody` is ever unverified. Reopener: a caller that forwards blobs *as authoritative* to something that does not validate them. |
| **Q3** (SCR-9) | Narrow `ReadSnapshot::open_table` / `open_multimap_table` to `pub(crate)` in this increment? | **Yes**, commit 6. | Leave public until S-OUT-KI / S-TX land their typed reads. | Every table *this* surface reads has a typed read after commit 4; a raw handle now only bypasses. Later surfaces add their own reads and re-narrow nothing. Zero external callers at the pin. |
| **Q4** (SCR-10) | Shape of the FL-R3 fields: a Rust-only `block_fold` table, or two more `BlockInfo` fields? | **`block_fold` table**, 16 B, dense, `InsertTable` under SI-2, journaled. | Widen `BlockInfo` 88 → 104 B. | `BlockInfo`'s contract is the LMDB struct minus the key, and its bytes are what E2 diffs; a Rust-only table keeps the diff's `block_info` leg byte-comparable and puts the two new fields where the digest sees them as a new row (Small class, full-domain). `SCHEMA_VERSION` bumps either way. Sub-question, same default: `long_term_effective_median` is a `ConnectFacts` pass-through (consensus computation), `cumulative_tx_count` is store-maintained (storage count). |
| **Q5** (SCR-12) | Does the store-side `RecordedTip` carry `connect: ConnectState`? | **Yes**; `ReadSnapshot` holds `&'store ChainStore`. | `tip()` returns recorded facts only; the daemon adapter reads `connect_state()` separately. | §3.6.2 puts the halt on the tip so a reader cannot present a stale tip as current; a type that cannot omit it is the cheaper enforcement. The ordering argument (§3.4) holds because the halt is monotonic. |
| **Q6** (SCR-2) | Does the dead `get_output_distribution` chain get a FOLLOWUPS deletion row? | **No row.** It is recorded here and in the DRS §7 S-CHAIN-R row cell; the C++ path dies at cutover with the LMDB store. | A pre-genesis FOLLOWUPS row "delete `core::get_output_distribution` … `get_block_cumulative_rct_outputs`". | Rule 23: a REJECTED name leaves its reasoning where humans look; a FOLLOWUPS row for C++ that the cutover deletes wholesale is a queue entry nobody will act on separately. S-OUT-KI's pre-flight inherits the finding for its `get_output_distribution`. |
| **Q7** (SCR-11) | Where does the redb-side digest v0 reader land? | **E2's first commit**, as `ReadSnapshot::digest_v0()` reading tables directly. | Here, as a public read; or after S-OUT-KI. | It is E2's instrument (a callee with no caller until the harness exists — rule 22); it reads three surfaces' tables and should not wait on their public reads; and the accumulator module already owns the fold's contract. |
...
- `LMDB_SCHEMA.md` / `LMDB_WRITE_ATOMICITY_AUDIT.md`: no change (no LMDB table changes; `block_fold` is Rust-only and lives in the bijection gate's allowlist).
