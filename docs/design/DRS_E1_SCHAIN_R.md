...
**A second upstream dependency, ruled 2026-09-16 (DRS §7.7).** Commit 2's
layout carries the `txs_pqc_auth_hash` row (amendment A3), written from
`TxIdentity.pqc_auth_hash` — a field the rules crate does not have yet, fed
by a `Transaction::pqc_auth_hash` the wire crate does not expose yet
(`hash_with_prunable` computes and discards it). Both halves, plus
`Transaction::hash_with_supplied_pqc_auth`, land as their owners' small
PRs before this increment's PR cuts; commit 2 waits on them exactly as
commit 3 waits on `Tip`. The row is not added with a locally computed
...
minus the key" framing in the same commit (commit 2, the layout commit) (rule 91: a doc that argues from
...
| **Amendment A3:** `txs_pqc_auth_hash: u64 → Hash32` — the digest of a transaction's `pqc_auths`, written by `connect` beside `txs_prunable_hash` from `TxIdentity.pqc_auth_hash`; Rust-only until an LMDB twin exists (`RUST_ONLY_TABLES`, SCW-11); journaled; SI-9-fresh under `tx_id`; sealed by A2 | `schema.rs`, `store/connect.rs` | DRS **§7.7** (ruled 2026-09-16, from the pruning lane): `PDM-Q6`'s second occupant is archival subject only if its hash is a persisted per-tx row, and the store had committed the identity to the first occupant alone. **Depends on** `TxIdentity.pqc_auth_hash` (rules crate) and `Transaction::hash_with_supplied_pqc_auth` (wire crate) landing first — commit 2 waits on them as it waits on `Tip` (§1) |
...
- `ARCHIVAL_PRUNED_DAEMON_MODE.md` `PDM-Q6` item 2: the pruning lane's to update when the row lands — the "32-byte `txs_pqc_auth_hash` row per tx" it describes exists from S-CHAIN-R commit 2; named here, not edited from this lane.
...
| 2026-09-16 | **Amendment A3 added to the layout commit (DRS §7.7, ruled from the pruning lane).** `TxIdentity` committed the per-tx identity to `PDM-Q6`'s first occupant alone; the `txs_pqc_auth_hash` row rides commit 2 so the bump is paid once, and commit 2 gains a second upstream dependency (rules-crate field + wire-crate constructor) beside `Tip`. |
