...
| Coinbase leaf commitment | **Real per-output `CM = H_ℓ(hybrid_pk)·G_k + r·J`** (`PL-D3` (`FCMP_SPEND_LINKABILITY.md` §6.2, 2026-09-14); formerly the Blake2b key hash `h_pqc`), computed by `shekyl_construct_output` (miner self-KEM, per-output) and stored **on-chain in `tx_extra` tag `0x07`** (`TX_EXTRA_TAG_PQC_LEAF_HASHES`, `N×64` entries `CM ‖ record` in vout order). | `derivation.rs` (`derive_pqc_leaf`), `cryptonote_tx_utils.cpp` |
...
| Corner | Rule | Source |
|--------|------|--------|
| Genesis creation | Generated when DB is empty; founder pool = genesis **coinbase** outputs (`txin_gen`, `version=3`, `CTTypeNull`, `unlock_time = 60`; built offline by `geblock` — `shekyl-genesis-tool::builder::build_genesis_tx` — and pinned as `GENESIS_TX`). | `blockchain.cpp:465-472`, `rust/shekyl-genesis-tool/src/builder.rs`, `GENESIS_TRANSPARENCY.md:122-198` |
...
| **Empty-tree root** | `get_curve_tree_root()` returns **`selene_hash_init`** when no leaves (`MDB_NOTFOUND`); the genesis header is pre-set to `shekyl_curve_tree_selene_hash_init`. Distinguish empty via `get_curve_tree_leaf_count() == 0`. **NOT** `null_hash`, **NOT** all-zeros, **NOT** `build_layers([]) == [[]]`. | `db_lmdb.cpp:5909-5921`, `cryptonote_tx_utils.cpp` `generate_genesis_block`, `core_rpc_server.cpp:3875-3877` |
