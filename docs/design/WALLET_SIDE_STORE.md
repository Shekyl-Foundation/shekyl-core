...
| **WSS-8** | **`prune_frozen` has no production caller at this sha, and the archiving wallet disables pruning outright.** Every call site is a test (`shekyl-p-host/tests/composition.rs` ×7, `shekyl-p-serve/tests/store_axis.rs:125`) or a doc comment reasoning *about* it as a hazard (`p-host/src/serve_set/{report,staleness,witness}.rs`, `shekyl-operator-alarm/src/lib.rs:341`). Production instead declares the one-way prune-disabled posture — `curve_tree_actor.rs:402` calls `set_prune_disabled` — so an archiving wallet **never prunes**. **"Unused" is a hypothesis, not a verdict:** the capability is designed-for and guarded by live detectors (`PostureDeclaration`, pins, staleness, witness), so the finding is *the discard has no caller*, not *the machinery is dead*. What follows is a question, not a deletion: **does the rebuilt store prune at all, and what bounds obligation A's growth if it does not?** | `WSS-Q2`; §8 bucket (ii)'s scope; the size discipline nobody has ruled |
| **WSS-9** | **Retiring the freeze retires the *consensus meaning* of the leaf segment, not the leaf segment.** The tie today is `SEGMENT_LEAF_COUNT == leaves_per_segment()` (`PDM-Q-F33` (ii)); under `PDM-Q6`/`F32` the consensus partition becomes `b_*`, a **byte-bounded `tx_id` range**, while `root_at_count`'s composition boundary is a **leaf count** fixed by the tree's arity (`outputs_per_node(SEGMENT_LAYER_J)`). These cannot be the same object. So after E4 the leaf segment survives **inside obligation A as a local root-composition geometry with no consensus meaning**, and the `F33` (ii) interim assert must die with the freeze rather than be re-pointed at `SHARD_BYTES`. | §5 rows 1–2; §8; the interim-tie PR's scope |
...
| **WSS-Q2** | **Does the rebuilt store prune, and what bounds obligation A's growth?** | `WSS-8` (no production caller; prune-disabled is the archiver posture), `WSS-12` (full leaf set in RAM), `76-device-provisioning-floor` | Proposed: **B prunes by shard lapse; A does not prune and owes a stated growth bound at the Pi-4 floor.** A growth bound is owed either way |
...
## 7. `CTS-`'s disposition — successor round, ruled on steering's answer (3)
...
**Ruled: `CTS-` closes as record; this document is its successor.** Not a
scope amendment. Amending `CTS-` in place would leave one document holding a
...
| --- | --- |
| **`CTS-4`, `CTS-Q5`** — `FrozenSegmentRecord`'s codec and its BE→LE move | The record is retired with the freeze (`PDM-Q12`). **But see `WSS-4`:** if `WSS-Q1`/`Q3` keep a root-composition cache for obligation A, it needs *a* record — one this round designs against the leaf geometry, not `CTS-4`'s consensus-shaped one |
