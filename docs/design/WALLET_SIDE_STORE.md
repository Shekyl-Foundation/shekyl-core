...
| --- | --- | --- | --- |
| **WSS-Q1** | **Which identity owns which state** (§6.1 — *not* "one file or two", which was the wrong axis) | §4's attribution; `WSS-13` (the ground), `WSS-4`, `WSS-5`, `WSS-6`, `WSS-7`; CT-1 §3.6; CT-5 §3.1; steering (1) | **Proposed — split by identity.** (a) **`P`'s serving store** is its own file, owned by the `StakeEngine`, and is **the wallet's only redb**. (b) **The principal's proving state is not a store at all** (§6.3): a public frontier at `F`, a buffer of recent blocks, and per-output membership paths in each identity's existing sealed file. Still Round 1's to rule, but **no longer a choice between two open arms** — §6.1 rejected one-file as the arrangement `WSS-13` reports as a defect, and §6.3 resolved the proving side |
...
| **Log file** (`--log-file`, mode `0600`) | **Plaintext** | **`P`** — released shard ids at `info` (`serve_set_source.rs:261-263`) matched against the chain's bond history (`WSS-20`). **A new gap** |
...
1. **Encrypting `P`'s store does not close the gap on its own.** `WSS-20` is a
   second route to `P` for the same adversaries. Both actions are needed:
   **strip the ids from the log** (count only, plus a lint or test that no
   `P`-correlated id reaches a log at any level) and **seal the store**.
...
**So `WSS-19`'s remaining work is exactly `WSS-20`.** With those two actions —
strip the ids from the log, seal the store — **sealing `P`'s store closes the
at-rest route to `P`**,
...
### 6.5 When the fill runs — `WSS-Q4` (**open**)
...
### 6.6 The serving read — `WSS-Q7` (**open**)
...
### 6.7 The `WSS-13` unwind (**design review — not ruled**)

Everything in this section is design-review material awaiting steering's
ruling. The two **rulings** it rests on, `R-A` and `R-B`, are at §1.2.
