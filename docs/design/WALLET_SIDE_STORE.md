...
| **WSS-Q8** | **The lapse tail** — how long an archiver serves a shard it no longer bonds | [`FOLLOWUPS.md`](../FOLLOWUPS.md) (the archiver retention-horizon row), owner [`ARCHIVAL_SERVING_ROUTE.md`](ARCHIVAL_SERVING_ROUTE.md), **enforcing site this store**. **This row has no builder today** | **Proposed: hold until the pair leaves the drawable set — erase at the close of the epoch in which the drop connected, plus `D_max`.** `WSS-16` ran the check and found one consumer: the recovery fetch draws from an epoch-open snapshot that `SO-D8` Q3 deliberately does not filter at the tip. **Why this shape:** no new tunable (the horizon is `E` and `D_max`, both already ruled); never earlier than drop-connect + `D_max`, since the epoch's close is never before the drop connected, so the reorg protection is preserved unchanged; at most one shard of ≤ 4.33 MB for ≤ one epoch + 720 blocks; and it leaks nothing, because the data is public and the drop is on-chain. It also makes the drawable set **honest for recovery, not only for challenges** — every draw from `D` finds data. **Alternative rejected:** filtering `D` against holdings at tip reopens `SF-D10` **and** `SO-D8` Q3, both ruled, to save ~4 MB. **Also rejected:** accepting the misses, which pushes a known avoidable cost onto other operators. The rule stays the serving route's; the enforcement and its test are this round's |
...
- **The serving store is `P`'s.** The serve set is a persona's bonded
  ...
  **encrypted at rest under a key from the wallet's existing key hierarchy**,
  and be **erased once the pair leaves the drawable set** — at the close of the
  epoch in which the drop connected, plus `D_max` (§6.2).
...
**And later still than that:** a dropped pair stays in the epoch's drawable set
until the epoch closes (`WSS-16`), so the erasure point is **epoch-close +
`D_max`** — which is never earlier than drop-connect + `D_max`, so this
subsumes Pin 3's protection rather than replacing it.
...
2. **Erase once the pair leaves the drawable set — at the close of the epoch
   in which the drop connected, plus `D_max`** (`WSS-Q8`, `WSS-16`). The tail
   is not zero: a recovery fetch draws from an epoch-open snapshot that
   `SO-D8` Q3 deliberately leaves unfiltered at the tip, so a dropped pair
   stays drawable for the rest of its epoch. The horizon introduces **no new
   tunable** — it is `E` and `D_max`, both already ruled — and is never earlier
   than drop-connect + `D_max`, so Pin 3's reorg protection is unchanged. This reclaims storage and makes "not held" true. With the store
