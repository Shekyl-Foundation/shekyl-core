...
| `.wallet.tor/` | Tor's own persistent state (entry guards, DQ-T0.7) | The onion key is **not** there (`ADD_ONION` with a minted key; `Detach` unrepresentable — `WSS-18`). **Unverified:** whether the pinned Tor version records anything per hosted service in its `state` file |
...
2. **The existence of `.pscan` / `.pending` is an accepted residual.** It
   ...
   not mitigated.**
3. **The Tor `state` file is a factual check**, not a design question: inspect
   it after a serving session on the pinned Tor version. `WSS-19` stays open
   until it runs.

**With those two actions, sealing `P`'s store closes the at-rest route to `P`**,
