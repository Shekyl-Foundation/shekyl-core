...
| CEN-I20 | 1 | the same DB-add read (`shekyl_tx_extra_leaf_entries` judges the coinbase grammar with I19, abort-unreachable) | **E3 S-CURVE** with I19 — one call, one belt; the rule is admission's (`check_tx_extra_shape`) and E6 slice 4's to apply in the judge |
