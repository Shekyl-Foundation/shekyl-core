...
> current 122** — 100 / 1 / 1, via **DRS-P0f** — so outside the 100 the digest is
...
| Class | **RECORD-AND-SPECIFY** — settled 2026-09-08 as **DRS-W15**. The 2026-07-27 proposal below was FIX-IN-CPP-FIRST; the countermand inverted it and the A3 narrow exception was tested against this row and did not fire, because no *ratified, conformance-checked* row covers post-reorg hardfork reconstruction — CEN-B3 is the only census row over this machinery and sits at **bucket 4**, held for R4 — so the exception has no subject and cannot fire. It re-runs if R4 ratifies such a row. Note what is **not** claimed: the read-back is established **load-bearing**, not established correct; that question is R4's, alongside the mechanism. Which mechanism replaces the read-back in Rust is census **R4**'s. The **Forbidden** clause below is unchanged and binding. |
...
| **P0b** | Atomicity + journals + RAW + **transcriptions** | Rewrite audit for all tables + journals; **RAW edges**; CI every `MDB_dbi` in audit. **Transcribe (not invent):** (A-2) **height-base per journal** (hook height vs block-index *N*); (A-4) **revert partial-order table** (journal × fields × predecessors × reason) from `pop_block` comments; (A-6) note in-code `m_write_txn` assertions + error-type inconsistency. **CI housekeeping (from #624 review, Rick):** the schema-coverage gate rides a workflow named `doc-links` — rename the workflow for what it gates; the coupling is recorded at the trigger site, this closes it. *(Delivered 2026-09-05 — audit rewritten in place over every write path with the §10 gate-pinned matrix; A-2/A-4/A-6 transcribed from the pop-funnel comments and the guard census; RAW set enumerated with the dead §6.6 seed retired; workflow renamed `docs-gates.yml`; findings DRS-W1 through DRS-W11 recorded, none S-graded, no C++ touched. The RAW **edge** enumeration beyond the audit's R-1…R-6 — full read-set tracing per write path — remains with P0d's digest work, where the reads become the digest's inputs)* | Apply/pop design; E2 pop_block |
| **P0c** | Wart register — **RECORD-AND-SPECIFY** (inverted 2026-09-01; was “cheap FIX-IN-CPP”) | Call-graph warts, registered as **DRS-W12 through DRS-W15** in [`LMDB_WRITE_ATOMICITY_AUDIT.md`](../LMDB_WRITE_ATOMICITY_AUDIT.md) §9 — the rows are there, not restated here. (A-1) → **DRS-W12**, the `= 0` + `BaseTestDB` stub patch **withdrawn**, not deferred; (A-3) → **DRS-W13**; (A-5) → **DRS-W14**; `hf_versions` → **DRS-W15**, where the A3 narrow exception was tested and **did not fire**, closing “FIX or REPLICATE” on **REPLICATE** with the mechanism routed to census **R4**. *(Delivered 2026-09-08 on a short-lived branch off `dev`; no C++ written, none owed. The A-6 129-dereference dominance analysis this row was to supply the call graph for is **declined and closed** on DRS-W3's reasoning — a possession-typed write handle makes the precondition unrepresentable, so the count measures a question the rewrite deletes.)* | Silent-failure classes removed pre-redb |
...
| **R2-1** | `hf_versions` DIVERGE was a bug; load-bearing pop read-back | **Accept** — evidence = call-graph. Class was FIX-IN-CPP-FIRST at the 2026-07-27 round; **inverted by the countermand and settled 2026-09-08 as DRS-W15** (REPLICATE; mechanism to R4) |
