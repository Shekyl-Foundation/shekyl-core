...
2026-08-21, §14). Engine-swap (**DRS-E\***) not started; **P0a–P0d delivered 2026-09-10**; **DRS-0 remains blocked** on CEN-B5 S1 re-verification. Prior premature “ratified” banner remains withdrawn.
...
| **P0d** | Digest v0 | Core chain + spent keys + curve root **minimum**; **must expand archival journals before S-ARCH / DRS-E archival port** (A-1 composes with blind min oracle — see §7.1.1). *(Delivered 2026-09-10 — `shekyl-chain-store::digest_v0` (cSHAKE256, layout-independent); FFI `shekyl_logical_state_digest_v0`; `BlockchainLMDB::logical_state_digest_v0` walks production LMDB; RAW read-set in the atomicity audit §6.1; archival journals remain the named §7.1.1 exclusion.)* | C oracle; logical-state definition |
...
**“DRS-0 blocked on P0”** = **P0a–P0d** (P0e may trail with named exclusions; **P0f** gates E2's *correctness* arm, not DRS-0). **P0a–P0d are delivered** (2026-09-05 / -05 / -08 / -10). DRS-0 remains blocked on the CEN-B5 S1 re-verification at its merged sha (front-matter), not on a missing digest.
