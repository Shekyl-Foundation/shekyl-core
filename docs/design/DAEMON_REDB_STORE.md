...
> **DRS-P0f row coverage is complete** (2026-09-02): 102/102 bucket-1/2 rows disposed — **96 CHECKED-CONFORMANT**, 5 DIVERGENT (CEN-L11 with L12 coupled; **CEN-M8 at S0** with G4/J26 coupled — see FOLLOWUPS), 1 failed closed. P0f is the per-row conformance review (*not* P0d, which is Digest v0); bucket-3/4 rows remain UNREVIEWED pending their design rounds. This invalidates the unhedged
...
> conformance register is **complete over the 102 bucket-1/2 rows** (96 / 5 / 1
> — via **DRS-P0f**); outside the 96 the digest is a *regression* instrument,
> and **CEN-M8 is DIVERGENT at S0**, CEN-G4/J26 coupled to it (see FOLLOWUPS) — **CSR-3a**; register seeded
> with CEN-L11) and
...
| **DRS-D11** | **Logical state digest** is a first-class artifact, built **against LMDB** in P0/C (E-1). | Oracle for DRS-C; input definition for DRS-D8; harness exercised before redb. **Scope (CSR-3 / CSR-3a):** oracle only where **both** hold — ratified on record **and** an **affirmative conformance record** exists. Three states: CHECKED-CONFORMANT (oracle), DIVERGENT (register), UNREVIEWED (**default**); the latter two are **regression** instruments. The register is **complete over the bucket-1/2 set** (2026-09-02; round 2 2026-09-03): 96 CHECKED-CONFORMANT / 5 DIVERGENT / 1 failed closed over all 102 bucket-1/2 rows — **DRS-P0f** populated it per row (P0d is Digest v0 and cannot). |
