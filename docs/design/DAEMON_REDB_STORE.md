...
> **DRS-P0f row coverage is complete** (2026-09-02): 102/102 bucket-1/2 rows disposed — **97 CHECKED-CONFORMANT**, 3 DIVERGENT (CEN-L11 with L12 coupled; CEN-B5, the rule-71 FAKECHAIN skip — see FOLLOWUPS), 2 failed closed (CEN-L8, CEN-I12). **Both S-graded findings are FIXED and re-verified:** the S0 by PR #602 (M8/G4/J26 promoted) and the S1 by PR #604 (CEN-D2/D1 promoted, 2026-09-03). P0f is the per-row conformance review (*not* P0d, which is Digest v0); bucket-3/4 rows remain UNREVIEWED pending their design rounds. **No §7.2 S-graded divergence remains** — the S0 (PR #602) and the S1 (PR #604) are both merged and re-verified, so the register no longer gates DRS-0. This invalidates the unhedged
...
> conformance register is **complete over the 102 bucket-1/2 rows** (97 / 3 / 2
...
| **DRS-D11** | **Logical state digest** is a first-class artifact, built **against LMDB** in P0/C (E-1). | Oracle for DRS-C; input definition for DRS-D8; harness exercised before redb. **Scope (CSR-3 / CSR-3a):** oracle only where **both** hold — ratified on record **and** an **affirmative conformance record** exists. Three states: CHECKED-CONFORMANT (oracle), DIVERGENT (register), UNREVIEWED (**default**); the latter two are **regression** instruments. The register is **complete over the bucket-1/2 set** (2026-09-02; review rounds and both post-fix re-reviews, 2026-09-03): 97 CHECKED-CONFORMANT / 3 DIVERGENT / 2 failed closed over all 102 bucket-1/2 rows — **DRS-P0f** populated it per row (P0d is Digest v0 and cannot). |
