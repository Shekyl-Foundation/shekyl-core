...
> (`CEN-*`) enumerates **174** rules (**165** consensus-flagged + **9** policy-flagged);
> **21 of 175 census rows** cite a file under `src/blockchain_db/` as an
...
### 3.5 DRS-C surface map (95 methods from `blockchain.cpp`)
...
one** surface: 95 methods (100 → 95 on 2026-09-21: the four stripe-engine
methods deleted under `PDM-Q7`, and `prune_tx_data` no longer reached — its
only callers were that engine), 10 surfaces, no method in two and none in none.
...
| **S-TX** | Tx blob and existence — **LANDED 2026-09-19 (DRS-E1 increment 6, [`DRS_E1_STX.md`](../completed/DRS_E1_STX.md))**: 9 methods + `get_tx_block_height` re-homed from S-OUT-KI → 6 reads on `ReadSnapshot` — T1 `tx_location(&TxHash) -> Option<TxLocation>`, T2 `tx_count()`, T3 `tx_record(&TxHash) -> Option<TxRecord>` (pruned + `pqc_auths` segments, both permanent hash rows), T4 `tx_prunable(TxStorageId) -> AtIndex<Prunable>`, T5 `tx_output_indices(TxStorageId) -> AtIndex<TxOutputIndices>`, T6 `tx_locations(RangeInclusive<LmdbHashKey>)`; T3 + T4 recompose `get_tx_blob` (`TxRecord::wire_bytes`). `get_tx_unlock_time` not ported (transitively callerless, STX-Q2 A); `for_all_transactions` dissolves into T6. The three prune-shaped reads are the archival good's read path per `PDM-Q6`, ported — see the plan's §2.3 before reading `PDM-Q7` as their deletion. No layout change (`SCHEMA_VERSION` is untouched by this increment; it was 7 at the cut, moved by DRS-E6 slice 2) | 9 | `for_all_transactions` `get_prunable_tx_blob` `get_prunable_tx_hash` `get_pruned_tx_blob` `get_tx_amount_output_indices` `get_tx_blob` `get_tx_count` `get_tx_unlock_time` `tx_exists` | **5** — Tx blob and existence reads, dependent on chain-R for height context. No writer of its own in this vocabulary — `blockchain.cpp` writes txs only through `add_block`. |  |
| **S-CURVE** | Curve-tree reads — **LANDED 2026-09-21 (DRS-E1 increment 7, [`DRS_E1_SCURVE.md`](DRS_E1_SCURVE.md))**: **5 methods → 3 typed reads** on `ReadSnapshot` (`store/curve_reads.rs`) — C1 `curve_tree() -> CurveTreeState` (root + depth + leaf count as **one row**, `SCU-Q1`; replaces `get_curve_tree_root` / `_depth` / `_leaf_count`), C2 `root_at(BlockHeight) -> AtHeight<CurveTreeRoot>` (the body `ChainView::root_at` had, made public; the view delegates; replaces `get_curve_tree_root_at_height`), C3 `leaves(Range<TreePosition>) -> AtIndex<Vec<TreeLeaf>>` (replaces `get_curve_tree_leaf_chunk`; consumer E3, `SCU-Q4`). Layout **8 → 9**: `curve_tree_leaves` → `Coded<tree_leaf>`, `curve_tree_layers` → `(u8, u64)` via `LayerChunk::key` → `Coded<layer_hash>` (`SCU-Q3`, packing not inherited), `curve_tree_meta` → one `Coded<curve_tree_state>` row under `()`, **`EMPTY` written by the seal** (SCU-1: absence is SI-7, never a default; a grown summary's root must equal the live root, SI-12, and `EMPTY` does not make that claim). `TreePosition` moved to `shekyl-types`, `TreeLeaf` minted there (`SCU-Q2`). SI-11 (`LeavesNotDense`) armed by the reads until E3 writes. | 5 | `get_curve_tree_depth` `get_curve_tree_leaf_chunk` `get_curve_tree_leaf_count` `get_curve_tree_root` `get_curve_tree_root_at_height` | **6** — Reads only; the arithmetic lives in `shekyl-fcmp`, not here. Depends on chain state but nothing depends on it, so it can move once the chain surfaces are stable. | Storage only; math in `shekyl-fcmp` |
| **S-ARCH** | Archival reads/writes reached from `blockchain.cpp` — **read half LANDED 2026-09-23 (DRS-E1 increment 8, [`DRS_E1_SARCH.md`](DRS_E1_SARCH.md))**: **18 methods → 9 typed reads** on `ReadSnapshot` (`store/archival_reads.rs`) — A1 `bond_record(&PCanonicalId) -> Option<BondRecord>` (replaces `get_archival_bond_value` / `_hybrid_pubkey` / `archival_bond_join_epoch`; the `u64::MAX` join epoch gone by the type, SAR-1), A3 `last_served_epoch(p, shard) -> Option<SettlementEpoch>` and A4 `served_shards(p) -> Vec<ServedShard>` (one reverse seek per shard over the tuple key `ServeCreditKey`, never a full walk; replace `archival_bond_last_served_epochs` / `_all_last_served_epochs`), A5 `pass_count(p, shard, E) -> PassCount` (`archival_serve_credit_pass_count`), A6 `r_market(shard, E) -> Option<RMarket>` / A7 `sigma_work(E) -> Option<SigmaWorkMilli>` / A8 `budget(E) -> Option<AtomicUnits>` (`get_archival_r_market` and the gather's two legs — **`None` ≠ 0**, SAR-8), A9 `last_settled_slash_epoch() -> Option<SettlementEpoch>` (`get_archival_last_slash_epoch`; the `properties` cell typed, sentinel gone), A10 `attestation_witness_at(h) -> AtHeight<Option<Vec<u8>>>` (`get_archival_attestation_witness_at_height`; above-tip and empty-witness told apart). **Not ported, each by ruling:** `gather_archival_emission_epoch_snapshot` deleted-not-ported (SAR-4, §3.4 rule 3 — slice 8 composes `EmissionEpochSource` from A1/A5/A7/A8, `SAR-Q4`); `archival_bond_holds_shard` and its slash-log read are **E4 commit 1** (`SAR-Q7`: the port deferred, the row minted now as CEN-L16); `archival_shard_freeze_height` / `get_archival_shard_segment_at_height` dead (`PDM-Q12`, SAR-5); `get_archival_prune_watermark_epoch` S-PRUNE's (SAR-6); the alt-witness pair E5 S-ALT's (`SAR-Q5`); `archival_bond_good_through` already a Rust fold behind an FFI shim; `set_archival_serve_credit_bit` E4's. **Layout 10 → 11:** six tables leave `Unshaped` (`archival_bond` → `Coded<BondRecord>`, `archival_serve_credit` → `([u8; 32], u64, u64, u64) → Present`, `archival_r_market`, `archival_sigma_work`, `archival_budget`, `archival_attestation_witness`) and `properties` gains the `archival_last_slash_epoch` cell; the journals, `archival_settlement` and the retired freeze table stay `Unshaped` for E4 (`SAR-Q1`). **The bond record's first Rust type** (`shekyl-chain-store::codec::archival::BondRecord`, `Canonical`, re-specified per `SAR-Q3` — same semantics, not byte-compatible) is cross-checked against the C++ v7 encoder over a checked-in corpus (`docs/test_vectors/ARCHIVAL_BOND_RECORD_V7.json`, written by `tests/unit_tests/archival_bond_record_v7_corpus.cpp`); the shared vocabulary (`ShardSet`, `HoldingsKind`, `BadInterval`, the four frozen caps) moved to `shekyl_types::archival` (`SAR-Q2`). SI-14 `ruled` (the type refuses what it would observe), SI-15 `built` (`ServeCreditWithoutBond`). **The write half stays E4's** (CEN-L7 … L10, the journals, the C++ marshal retirement) | 18 | `archival_bond_all_last_served_epochs` `archival_bond_good_through` `archival_bond_holds_shard` `archival_bond_join_epoch` `archival_bond_last_served_epochs` `archival_serve_credit_pass_count` `archival_shard_freeze_height` `gather_archival_emission_epoch_snapshot` `get_archival_alt_attestation_witness` `get_archival_attestation_witness_at_height` `get_archival_bond_hybrid_pubkey` `get_archival_bond_value` `get_archival_last_slash_epoch` `get_archival_prune_watermark_epoch` `get_archival_r_market` `get_archival_shard_segment_at_height` `set_archival_serve_credit_bit` `store_archival_alt_attestation_witness` | **7** — Largest surface (18) and **gated on the P0b journal audit** — its write paths are the ones whose atomicity is still being characterised. Extracting before that audit ports an unaudited contract. **UPDATE 2026-09-23: the gate lifted 2026-09-05 (P0b RECONCILED; readiness row A1) and was re-read eighteen days later — Round-0 pre-flight [`DRS_E1_SARCH.md`](DRS_E1_SARCH.md) (DRS-E1 increment 8; Round 1 RULED on PR #840, **increment LANDED 2026-09-23** — see the Role cell): the *read* half of this row (eighteen methods → nine typed reads, six tables and one cell shaped, the bond record's first Rust type); the *write* half (CEN-L7 … L10, the journals, the C++ marshal retirement) stays E4's, whose first question the pre-flight names (SAR-7: which journals are history the read path needs).** | Cursor surface for retention (E4) |
| **S-POOL** | Tx pool — **LANDED 2026-09-24 (DRS-E1 increment 9, [`DRS_E1_SPOOL.md`](DRS_E1_SPOOL.md))**: **nine methods (the row's eight, plus the LMDB-level txpool_has_tx that `tx_memory_pool::have_tx` reaches past `Blockchain::` — outside this map's denominator, which is what `blockchain.cpp` reaches, so it is counted in `DRS_E1_SPOOL.md` §2.1 #9 and not here — SPL-15) → 7 typed operations** on a **second file**, `shekyl-chain-store::pool::PoolStore` — §5.1's pick built and its falsifier closed: `txpool_meta` / `txpool_blob` are **evicted** from this store's catalogue (layout 11 → 12, `tables.snap` 49 → 47, `Unshaped` 22 → 20; `schema::MIRRORED_ELSEWHERE` names the twins and the bijection gate reads it). P1 `insert(txid, &PoolRecord, blob)` (both rows; `AlreadyHeld`, `EmptyBlob`, `NullFcmpCache`; a key in one table only is SI-16), P2 `update(&txid, &PoolRecord)` (meta only; `NotHeld`; **`Relay(RelayRefusal)` — the stored `RelayState::follows` verdict carried through: `OriginChanged`, provenance enforced not asserted; `PhaseNotForward` when the phase is not the stored one and not a forward step; `ResponsibilityRearmed`**), P3 `remove` (both rows, idempotent), P4 `record -> Option<PoolRecord>`, P5 `blob -> Option<Vec<u8>>` (no category parameter; either row alone is SI-16), P6 `len`, P7 `entries()` (natural key order, SPL-8; blob a second read). `for_all_txpool_txes`'s class filter, `get_txpool_tx_count`'s per-class scan, `txpool_tx_matches_category` and txpool_has_tx dissolve into P4/P7 + `RelayMethod::matches` at the pool (SPL-3, SPL-15). **The record** (`codec::pool::PoolRecord`) re-specifies the 192-byte `txpool_tx_meta_t` along `DAEMON_RELAY_PRIVACY.md` §92.4's seams — `Origin { Originated, Arrived { zone } }` (permanent), `RelayPhase { Held, Stem, Fluff, Block }` each with its own clock word (SPL-5's overload dissolved), `RelayState` (phase enum chosen by provenance: originated is `Held` or `Block` with responsibility on that arm; arrived is `Stem`, `Fluff` or `Block`) — `SPL-Q9`, pin made structural on review; the relay discriminant has no default arm and `Fluff` is reached only by its own byte (SPL-14). `RelayMethod` / `NetZone` moved to `shekyl_types::relay` as the FFI seam's words (`SPL-Q6`). **Write API is the closure** `PoolStore::write(…)` (the store commits or aborts at the closure's exit; no caller holds a batch) — `LockedTXN`'s abort-on-drop (the mechanism of the relay-timestamp bug) is not carried (SPL-16). **Header:** own version cell; a sealed file at another layout is **recreated** (`PoolOpen::Recreated`), a non-redb file / no header / undecodable cell is **refused, never deleted** (`SPL-Q8` with its precondition). D9 durability (`SPL-Q4`). **SI-16** built. **The pool itself — admission, the relay clock, eviction, indices — is E5's**; the contract it inherits: chain commits first, pool second, pool reconciles at open (`SPL-Q5`, new E5 work) | 8 | `add_txpool_tx` `for_all_txpool_txes` `get_txpool_tx_blob` `get_txpool_tx_count` `get_txpool_tx_meta` `remove_txpool_tx` `txpool_tx_matches_category` `update_txpool_tx` | **8** — No consensus state and no dependency on the chain surfaces, so it can parallelize with 4–7 if there is capacity. Ordered here rather than earlier because it is privacy-sensitive (Dandelion++) and deserves attention that is not competing with the consensus path. **UPDATE 2026-09-24: LANDED on the increment PR.** Round 0 and Round 1 (PR #849; SPL-14) sized it; the file, the eviction and layout 12 are built. Nine DB methods — the eight `Blockchain::` forwards plus `txpool_has_tx` (SPL-15), every caller `tx_memory_pool` — map to seven operations. The §5.1 falsifier is closed: no pool table is in this catalogue. Review the same day made the pin a type (`RelayState`; `DRS_E1_SPOOL.md` §11). The pool itself — admission, relay clock, eviction, indices — stays E5's.** | Privacy-sensitive (Dandelion++) |
| **S-ALT** | Alt chain — **LANDED 2026-09-25 (DRS-E1 increment 10, [`DRS_E1_SALT.md`](DRS_E1_SALT.md))**: six methods here plus the alt-attestation witness pair (S-ARCH's #16/#17 by this map's count, S-ALT's by `SAR-Q5`) and the remover no `Blockchain::` method reaches → **seven typed operations on the consensus file's existing `WriteBatch` / `ReadSnapshot`** (`SAL-Q1` RULED: same file, same transaction — a switch is pop, demote, connect and remove in **one** `ChainStore::write` closure, SAL-1, so `rollback_blockchain_switching` is deleted at cutover rather than ported, SAL-14; the pool's grounds for a second file were checked against this surface and are absent, SAL-10). AL1 `insert_alt_block` (`AltCannot::AlreadyHeld` — CEN-K3's belt, typed; `AltCannot::IdentityMismatch` — the key must hash the block, the C++ trusted the caller), AL2 `remove_alt_block` (`AltCannot::NotHeld`, a caller-contract violation), AL3 `drop_alt_blocks`, AL4 `alt_block` (hands out a **parsed** `Block` — every C++ consumer parsed; no raw-bytes surface), AL5 `has_alt_block` (membership without a decode, the p2p `have_block` path), AL6 `alt_block_count`, AL7 `alt_blocks` — reads on `ReadSnapshot` **and** on the batch, never `ChainView` (SAL-7). `alt_block_data_t ‖ blob` → **`AltBlock`** (`codec/alt.rs`): the u128 as one field, the weight's zero sentinel as `Option`, the block bytes and the CW-2 witness as fields — `archival_alt_attestation_witness` **folded** into `alt_blocks` (`schema::FOLDED_INTO`, the bijection gate's fourth direction, SAL-2); layout **12 → 13**, `tables.snap` 47 → 46, `Unshaped` 20 → 18. **The digest boundary is a test** (SAL-15): an alt block with a witness moves no digest — the audit's "declared, not enforced" item for this class, closed as a consequence of the one-row shape. Not in scope: the admission ladder, fork choice, the switch itself, `--keep-alt-blocks` — E5's. | 6 | `add_alt_block` `drop_alt_blocks` `for_all_alt_blocks` `get_alt_block` `get_alt_block_count` `remove_alt_block` | **9** — Alt-chain storage depends on both chain surfaces being settled; its reorg path is the one place both are exercised together. **UPDATE 2026-09-25: LANDED on the S-ALT increment PR (three commits stacked on #856).** UPDATE 2026-09-25: `DRS_E1_SALT.md` §9 RULED (PR #856) — the increment may be cut. UPDATE 2026-09-24: pre-flight posed. |  |
| **S-PRUNE** | Pruning — **the retention prune LANDED 2026-09-25 ([`DRS_E1_SPRUNE.md`](DRS_E1_SPRUNE.md) §14; layout 13 → 14): the boundary batch inside `connect` at `E·SEB` discards `D(E)`'s shards (`txs_prunable` + `txs_pqc_auths`) and retires undo rows below `tip − D_max`, the `undo_log_floor` cell checked against the journal's first key (SI-6 `FloorMismatch`), `Horizons` with `cap ≤ retention < SEB` refused at open — the cap the in-force rule set's (`RuleSet::reorg_cap`, SPR-8), re-checked by `connect` — `pop` refusing below the floor, `D_MAX` / `journal_horizon` / `SHARD_TX_COUNT` built; the regtest lever's parse refuses `SEB ≤ cap` (SPR-9).** The C++ statement below stands: nothing was extracted. | 1 | `pop_target_allowed` (was 6: check_pruning, get_blockchain_pruning_seed, prune_blockchain, update_pruning **DELETED from C++ 2026-09-21**, `PDM-Q7`; prune_tx_data unreached from blockchain.cpp since the same day and **DELETED 2026-09-22** with the output_metadata and txs_prunable_tip tables and its watermark, LMDB v15) | **NOT EXTRACTED** — four of the original six were the Monero-era stripe engine, superseded before they could be ported and gone 2026-09-21; the fifth, prune_tx_data (Shekyl's C++ depth-based tx-data discard), went 2026-09-22 once it had no caller. Nothing of either is carried into S-PRUNE (see the PDM note below and its two dated updates). `pop_target_allowed` **dissolved 2026-09-15 (S-CHAIN-W SCW-7)**: pop-ability is "does `undo_log[h]` exist", so the retention prune that eventually lives here inherits a contract before it has a home — **it deletes `undo_log` rows below its watermark in its own transaction, and the watermark may not go shallower than `D_max` blocks below the tip** (`ARCHIVAL_PRUNED_DAEMON_MODE.md` PDM-Q11, **RULED 2026-09-18** — `D_max = 720` PROVISIONAL): undo-log retention ≥ `D_max`, or a legal reorg returns `StoreCannot::PopBelowFloor`. The surface landed 2026-09-25 ([`DRS_E1_SPRUNE.md`](DRS_E1_SPRUNE.md)): the floor is the `undo_log_floor` cell, written by the boundary batch, and `pop` below it is `PopBelowFloor`. [`DRS_E1_SCHAIN_W.md`](../completed/DRS_E1_SCHAIN_W.md) §5.4. The plan doc `PDM-Q-F31` asked for is that file, built rather than owed. It holds the predicate (Q2, no exceptions — Q9 RULED on #775: all daemons prune uniformly), `Horizons`, the three-leg store invariant (A4 withdrawn, `PDM-Q6` item 5: no length rows), Q3's instrument, and the named inputs (`storage_ids_through` is the storage-id total, SPR-1). Outside this row: S-ARCH retires its journals at `journal_horizon`, and §11's serve-credit precondition stays the E3 ordering. | Bootstrap / prune tools |
...
- **The pop journal** (Q5; increment 3, `store/undo.rs`, `codec/undo.rs`):
  ...
  the refusal is `StoreCannot::PopBelowFloor` at genesis and below the
  persisted `undo_log_floor` (S-PRUNE, landed 2026-09-25: the boundary
  batch retires undo rows below `tip − retention`, at least the in-force
  rule set's reorg cap and `D_max` in production, and records the lowest
  height kept — `store/prune.rs`). The cell is kept because the table
  cannot name the floor once every retained row has been popped
  (`DRS_E1_SPRUNE.md` SPR-4), and it is checked: a journal whose first key
  is not the cell's is SI-6 (`FloorMismatch`) at `pop` and at every
  boundary. A recorded tip at or above the floor with **no** journal row is
  SI-6 (`NoRowForTip`), not the floor.
...
- **The transaction reads (S-TX, DRS-E1 increment 6, 2026-09-19;
  [`DRS_E1_STX.md`](../completed/DRS_E1_STX.md) §3):** `tx_location(&TxHash)` /
...
| --- | --- | --- |
| Freed-page / COW residue | Forensic recovery of txpool / Dandelion++ timing state differs by engine | **PICKED — the pool does not live in the consensus store file.** Today it does: `m_txpool_meta` / `m_txpool_blob` are opened in the same env as the chain (`src/blockchain_db/lmdb/db_lmdb.cpp:1677–1678`), so consensus-store free pages carry relay-timing residue that outlives the tx. Pool state is **not consensus state and not reconstructible from blocks** (D10 does not reach it), so it belongs in a **separate store file that may be discarded wholesale** — which also makes residue a *policy* question rather than an engine-reclamation question. *Falsifier:* any pool table in the consensus store's `TableDefinition` set. **Depends on the slice-B schema map**; if one file is ruled instead, the fallback pick is wipe-on-open for pool tables, which is strictly weaker (it bounds residue lifetime rather than removing it). **Read against the tree 2026-09-24 (S-POOL Round 0, SPL-1):** the falsifier has been firing since slice B — `TXPOOL_META` / `TXPOOL_BLOB` sit in `schema.rs` (`:350`, `:353`), `Unshaped`, mapped 1:1 from LMDB without either pick being chosen. The pick stands; its increment is DRS-E1 S-POOL ([`DRS_E1_SPOOL.md`](DRS_E1_SPOOL.md)), which builds the file and evicts the two tables. **BUILT 2026-09-24 (S-POOL): `shekyl-chain-store::pool::PoolStore` is the separate file; `txpool_meta` / `txpool_blob` are out of this store's `TableDefinition` set (layout 12) — the falsifier is closed, and the bijection gate now holds the direction (`schema::MIRRORED_ELSEWHERE`).** **The bug this row should have cited from the start** (2026-09-24): the pool's `LockedTXN` aborts on uncommitted exit, and `get_relayable_transactions` once returned without `lock.commit()`, silently rolling back every Dandelion++ relay-timestamp write — origin-disclosure class (CHANGELOG "Dandelion++ relay timestamp rollback"; fix at `tx_pool.cpp:1232`, atomicity audit `get_relayable_transactions` row). A discardable file does not license a write handle whose `Drop` discards silently; the pool file's write API is the chain store's closure shape ([`RELAY_STATE_REFERENCE_SHAPES.md`](RELAY_STATE_REFERENCE_SHAPES.md) §1) |
...
behavior for those paths before claiming parity. *Read against the tree
(2026-09-23):* the S-ARCH **read** half landed (DRS-E1 increment 8,
[`DRS_E1_SARCH.md`](DRS_E1_SARCH.md) §2.5, SAR-11) with **no digest family
added** — a family over tables no Rust writer fills would be a MATCH over
emptiness — and with **no parity claim** on any S-ARCH census row; what this
rule binds is the apply/revert path (E4's writers, CEN-L7 … L10) and the E2
...
is ported. Only **21 of 175 census rows** name a store file at all, and seven
of those are already bucket 3. The other **140 of 154 live consensus rows** (141/153 when this was written; CEN-F12 went to bucket 3 with E6 slice 4's precursor, 2026-09-21; CEN-I20 joined the bound set 2026-09-23; CEN-L16 minted store-bound the same day by S-ARCH's pre-flight) —
...
| CEN-I20 | 1 | the same DB-add read (`shekyl_tx_extra_leaf_entries` judges the coinbase grammar with I19, abort-unreachable) | **E3 S-CURVE** with I19 — one call, one belt; the rule is admission's (`check_tx_extra_shape`, I19 + I20 in one function) and the 4.I slice's (E6 slice 6) to apply in the judge — slice 4 (4.F) landed 2026-09-22 without it, `CHAIN_RULES_SLICE_4.md` S21 |
| CEN-L11 | 1 | curve-tree growth per accepted output | **E3 S-CURVE** |
| CEN-L12 | 1 | deferred-insertion maturity | **E3 S-CURVE** |
| CEN-L7 | 1 | archival connect-writer fatal backstops | **E4 S-ARCH** — shapes minted by E1 S-ARCH (2026-09-23): the pass bit is a `Present` row under `ServeCreditKey`, the record a `Coded<BondRecord>`; the unknown-persona refusal is SI-15's first check when this hook lands |
| CEN-L8 | 1 | epoch close + settlement hooks in `add_block` | **E4 S-ARCH** — the three close rows it writes are shaped by E1 S-ARCH (`Coded<RMarket>`, `Coded<SigmaWorkMilli>`, `Coded<AtomicUnits>`; A6–A8 read them as `Option`, `SAR-Q6`); `archival_settlement` and the journals stay E4's to shape |
| CEN-L9 | 1 | slash processing per height | **E4 S-ARCH** — its watermark is the `ArchivalLastSlashEpochCell` E1 S-ARCH typed (A9); the slash log's shape is E4 Round 0's (SAR-7) |
| CEN-L10 | 1 | segment-freeze registry, CREATE-only | **E4 S-ARCH** — retired by `PDM-Q12`; E4 deletes the C++ and moves the row to bucket 3 (`DRS_E1_SARCH.md` SAR-5) |
| CEN-L16 | 1 | the as-of-height holds-shard predicate evaluated inside the store (`archival_bond_holds_shard_of`, reading `archival_slash_log` as history) | **E4 S-ARCH** — the fold moves to `shekyl-archival-retention` with E4's journal ruling; E1 S-ARCH mints the typed reads it composes from (A1 + A2, `DRS_E1_SARCH.md` `SAR-Q7`); twin CEN-J8 |
...
**Table 3 — the 140 of 154 live consensus rows that are surface-free, by census subsystem (E6's slices).**
...
| 4.F Miner transaction | 13 | 4 | 4 | 21 | slice 4 — emission / burn arithmetic in `shekyl-economics` (adopt); needs header + weights. Was `5 / 22`: CEN-F12 (dead gate) deleted to bucket 3 by the slice-4 precursor, Q2 (a) |
...
| 4.J Archival families | 26 | 0 | 0 | 26 | slice 8 — all verdicts already Rust-side; adopt behind the crate, C++ marshalling retired with E4. **Inherited forward-action (`SAR-Q6`, RULED 2026-09-23):** the store's A6/A7 reads return `Option` where the C++ returned `0` on absence; whether a bond-post admission treats a held shard whose epoch has no market row as zero co-holders or as a distinct fact is this slice's to decide (`ARCHIVAL_CONSENSUS_STATE.md` §3.3–3.5), and the `Option` is what makes the question askable |
| 4.K Reorg / alt chains | 1 | 10 | 0 | 11 | slice 9 — re-validation over an alt `ChainView`; after everything above |
| 4.M Mempool (consensus rows) | 1 | 0 | 1 | 2 | slice 10 with E5 — the nine `P` rows enter as `AdmissionPolicy` here, never as `RuleSet`. **The pool store E5 writes into landed 2026-09-24** (DRS-E1 S-POOL, `shekyl-chain-store::pool`): P1–P7 are the contract; E5 inherits chain-first / pool-second with reconciliation at open (`SPL-Q5`), and the relay loop that branches on the record's relay fields is the Zone's job, not the pool's (`RELAY_STATE_REFERENCE_SHAPES.md` §2) |
...
| --- | --- | --- |
| **Not chain state** | ~~`txpool_meta`, `txpool_blob`~~ (**moved out of this file 2026-09-24, S-POOL, layout 12** — the boundary this row predicted, drawn: they are `pool_meta` / `pool_blob` in `shekyl-chain-store::pool`'s own file and cannot be in a digest over this one), `alt_blocks` (~~`archival_alt_attestation_witness`~~ — **folded into `alt_blocks` 2026-09-25, S-ALT, layout 13**: the witness is `AltBlock::attestation_witness`, one row, one lifetime) | Replaying **main-chain** blocks produces the main chain. The pool is unconfirmed by definition and the alt surface is by definition what the chain did not take; two honest nodes at one height legitimately differ. Slice A excludes these from **all future digests** on the same ground. Slice C's §5.1 pick moves the pool out of the consensus store file entirely, which makes this a boundary rather than an exception — **built by S-POOL**; the alt surface's file was S-ALT's question (`SPL-Q2`'s reopener) and was **answered the other way** 2026-09-25 — it stays in this file, `Excluded`, because a switch is one transaction with `connect`/`pop` or it is not a switch (`DRS_E1_SALT.md` SAL-1, SAL-10); the boundary is now **enforced** by a test, not only declared (SAL-15) |
