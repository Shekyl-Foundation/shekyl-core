...
> **DRS-P0f row coverage is complete** (2026-09-02): 102/102 bucket-1/2 rows disposed — **95 CHECKED-CONFORMANT**, 5 DIVERGENT (CEN-L11 with L12 coupled; **CEN-D2 at S1** with D1 coupled; CEN-B5, the rule-71 FAKECHAIN skip — see FOLLOWUPS), 2 failed closed (CEN-L8, CEN-I12). **CEN-M8's S0 is FIXED and re-verified** (PR #602 merged; M8/G4/J26 promoted, round 6). P0f is the per-row conformance review (*not* P0d, which is Digest v0); bucket-3/4 rows remain UNREVIEWED pending their design rounds. **Per §7.2, CEN-D2's S1 (CEN-D1 coupled) blocks DRS-0 until fixed or accepted** (CEN-M8's S0: fixed by PR #602, merged and re-verified 2026-09-03). This invalidates the unhedged
...
> conformance register is **complete over the 102 bucket-1/2 rows** (95 / 5 / 2
> — via **DRS-P0f**); outside the 95 the digest is a *regression* instrument,
> and **CEN-M8's S0 is fixed and re-verified** (PR #602 merged; M8/G4/J26 promoted round 6) with **CEN-D2 at S1** (CEN-D1 coupled), a DRS-0 blocker per §7.2 (see FOLLOWUPS) — **CSR-3a**; register seeded
> with CEN-L11) and
...
| **DRS-D11** | **Logical state digest** is a first-class artifact, built **against LMDB** in P0/C (E-1). | Oracle for DRS-C; input definition for DRS-D8; harness exercised before redb. **Scope (CSR-3 / CSR-3a):** oracle only where **both** hold — ratified on record **and** an **affirmative conformance record** exists. Three states: CHECKED-CONFORMANT (oracle), DIVERGENT (register), UNREVIEWED (**default**); the latter two are **regression** instruments. The register is **complete over the bucket-1/2 set** (2026-09-02; review rounds through 6, 2026-09-03): 95 CHECKED-CONFORMANT / 5 DIVERGENT / 2 failed closed over all 102 bucket-1/2 rows — **DRS-P0f** populated it per row (P0d is Digest v0 and cannot). |
