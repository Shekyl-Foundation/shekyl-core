...
> the checked set holds **one row as of 2026-09-02** — CEN-H5, promoted by **DRS-P0f** (the conformance register; *not* P0d, which is Digest v0); everything else is UNREVIEWED. This invalidates the unhedged
...
> checked set holds **one row as of 2026-09-02** (CEN-H5, via **DRS-P0f**), so
> outside that row the digest is still a *regression* instrument (**CSR-3a**; register seeded
> with CEN-L11)) and
...
| **DRS-D11** | **Logical state digest** is a first-class artifact, built **against LMDB** in P0/C (E-1). | Oracle for DRS-C; input definition for DRS-D8; harness exercised before redb. **Scope (CSR-3 / CSR-3a):** oracle only where **both** hold — ratified on record **and** an **affirmative conformance record** exists. Three states: CHECKED-CONFORMANT (oracle), DIVERGENT (register), UNREVIEWED (**default**); the latter two are **regression** instruments. The checked set holds **CEN-H5** as of 2026-09-02 — **DRS-P0f** populates it per row (P0d is Digest v0 and cannot). |
