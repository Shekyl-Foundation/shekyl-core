...
| **S-OUT-KI** | Outputs and key images — **plan + Round-0 pre-flight OPEN 2026-09-18** ([`DRS_E1_SOUT_KI.md`](DRS_E1_SOUT_KI.md); DRS-E1 increment 5; 8 methods → 4 reads; `output_amounts` multimap → keyed tuple table, SOK-1; **`get_output_histogram` DELETED 2026-09-18** — SOK-Q3 ruled B, a statistical disclosure surface with no consumer on a ringless chain; RPC, CLI command and the callerless store chain removed by their own PR; the count moved 8 → 7 because the deletion is real, unlike SCW-2/SCR-2's kept names) | 7 | `for_all_key_images` `for_all_outputs` `get_output_distribution` `get_output_key` `get_output_tx_and_index` `has_key_image` `has_key_images` | **4** — Consensus-critical (double-spend admission) and needs chain reads for height context, so it follows S-CHAIN-R rather than racing it. |  |
