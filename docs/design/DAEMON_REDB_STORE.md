...
| **S-POOL** | Tx pool — **LANDED 2026-09-24 (DRS-E1 increment 9, [`DRS_E1_SPOOL.md`](DRS_E1_SPOOL.md))**: **8 methods (+1 the row never counted, SPL-15) → 7 typed operations** on a **second file**, `shekyl-chain-store::pool::PoolStore` — §5.1's pick built and its falsifier closed: `txpool_meta` / `txpool_blob` are **evicted** from this store's catalogue (layout 11 → 12, `tables.snap` 49 → 47, `Unshaped` 22 → 20; `schema::MIRRORED_ELSEWHERE` names the twins and the bijection gate reads it). P1 `insert(txid, &PoolRecord, blob)` (both rows; `AlreadyHeld`, `EmptyBlob`, the shape refusals), P2 `update(&txid, &PoolRecord)` (meta only; `NotHeld`; **`OriginChanged` — provenance enforced, not asserted**), P3 `remove` (both rows, idempotent), P4 `record -> Option<PoolRecord>`, P5 `blob -> Option<Vec<u8>>` (no category parameter), P6 `len`, P7 `entries()` (natural key order, SPL-8; blob a second read). `for_all_txpool_txes`'s class filter, `get_txpool_tx_count`'s per-class scan, `txpool_tx_matches_category` and `txpool_has_tx` dissolve into P4/P7 + `RelayMethod::matches` at the pool (SPL-3, SPL-15). **The record** (`codec::pool::PoolRecord`) re-specifies the 192-byte `txpool_tx_meta_t` along `DAEMON_RELAY_PRIVACY.md` §92.4's seams — `Origin { Originated, Arrived { zone } }` (permanent), `RelayPhase { Held, Stem, Fluff, Block }` each with its own clock word (SPL-5's overload dissolved), `Responsibility` (`Originated` only; `Arrived` + `Armed` refused at decode and write) — `SPL-Q9`; the relay discriminant has no default arm and `Fluff` is reached only by its own byte (SPL-14). `RelayMethod` / `NetZone` moved to `shekyl_types::relay` as the FFI seam's words (`SPL-Q6`). **Write API is the closure** `PoolStore::write(…)` (the store commits or aborts at the closure's exit; no caller holds a batch) — `LockedTXN`'s abort-on-drop (the mechanism of the relay-timestamp bug) is not carried (SPL-16). **Header:** own version cell; a sealed file at another layout is **recreated** (`PoolOpen::Recreated`), a non-redb file / no header / undecodable cell is **refused, never deleted** (`SPL-Q8` with its precondition). D9 durability (`SPL-Q4`). **SI-16** built. **The pool itself — admission, the relay clock, eviction, indices — is E5's**; the contract it inherits: chain commits first, pool second, pool reconciles at open (`SPL-Q5`, new E5 work) | 8 | `add_txpool_tx` `for_all_txpool_txes` `get_txpool_tx_blob` `get_txpool_tx_count` `get_txpool_tx_meta` `remove_txpool_tx` `txpool_tx_matches_category` `update_txpool_tx` | **8** — No consensus state and no dependency on the chain surfaces, so it can parallelize with 4–7 if there is capacity. Ordered here rather than earlier because it is privacy-sensitive (Dandelion++) and deserves attention that is not competing with the consensus path. **UPDATE 2026-09-24: Round-0 pre-flight OPEN and Round 1 RULED the same day (PR #849; Q6's default corrected — the relay-state decoder refuses every unrecognised state, `fluff` never by fall-through, SPL-14), [`DRS_E1_SPOOL.md`](DRS_E1_SPOOL.md) (DRS-E1 increment 9): all eight are `Blockchain::` forwards whose only consumer is `tx_memory_pool`; they map to seven operations on a **separate pool file** — the §5.1 pick, ruled 2026-09-12 and unbuilt, whose falsifier (`txpool_meta` / `txpool_blob` in this store's `TableDefinition` set) has been firing since the schema map landed (SPL-1). The increment builds the file, evicts the two tables (layout 11 → 12) and re-specifies the 192-byte record; the pool itself — admission, relay clock, eviction, indices — stays E5's.** | Privacy-sensitive (Dandelion++) |
