...
**`blockchain.cpp`** (268 store call sites, 100 store methods), not the
storage class alone.
...
| **S-OUT-KI** | Outputs and key images — **plan + Round-0 pre-flight OPEN 2026-09-18** ([`DRS_E1_SOUT_KI.md`](DRS_E1_SOUT_KI.md); DRS-E1 increment 5; **7 methods → 4 reads** (histogram **DELETED 2026-09-18**, SOK-Q3 B — was 8); `output_amounts` multimap → keyed tuple table, SOK-1; RPC, CLI command and the callerless store chain removed by their own PR; the name does not stay on this row because the C++ method is gone from `blockchain.cpp`, unlike SCW-2/SCR-2) | 7 | `for_all_key_images` `for_all_outputs` `get_output_distribution` `get_output_key` `get_output_tx_and_index` `has_key_image` `has_key_images` | **4** — Consensus-critical (double-spend admission) and needs chain reads for height context, so it follows S-CHAIN-R rather than racing it. |  |
