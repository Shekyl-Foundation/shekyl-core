...
| **S-TX** | Tx blob and existence — **plan + Round-0 pre-flight OPEN 2026-09-19** ([`DRS_E1_STX.md`](DRS_E1_STX.md); DRS-E1 increment 6; 9 methods + `get_tx_block_height` re-homed from S-OUT-KI → 6 reads on `ReadSnapshot` (five point reads and a bounded tx walk); the three prune-shaped reads are the archival good's read path per `PDM-Q6`, ported — see the plan's §2.3 before reading `PDM-Q7` as their deletion) | 9 | `for_all_transactions` `get_prunable_tx_blob` `get_prunable_tx_hash` `get_pruned_tx_blob` `get_tx_amount_output_indices` `get_tx_blob` `get_tx_count` `get_tx_unlock_time` `tx_exists` | **5** — Tx blob and existence reads, dependent on chain-R for height context. No writer of its own in this vocabulary — `blockchain.cpp` writes txs only through `add_block`. |  |
