...
| Class | **RECORD-AND-SPECIFY** — settled 2026-09-08 as **DRS-W15**. The 2026-07-27 proposal below was FIX-IN-CPP-FIRST; the countermand inverted it and the A3 narrow exception was tested against this row and did not fire, because the tip-above read-back is *correct* — so the C++ still serves as a sound oracle over `hf_versions` and nothing here blocks it. Which mechanism replaces the read-back in Rust is census **R4**'s. The **Forbidden** clause below is unchanged and binding. |
