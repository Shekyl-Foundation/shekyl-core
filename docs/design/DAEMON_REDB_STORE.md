...
- **The pop journal** (Q5; increment 3, `store/undo.rs`, `codec/undo.rs`):
  ...
  batch retires undo rows below `tip − retention`, `D_max` in production,
  and records the lowest height kept — `store/prune.rs`). A recorded tip at
  or above the floor with **no** journal row is SI-6 (`NoRowForTip`), not
  the floor.
