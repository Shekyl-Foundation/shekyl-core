...
| **DRS-D11** | **Logical state digest** is a first-class artifact, built **against LMDB** in P0/C (E-1). | Oracle for DRS-C; input definition for DRS-D8; harness exercised before redb. **Scope (CSR-3 / CSR-3a):** oracle only where **both** hold — ratified on record **and** an **affirmative conformance record** exists. Three states: CHECKED-CONFORMANT (oracle), DIVERGENT (register), UNREVIEWED (**default**); the latter two are **regression** instruments. The register is **complete** (2026-09-02): 98 CHECKED-CONFORMANT / 3 DIVERGENT / 1 failed closed over all 102 bucket-1/2 rows — **DRS-P0f** populated it per row (P0d is Digest v0 and cannot). |
