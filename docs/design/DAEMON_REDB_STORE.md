...
| **A3** | Known durable-state **warts** are recorded (DRS-W1…DRS-W15); default **RECORD-AND-SPECIFY**. **Met 2026-09-08 (P0c):** four remaining rows registered in the audit §9; A-6 dominance analysis declined (possession-typed write handle). DRS-W15 Forbidden is DIVERGE-by-delete; rebuilt-window correctness and replacement mechanism are R4's, in that order | Security | DRS-P0 / C |
