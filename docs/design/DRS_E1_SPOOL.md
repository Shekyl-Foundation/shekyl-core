...
deviations from the ruled plan are in §11 (2026-09-24, "as built", and the
review row the same day): the commits re-cut two-plus-one, the class table's
two rows kept, one layout number for the crate's two files, `matches` on
`RelayMethod`, `PoolStore::create` returning `PoolOpen`, and the pin made
structural — `RelayState` chooses the phase enum by provenance, so
`PhaseWithoutOrigin` and `responsibility: Option` are not the shape that
landed. The census row's method for S-ALT is unchanged.
...
provenance, phase, responsibility — rather than a field-for-field
`RelayMethod`, which re-bundles what that round unbundled (**`SPL-Q9` RULED
2026-09-24: the decomposition**). The review on the increment builds that
decomposition as `RelayState`: the phase enum is chosen by provenance and
responsibility sits on the originated arm, and `RelayState::follows` is the
one verdict a write asks — `RelayRefusal::OriginChanged` compares `origin()`,
so a disarm or a phase step is not an origin change (§11).
**The increment may now be cut
...
| `Responsibility` | `enum { Armed, Disarmed }` — **a separate field, deliberately not nested inside `Origin::Originated`**, and legal only with it: `Arrived` + `Armed` was to be **refused at decode** (SI-7) **and at write** — **as built (§11 review), it is unrepresentable:** responsibility is a field of `RelayState::Originated` only, and the write-side refusal that remains is the re-arm (`RelayRefusal::ResponsibilityRearmed`) | with `PoolRecord` (`SPL-Q9` RULED) | §92.4's second clause and §92.5c item 1's third: re-broadcast responsibility ends when F-10's predicate fires; `on_stem_propagated` writes `Disarmed`. The C++ `observed_circulating` bit, given the only entry class it is defined for. **Why the first cut kept it a sibling field, and why that did not hold (review, 2026-09-24):** comparing the whole `Origin` value would treat a nested responsibility as an origin change. That is a property of the comparison, not of the lifetimes. `OriginChanged` compares `RelayState::origin()` — originated, or arrived over a zone — and responsibility lives on `RelayState::Originated` only, so a disarm is not an origin change and an arrival cannot carry one. The phase enum is chosen the same way (`OriginatedPhase` is `Held \| Block`; `ArrivedPhase` is `Stem \| Fluff \| Block`). See the §11 review row. |
...
| `PoolCannot` | **as built:** `{ AlreadyHeld, NotHeld, EmptyBlob, Relay(RelayRefusal), NullFcmpCache }`, with `RelayRefusal { OriginChanged, PhaseNotForward, ResponsibilityRearmed }` the codec's own verdict (*ruled as* `{ AlreadyHeld, NotHeld, EmptyBlob, OriginChanged, ResponsibilityWithoutOrigin }`) | `shekyl-chain-store::pool`; `RelayRefusal` in `codec::pool` | P1/P2's typed refusals — the pool's decisions, not faults (`StoreCannot`'s shape). `EmptyBlob` added on review (Copilot, PR #849): P1's row required the refusal and the enum did not carry it. The relay refusals are **one** verdict, `RelayState::follows(next) -> Result<(), RelayRefusal>`, asked by P2 and carried through unchanged — the store does not restate the type's transition rule as three separate checks (§11, elegance review). `Arrived` + `Armed` is unrepresentable, so `ResponsibilityWithoutOrigin` has nothing to refuse; SI-7 at decode covers what the bytes could still spell. |
| `PoolStore`, `PoolSnapshot`, `PoolBatch` | the handle, the read snapshot, the write batch **handed only to a `PoolStore::write` closure** (`#[must_use]`, never returned to a caller) | `shekyl-chain-store::pool` — `mod.rs` the handle, `header.rs` the seal, `read.rs` P4–P7, `write.rs` P1–P3, `schema.rs` the tables (the chain store's own `store/` layout) | One file, one writer (§3.6's process model, applied to a second file). The batch's shape is SPL-16's: the store decides commit or abort at the closure's exit; no code path holds a batch it could drop. |
