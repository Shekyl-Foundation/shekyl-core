...
responsibility sits on the originated arm, while `OriginChanged` compares
`origin()` so a disarm or a phase step is not an origin change (§11).
...
| `Responsibility` | `enum { Armed, Disarmed }` — **a separate field, deliberately not nested inside `Origin::Originated`**, and legal only with it: `Arrived` + `Armed` is **refused at decode** (SI-7) **and at write** (`PoolCannot::ResponsibilityWithoutOrigin`), the way the relay discriminant refuses an unknown byte | with `PoolRecord` (`SPL-Q9` RULED) | §92.4's second clause and §92.5c item 1's third: re-broadcast responsibility ends when F-10's predicate fires; `on_stem_propagated` writes `Disarmed`. The C++ `observed_circulating` bit, given the only entry class it is defined for. **Why the first cut kept it a sibling field, and why that did not hold (review, 2026-09-24):** comparing the whole `Origin` value would treat a nested responsibility as an origin change. That is a property of the comparison, not of the lifetimes. `OriginChanged` compares `RelayState::origin()` — originated, or arrived over a zone — and responsibility lives on `RelayState::Originated` only, so a disarm is not an origin change and an arrival cannot carry one. The phase enum is chosen the same way (`OriginatedPhase` is `Held \| Block`; `ArrivedPhase` is `Stem \| Fluff \| Block`). See the §11 review row. |
