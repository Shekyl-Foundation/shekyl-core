...
**Status:** **ROUND CLOSED 2026-08-14** — every PR merged (SA-1..6 +
SA-2b + the SA-3 batch + the bond-watch follow-on), every ruling
ratified (SA-R-1..7), and the conventions tail executed
(`feat/sa-rt-round-close`): the SA-R-7 pattern gate is live
(`shekyl-ffi/tests/ffi_boundary_ratchet.rs`, both-directions pins on raw
`from_raw_parts`/`with_capacity`; second helper clone absorbed), the
house conventions are rules (`40`/`30`/`42` additions + new
[`46-shell-gate-exits`]), and the FOLLOWUPS rewrap landed. Historical
status below kept as written. Previously: ROUND OPEN — rulings SA-R-1…SA-R-6 RATIFIED 2026-08-09; PR-SA-1
...
## 2.3 SA-R-7 — untrusted numeric conversions (RATIFIED 2026-08-14)

**Ratified 2026-08-14 after the completeness check the ratification was
conditioned on.** The reviewer's challenge — "the rule can't be 'codify
what's now enforced' if a sibling of the fixed bug is still live two files
over" — drove a full `with_capacity` census of the FFI crate (30 sites),
which settled a distinction the rule now encodes:

- **Wire-embedded counts** (parsed from *inside* a relayed blob, promising
  elements with no buffer behind them): the dangerous class.
  `parse_prove_witness`'s three counts were the **only** instances — fixed
  cap-before-reserve, pinned by test.
- **ptr+len ABI-pair counts** (the count *is* the declared length of an
  accompanying caller-provided buffer — `emission.rs` pairs/ids, the
  verify-path `ki/po/pqc` counts, FROST/relay spans): a hostile value here
  is a broken caller contract (UB at the slice, allocation or no
  allocation), and the sites carry `checked_mul` + `isize::MAX` guards so
  overflow cannot desync the byte length from the element count. One
  uniformity gap found and fixed in the census:
  `shekyl_fcmp_outputs_to_leaves`'s unchecked `count * 128`.
- **`.len()`-derived counts** (incl. division-derived like
  `scalars_len / 32`): bounded by materialized bytes, safe by construction.
...
**Enforcement degrees (stated honestly, per the ratification):** clause 1
...
outside `slice_from_ptr`. **LANDED with the conventions tail**
(`feat/sa-rt-round-close`): the FFI boundary ratchet
(`shekyl-ffi/tests/ffi_boundary_ratchet.rs`) pins raw `from_raw_parts`
and raw `with_capacity` per file in both directions, the seam helpers
are rule-40 obligations, and the pipefail convention is rule
`46-shell-gate-exits` (five exhibits by close). Clause 2 is now
enforced, not remembered; the residual is discharged and the round
closed.
...
(cSHAKE256, Keccak-256 — `shekyl_crypto_hash::keccak256`, renamed from
`cn_fast_hash` in SA-3d; C ABI export keeps `shekyl_cn_fast_hash` — SHA-512,
Blake2b512), and
...
| `RetiredPersonaRecord.{release,retired}_epoch` | ratified done-side authoritative-record provenance (2026-06-29 round); derive-forward keys on `p_slot` |
