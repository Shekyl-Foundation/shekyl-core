...
| PW-8 | Rekey/forward-secrecy schedule for long-lived peer-to-peer sessions (harvest-now-decrypt-later applies to session traffic, not just the handshake; the §Scope boundary puts operator-to-operator sessions on the RPC side — this row governs the adversarial P2P wire). **The named production precedents carry their own rekey schedules — a Signal-style ratchet is not needed to close this row.** Three candidate mechanisms, two with deployment history: (1) **BOLT-8-style symmetric KDF rotation** (Lightning rotates every 1,000 messages via a symmetric KDF chain): cheap, no new entropy on the wire, old keys deleted (forward secrecy), and every rotated key still derives from the hybrid handshake's ML-KEM-mixed root, so the HNDL property survives rotation; lacks post-compromise healing — a fully compromised session state stays compromised. (2) **WireGuard-style periodic full re-handshake** (WireGuard re-runs the Noise_IK handshake on ~120 s or a message-count bound, whichever first): fresh hybrid entropy each interval = post-compromise recovery and a refreshed PQ contribution; costs PW-3's ~2.7 KB hybrid handshake per interval — **interaction hazard: a re-handshake on a fixed timer is itself a PW-28-class cadence fingerprint**, so if chosen the interval gets jittered and the handshake padded under the same fixed-size band as the initial one, or PW-8's fix quietly recreates what PW-28 closes. (3) **PQ-KEM ratchet** (Signal-style asymmetric step, KEM-ified) — where PW-4 bites: the Double Ratchet's asymmetric half is DH-shaped and KEM-based ratchets are research-grade, not deployed prior art; keep in the wargame table as the control, likely rejected under the get-it-right posture. **Pattern-axis honesty note for the wargame:** WireGuard is IK and Lightning is XK — production rekey precedent concentrates on the authenticated static-key surface; the gossip wire's NN-family direction has thinner deployment history (recommendation unchanged — the anonymity requirement justifies NN and it is the simplest pattern, not an exotic one — but the wargame states the precedent gap rather than implying transfer). Compensation: on NN sessions option 2's re-handshake is nearly free of identity complications, since there is no static key to re-authenticate. | Steering round 2026-09-01 (this fold); WireGuard whitepaper (timer/message-count re-handshake), Lightning BOLT #8 (1,000-message KDF rotation) — **fetch both into the papers corpus and re-verify the exact bounds before the P2P-2 brief quotes figures** (PW-3's re-pin discipline) | Open — choose among the three named mechanisms, possibly differing by surface (authenticated vs NN gossip); option 2 only with jitter + fixed-size padding per PW-28/PW-3 |
