...
| PWC-A4 | Packet limit 256 KiB until the Levin handshake completes, raised to the session limit only by that completion | `header.rs:23`; `reader.rs:147`, `:153-169` | `pinned-not-re-derived` | 4 | — |
| PWC-A5 | Packet limit once a session is established is 100 MB (`DEFAULT_MAX_PACKET_SIZE`), inherited, not derived from any largest-legitimate-message argument | `header.rs:27`; `levin_base.h:86` | `pinned-not-re-derived` | 4 | PW-10 |
