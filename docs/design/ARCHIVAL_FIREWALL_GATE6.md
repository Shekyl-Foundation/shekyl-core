...
- **Round 4 drafted (2026-07-11, §12) — the drain-event firewall.** GF-7 (funding-in) already built +
  ...
  **dissolves by axis attrition** — ratification added the fourth row the draft's walk had
  ...
  aggregate-only surface LANDED (§12.4 build note); F-D2's `shekyl-gui-wallet`
  round-number/random-split default is a second repo and remains OPEN. R4 close:
  F-D2 GUI default is the last open item.**
...
**Build (2026-07-17) — core-side half LANDED; GUI default NOT (second repo).**
...
- **The wallet-flow default — NOT LANDED (out of scope for this PR).** "Never
  ...
  been touched. **F-D2 is not recorded as landed** until that flow is built; the
