...
- **Round 4 drafted (2026-07-11, §12) — the drain-event firewall.** GF-7 (funding-in) built; its
  verdict is **⛔ WITHDRAWN in full 2026-07-23** (was: graded PROVISIONAL-PASS, `r = 1.86`) — the
  graded channel was never on the chain (WI-4 §13.1 banner; GF4b-2's funding-input count is the
  surviving entry-seam channel); GF-4 (value-out) drafted as one event with three co-triggered
  ...
  **F-D5** → ~~§14.4~~ *(dangling — re-pointed 2026-07-17 to the §12.7-chartered disposition
  ...
  the joint grade. The reinstate/release FSM blocker is discharged (landed PR #303/#307).
  ...
  **dissolves by axis attrition** — ratification added the fourth row the draft's walk had
  ...
  (incl. the §16.4 funding default) on top (§12.4 build note). R4 close: that
  subsystem is the last open item.**
...
**Round-2 scope expansion — reinstate/release at genesis (R-1/R-2/R-3).** Bond is consensus-tracked
...
`JoinMarket` / `Reinstate`-after-slash / full `Release` + a release cooldown
...
**sim bond-mobility model reconciled to the reinstate/release FSM's frictions.** (c) cannot be
computed until the FSM (the now-genesis-scoped lifecycle, promoted 2026-06-15) is pinned — so the
**reinstate/release FSM is a pre-genesis-seal dependency for the sim reconciliation, not merely a
...
> **Mechanism correction (2026-08-03, maintainer ruling).** The bullet below names
> **separate Arti client instances** as the mechanism. **Arti is not the path** —
> the Tor Project owns Tor and this project consumes the **Tor Expert Bundle as an
> external, hash-pinned process** — so the Arti anchor never fires and this clause's
> *named mechanism* is unavailable. **The intent below stands unchanged:**
> `P`↔principal isolation matters, and the guard layer is where the residual lives.
> **What is actually done is recorded in
> [`ARCHIVAL_BOND_2D2_TRANSPORT_PLAN.md`](ARCHIVAL_BOND_2D2_TRANSPORT_PLAN.md)
> §"Two notes recorded (correct as-is — do not 'fix')":** one Tor process, `P` and
> the principal **share one guard set**, accepted as the deliberate §7 residual —
> because severing it means separate Tor instances, and a non-default config is
> itself a fingerprint to a guard observer (a weaker adversary than the correlator
> the shared guard exposes). **Do not split instances.** Measured corroboration:
> [`SP_T3_SKELETON_MEASUREMENT.md`](SP_T3_SKELETON_MEASUREMENT.md) §2a — the
> coupling is exactly guard-level (intro points and HSDir sets stay disjoint).

- **Independent Tor clients / guard sets.** *(Mechanism superseded — see the
  correction above; retained for history.)* `P`'s serving HS, `P`'s outbound
  broadcast, and the principal wallet's traffic use **separate Arti client
  instances with non-overlapping guard
...
- [x] **Reinstate/release recurring-surface** — §6 R4 re-scope acknowledged: GF-4 (drain output-count)
      and GF-7 (bond-funding separation) carried as **recurring** (partial-release / reinstate-topup),
      not one-time; the **reinstate/release FSM (promoted to genesis / V3.0, 2026-06-15)** named as the
...
(2) the reinstate/release FSM promotion gates the R-3 age-stratified sim reconciliation, which gates
...
- **To R4 (§6):** GF-4/GF-7 recurring surfaces under the genesis reinstate/release FSM; the exit-seam
...
- **GF-10 numeric width** above `S_min` → R4/GF-4 CB-3 joint grade (jointly with amount + holdings +
  ...
  grade."**
- **Extension to bond ops** — GF-10 reused for partial-release / reinstate-topup claim timing in R4 (§6);
...
identity, not a committed bound; full reading at §12.8 and WI-4 §13.5). *(2026-07-23:
the cover-blindness finding is metric arithmetic — `P(link)` scales as `1/N`, so
`r = P(link)·N` cannot see cover — and it **stands**; only the instrument that
demonstrated it is now fail-closed alongside the withdrawn `r = 1.86`, WI-4 §13.1
banner.)* This is the
...
OUTCOME)*;
**F-D6** is the anti-drift pin. R4's other half — **GF-7 (funding-in)** — was built + graded
PROVISIONAL-PASS (`r = 1.86`; §6 R4 cell) *(⛔ WITHDRAWN in full 2026-07-23 — WI-4 §13.1
banner; GF4b-2's funding-input count is the surviving entry-seam channel)*; this section is
the **value-out (GF-4)**. The channels are
...
- **The wallet-flow default — NOT LANDED, and larger than a "default".** "Never
  ...
  *(incl. the §16.4 funding default — reconciliation amendment 2026-07-19: the
  §12.9 R4 close wording "F-D2 (incl. funding default)" is authoritative and this
  build note previously omitted it, so R4's scope read under-scoped from this
  section alone; `ARCHIVAL_DRAIN_SEND_FD2.md` DS-6 carries the disposition)*:
  ...
  2. **A drain tx-assembly path.** `plan_drain` returns a `DrainPlan` (amount,
     ...
     nowhere.
  3. **The wallet-flow defaults on top (incl. the §16.4 funding default).** The
     round-number/random-split drain default *and* the §12.9-decision-3 funding
     default (self-fund via GF-4b lineage; external growth-funding a loud
     override) — both gating for the R4 close per §12.9's re-formed close
     wording.
...
- **GF-7 (funding-in):** built; verdict **⛔ WITHDRAWN in full 2026-07-23** (was:
  **PROVISIONAL-PASS**, `r = 1.86`, local-daemon; §6 R4 cell) — the graded channel was never
  on the chain (WI-4 §13.1 banner). The entry seam's surviving channel is GF4b-2's
  funding-input count — structural, gated, not yet closed — and the transport residual is
  defended by circuit isolation with the principal-side default flip as its open front. Its
  former residuals (effective-vs-nominal cover, cold-start) were graded on the withdrawn
  instrument.
...
- **Pre-seal blocker — DISCHARGED 2026-07-15:** the reinstate/release FSM (genesis-scoped, 2026-06-15)
  ...
  PR #303 2026-07-14) and landed as enforced consensus (`HoldingsUpdate`/`Release` #303, `Reinstate`
...
and its five standing conditionals were untouched by this round *(and since WITHDRAWN in
full, 2026-07-23 — WI-4 §13.1 banner)*.
...
- **2026-07-16 (F-W9 — F-D4 §13.3's repetition premise corrected; method note 4
  ...
  `Reinstate`; the common standing-only `Reinstate` moves no value by Pin 2), **zero**
...
- **2026-06-15 (`HoldingsUpdate` promoted to genesis scope):** Resolved R-1's open
  V3.1→genesis call: the full bond lifecycle (`JoinMarket / Reinstate / HoldingsUpdate /
...
- **2026-06-13 (Round 2 adversarial pass 1):** Worked the §10 forks + the reinstate/release scope
  ...
  carries `JoinMarket` / `Reinstate`-after-slash / full `Release` + release cooldown
  ...
  **reinstate/release FSM is a pre-genesis-seal dependency** for the R-3 sim reconciliation (§6 scope
