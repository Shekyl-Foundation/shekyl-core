...
| W3 | **Consensus: presence + 9-byte size, not content.** | `CtSigBase` serialization + size checks; no validator branch on sentinel vs cooperative plaintext. | ☐ |
| W4 | **`enc_label` prehash binding (blocking).** On-wire label octets are XOR-only (malleable); unlike amounts (Pedersen self-check), label integrity is **solely** from inclusion in `serialize_ctsig_base` → second component of `get_tx_prehash` — same discipline as `enc_amounts`. | Read `ct_types.h` (`enc_labels` after `enc_amounts`); confirm `get_tx_prehash` hashes that blob. **Not** satisfied by size/presence checks alone. | ☐ |
| W5 | **Tamper test locks prehash binding.** One-byte flip of `enc_labels[0]` changes the CtSigBase hash (and must break full tx verification when wired end-to-end). | CI: `fcmp.enc_label_binds_rctsig_base_prehash` green on FA-11 branch. | ☐ |
