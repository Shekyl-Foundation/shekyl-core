...
| W5 | **Tamper test locks prehash binding.** One-byte flip of `enc_labels[0]` changes the CtSigBase hash (and must break full tx verification when wired end-to-end). | CI: `fcmp.enc_label_binds_ctsig_base_prehash` green on FA-11 branch. | ☐ |
