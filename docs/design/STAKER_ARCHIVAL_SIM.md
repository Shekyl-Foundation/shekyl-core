...
`ALL` uses `sprdW`. `deep = deep_frac_under<0.10`; `churn_stable = churn<0.05`.
...
4. **Sim pin (dimensionless `bond_rate*`): `0.75`.** Highest rate in the feasible triple
   ...
   whale) caps the *consensus floor* lower. Atomic mapping lands at
   **`750_000_000` atomic (0.75 SKL)** per `FOUNDATION_GENESIS_IDENTITY_SET.md` §9.3.
...
|---|---|---|---|
| L1 | **`g(age)` value** (gate 4 / near-term pin) | `g` doesn't just clear deep — it **relocates the crux**: the unavoidable finite-provisioning residual sits on the **oldest tail at `g=1`**, the **shoulder at `g≈2`**, and **inverts (oldest over-covered, hot starving) at `g≥4`**. The shoulder (youngest deep shards, just aged out of hot, still partly replicated, *least* irreplaceable) is strictly less harmful than the tail (oldest, most irreplaceable, no recency backstop). **L2×L1 entanglement (2026-06-07):** spread at `bond_rate*` passes `sprdW` **thinly** (~0.001–0.007 margin); `gate4_fine_0.75_whale` windowed Gini **rises** 0.581→0.594 under windowing. Raising `g` for deep coverage must be calibrated against **whale-window trend**, not benign mean alone — the active Layer-2 constraint. | **Decided (target restated), leaning `g≈2`.** Seat residual on shoulder, not tail. **Open calibration:** `g` pin must clear deep **and** keep `sprdW` under 0.6 on **whale** rows at `bond_rate*`. Reopen if thick-state sweep or whale-window check moves shoulder-seating `g`. |
| L2 | **Bond upper bound** (gate 4) | `bond_high` (rate 8) → min-form `dS/dN=0.83` → deep starves; binding condition is **co-located min-form** (L8). **Fine sweep (2026-06-07):** live upper bound at L11 lean is **whale spread** (Gini), not min-form — fails from `bond_rate ≥ 1.00`. Pin **`bond_rate* = 0.75`**. **Spread windowing (`sprdW`, 2026-06-07):** zero snap-pass→win-fail; graze dissolves on benign rows (`giniW` 0.593–0.599 vs threshold 0.600 — **hair margin**); whale row **worsens** under windowing (0.581→0.594). Bond 1.00 fails both; `l11_bud_b50` ~0.84 both. **Keystone holds** → calibration not shape reopen. **Coherence:** `reward.rs` denominator is `Σ capped` (form C verified); see `REWARD_EMISSION_LEG.md` §1.1. | **Decided (numeric pin):** `bond_rate* = 0.75`; **`ARCHIVAL_BOND_FLOOR = 750_000_000` atomic**. Reopen if spread threshold, whale model, or endowment calibration moves intersection. **Active:** thin-margin `g(age)`↔spread calibration (L1 entanglement). |
