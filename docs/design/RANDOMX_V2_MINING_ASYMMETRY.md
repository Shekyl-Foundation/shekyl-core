...
| Field | Value |
|-------|-------|
| Status | **Measurement design — not yet executed.** This document is the plan and the provenance protocol; no number in it is a result until Appendix B is filled from a source-verified run. Phase 0's **constant-level fork delta is discharged** (§3.1: XMRig 6.26.0 @ `b2ca7248` cloned and every `RX_V2` tuning constant diffed byte-for-byte against `external/randomx-v2` v2.0.1 — no delta). The remaining Phase 0 task is the runtime **byte-equality differential** (§3.2), which is what actually proves hashing-semantics identity. Revised after **red-team rounds 1–2** (§11 — F1–F7 structural: four-factor decomposition, aggregate-H/s basis, whale-produces-Shekyl-blocks gap, threshold arithmetic, two-ended range; R1–R4 polish: decomposition-is-an-aid-not-identity, hash-core-vs-block-framing layer split, §6.4-pinned prototype greenlight, confirmed-huge-page provenance; Intel-first / Ryzen-rig two-machine plan). |
...
### 3.2 The remaining Phase 0 task — hash-core byte-equality differential at the raw-blob layer (NOT yet run)
Constant identity is **necessary, not sufficient**. What licenses a **ceiling number** is a runtime proof at the **hash-core / raw-blob layer**: XMRig's `randomx_calculate_hash` output (or its self-test / benchmark hash-dump) for a given `(seedhash, blob)` is **byte-identical** to C-full and to Rust-light **fed the same raw bytes**. This is deliberately the layer *below* block framing — it proves the four tweak code paths agree, which is exactly what a ceiling number needs and **all** it needs.
- **Corpus (R2):** the existing raw-blob corpus (`corpus_random.rs`, 32×32 seedhash×blob) is the correct corpus at *this* layer — raw bytes into the hash function, which is what XMRig's calculate-hash path also consumes. Do **not** conflate with the block-layer equivalence, which needs the §6.3 shim and a Shekyl-*block* corpus — a separate, larger deliverable, not a Phase-0 gate on the ceiling number.
- **On divergence** — enumerate the deltas and classify: **hash core** (fatal — XMRig can't be a stock ceiling) vs surrounding **schedule/consensus wrapper** (which XMRig never computes — harmless).

### 3.3 Ceiling-artifact decision + correctness proof
Whatever Phase 0 concludes, the chosen ceiling miner's correctness is **established by the shared-corpus differential before it produces any H/s**: XMRig-produced hash ≡ C-full ≡ Rust-light on a shared `(seedhash, blob)` corpus. Build this by **reusing** the Hole-1 C-full path (`RANDOMX_V2_PHASE3_PLAN.md` §7; `tests/randomx_v2_parity/randomx_v2_miner_kat.cpp`) and **adding** the XMRig leg. Concretely: capture XMRig's hash output at the **raw-blob layer** — its self-test / benchmark hash-dump, or a direct `randomx_calculate_hash` harness — and assert equality against the pinned canonical outputs the differential already owns. Do **not** derive the ceiling proof by mining a crafted template; that exercises the block-layer framing, which is the separate §6.3 path.
...
**Gate:** do not proceed to Phase 1's ceiling leg or Phase 3's whale until §3.0–§3.3 are answered and the XMRig leg passes the three-way differential.
...
## Appendix B — results (empty until a source-verified run fills it)

*No number is entered here until it carries its full §2 provenance. This section stays empty by design until Phase 0's prerequisite (§3.0) is cleared.*
