...
| **Mode** | light / full; JIT on/off; dataset present?; **thread count (first-class — the ratio moves with it, §4.3)**; **memory-subsystem tuning: 1 GiB huge-pages — reservation *confirmed successful, not merely requested* (the miner logs the page size actually used; a silent 1 GiB→2 MiB fallback measures a lower tuning factor = an optimistic error, R4); MSR / prefetcher writes applied?** (a privileged fourth factor, §4.2); `RANDOMX_FLAG_V2` set? (program size 384 vs 256 — a wrong-algo run is a wrong-hash run). **CEILING PRECONDITION (not a knob): the XMRig ceiling leg MUST run full-dataset mode.** Its light and light-JIT paths are v2-**incomplete** (proven at Phase 0, §3.2 — they compute a *different, wrong* hash) and must never be the measured leg. This is a hard precondition of the same class as confirmed-`FLAG_V2` and confirmed-1 GiB-pages, not a configuration choice — a future run must not casually measure XMRig's light path and believe it is comparing like-for-like. |
...
- **The mode is load-bearing (a refinement of R2).** XMRig's **light / verification** path is v2-**incomplete**: its interpreter applies only `Tweak_V2_PREFETCH` (`vm_interpreted.cpp:82`); the other three tweaks (`AES`/`CFROUND`/`COMMITMENT`) exist only in its JIT, and even its light-JIT diverges from its own full-dataset result (`2fe105f9…` vs the canonical `34f8b017…`). So XMRig's own mode-invariance is broken **in the verification path miners never use**. The ceiling comparison therefore must use XMRig's **full-dataset** hash — which is exactly the mode an adversary mines with.
- **Why this is the expensive kind of wrong (what "necessary, not sufficient" was guarding against).** §3.1's constants all matched, and a naive tester would trust XMRig's *default verification* path — which is **light**. Testing there produces a **false divergence** on the ceiling artifact, and a false divergence is the costly failure: it either (a) sends you chasing a nonexistent bug in Shekyl's own fork, or (b) — far worse — gets "resolved" by someone **patching Shekyl to match XMRig's incomplete light path**, i.e. corrupting consensus to match a miner's verification shortcut. This is the concrete attack the layer split (§1.4) and the "necessary, not sufficient" stance were guarding against; the plan would have caught the trap by construction (the raw-blob differential is the licensing gate, not the constant diff). The finding also **retroactively validates** the §3.2/§6.3 hash-core-vs-block-framing split from red-team round 2.
...
### 6.5 Watch item carried from Phase 0 — the whale's self-verification may disagree with its mining
Phase 0 established that XMRig's **light** path is v2-incomplete (§3.2): it computes a *different* hash than XMRig's own full-dataset mining path. At the **hash layer** this creates **no** honest/attacker asymmetry — any XMRig-based attacker must mine full-dataset (which they want anyway for speed), so a light/full split does not hand the attacker anything. But there is a **network-stability dual** worth instrumenting rather than dismissing: does any real miner ship a configuration where the **light path is used to self-verify blocks it mined with the full dataset**? If so, that miner could (a) produce blocks it *believes* valid that the network *rejects*, or (b) accept blocks it should not — a source of **orphans independent of the difficulty dynamics** §6.2 already watches. This is **upstream XMRig's** behavior, not Shekyl's bug to fix; but a whale whose own verification disagrees with its mining is a distinct orphan generator, so Phase 3 instrumentation should **attribute orphans to their cause** (difficulty transient vs self-verification divergence) rather than lump them. Recorded as a Phase-3 watch item so it does not disappear into "we use full-dataset."
