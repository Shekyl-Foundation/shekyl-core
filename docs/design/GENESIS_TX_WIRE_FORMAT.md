...
| `0x05` | `txin_archival_bond_post` (:264) | **Spec + ratify + UNIFY** (gate-4 §3.4.1; JoinMarket-only) | Full layout in **§9.11** — **incl. `bond_spend_pk`** (GF-1 debit authorizer, JoinMarket-only, on wire; authorized via the surface-A `pqc_auths` slot, SA-2b). **The current C++/Rust impl omits `bond_spend_pk` — must be added.** |
