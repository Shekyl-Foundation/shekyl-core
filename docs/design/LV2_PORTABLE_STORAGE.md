...
| `peerlist_entry` | `p2p_protocol_defs.h` | `adr` is `network_address`; `last_seen` / `pruning_seed` are `OPT` (`rpc_port` / `rpc_credits_per_hash` deleted in PR #587 — dead RPC advertisement, census-1 U-5/L-6) |
