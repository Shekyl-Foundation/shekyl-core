...
| Type | File | Notes |
| --- | --- | --- |
| `basic_node_data` | `p2p_protocol_defs.h` | `network_id` is `VAL_POD_AS_BLOB`; `support_flags` is `OPT` (`rpc_port` / `rpc_credits_per_hash` deleted in PR #587 — dead RPC advertisement, census-1 U-5/L-6) |
| `peerlist_entry` | `p2p_protocol_defs.h` | `adr` is `network_address`; `last_seen` is `OPT` (`rpc_port` / `rpc_credits_per_hash` deleted in PR #587 — dead RPC advertisement, census-1 U-5/L-6; `pruning_seed` deleted 2026-09-21 — `PDM-Q7`, the stripe engine and its self-asserted wire attribute) |
