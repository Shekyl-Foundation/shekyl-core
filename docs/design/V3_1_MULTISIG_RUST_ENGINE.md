...
| **Address bech32m** | After **MSW-8**, canonical payload is `6 + N×1216` (KEM only; `HEADER_LEN + N×PER_PARTICIPANT_LEN`). N=2 ≈ QR-able. Does not set MAX. §15.3 registry **re-priced** — optimization, not prerequisite. |
...
it is now **bounded: no const generics, and `MultisigSigner` is
unparameterized.** The last unrecorded reason in the multisig surface is
recorded — at the name (`signer.rs`) and here.
...
`MultisigSigner<N, K>` is retired (`MultisigSigner`, unparameterized). Recorded
at the name (`signer.rs`) and in "MS-1 under coexistence" above. MS-1's remaining
...
| 2026-07-16 | **MS-1 `N`/`K` DECIDED:** the multisig kind takes **no const generics**. `N` (participants) / `M` (threshold) are runtime payload values (`n_total`/`m_required`); the coexistence axis (version byte) is type/dispatch, not a const generic (and not compile-time — the version is read from the wire). `MultisigSigner<N, K>` retired; `MultisigSigner` is unparameterized. Recorded at the name (`signer.rs`) + "MS-1 under coexistence". MS-1's remaining call is only the coexistence *shape* (per-version markers vs runtime version field), bounded by this. |
