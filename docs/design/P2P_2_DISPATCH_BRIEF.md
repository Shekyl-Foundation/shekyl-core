...
| id | Decision | Grounded in |
| --- | --- | --- |
| PWD-I1 | Session identity — **DELIVERED, and the verdict reversed on amendment (2026-09-02): no peer identifier on the wire at all.** `peer_id` leaves `basic_node_data`, `id` leaves `peerlist_entry`; five jobs replaced by a zone-scoped nonce, an address-based same-host cap (PWD-B9), a local session-established flag (`session_established()`, §3.5 *Two handshakes* of the protocol doc), and deletion of the back-ping (PWD-B10) | PW-19, PW-19a, PWC-D4 |
