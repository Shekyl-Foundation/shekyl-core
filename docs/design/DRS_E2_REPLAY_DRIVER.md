...
| 2 | `Stale::Seed` → bounded retry — **BUILT for replay, increment 2 (commit 5): surfaced as a run-ending fault on first occurrence, the validator's `Retry` carried on it; the wrong-seed test. The re-`form` loop that consumes that `Retry` has no driver in replay (RD-Q5's second sentence) and is E3's, with the live feed as its caller** | `fault.rs` (`FormAttempt`, `Retry`, `MAX_FORM_ATTEMPTS`); slice 2 Q8 (the ruling that minted the arm and its bound) | In replay any `Stale::Seed` is a **driver defect** — record and surface the first occurrence (RD-Q5, RULED); the bounded loop lands with the source that can honestly re-`form` (E3), the bound already typed on the fault. |
...
- **RD-F19 (commit 6b, 2026-09-20; corrected 2026-09-21) — there is no
  Fakechain seed-epoch override for the driver to take, and a regtest daemon
  run with one produces a chain the validator's schedule cannot replay.** The
  validator derives the seed height with the mainnet constants at every
  nettype and reads no environment (slice 2 F5, CEN-D3); the
  `SEEDHASH_EPOCH_*` fast-epoch lever lives only at the C++ FFI boundary. So
  the driver claims seeds on the validator's own `seed_height`
  ...
  honest run can use). **What such a chain does under replay depends on the
  target** (the 2026-09-21 review's correction of this finding's first
  form, which said "refused by design"): under a live target CEN-D1 refuses
  the first block mined under the fast schedule, because its longhash was
  computed under the wrong seed; under `--fixed-difficulty 1` every
  longhash satisfies the target, every block connects, D1–D3 grade
  derived-and-conformant and the digest — which does not cover PoW —
  MATCHes, so the schedule mismatch is **invisible in the chain data**.
  Nothing in a corpus or a trace can record the daemon's schedule; the
  exporter therefore refuses to run with `SEEDHASH_EPOCH_*` set in its own
  environment (first evidence, since the recipe runs daemon and exporter
  from one shell — not proof), and the recipe states the daemon's
  environment. Commit 8's regtest daemon was started without
...
- **RD-F21 (review, 2026-09-21) — the seed ledger's window premise fails
  under a rewind across a seed-epoch step.** The ledger forgot every height
  below the current seed height on the premise that seed heights never
  decrease in the connecting height; a `Rewind` moves the connecting height
  backwards, so after block 2113 was formed (seed height 2048, height 0
  forgotten) a rewind to 2111 asked for height 0 again and the run ended
  with `SeedUnknown` — the store fallback the docs promised did not exist.
  A depth-two reorg at every epoch rollover is routine on a live chain and
  E3 would have inherited the fault. Fixed by what the docs said: the ledger
  answers `Unread` for a height outside its window and the driver asks the
  store through the connector (`HashAt`), recording the answer; the store
  holds every connected block, and the barrier rule guarantees the pops
  have committed before the claim. The run-start warm-up that pre-read the
  store went with it. Pinned by
  `a_rewind_across_a_seed_epoch_step_claims_the_seed_from_the_store`
  (main 0..=2113, rewind to 2111, fork 2112'..=2114'; MATCH at the tip).
- **RD-F22 (review, 2026-09-21) — a refusal reached the exit code through
  exactly one path.** The grader iterated only the register's rows, so a
  refusal on a row the register does not carry (CEN-B2/B6/B7/D1b/D7 are
  implemented and bucket 4, outside the coverage gate's denominator by
  construction) was never graded; on an UNREVIEWED row it graded
  `RegressionSignalOnly` and was dropped; and without `--register` a refusal
  or a DIVERGE was an `eprintln!` followed by exit 0 — a run that stopped on
  Rust refusing a block the chain holds read as a pass. Fixed by grading the
  refusal **once, as its own typed field** (`GradedRun::refusal`), with an
  absent row UNREVIEWED by absence; a refusal is open unless the register
  records the row DIVERGENT (then owed); and the binary's exit is non-zero on
  any disagreement the run reports unless a register adjudicates it. Both
  are the lane's reading of §1.3 ("no unadjudicated disagreement"), recorded
  in §9 for the maintainer.
...
4b. **LANDED (increment 2)** `ingest: the trace — the six facts, cumulative difficulty, digest checkpoints; Rust-minted format; the LMDB exporter as a C++ harvest shim handing bytes across the FFI; the RPC corpus fetch feeding CorpusWriter` (RD-Q2). Trace layout §3.9. A checkpoint is the **outer** `digest_v0` (32 bytes) — the redb read returns no components, so a DIVERGE names the height, not the family; widening both sides is the diagnosis step (reopen on the first DIVERGE at a checkpoint on a real chain). The exporter writes **one checkpoint, at the tip** (RD-F18) and takes the daemon's `--regtest`; `--block-stop` below the tip writes no checkpoint (observed: 0 digested when heights 0–300 were exported from a 2301-block chain). **Review 2026-09-21:** the exporter now holds **one LMDB read snapshot** (`db_rtxn_guard`) across the tip, every facts row and the checkpoint — as the digest oracle it must agree with does — so a daemon still appending cannot move the tip between the facts and the checkpoint (a false DIVERGE); the checkpoint is `BlockchainLMDB::logical_state_digest_v0` itself, handed to `shekyl_e2_trace_push_checkpoint(writer, digest)` finished (the families-shaped FFI and its second copy of the family walk are deleted); a zero `curve_tree_roots` row is refused, not exported; `--block-stop` is read by presence (`0` is genesis alone; above the tip is refused, not clamped); the trace refuses a facts row past `u64::MAX` and bytes after the trailer on both sides; the long-term effective median is stated as the shim's re-derivation of the daemon's private function (equal in every daemon configuration — the window override is test-only), not as a daemon output.
5. **LANDED (increment 2)** `ingest: form workers → sequencer → validate+connect actor (kameo, no-restart); Corrupt → halt, terminal — halt-then-assert-not-restarted test; Stale::Seed surfaced; injected-wrong-seed test` (RD-Q5, RD-Q11). The pipeline assigns heights (the source's `Extend` carries none) and **asserts them against `Source::first_height`** before forming anything (a misaligned corpus is `PipelineFault::Misaligned`, not a verdict); the ledger claims on the validator's own `seed_height` (one function in `shekyl-chain-rules` for D3, the harness and the driver; RD-F19) and asks the store for a seed height outside its window (RD-F21); sequence numbers are asserted consecutive at the event (a hole is a loud `SequenceError::NotNext`, never a parked item); the writer's latch is structural (`connector::Writer` is the only write path); `PipelineConfig::window` bounds what is formed ahead of the writer — in flight plus waiting — and so the length of one `Apply` run, while `PipelineConfig::hashers` (default: the host's parallelism; `--hashers`) bounds RandomX concurrency, with every finished worker drained before the writer is asked; `RunReport::switches` carries the digest after each committed pop (§3.8). **Test order inside the commit: the no-restart test is written first, while the actor is three lines** — it is cheap then and awkward once supervision is configured and a channel is plumbed through; it is also the test most likely to be deferred as obvious, and the failure it guards (a halt laundered into a retry) is silent by construction.
...
6b. **LANDED (increment 2)** `ingest: --fixed-difficulty → RuleSet::fakechain on regtest; a schedule that names it` (RD-Q7): `schedule::ChainRules` — `Scheduled(Network)` through `RuleSchedule::for_network(..).rules_at(h)` → `RuleSet::for_id`, or `Regtest { fixed_difficulty }` → `fakechain(n)` / `GENESIS` (the daemon's own default: `--fixed-difficulty 0` is *not fixed*); the flag refused off regtest by construction; read per block by `form` and `validate`. *[2026-09-21, E6 slice 3 c7 (PR #814): `ChainRules::trust()` added beside `in_force` — `Scheduled(net)` → `Trust::full(ReleaseAnchors::for_network(net))`, `Regtest` → `Trust::UNANCHORED` — and passed as `validate`'s fourth argument at the connector's one call; `CHAIN_RULES_SLICE_3.md` Q1/Q2.]* The binary `shekyl-chain-replay` (`fetch --chain`, `replay --chain --fixed-difficulty`) carries it.
7. **LANDED (increment 2)** `ingest: CSR-3a grading — extractor, artifact, the two-clause evidence rule (verdict-evidence and component-evidence as typed row fields, RD-Q9), grader wiring` (RD-Q6, RD-F7): `scripts/ci/export_conformance_register.py` serializes the coverage gate's own parse (`--selftest` in `docs-gates.yml` parses the serialized text back and asserts the grader's shape; `--check-fixture` holds `rust/shekyl-chain-ingest/fixtures/conformance_register.json` — the committed copy `grader.rs`'s test parses with `Register::from_json` — equal to the live derivation, so a key rename on either side goes red in CI); `grader.rs` emits `shekyl_e2_grade_v1` with `VerdictEvidence` / `ComponentEvidence` per row, producers from `ConnectFacts::DELETED_BY`, and the refusal that ended the run graded once as `GradedRun::refusal` (RD-F22); `passes()` = no open adjudication on either clause or on the refusal; `replay --register --grade-out` exits non-zero on an open adjudication. The inversion is live in the tests.
7b. **LANDED (increment 2)** `ingest: metrics sink — light-mode RandomX wall-clock per hash / per block; the dataset-mode measurement artifact` (RD-F11, item 7): `metrics::Metrics` / `MetricsArtifact` (`shekyl_e2_metrics_v1`), derives timed apart from hashes, waits apart from derives (RD-F20), the measuring host recorded; `replay --metrics-out`. **The number** (item 8 below): light mode, Rust interpreter, x86_64 16 threads (not the provisioning floor) — **0.60 s CPU per hash**; 0.64 s wall per hash at 16-wide (0.34 s at 1-wide: light mode is memory-bound and the per-hash wall grows with concurrency while throughput still rises). **Read with its concurrency (review 2026-09-21):** that run had no hasher bound — `window` doubled as the number of blocking `form` workers, so 16 hashers shared a 16-thread host and the per-hash wall includes their contention; the artifact is now `shekyl_e2_metrics_v2` and records `concurrency { hashers, window }`, and `--hashers` defaults to the host's parallelism so the next number is the verifier's; a cache derive 0.39–0.46 s; 2301 blocks in 93 s wall. Against the C light-VM-JIT baseline of 10–15 ms/hash (`RANDOMX_V2_RUST.md` §9) that is ~40×; `BENCH_RESULTS.md`'s single-thread interpreter median (~300 ms) agrees. Recorded, not judged: the dataset-mode decision reads this artifact.
...
8d. **LANDED (increment 3, 2026-09-22)** `ingest: the mutation family — systematic invalidations of a valid corpus with spec-first expected verdicts` (§3.10 is the spec, written before the code). `mutation.rs`: nine `Mutation`s each naming its census row (`expected()`), `Mutated<S>` the Extend-only `Source` wrapper, `Environment`/`Pow` for the clock and the wrong-seed mining. Six rows `Implemented` (B1, A2, B5, C1, C2, D1) refuse exactly as named through the real pipeline; three `Pending` (F13, G2, I7) are **pinned at today's shape** — connects, connects, SI-1 halt — with the census's own `CenRow::status` choosing the test's branch, so an E6 port turns the pin red (the falsifier is the census, not a date). D1 runs under a seed-sensitive mock longhash at `fixed_difficulty = 2` with the chain mined against it. Trace tag `0x03` stays RESERVED; the regtest C++ leg is item 8's. *Was: a split inside this ruled plan (rule 22 §1), Extend-only, on the pipeline as increment 2 left it.*
