...
| **record** × n | | tag `0x01` **Extend**: `height` u64 (must equal first_height + i), `block_len` u32, block bytes, `tx_count` u32, then `tx_count` × (`tx_len` u32, body bytes). Tag `0x02` **Rewind** (RD-Q13, **built in commit 8c, 2026-09-20**): `to` u64, no payload. Writer and reader keep the lineage (every height's hash since `first_height`, truncated by a rewind), so the next `Extend` carries `to + 1` and chains onto the block at `to`; `to` must lie in `[first_height, tip)` (`RewindNotBackward`, `RewindOutOfCorpus`, `RewindOnEmpty`). `count` counts both kinds; `tip_hash` is the lineage's tip. Was RESERVED from commit 4a until this commit. |
...
8c. `ingest: Rewind — the reorg source family; pop + connect through the actor with the barrier rule; digest after each switch` (RD-Q13; §3.8) — **LANDED 2026-09-20**: the corpus `Rewind` record (tag `0x02`, §3.9), `CorpusReader` as the reorg family's `Source`, `RunReport::switches` carrying the digest after each committed pop (pop symmetry through the actor: the state at `to` equals the state when `to` was first the tip), the fixture generator `test_support::reorg`. The **mutation family** (§3.8) does **not** land in this PR: it is Extend-only and separable, and this PR is past the rule-06 ceiling — it is the next E2 PR, owner this plan (§7 item 8d below), so this file stays in `design/` until it lands.
8d. `ingest: the mutation family — systematic invalidations of a valid corpus with spec-first expected verdicts` (§3.8; header flips, wrong reward, reordered transactions, double spend, bad seed, future timestamp closing §5's FTL row). **Next E2 PR.** Falsify by `rg 'mutation' rust/shekyl-chain-ingest/src/` finding a `Source`.
...
| 2026-09-20 | **§7 commit 8c landed — the reorg family.** The corpus grows the `Rewind` record it had reserved; `CorpusReader` emits `IngestEvent::Rewind`, so the reorg family is corpus files, not a second `Source` type; the pipeline records a `Switch { to, popped, digest }` after each committed pop (§3.8's digest after each switch, and the actor-level pop-symmetry check); a fixture reorg (main 0..=4, rewind to 1, fork 2'..=5') replays through the real reader with the post-pop digest equal to the state at 1 and the fork's tip matching its trace checkpoint. Checkpoints under reorgs: compared the first time a height is the tip, so a fixture's checkpoint sits beyond every pre-switch tip and a C++ trace is tip-only by RD-F16 (corpus module docs). **Disclosed deferral:** the mutation family is 8d, the next E2 PR (rule 22: this PR is past the rule-06 ceiling; the family is Extend-only and separable; owner this plan, which stays in `design/`). |
