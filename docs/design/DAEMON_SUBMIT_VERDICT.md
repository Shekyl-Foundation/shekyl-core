...
| E11 | Fee-input FCMP++ over the `ToKey` subset; `n_fee == 0 ⇒ fcmp_proof` **empty**, `n_fee > 0 ⇒` non-empty | `:4046-4108` | **C** — the shared funding-subset K12 leg (bond-arm shape); the empty/non-empty coupling is an engine Phase-A static (wire `validate()` does not pin it) |
