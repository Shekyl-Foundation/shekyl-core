...
| 4 | Go-live gate (§3.2, structural per GF4b-5) | **Post-SP-R0 criterion (2026-07-18):** the `SpentRecordsDurablyPruned` witness has **exactly one** production constructor — `arm1_watch_pruning_live`, minted by SP-R0 arm #1 (key-image watch + prune-at-ingest, logic-discharged per the DQ-F split; production-firing gated on the staker-activation round) — and **C-1 must not add another**; the tripwire asserts exactly-one. *(Original criterion — zero production constructors, the sole mint reserved for SP-R0 — DISCHARGED 2026-07-18 by that mint.)* |
