...
|---|---|---|---|---|
| P-1 | Does the SDDL string produce the DACL we meant? | Build the descriptor, read it back with `ConvertSecurityDescriptorToStringSecurityDescriptorW`, compare to the input | Round-trips to an equivalent string: owner ACE present, **no** second allow-ACE beyond the logon SID, `S:(ML;;NW;;;ME)` present | **WP-D2** — if the OS does not preserve what we wrote, SDDL is not the reviewable-string win it was chosen for, and hand-assembled ACLs come back on the table |
