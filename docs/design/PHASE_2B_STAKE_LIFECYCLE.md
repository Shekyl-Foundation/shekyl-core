...
> This is the wallet mirror of the consensus nullifier-reorg requirement, which the
> consensus design **commits to** in [`CONFIDENTIAL_STAKING.md`](../CONFIDENTIAL_STAKING.md)
> §11 item 3 (nullifier-table per-block journal + atomic revert). The wallet therefore
> mirrors a committed spec, not an unbacked assumption. If the consensus pop path fails to
> revert nullifiers, a re-mined claim is permanently rejected; the wallet cannot fix that —
> but that is ordinary consensus-correctness reliance (the wallet relies on consensus
> implementing every confidential rule it mirrors), not a §5-specific gap, and the §11
> implementation rides the general consensus gate (§8.0). The wallet's obligation is only to
> keep its own `claimed_epochs` consistent with the post-reorg chain, which the §5.2
> full-rebuild does by construction.
...
| **T6** | Nullifier-reorg desync | Adversarial reorg shapes (straddling-lock, deep reorg, reorg-then-re-mine) against the §5.2 rebuild; validate the mirror against the **committed** CONFIDENTIAL_STAKING §11 item-3 nullifier-revert spec. The §11 implementation is verified at Stage-3 integration as part of the general consensus gate (§8.0), not a §5-specific carry. |
...
| **G7** | Long-range reorg of a confirmed claim | Covered by the §5.2 rebuild against the committed §11 item-3 revert spec — confirm no *additional* wallet gap; the §11 implementation rides the general consensus gate (§8.0), not a §5-specific carry. |
...
**Round-3 dependencies / cross-track asks:** (1) **T6** / **G7** validate the §5.2 mirror
against the **committed** CONFIDENTIAL_STAKING §11 item-3 nullifier-revert spec; the §11
*implementation* is part of the general consensus gate (§8.0), not a §5-specific carry;
(2) upstream Round 2 wire/KAT gates (§8.0) are
...
§5's reconciliation mirrors the committed [`CONFIDENTIAL_STAKING.md`](../CONFIDENTIAL_STAKING.md)
§11 item-3 nullifier-revert spec; it adds **no** §5-specific external dependency beyond
"the confidential mechanism must be implemented," which this section already governs.
Treating the nullifier-revert as a distinct standing carry over-weights one line item of
the same Stage-3/consensus-Round-2 implementation the whole document waits on — the entire
confidential stake/claim/unstake path (subtree, nullifier table, `h_bind`, rate-epoch
records, and their `pop_block` reversal) is unimplemented together, and is gated together.
...
- [x] **§5 consensus-reliance — design dependency satisfied; implementation rides the general consensus gate, not a §5 standout (clarified 2026-06-05):** §5's reorg correctness relies on the consensus `pop_block` path reverting the stake-claim nullifier set so a reorged-then-re-mined claim stays re-claimable. This behavior is **already committed upstream** — [`CONFIDENTIAL_STAKING.md`](../CONFIDENTIAL_STAKING.md) §11 item 3 (nullifier-table per-block journal + atomic revert). The design-level dependency is therefore **discharged**: the wallet mirrors a committed spec, not an unbacked assumption. The *implementation* of §11 is part of the **general** confidential-consensus implementation gate (§8.0; §12 code-site map) the entire Phase 2b waits on — the whole mechanism is unimplemented Stage-3/consensus-Round-2 code, and the nullifier-revert is one line item in it, not a §5-specific external carry. The wallet-cannot-fix-it property (a broken consensus revert permanently rejects a re-mined claim) is **ordinary consensus-correctness reliance**, identical in kind to the wallet's reliance on consensus implementing every other confidential rule it mirrors; it is verified end-to-end at Stage-3 integration via the §7.4 T6/G7 wargame, not gated as a distinct open item here.
