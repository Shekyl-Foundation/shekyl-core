...
- [ ] **Cross-track carry (consensus-track, not wallet design) — the sole remaining §5 dependency:** §5's reorg correctness **assumes the consensus `pop_block` path atomically reverts the stake-claim nullifier set** (cross-tree atomicity; §5.2 closing note). If it does not, a reorged-then-re-mined claim is permanently rejected and **the wallet cannot fix that** — the mirror is already maximally correct (it rebuilds `claimed_epochs` from whatever the post-reorg chain set is). This carry is **external**: it closes by confirming the `pop_block` nullifier-revert behavior against [`CONFIDENTIAL_STAKING.md`](../CONFIDENTIAL_STAKING.md) (consensus Round 2: cross-tree atomicity), **not** by any wallet change. Tracked as a Round-3-readiness dependency; reopen/close-trigger is the upstream `pop_block` spec confirmation.
