...
| §3.10.2 dust nominal | `coin_select.rs` carries a nominal `dust_threshold = 1_000_000` to migrate to `dust()` | **migrated** (2026-08-17): the nominal is gone; `SelectionCriteria::for_rate(target, rate)` composes `tx_fee::dust_threshold` with `shekyl_tx_weight::marginal_input_weight_at_d_ref()`, and there is no `Default` that could reconstruct a rate-free threshold | **Superseded** — the migration this row tracked has landed. |
