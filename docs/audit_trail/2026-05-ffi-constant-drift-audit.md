...
`fix/legacy-account-generate-network-guard`.**

| Field | Value |
| --- | --- |
| Symptom | The legacy `account_base::generate(recovery_key, recover, two_random)` wrapper hardcoded `cryptonote::FAKECHAIN` for raw-seed derivation regardless of caller network. Three production callers reached it: `wallet2::generate(name, password, recovery, recover, ...)` at `src/wallet/wallet2.cpp:5946`, `wallet2`'s 0-change dummy-destination address generator at `src/wallet/wallet2.cpp:8269`, and `wallet_rpc_server::on_stop_background_sync`'s seed-recovery path at `src/wallet/wallet_rpc_server.cpp:2339`. |
| Result on testnet | A `wallet2::generate(...)` instantiated on `TESTNET` derived keys under `FAKECHAIN` salt; on reload, `rederive_from_master_seed(TESTNET)` returned `false` because the master seed disagreed. Same shape on `wallet_rpc_server::stop_background_sync`: spend key recovered under `FAKECHAIN` salt couldn't match the wallet's stored `TESTNET`-salted key, so the RPC's post-recovery key check always failed. |
| Result on mainnet/stagenet | The same path on `MAINNET/STAGENET` is doubly broken: (a) `FAKECHAIN`-derived keys disagree with `MAINNET/STAGENET` rederive salt; (b) `MAINNET/STAGENET` don't permit `RAW32` at all. But the user-facing path on mainnet is BIP-39 only, which routes through the missing wrapper above (Bug 4), so the `wallet2::generate` wallet-creation slot is never reached on mainnet today. The `wallet_rpc_server::stop_background_sync` Electrum-words slot, however, *was* reachable on mainnet — and was failing key-comparison silently for the entire pre-fix window. |
| Form of fix (delivered) | Added `account_base::generate(recovery, recover, two_random, network_type nettype)` overload that threads the caller's network through `generate_from_raw_seed`. Migrated `wallet2::generate(...)` and `wallet_rpc_server::on_stop_background_sync` to pass `m_nettype` / `m_wallet->nettype()`. The 3-arg overload remains as a FAKECHAIN-only test convenience for ~25 unit / integration / performance tests, scheduled for deletion at V3.2 alongside the `wallet2.cpp` cutover (FOLLOWUPS V3.1+ entry: "Delete `account_base::generate` 3-arg overload alongside `wallet2.cpp` cutover"). The 0-change dummy-destination caller is intentionally still on the 3-arg overload — it's a transient one-shot whose address is used as a stand-in destination but whose secret keys are discarded; properly network-matching the dummy address requires a BIP-39 path on MAINNET / STAGENET (RAW32 isn't permitted there) and is filed under a separate FOLLOWUPS V3.2 item. |
| Failure-mode change | On MAINNET / STAGENET, the migrated callers now throw cleanly via the FFI's `permitted_seed_format` check instead of silently producing FAKECHAIN-salted unspendable wallets / unmatchable spend keys. On TESTNET, the migrated callers now produce TESTNET-salted accounts that round-trip through `wallet2::load`, which they couldn't before. |
| Mainnet test coverage check | `wallet_rpc_server::on_stop_background_sync` is exercised only via `tests/functional_tests/transfer.py::check_background_sync` and `check_background_sync_reorg_recovery`, which run on FAKECHAIN (`assert res.nettype == "fakechain"` per `daemon_info.py:80`). No mainnet integration test exists. The Electrum-words RPC seed-recovery path is itself defunct post-Monero-fork (Electrum-style 25-word recovery is gone per `account_base::generate`'s comment); on the post-fix MAINNET/STAGENET path the call now throws cleanly rather than silently mismatching. Filed as FOLLOWUPS V3.2: replace Electrum-words with a BIP-39 entry alongside the `shekyl-wallet-rpc` Rust cutover. |
| Regression test added | `tests/unit_tests/account.cpp::legacy_generate_4arg_overload_uses_nettype_argument` pins (a) the 4-arg overload produces the same TESTNET-salted account as `generate_from_raw_seed(..., TESTNET)`, (b) the 3-arg overload remains FAKECHAIN-salted (and therefore differs from the 4-arg TESTNET account), and (c) the 4-arg overload throws on `(MAINNET, RAW32)` and `(STAGENET, RAW32)` at the same `permitted_seed_format` check `generate_from_raw_seed` enforces. |
...
| `fix/legacy-account-generate-network-guard` (sibling) | Bug 4-adjacent fix: 4-arg `account_base::generate(.., nettype)` overload added; production callers migrated (`wallet2::generate(...)` and `wallet_rpc_server::on_stop_background_sync`); `tests/unit_tests/account.cpp::legacy_generate_4arg_overload_uses_nettype_argument` regression test added; 3-arg overload retained as FAKECHAIN-only test convenience with FOLLOWUPS V3.2 deletion target alongside `wallet2.cpp`; 0-change dummy generator and Electrum-words seed-recovery filed for V3.2 follow-on. Note: this branch does **not** add a `wallet2::generate_from_bip39` wrapper — that absence is by design (Bug 4 above) and defended by the `static_assert` tripwire on `fix/wallet-storage-test`. | Delivered |
