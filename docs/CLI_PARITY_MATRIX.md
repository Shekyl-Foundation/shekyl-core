...
| 9 | `unstake` | N/A | Planned | No native RPC surface yet — but **no longer "design pending"** (corrected 2026-08-26, PR-P4). The producer is built: `build_unbond_vin` for the witness and `AssembleUnbond` for the full persona-bound exit transaction, with the surface-A auth slot under `bond_spend_pk`. The remaining gate is **reachability**, not design and not construction — nothing on the exit path becomes callable from RPC or CLI until the regtest walk has exercised the retire path end to end (the wipe, the funded gate, the seal-then-act crash ordering), and that walk lands as its own PR. Reopen criterion lives in `api/wallet_rpc.yaml`'s RESERVED `unstake` entry |
