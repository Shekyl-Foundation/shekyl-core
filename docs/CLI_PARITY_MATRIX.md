...
| 9 | `unstake` | N/A | Planned | No native RPC surface yet — but **no longer "design pending"** (corrected 2026-08-26, PR-P4). The producer is built: `build_unbond_vin` for the witness and `AssembleUnbond` for the full persona-bound exit transaction, with the surface-A auth slot under `bond_spend_pk`. Remaining gates: **reachability** (no RPC method, no CLI verb), and **dispatch** of the assembled bytes. Native `/submit_transaction` admission was a third gate until 2026-08-29: the battery is no longer JoinMarket-only, and a dispatched Unbond would now be accepted (§8.7.1.1). Reopen criterion lives in `api/wallet_rpc.yaml`'s RESERVED `unstake` entry |
