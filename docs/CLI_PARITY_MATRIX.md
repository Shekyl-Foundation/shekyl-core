...
| 9 | `unstake` | N/A | Planned | No native RPC surface yet — but **no longer "design pending"** (corrected 2026-08-26, PR-P4). The producer is built: `build_unbond_vin` for the witness and `AssembleUnbond` for the full persona-bound exit transaction, with the surface-A auth slot under `bond_spend_pk`. Remaining gates: **reachability** (no RPC method, no CLI verb) and the **composed verb** (`unstake` = post + a decorrelated drain). Dispatch itself landed with PR-B (`Engine::submit_unbond`, `pub(crate)`; the daemon walk ran submit-accept + block-connect green), and native `/submit_transaction` admission was a gate until 2026-08-29 (§8.7.1.1) — neither lifted reachability. Reopen criterion lives in `docs/api/wallet_rpc.yaml`'s RESERVED `unstake` entry |
