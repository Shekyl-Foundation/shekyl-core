...
| 25 | `verify` | `verify` | Covered | Native `verify_message` (PR-SM-2), **session-less** — works with no wallet open (SM-R-6). Signature pastes are kept out of readline history; `@path` reads a file so a mail-wrapped 21.7 KB line does not have to survive the REPL tokenizer. Every current address answers -29803 ("this address format cannot verify message signatures") until the fork-(ii) v2 address layout lands in code |
