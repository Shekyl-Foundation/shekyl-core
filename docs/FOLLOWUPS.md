...
- **`SO-D9` — `ERR_EPOCH_MISMATCH` on the serve-credit admission path is a tautology; RULED (i) 2026-09-13, implementation owed.** `serve_credit.rs:168` refuses when `response.settlement_epoch != ctx.settlement_epoch`, but `ctx.settlement_epoch` is set at `blockchain.cpp:5304` from `sc_settlement_epoch`, parsed out of the same record at `:5124`; `shekyl_archival_verify_serve_credit_vin` has one caller (`:5315`). The check compares the record to a copy of itself — rule 50 "check that cannot fail", second instance in the SO arc. **Ruled (i):** populate `ctx.settlement_epoch` from `shekyl_archival_settlement_epoch_at_height(current_height)` (`shekyl_ffi.h:2594`), an explicit single-site rule that the record's epoch is the block's — not the *when*-bound `h_close` leaves implicit. Consensus tightening; one C++ line plus a test. Ordering to preserve: **two** C++ gates run before the FFI — `current_height > h_close(E)` (`:5188`) refuses a stale header, and `challenge_seal_on_chain(h_open(E), current_height)` (`:5201`, needs `E·N + 1 < current_height`) refuses a **future** one — so the only header reaching `EPOCH_MISMATCH` with `E ≠ epoch(current_height)` is the boundary case `current_height = h_close(E)` (a record for `E` in the first block of `E+1`), which the tautology admits today and (i) refuses on the pre-cutover path; under R-B the operand is the validated issuing block `h`, and that record is admitted again when `epoch(h) = E`. Falsify by `rg -n 'ctx.settlement_epoch = sc_settlement_epoch' src/cryptonote_core/blockchain.cpp` returning nothing AND an isolated verifier test (`shekyl-ffi/src/archival_ffi/tests.rs` calls `shekyl_archival_verify_serve_credit_vin` directly) that a header whose `settlement_epoch ≠ ctx.settlement_epoch` derived from height is refused with `SHEKYL_ARCHIVAL_VERIFY_ERR_EPOCH_MISMATCH` — a future-epoch header cannot be the full-path vector, the seal gate refuses it first. Owner: [`ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md`](design/ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md) `SO-D9`; [`ARCHIVAL_SETTLEMENT_WRITER.md`](design/ARCHIVAL_SETTLEMENT_WRITER.md) §13.
  - Target: pre-genesis
...
- **Per-tx identity for both `PDM-Q6` occupants, before E2's first production writer (DRS §7.7)** — three pieces, each in its owner's crate: `TxIdentity.pqc_auth_hash: PqcAuthHash` beside `prunable_hash` (rules crate, from the one `validate`); `Transaction::hash_with_supplied_pqc_auth` sharing `hash_with_prunable`'s body (wire crate — without it a node discarding `pqc_auths` cannot compute a txid even holding the digest); the `txs_pqc_auth_hash` row written by `connect` (store, S-CHAIN-W amendment A3 on S-CHAIN-R's layout commit). Deadline is E2's first production writer: after it the cost is every replay-derived identity row, KAT and fixture. Carrier: the two upstream halves as small PRs now; the row with S-CHAIN-R commit 2; `PDM-Q6` items 1–2 ruled by the pruning lane before E2's pre-flight closes. Falsify by `rg pqc_auth_hash rust/` returning the field, the constructor and the row.
  - Target: pre-genesis
...
- **Daemon shard-fetch client (`SF-` round): sub-PR 1 BUILT 2026-09-13 as one PR stacked on #734; §9.1 (c) DONE 2026-09-16 (`N = 8`) — what remains** — `shekyl-p-fetch` (dial, `shekyl-pass-request` header, envelope, SOCKS5h reuse, `MAX_INFLIGHT = 8` pinned, `SF-D6` taxonomy, countersignature verify via the shared `verify_pass_transcript`, `Vec<u8>` body, `ContentVerify` hole), the serve-side `SF-D8` countersignature and pre-sign gate, the `PassSigner`/`PassKey` seam, the `serving_route` home, the `RF-R1` update, and `check_p_fetch_dep_cut.py` all landed in that PR plus #746 ([ARCHIVAL_SHARD_FETCH.md](design/ARCHIVAL_SHARD_FETCH.md) §9.1). Open, each with its carrier:
  - Target: pre-genesis
  - **Sub-PR 2 (blocked on `PDM-Q6`, PR #723):** the frame codec and the content-verify — today `ServedFrameHeader` (`RF-D4`) + `recompute_segment_r_k` over a leaf shard, which `PDM-Q6` is expected to retire for a tx-range body verified per-tx against `txs_prunable_hash` — plugged into `ContentVerify`; the scheduler that constructs `PFetchClient` (challenge and organic callers, `SF-D1`, `SF-D10`); `max_body_bytes()` re-derived from the ruled unit. Falsify the block by `PDM-Q6`'s row in `IMPLEMENTATION_INDEX.md` reading RULED.
...
- **`PDM-Q6` items 1–2 ruled before DRS-E2's first production writer; `TxIdentity` and the store carry `pqc_auth_hash`** (`PDM-Q-F26`, 2026-09-16, [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) §6 / §8). Two owed items with one clock. **(1) DRS-E lane:** `TxIdentity { hash, prunable_hash }` (`rust/shekyl-chain-rules/src/block.rs`) carries one of Q6's two occupants; add `pqc_auth_hash: Option<PqcAuthHash>` — the spend txid's third component, `H(varint(count) ‖ auths)` as `Transaction::hash()` computes it and as the C++ oracle serialises it (`cryptonote_format_utils.cpp:1319-1325`), **not** `keccak256` of the raw `txs_pqc_auths` segment; `None` ⇔ 3-part txid (coinbase, and a spend with empty `pqc_auths`), never a sentinel, since `TxIdentity` is also `miner_tx`'s — with a KAT against the txid, and a store row beside `txs_prunable_hash`, present ⇔ 4-part and never deleted — hash row without segment is *discarded*, the steady state below `W`, not a fault (SCW-11's `RUST_ONLY_TABLES` map admits the table). Contract on the row before the implementation that omits it (SCW-7's standard). Not optional: a Q6 ruling that keeps `pqc_auths` universal retires the row, not the component. **Absorbs and corrects** the 2026-08 row *"A pruned node keeps whole `pqc_auths` because they have no hash table"*: its "unmeasured" hold is discharged by F14 (~60 % of spend bytes), and its claim that the hash is over the raw segment bytes was wrong at source. **(2) Steering:** rule Q6 items 1–3 (a transcription of F13/F14/F17 with rule-21 shape against §3's six adversarial items) and name a consensus-side owner for `PDM-Q11` (`D_max`) — the shape is written, the person is not. **SO- contact:** the serve-credit tx's prunable region is F13's good; the SO-D8 writer re-homed onto chain-rules inherits the same E2 ordering (PDM §5 first bullet). Falsify (1) structurally, not by token (the doc comment already carries the name): `rg '^\s*pub pqc_auth_hash:' rust/shekyl-chain-rules/src/block.rs` matches a field declaration, `rg 'TableDefinition.*"txs_pqc_auth_hash"' rust/shekyl-chain-store/src/schema.rs` matches a table constant, and `cargo test -p shekyl-chain-rules pqc_auth_hash` runs at least one test green. **Expiry, not just pending:** if a DRS-E2 replay PR opens while the `PDM-Q6` index row reads OPEN, this deferral is void — that PR states which Q6 items it rules by construction, or it does not merge.
  - Target: pre-genesis

- **E6 shaping under `PDM-Q5` / `PDM-Q3`: a below-anchor `RuleSet`, and `ChainView` without a recorded-body accessor** (`PDM-Q-F27`, `F29`, 2026-09-16; [CHAIN_RULES_CRATE.md](design/CHAIN_RULES_CRATE.md) §13, DRS-E6 row). **(1)** `PDM-Q5` band 1 — a fresh node below the release anchor `C`, skeleton only — cannot pass a proof-checking `validate`, so under DRS-D12 it has **no writer** unless `shekyl-chain-rules` issues a `RuleSet` whose `enforced` omits the proof rows; the seam exists (`RuleSet { id, enforced }`, `ISSUED`), the set does not, its selector is anchor-relative (not `RuleSchedule`), and the anchor check's home (census row vs node policy) is Q5's to rule. Owed when `ISSUED` is next touched, as a decision either way. **(2)** `PDM-Q3`'s instrument is the trait surface: no `ChainView` method returns recorded transaction bytes without a `CenRow` and an above-`W` marking — true by construction at `645d09dc3`, held as a standing property. Falsify (1) by `rg 'const ISSUED' rust/shekyl-chain-rules/src/rule_set.rs` still listing only `GENESIS` when the read-side slice consuming `RuleSetId` from the store lands; falsify (2) by a `ChainView` method whose return type carries a `Transaction` or tx byte slice with no row named in its doc comment. Named blocker for (1): `PDM-Q5`'s anchor ruling (which rows a skeleton is held to) — the *seam* is owed now, the *set's contents* after Q5.
  - Target: pre-genesis

- **Skeleton block payload grows `pqc_auth_hash`** (`PDM-Q-F28`, 2026-09-16; owner **`LV-` / `PWC-`**, not DRS-E). `tx_blob_entry { blob, prunable_hash }` (`cryptonote_protocol_defs.h:49-58`) and its KAT-pinned mirror `shekyl-levin::payload::block::TxBlobEntry` carry the one txid component a pruned receiver cannot compute; under `PDM-Q6` item 2 the skeleton also drops the `pqc_auths` slice, so a band-1 receiver needs **two** to rebuild the 4-part txid and verify the block's tx list. Entry grows an `Option`-shaped field on the identity's predicate; `shekyl-wire` gains the two-supplied form beside `hash_with_supplied_prunable`; a `PWC-` census row records the growth; the levin KATs move. **Blocked on `PDM-Q6` item 2** — the field exists only if the slice is discardable; falsify the block by the Q6 row in `IMPLEMENTATION_INDEX.md` reading RULED on item 2. Expiry: Q6 item 2 rules discardable while `TxBlobEntry` still has one hash field and no `PWC-` row names the growth.
  - Target: pre-genesis
