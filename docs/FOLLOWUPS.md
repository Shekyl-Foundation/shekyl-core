...
- **Gate PWD-T7's compression invariant mechanically: assert that only notify routes reach `try_compress_message`, and that no message carrying confidential material of any lifetime — session key, node-local secret, reused token — is routed through one.** **Blocker cleared 2026-09-03 by PWD-B3's route classification** (this was "nothing in the tree classifies a command as confidentiality-bearing"): every p2p command is an *invoke* (`net_node.inl:1078`, `:1165`, `:2581`, `:2623`) and all three compressor call sites finalize as notifications, so the reachable set is the cryptonote notify family — public consensus data. Executable now: the check is on the **routes**, not on the command table, since a table-shaped gate would pass while an invoke-carried secret was re-routed through a notify. The invariant is stated at the dispatch site (`rust/shekyl-levin/src/compress.rs`) in the meantime, which is placement, not enforcement — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-T7, PWD-B3
  - Target: pre-genesis
- **Size the Levin bucket header's length field, once `NOTIFY_RESPONSE_GET_OBJECTS`'s byte budget has a value.** **Blocker: three inputs are still open** — *(1)* the response byte budget itself (owed: a bandwidth/latency trade for initial sync, needs sync measurements); *(2)* `margin` in `entry_max`, owed to the **consensus lane — C2-R2 owns the derivation** (CEN-G6/G6b), **scoped to the tip-adjacent announce path only** since 2004 is byte-budgeted and `entry_max` has one consumer (2008). **C2-R2 has a proposed value — `margin(k) = min(2^k, 50·LTEM/EM)` for announce-lag `k`, both operands receiver-local — recorded in `CONSENSUS_C2_R2_WEIGHT_FEES.md` §6 on branch `docs/c2-r2-weight-fee-constants`, NOT yet on `dev` and pending R2 signature**, so it is cited here as provenance and not as a resolvable reference. **Carry the function, not the ×4 constant**: the announce path is gated on `is_synchronized()` (`src/cryptonote_protocol/cryptonote_protocol_handler.inl:538`, definition `cryptonote_protocol_handler.h:111`), which is a **latched boolean, not a height comparison**, so nothing enforces a numeric `k` — the `min`'s absolute arm is what keeps the bound safe when lag exceeds the assumed value; *(3)* the **relay batch bound for `NOTIFY_NEW_TRANSACTIONS`** — **the mechanism is now ruled by PWD-B12** (a byte cap on what a flush releases *and* a byte cap on what a zone holds, with admission refused at the second), **but its two numeric values are owed**, so 2002 still has no computable cap and 2004 is not yet proven to be the largest value the header must express. *This item was briefly marked cleared on 2026-09-03 and re-opened: PWD-B3 fixed the cap's shape, not its value; PWD-B12 fixed 2002's mechanism, not its value.* PWC-A2 — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-T6, PWD-B3, PWD-B12
  - Target: pre-genesis

- **Re-derive `drop_connections(address)`'s host-keyed severing together with the anonymity-zone inbound bound.** It drops **every** connection sharing a host and adds a host-fail score of 5 (`src/cryptonote_protocol/cryptonote_protocol_handler.inl:2874-2912`). *Blocker corrected 2026-09-04 (review finding, verified at source): the earlier "`is_same_host` needs an IP" claim was wrong — the method dispatches to the concrete type (`contrib/epee/src/net_utils_base.cpp:90-98`) and Tor/I2P compare hostname strings (`src/net/tor_address.cpp:181-184`, `src/net/i2p_address.cpp:165-167`).* **One of the two named blockers is now closed** (`fix/anon-zone-address-keying`): the sentinel still exists, but `is_same_host` refuses to equate two addresses that name no host and `drop_connections` returns early on one, so the sweep no longer severs a zone. *Recorded as it stood:* every inbound anonymity-zone peer is the zone's `unknown()` sentinel (`src/p2p/net_node.cpp:336`, `:340`), and host-keying compared those equal. **The remaining blocker still defers this item and is unaffected by that fix:** a learnable onion identity is free to mint, so a per-host key prices nothing — a sweep that *worked* on an anonymity zone would still be the wrong instrument. Same identity problem as PWC-E11's inbound cap (absent via `has_too_many_connections`' explicit non-public early return, `src/p2p/net_node.inl:3091-3092`) — **deferred by PWD-B7 to the PWD-I4 sub-round** so the two re-derive once, count- or work-based. PWC-E9 — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B7, PWD-I4
  - Target: pre-genesis

- **Carry the peer's endpoint in the handshake, and delete the `unknown()` sentinel with it.** **Scoped to anonymity zones — clearnet's split is deliberate and stays.** `basic_node_data` announces a **port and no address**, and on clearnet that is right: the address is *observed* from the connection, and the port is the only field observation cannot supply, because an outbound dial's source port is an ephemeral NAT mapping bound to that one flow and unreachable to anyone else. Announcing `IP:port` there would replace an observed fact with a claim. **On an overlay nothing is observed at all**, so the endpoint itself must be announced — and unlike a clearnet port it is directly checkable, since dialling the `.onion` *is* the verification. That is the field the zone-wide sentinel stands in for; note `zone.m_can_pingback` is false off the public zone (`src/p2p/net_node.inl:444`, `:818`), so today an anonymity zone verifies nothing. **This deletes work, it does not add it:** the sentinel goes, and with it `identifies_a_host()` in `contrib/epee/src/net_utils_base.cpp`, the `drop_connections` early return, and their comments. Verification needs no new mechanism: PWD-B10 already ruled that **the outbound dial is the verification**, so a false announcement produces a failed dial and ages out. Admission policy (no endpoint ⇒ not a peer) is a separate ruling with a `--hide-my-port` consequence, with Rick. — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-I1, PWD-B10
  - Target: pre-genesis

- **Sweep the tree for comparison tests built only from separately-constructed operands.** A test that constructs both sides independently **never creates the aliasing condition**, so it cannot exercise a short-circuit that fires on *identity* — not weak, structurally incapable: the defect lives on an axis the test never enters. **Measured, not argued:** with the anon-zone guard misplaced, the separately-constructed limbs (`tests/unit_tests/net.cpp:263-264`) **pass** while the alias limbs (`:276-277`) **fail**; with it absent, all eight fail. Any equality, ordering or identity check with a pointer short-circuit has the same blindness available. Mechanical: find comparison tests, check whether any construct both sides from one source, add an alias limb where not. Found by `fix/anon-zone-address-keying` — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B7
  - Target: pre-genesis

- **Execute PWD-B8's deletion of `m_bad_peer_checker`.** `once_a_time_seconds<43>` has exactly one occurrence tree-wide — its declaration at `src/cryptonote_protocol/cryptonote_protocol_handler.h:189` — and `on_idle` never calls it, so a cadence constant no code reads currently reads as one to anyone auditing the file. **The two lineage-dead structs PWD-B8 also rules are tracked by the `network_address_old` / `connection_entry_base` item above**, which already carries the `object_sizes.cpp` warning — not restated here, so the queue has one owner per subject. PWC-E4a — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B8
  - Target: pre-genesis

- **Implement PWD-B7's affirmative drop verdict — internal failures must not sever peers.** Verified at source (review round of #616): two our-own-state paths return false from `add_tx` with `m_no_drop_offense` unset — `insert_key_images`' pool-bookkeeping invariant (`src/cryptonote_core/tx_pool.cpp:785-787`, message literally "internal error"; from `:337`/`:496`) and the catch-alls (`:344`, `:506-509`) — and fall through `!tvc.m_no_drop_offense` (`src/cryptonote_protocol/cryptonote_protocol_handler.inl:909-913`) to `drop_connection`: **our own storage exception disconnects an innocent peer**, which PWD-B7 forbids. The fix is a **typed tri-state verdict** (attributable form failure = drop / policy-or-state rejection = no drop / internal failure = no drop, loud log) so absence of a classification defaults to no-drop — not another boolean whose unset arm means "sever". The announce path's `check_incoming_block_size` arm (`cryptonote_protocol_handler.inl:545-549`) is the same class (compares against OUR limit; narrow window in practice, wrong axis regardless) and rides the same change; the parse arm stays a drop (genuine form). Belongs to the P2P-3 implementation of the drop rule, Rust-side per rule 20 as the handler surface crosses. PWC-E7, PWC-E8 (the no-drop findings this implements; **not** PWC-E2, which is the rate-limit absence owned by the PWD-B1 item below, and the announce-size arm has no census row of its own) — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B7
  - Target: pre-genesis
