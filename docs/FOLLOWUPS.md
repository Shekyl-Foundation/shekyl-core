...
- **`ERR_EPOCH_MISMATCH` on the serve-credit admission path is a tautology.** `serve_credit.rs:168` refuses when `response.settlement_epoch != ctx.settlement_epoch`, but `ctx.settlement_epoch` is set at `blockchain.cpp:5304` from `sc_settlement_epoch`, parsed out of the same record at `:5124`; `shekyl_archival_verify_serve_credit_vin` has one caller (`:5315`). The record is compared to a copy of itself and the check refuses nothing — rule 50 "check that cannot fail", second instance in the SO arc. Disposition is a ruling because option (i) is a consensus tightening: (i) populate `ctx.settlement_epoch` from `shekyl_archival_settlement_epoch_at_height(current_height)` so the record's epoch must be the including block's, or (ii) delete it and let the C++ `h_close` gate (`:5186`) be the single expression. Independent of how `SO-D8` resolves; under R-A (i) is the boundary rule. Falsify (i) by a test that a record carrying `E` in a block of `E+1` is refused with `ERR_EPOCH_MISMATCH` *before* `h_close` is consulted; falsify (ii) by `rg ERR_EPOCH_MISMATCH rust src` returning nothing. Owner: [`ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md`](design/ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md) F2.
  - Target: pre-genesis
