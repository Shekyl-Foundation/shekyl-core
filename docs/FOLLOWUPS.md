...
- **CEN-I4's input cap (8) is inherited and unjustified — derive it from a verifier budget at the device floor, or delete it as redundant with `MAX_TX_SIZE`; price the consolidation-sequence cost in the same round.** The number arrived from RingCT's era with *"bounds proof generation time and tx size"* as its only rationale. Measured 2026-09-24 (`shekyl-wire/tests/input_cap_cost.rs`) on an i9-11950H **and on the Pi 4 floor (skl-pi)**: 6.4 KB per input on the wire; 11.9 ms (i9) / **64.9 ms (floor)** of verifier time per input, linear on both; `MAX_TX_SIZE` alone admits 153 inputs, so the cap binds by 19×; per-input linearity means the cap moves no per-block bound (the weight limit does) and splitting a consolidation across transactions costs the verifier *more*; what it bounds is per-transaction work on a transaction that proves invalid at relay — **at the floor, ~0.62 s at 8 and ~10 s at 153** — a relay-policy quantity. The privacy side — a wallet with twenty outputs forced into three transactions in a window — is unpriced. **What the floor decided and what it could not:** it changed the absolute milliseconds (5.5×) and not one ratio — linearity, positive per-transaction overhead, splitting-costs-more, and the 19× are structural and held there; the floor set only the *value* of the relay verifier budget, which is now in hand. Round: state the relay verifier budget against the floor's 0.62 s / 10 s; derive the cap or move it to policy and delete the consensus rule. **The deletion is a widening** — the chain would accept nine-input transactions it now refuses — which is free pre-genesis and a hard fork after; the window for making it free closes at genesis. **Two constants part company at that moment and both ends are named here so neither orphans:** the consensus cap (`shekyl_wire::MAX_FCMP_INPUTS`, read by `rules/tx.rs::I4`; the C++ `FCMP_MAX_INPUTS_PER_TX` until E4 retires it) and the prover/verifier cap (`shekyl_fcmp::MAX_INPUTS`, refused at `proof.rs`'s prove and verify entry points) — the second must move to whatever bound the round states (the relay budget's, or `MAX_TX_SIZE`'s implied 153), or a prover that refuses at 8 becomes the cap by accident. Falsify by the census row's rationale cell citing a stated budget and a floor measurement, or the rule's deletion with both constants moved. Full analysis: [`design/CHAIN_RULES_SLICE_6.md`](design/CHAIN_RULES_SLICE_6.md) §5.4.
...
- **Captured spends for the verification era: five transactions, generated once through the Engine and committed as test data, before slice 6 lands the proof bodies.** Every spend fixture in `shekyl-chain-rules`, `shekyl-chain-store` and `shekyl-chain-ingest` carries filler proof material by declaration (`fixture::bp_plus_layout_for`, `fcmp_proof: [0xF0]`, `pqc_auth_filler`); slice 6's BP+ / FCMP++ / PQC verification will refuse all of them at once, and the fix is real proofs, which are expensive to make — so capture, not per-test proving (the `TX_EXTRA_PQC_ROUND_TRIP.json` shape). The surface, scoped at `6e7cc9db4` from the fixtures' actual use ([`CHAIN_RULES_SLICE_5.md`](design/CHAIN_RULES_SLICE_5.md) §5.1): 1-in/1-out, 1-in/2-out, one bond-post and one emission with one fee spend each, and one depth-3 variant — each a `.tx` blob with its block context beside it. The generator is `e2e_fcmp_spend_accepted_by_daemon` (`regtest_e2e.rs:865`); the depth-3 mine exists (`:1064`). A captured spend has one key image, so the store's `spend(key_image: u8, …)` constructor does not survive capture and is redesigned with it. Falsify by: `rust/shekyl-chain-rules/tests/vectors/*.tx` present and `harness::fixture::listed` loading one, before any 4.I row flips to `implemented`.
  - Target: pre-genesis
  - Owner: [`CHAIN_RULES_CRATE.md`](design/CHAIN_RULES_CRATE.md) — the harness contract (§8); consumer: slice 6's **verification commits**, which do not land until the vectors exist (the "pre-flight does not open" wording was read as gating those commits, not the document — slice 6 Q1, RULED confirming the reading 2026-09-24). **Amended by slice 6 §5.2:** the capture is **blocks**, not transactions, and its consumer of record is `shekyl-chain-ingest` (replay against a real store), not `MockChain`.
...
- **Rule connection admission before channel established: the *rate*, and the anonymity-zone gap.** PWD-T5 routed adaptive resource exhaustion to PWD-B1/PWD-B9; PWD-B1 covers only connections with an **established channel** (its bucket sits on the four invoke entry points), and **PWD-B9 is outbound slot diversity**, so neither reaches an inbound flooder that forces one ML-KEM decapsulation per connection. **UPDATED 2026-09-22: `has_too_many_connections` is DELETED**, so the per-host bound this row named no longer exists and the anonymity-zone asymmetry it flagged is now the uniform state — every zone's inbound bound is the total ceiling, which is itself awaiting its measurement (row above). A concurrency cap still does not bound churn in any zone, which is what this row actually owes. **Unruled, not blocked**: no decision waits on another. Needs its own row rather than a widened PWD-B9, since B9 means outbound in the brief and the index. PWC-E11, PWC-E2 — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B1, PWD-T5
  - Owner: [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md)
  - Target: pre-genesis
...
- **S-PRUNE needs a rule-26 plan doc before its first increment** (`PDM-Q-F31`, 2026-09-17; owner **DRS-E lane**). The store-side implementation items in [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) §8 — the shard-granular discard predicate `current_epoch ≥ close_epoch(k) + 2` (Q2 re-ruled 2026-09-22 — bodies retire at the epoch boundary after the freeze epoch, `W` retired; F32 — no exceptions on any daemon, Q9), the pop floor at the highest discarded shard's `close_height`, the undo-log floor (SCW-7), the `⌊tx_id / T⌋` partition (Q6 item 5, 2026-09-23 — no length rows), the `pqc_auths` discard consuming `txs_pqc_auth_hash` (F26), the `txs_pqc_auths` accumulator re-grade to `Excluded` — land on or adjacent to S-PRUNE (**not** F27's below-anchor mode, which is E6's, nor F28's wire field, which is `LV-`/`PWC-`'s), which is deliberately not extracted ([DAEMON_REDB_STORE.md](design/DAEMON_REDB_STORE.md) §7) and already carries two contracts on its row with no home. E1 got `DRS_E1_SCHAIN_W.md` before its writers; S-PRUNE's successor gets `DRS_E*_SPRUNE.md` before its first increment, and it is where the store invariant (three legs landed, A4's fourth owed), the predicate's enforcement point and §3's reorg discard-side half stop being scattered. Writable now against the charter with the epoch horizon, `SEB`, `D_max` and `T` as named inputs (skeleton §12, re-keyed 2026-09-23); compatible with `PDM-Q-S0` (design, not implementation). **Skeleton landed 2026-09-18** ([DRS_E1_SPRUNE.md](design/DRS_E1_SPRUNE.md), thirteen sections each naming the contract it consolidates, `Status: SKELETON, not a plan`); the plan — DRS-E's increment ordinal, Round-0 pre-flight (carrying Q1's journal-horizon check), commit sequence — is still owed; **its `PDM-Q1` gate cleared 2026-09-18** (Q1 RULED), so it may open now. Falsify by that file's banner still reading `SKELETON` when the first S-PRUNE increment PR opens.
  - Target: pre-genesis
...
- **Journal horizon asserted at the journals' retirement site** (`PDM-Q1` RULED 2026-09-18, [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) Q1; F19's "what remains owed is the check"; owner **DRS-E / S-ARCH** — *re-homed 2026-09-22 from S-PRUNE*: the seven journals have no Rust writer outside S-ARCH, so there is no retirement site in S-PRUNE to assert at; S-PRUNE mints the function, [DRS_E1_SPRUNE.md](design/DRS_E1_SPRUNE.md) §12). The seven window-retired archival journals (`PDM-Q-F16`) retire at `tip − (CRB + n·SEB + D_max)` — **a second horizon beside the body horizon by design** (Q2 re-ruled 2026-09-22; was "equal to `W`") — through one function, never a literal. *Corrected 2026-09-22:* that function does not exist yet — `shekyl_archival_failure_window_params` returns the m-of-n slash window's `(m, n, serve_budget)`, one operand, not the horizon; and `D_max` has no constant. Both entry paths are bounded but neither is enforced (F19). The assertion lands where retirement is decided; a violated horizon is a refused retirement. Falsify by a journal row retired at a height not derived through the horizon function, or by the horizon expression appearing as a literal in a second site.
  - Target: pre-genesis

- **`SEB > D_max` const-asserted at `D_max`'s home, and the journal-horizon function beside it** (`PDM-Q2` RE-RULED 2026-09-22, [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) Q2 "Pops — checked, not argued"; `PDM-Q11` names the home `CEN-E2`). The guarantee that a pop never reaches a discarded body moved from the argued `W ≥ D_max` to `SEB > D_max`; it is asserted, not argued, once `D_max` has a constant — which it does not (`rg 'D_MAX' rust/shekyl-chain-rules/src` matches nothing). Owed together, in S-PRUNE's constants commit (with `T`'s home; A4 is not owed — `PDM-Q6` item 5): `D_MAX` at `CEN-E2`, `const _: () = assert!(SETTLEMENT_EPOCH_BLOCKS > D_MAX)` on the production constants — **the only assertion** — and `journal_horizon_blocks() = CRB + FAILURE_WINDOW_N·SEB + D_MAX` in `shekyl-archival-retention` for S-ARCH's retirement site. **And the fakechain conforms** (the fix, not an assertion site): the regtest `SHEKYL_SETTLEMENT_EPOCH_BLOCKS` override (`constants.rs:257`) admits `2..=SETTLEMENT_EPOCH_BLOCKS` in isolation, which puts `SEB ≤ D_max` under a short-epoch fakechain — a rejected configuration on any nettype (rule 71). Either one knob overrides both and preserves the ratio, or the parse refuses `SEB ≤ effective D_max`. Falsify by `SEB ≤ D_max` compiling on the production constants, by any nettype or override reaching `SEB ≤ D_max` at runtime, or by the horizon expression written as a literal anywhere.
  - Owner: [`ARCHIVAL_PRUNED_DAEMON_MODE.md`](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) Q2 / Q11
  - Target: pre-genesis
