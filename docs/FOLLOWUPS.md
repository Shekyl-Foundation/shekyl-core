...
- **`CEN-D2`/`CEN-D1` re-review once the PoW-verifier fix is merged.** The S1 was: on RandomX FFI failure the longhash was forced to the `0xff…` sentinel and the validation paths relied on `check_hash` rejecting it — but `check_hash(0xff…, 1)` passes, both call sites ignored the verifier's returned bool, and difficulty 1 is reachable (`--fixed-difficulty` on any nettype; organic LWMA-1 has no floor above CEN-D6's zero-guard), so under (FFI failure ∧ difficulty 1) a block could connect with its PoW never verified. **Ruled FIX 2026-09-03 (Rick) and fixed by PR #604** — the verifier's bool now gates all three consumer sites (main connect, alt path, and the longhash precompute worker, which no longer caches an uncomputed hash); the sentinel remains as a belt for display-only callers. **What remains:** both register rows stay DIVERGENT until re-reviewed at a sha containing the merged fix (the `CEN-M8` precedent), which takes the register to 97/3/2 and clears the last S-graded DRS-0 blocker. Owner: [`CONSENSUS_STORE_RECONCILIATION.md`](design/CONSENSUS_STORE_RECONCILIATION.md) §5.4.1.
  - Target: pre-genesis (event-driven: re-review at a merged sha)
