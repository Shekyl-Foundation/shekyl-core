...
- **Count-versus-height: height-semantics Phase 2c retyped wire/FFI inland decode; Phase 2d remains.** Census has no unclear family; wire table and inland/wire convention are in [`HEIGHT_SEMANTICS.md`](design/HEIGHT_SEMANTICS.md) §3. Phase 2c: `ChainTip` / `BlockHashAt` / `BlockHeaderAt` / `BlockAt.chain_height` are `ChainCount`; `target_height` is `Option<ChainCount>`; `Rpc::get_height` returns `ChainCount`; `ref_age_window` takes `ChainCount` vs `BlockHeight`; inland handlers bound with `has_block` / name the top with `tip()` (no numeric change). Remaining: Phase 2d inland bare-`u64` tail, with `compile_fail` per new conversion boundary. Falsify 2c by `ChainTip.chain_height` or `Rpc::get_height` returning anything but `ChainCount`. Falsify Phase 1 by a `daemon_claimed_tip` consumer missing from that table, or a consumer whose protocol quantity is ordinal and whose stamp is compared to a count from a different clock. Filed from PR #792 as the **third instance in a week** of a count read as a height (`WSS-24`'s producer-side conversion, #792's own fixture answering `get_info` with a tip where a count was required, and this wrap) — which is why the order is walk-then-retype and not a bare newtype.
  - Owner: [`HEIGHT_SEMANTICS.md`](design/HEIGHT_SEMANTICS.md)
  - Target: pre-genesis
  - Owner: [`HEIGHT_SEMANTICS.md`](design/HEIGHT_SEMANTICS.md) §3 — the wire table and the inland/wire convention Phases 2c/2d are walked against; it outlives each phase's PR
...
- **`--max-connections-per-ip` defaults to `1`, so two nodes behind one NAT cannot both hold a public-zone connection to the same peer — and the refused node is told nothing.** Observed 2026-09-21 on a live testnet pair sharing one residential WAN address: daemon A (synced, height 7101) held each seed's inbound slot; daemon B was refused by every seed — 34 consecutive handshakes ending `LEVIN_ERROR_CONNECTION_DESTROYED`, `white_list: 0 / gray_list: 0`, height stuck at 1. `has_too_many_connections` ([`net_node.inl`](../src/p2p/net_node.inl):3236, called at `:241`) counts **inbound** same-host connections against `max_connections`, default `1` ([`net_node.cpp`](../src/p2p/net_node.cpp):191); `is_same_host` compares `ip()` only ([`net_utils_base.h`](../contrib/epee/include/net/net_utils_base.h):83), so a distinct `--p2p-bind-port` does not separate the two nodes — verified by moving B to 12022. Anchors pinned to `dev` `059aca264`. **Not PWD-I1's cap**, which is outbound-only by construction ([`net_node.h`](../src/p2p/net_node.h):144). **Tor is not an escape hatch**: the admission cap skips non-public zones (`:3238`) so an onion peer connects, but [`cryptonote_protocol_handler.inl`](../src/cryptonote_protocol/cryptonote_protocol_handler.inl):452 refuses chain sync on exactly those zones — measured, six onion peers at height 7131, zero blocks in nine minutes on an empty database. **Re-diagnosed 2026-09-21: the cap is the right mechanism pointed at the wrong noun** — `is_same_host` answers two questions with one quantity, which is why no value of the cap works. **The narrative, the pricing, the trade and the sequencing live in the owning plan doc** (PWD-I7 and PWD-I8), per this file's header; what stays here is the claim, the anchors and the falsifier. Falsify by two daemons behind one NAT each holding a public-zone connection to the same seed (raise the cap on **every** seed, restart **both** daemons to clear the failure cache, and read the result off the seed), or by the refusal reaching the refused node's own log.
  - Target: pre-genesis
...
- **`seedeu` (45.77.66.189) is compiled into the testnet seed list and is externally unreachable on `12021`, so every node on the network burns a connect-failure and a 3600s suppression on it.** Found 2026-09-21 by direct survey while establishing the live seed set for PWD-I7's falsifier. **Worse than a down seed, because every host-side health check passes**: the daemon is active and bound to `0.0.0.0:12021`. `12021` is closed from three independent seeds while the control (`seedjp:12021`) is open from the same sources, so the block is `seedeu`'s ingress, not any prober's egress; its established-connection count is **5 against 10** on healthy seeds, outbound-only — the signature. Host firewall reports `ENABLED=yes` and the rule set needs privilege to read, so the **cause is not established** — missing `ufw` rule or provider firewall, both needing access this lane does not have. All six seeds otherwise run the identical binary (`8a8b1db0…`) at the same height (7408) with `in-peers=128`, and **none sets `--max-connections-per-ip`**, so the cap is 1 fleet-wide. **RECOVERED 2026-09-21, and the original diagnosis was WRONG in its cause.** Re-probed after the Frankfurt incident cleared: `12021` is now **OPEN from all five vantage points** that previously measured it closed — this box, `seedaus`, `seedjp`, `seeduse` and `skl-foundation`. So it was a **transient provider outage, not a standing firewall misconfiguration**, and the row's first writing named the wrong cause. **The lesson is the one worth keeping** (rule 47): daemon-active + bound + externally-closed-from-many-sources is consistent with BOTH a standing block and a transient outage, and a **point-in-time reachability probe cannot distinguish them**. The discriminator is *time* — re-probe later — and the first writing did not have it and did not say so. **The row stays open** because the impact claim survives its cause being corrected: while unreachable, a compiled-in seed costs every node on the network a connect-failure and a flat 3600s suppression, and every host-side health check passes throughout. What is owed is **detection**, not a firewall fix — nothing on the fleet notices a seed that is advertised and unreachable. **Excluded from PWD-I7's falsifier rather than repaired**, with the exclusion recorded in that row; its noise is separable there because a dead seed fails on the CONNECT arm while the cap refuses on the HANDSHAKE arm, two distinct log strings. **The row stays open**: exclusion unblocks one run, it does not fix a compiled-in seed that every node on the network still dials and suppresses for an hour. Falsify by `12021` answering from an off-fleet host, or by the daemon logging an inbound connection.
  - Target: pre-genesis
...
- **A refused inbound connection is charged a flat 3600s public-zone failure window whose own justification does not describe this failure class.** Sibling of the row above, filed separately: that row explains why one connection is refused, this explains why one refusal becomes a **dead node**. A per-IP refusal destroys the connection *after* TCP succeeds, so it is a **handshake** failure ([`net_node.inl`](../src/p2p/net_node.inl):1561 and :1619 — not `:1549`/`:1609`, which are the connect-fail arms), and `failed_addr_cache::window` ([`net_node.h`](../src/p2p/net_node.h):239-240) returns the flat hour on the FIRST failure. Anchors pinned to `dev` `059aca264`. **The constant's justification is the defect** ([`cryptonote_config.h`](../src/cryptonote_config.h):197-201): *"a failed dial usually means a down host"* — false here, the seed is up and refusing on policy, so each attempt blacklists it for an hour across every seed and recovery is impossible rather than slow. **The selector's stated premise is the deeper defect** ([`net_node.h`](../src/p2p/net_node.h):221-225): *"The window is a property of the TRANSPORT, not of the peer"* — a per-IP refusal is a property of the peer **pair**, so no tuning of a zone-keyed selector reaches this class; and the anon branch's own justification (*"up, correct, and answering everyone else"*) describes this failure word for word while being unreachable for it. **Not fixed here, and the blocker is named** (rule 22): the anon 240s cites a measurement ([`cryptonote_config.h`](../src/cryptonote_config.h):251, p90 of measured recovery) while the public hour's justification is an assertion, so a policy-refusal window is a **new number needing its own derivation** (rule 76), not a reuse. `record_addr_failed` ([`net_node.inl`](../src/p2p/net_node.inl):1642) takes an address and nothing else, so the class is known one line above the call and discarded crossing it — which is where a fix would go. Falsify by a refused node re-reaching the same seed inside 3600s on an unmodified build, or by the handshake-fail arms proving unreachable from an accept-side refusal.
  - Target: pre-genesis
