...
- **Implement the daemon shard-fetch client (`SF-` round RULED, halt lifted 2026-09-13; SPLIT at the verify seam 2026-09-13 per `PDM-Q-F25`)** — **Sub-PR 1, unblocked:** dial, header, envelope, SOCKS reuse, in-flight cap, miss/timeout taxonomy, **and the envelope's countersignature verify in full** (P's `HybridSignature` over the decoded 72-byte header `nonce ‖ anchor_height ‖ anchor_hash` followed by `shard_id_le` under the bond-record key — transport authenticity, unit-independent), against a body of `Vec<u8>` and one **content-verify** hole `verify(body, expected) -> Result` — the body type is `Vec<u8>` and nothing else (a unit-aware type or leaf/tx polymorphism means the split leaked and is reverted), and `expected` is opaque until `PDM-Q6` names it. The countersignature check is **not** inside the hole. **Sub-PR 2, blocked on `PDM-Q6` (PR #723):** the frame codec and the content-verify — today `ServedFrameHeader` (`RF-D4`) + `recompute_segment_r_k` over a leaf shard, which `PDM-Q6` is expected to retire for a tx-range body verified per-tx against `txs_prunable_hash`. Falsify the block by `PDM-Q6`'s row in `IMPLEMENTATION_INDEX.md` reading RULED. The ruled contract (leaf-shaped items marked **[sub-PR 2]** below; everything else is sub-PR 1's): one `shekyl-p-fetch` client for challenge and organic reads ([ARCHIVAL_SHARD_FETCH.md](design/ARCHIVAL_SHARD_FETCH.md), Round 1 CLOSED 2026-09-13). Every `SF-D` question is disposed; `SF-D9` is done; `SF-D11` is withdrawn; `SF-D12` is a corollary of `SF-D10`. The request carrier is closed in shape: tor-zone SOCKS, `GET /shard/{id}` on port 80, one required header decoding to 72 bytes `nonce[32] ‖ anchor_height_le[8] ‖ anchor_hash[32]` (fresh random plus the requester's chain anchor at `tip − 720`, **both** callers — not `attestation_nonce`; `P` gates `anchor_height` against a host-supplied height, `[p − 720 − L, p − 720 + L]`, identical 404 outside it, `L` pinned in `RF-R1`), and one shared admission path (`fetch(destination, shard_id, header)`; schedulers name `P`). The implementation PR pins the exact header name/encoding in code plus tests and updates the `RF-R1` living contract in the same change. Verification is P's hybrid countersignature under the stable bond-record identity key (sub-PR 1) plus content-verify — local `R_k` today **[sub-PR 2]**; parse and root mismatch **[sub-PR 2]** and bad countersignature (sub-PR 1) remain typed. `SF-D8`: `P` signs the **decoded** 72-byte header followed by `shard_id_le[8]` (server-parsed route id) — the binary, never the header's textual form; the challenge tuple and `cb_out_key` stay off the fetch — the mechanism proves `P` served, not which miner asked; the pass record carries `nonce[32] ‖ anchor_height[8]` and admission reads `anchor_hash` from the connecting chain in the window `[h − 720 − L, h − 720]` (**verifier LANDED as step (a0), PR #734**: `archival_reorg_depth_blocks` + `archival_attestation_anchor_lag_blocks = 4` PROVISIONAL, window table across the FFI, alt-chain fill above the fork point, genesis reject below 724); the HTTP body is a fixed-length outer envelope holding the canonical `HybridSignature` (sub-PR 1 parses and verifies it) followed by the `RF-D4` frame — unchanged by `SF-`, but the frame is `ServedFrameHeader` **[sub-PR 2]**; sub-PR 1 hands its bytes to the hole unparsed. **Sub-PR 1 not blocked.** The implementation PR owes: header spelling/encoding plus `RF-R1` contract update, the `PServeEndpoint` signer callback plus the host-supplied height for the pre-sign gate (crate still holds no keys and no chain view; SH-2 wires the live secret) — the signature domain (`shekyl/archival-attestation-scheme-v2`), its KATs and the `verify_pass_countersignature` v2 verifier are already landed by (a0), in-flight cap `N` (the Pi 4 memory floor was derived as `N × SHARD_BYTES`, the leaf-shard figure — **re-derive under `PDM-Q6`'s unit** once the body size is a tx-range's, not before; sub-PR 1 ships the cap with the leaf figure as its stated provisional bound; no unbounded buffer, schedulers wait for a slot) with its lower-bound rationale, the `SF-D4` dep-cut gate, and the W₂ measurement as its Round 0. Organic draw bound `k` is `TJ-D`'s (the fill scheduler names the next `P`); not this PR. Archive the round doc to `docs/completed/` when the client lands.
