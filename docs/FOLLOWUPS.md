...
- **Implement PWD-B7's affirmative drop verdict — internal failures must not sever peers.** **Three sites, all verified at source:** *(1)* two our-own-state paths return false from `add_tx` with `m_no_drop_offense` unset (`src/cryptonote_core/tx_pool.cpp:785-787`, message literally "internal error"; and the catch-alls `:344`, `:506-509`) and fall through `!tvc.m_no_drop_offense` (`cryptonote_protocol_handler.inl:909-913`) to `drop_connection`; *(2)* the announce path's `check_incoming_block_size` arm (`:536-540`) compares against OUR limit — wrong axis, narrow window; *(3)* the block-sync `prepare_handle_incoming_blocks` arm, whose ten `return false` sites carry **fifteen** conditions (`SCAN_TABLE_QUIT`'s one return serves six) in **three** classes — our state (`m_cancel` ×4, `!waiter.wait()` ×2), the sender's input (unparseable block ×2 or tx, duplicate tx or key image, empty span), and **our own internal invariants** (`tx_index is out of sync` ×2, `Tx not found on scan table`) — so **the boolean cannot say which fired and neither the sever nor the score is separable without the type**. #628 withdrew a proposed score removal after two enumerations of this call each undercounted it; that reversal is the sharpest evidence for the tri-state. The fix is a **typed tri-state verdict** (attributable form failure = drop / policy-or-state = no drop / internal = no drop, loud log) so an unclassified arm defaults to no-drop. The parse arm stays a drop. Rust-side per rule 20. PWC-E7, PWC-E8 (**not** PWC-E2 — that is the rate-limit absence owned by the PWD-B1 item; the announce-size arm has no census row of its own) — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B7
  - Target: pre-genesis
...
- **Size FL-R19's relay-floor clamp margin, and wire it (RULED (b), round 16).** The served economy rung must be clamped a fixed deterministic multiple above the unbuffered relay floor, so a median contraction between construction and broadcast cannot refuse an honestly-quoted transaction. Named blocker: the margin cannot be sized from this lane — the acceptance criterion is pre-registered as *the plausible median contraction over the p95 construction-to-broadcast gap for the OFFLINE-SIGNING flow*, and the chain traces are structurally blind to that gap; the quantity is the construction-to-broadcast gap distribution across real wallet flows, which is wallet instrumentation. **Binding on whoever implements it: the margin MUST be a fixed deterministic multiplier — never randomized, never per-wallet**, or the quote stops being derivable from chain state and manufactures the fingerprint FL-R18 established does not otherwise exist. Reopening criterion: the wallet lane produces that gap distribution, or ships offline signing, whichever is first; if the distribution is sub-block for essentially every flow the margin may be nominal, but the clamp still lands. Owning record: [FEE_LADDER_DERIVATION.md](design/FEE_LADDER_DERIVATION.md) §8 FL-R19.
  - Target: pre-genesis

- **Disclose the fee-tier privacy trade in the wallet/CLI tier picker (rule-81 obligation created by FL-R17's signature).** Three tiers put a privacy cost behind a user-facing choice — operative divisors ×1.25 default / ×6.7 / ×20 — and a user selecting "priority" cannot be expected to price a candidate-set divisor, so the surface must state the trade in terms a non-protocol user can act on or the choice is uninformed by construction. Named blocker: copy is owned by the wallet/GUI lane (rules 80/81/82), not the derivation round, and was handed off via the round record only. Reopening criterion: the engine tier-mapping change, which is its natural carrier. Owning record: [FEE_LADDER_DERIVATION.md](design/FEE_LADDER_DERIVATION.md) §7.
  - Target: pre-genesis
