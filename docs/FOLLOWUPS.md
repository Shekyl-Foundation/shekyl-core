...
- **`SEB > D_max` const-asserted at `D_max`'s home, and the journal-horizon function beside it** (`PDM-Q2` RE-RULED 2026-09-22, [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) Q2 "Pops — checked, not argued"; `PDM-Q11` names the home `CEN-E2`). The guarantee that a pop never reaches a discarded body moved from the argued `W ≥ D_max` to `SEB > D_max`; it is asserted, not argued, once `D_max` has a constant — which it does not (`rg 'D_MAX' rust/shekyl-chain-rules/src` matches nothing). Owed together, in S-PRUNE's A4 / precursor commit: `D_MAX` at `CEN-E2`, `const _: () = assert!(SETTLEMENT_EPOCH_BLOCKS > D_MAX)`, and `journal_horizon_blocks() = CRB + FAILURE_WINDOW_N·SEB + D_MAX` in `shekyl-archival-retention` for S-ARCH's retirement site. Falsify by `SEB ≤ D_max` compiling, or by the horizon expression written as a literal anywhere.
