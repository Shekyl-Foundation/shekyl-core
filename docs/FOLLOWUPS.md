...
- **p2p lane: the anchor dial path destroys the persisted anchor set on first use, and yields AT MOST ONE anchor-backed connection rather than `ANCHOR_CONNECTIONS_COUNT` — zero if every persisted anchor fails to handshake, since the container is cleared before any dial is attempted.** `get_and_empty_anchor_peerlist` (`net_peerlist.h:504-522`) copies every anchor into a caller-local vector and **clears the container**; `make_new_connection_from_anchor_peerlist` (`net_node.inl:1438-1470`) returns after the **first** successful dial; only that peer is re-inserted (`net_node.inl:1361`). The remainder goes out of scope and is lost. So a cold restart gets at most 1 anchor-backed connection, not 2, and post-restart "anchors" are repopulated from ordinary outbound draws rather than the persisted trusted set — **the anti-eclipse defence is materially weaker than its constant advertises.** Directly bears on Q-10's `g_max`, whose §7 framing asks "≥ k slots are anchor-backed and thus not re-rollable": `k = 1` at best. Found by a review challenging the figure rather than the reasoning — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-I4
  - Target: pre-genesis

- **Relay lane: `fluff_return_ms` has no gate tying it to the zones actually configured — adding a zone slower than Tor under-provisions `F′` silently.** It is a hardcoded scalar (`shekyl-relay-privacy/src/params.rs:381` = 3250) chosen by hand as the worse of two measured zones, and every consumer reads that single value. `get_seed_nodes` fails **loudly** on an unknown zone; this one fails **silently**, and §44.3's own argument makes under-provisioning the unsafe direction (over-estimating `F` lengthens the embargo and *reduces* the §6.7 prefix-fire leak; under-estimating costs privacy). A derivation check asserting the constant equals the max over measured zones would convert it to a loud failure — the shape `tests/carrier_window.rs` already uses. Found while verifying the N-ary constraint for the 2026-09-01 graph ruling ([`DAEMON_RELAY_PRIVACY.md`](design/DAEMON_RELAY_PRIVACY.md) §91.2)
  - Target: pre-genesis

- **~~Relay lane: the propagation-graph default question has a SECOND cost~~ — CLOSED 2026-09-01: the option was declined; the graph stays flexible and N-ary (`DAEMON_RELAY_PRIVACY.md` §91.2). Retained one release as the record of what the decline avoided.** Whether the graph default moves toward Tor-only was open and owned by the relay lane (`DAEMON_RELAY_PRIVACY.md` §91.2 + the 2026-09-01 disambiguation note). Cost 1 is already quantified there: `F′` at the worst zone, `ANON_ZONE_TRANSIT_ASSUMPTION_MS = 1625` vs clearnet's `50`, with no clearnet graph to take the *min* over. **Cost 2, found by P2P-2 and recorded so that decision does not miss it:** §12.10's eviction floor is bounded partly by **re-entry cost**, and on an anonymity network both halves of that tend to zero — §33.5 (*"key-minting is free, so that wait is the entire cost"*) and F-8 (the adversary reconnects rather than mints). §6.10's deterrent is explicitly economic and aimed at the *budget-constrained* observer, so it is the thing that evaporates. Argue, do not assume, whether the floor retains any cost term on Tor — §54.4(a)'s *inert → degraded* softening is why it is survivable. Both costs, one decision, one owner — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-I3
  - Target: pre-genesis
...
- **Shi et al. (NDSS 2025) graylist and whitelist sub-attacks — *ruled in design 2026-09-02, unimplemented in the tree*; the double-spend arm is refused only by an inherited, never-examined guard.** Both sub-attacks are now closed by PWD-I2's three rules ([`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md)); what remains here is the C++ change and the inherited guard. The guard is upstream Monero's `f7fd209ed`; Shekyl has no record examining it, so a rewrite re-deriving tx ingest drops it silently. PWC-E7, §5.2 — [`P2P_1_WIRE_CENSUS.md`](design/P2P_1_WIRE_CENSUS.md)
  - Target: pre-genesis

- **An inbound peer writes *itself* into the white list, bypassing gray — Shi §III-B's channel, open in the tree.** `handle_handshake` is inbound-only (`net_node.inl:2700-2706`) and on a successful back-ping calls `append_with_peer_white(pe)` at `:2766` with `last_seen = now`; `trim_white_peerlist` evicts oldest-`last_seen` first, so 1,000 IPs at one inbound connection each cycle the honest set out. **Fix per PWD-I2's third rule: route `:2766` to `append_with_peer_gray`** — `gray_peerlist_housekeeping` (`:3108`, every 60 s) then promotes it only after an outbound dial succeeds, which is the verification every other candidate already passes. Public-zone only (`m_can_pingback`-gated). **Behavioural, not cosmetic:** it changes white/gray and therefore post-restart dial composition, so PWD-I4's `ρ`/`g_max` sub-round must derive against the fixed behaviour. PWC-D11, PWD-I2 — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md)
  - Target: pre-genesis
