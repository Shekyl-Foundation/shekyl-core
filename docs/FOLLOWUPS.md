...
- **PDM: is `assumevalid = 0` at first release "genesis is the anchor" or "no anchor at all"?** E6 slice 4 made genesis the height-0 entry of every public network's `ReleaseAnchors` (Q3 (a), 2026-09-22): CEN-E1 judges a height-0 candidate against it and CEN-E5 refuses a wrong-network file at open. `ARCHIVAL_PRUNED_DAEMON_MODE.md` `:304` ships the first release with `assumevalid = 0`. The two readings coincide behaviourally today and diverge the moment anyone asks whether band 1 (`≤ C`) is empty at first release — so the question is put in that form, and the answer is a sentence: where `PDM-Q5` says "`C`", the slice reads "the last anchor above genesis" when the distinction matters (`rust/shekyl-chain-rules/src/anchors.rs` module docs). Falsify by: the PDM lane recording the reading beside `:304`.
  - Target: pre-genesis
  - Owner: [`ARCHIVAL_PRUNED_DAEMON_MODE.md`](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) — the PDM lane (`PDM-Q5`); the anchor table is [`CHAIN_RULES_SLICE_3.md`](design/CHAIN_RULES_SLICE_3.md)'s and the genesis entry [`CHAIN_RULES_SLICE_4.md`](design/CHAIN_RULES_SLICE_4.md) Q3's
