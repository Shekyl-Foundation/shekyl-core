...
- **Rule connection-admission *rate*, not only the concurrent-connection cap.** PWD-T5 routed adaptive resource exhaustion to PWD-B1/PWD-B9; PWD-B1 turned out to cover only the **post-handshake** phase, since its bucket sits on the four invoke entry points. The pre-handshake flooder prepends the correct prefix and forces one ML-KEM decapsulation per connection, then disconnects and repeats — **PWD-T6 bounds the per-connection buffer and PWD-B9 caps concurrency, but neither bounds churn.** Blocker: none; it is unruled, not blocked, and **PWD-B9 as briefed is a cap rather than a rate**, so B9 must carry both or a row must be minted. PWC-E2 — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B1, PWD-B9, PWD-T5
  - Target: pre-genesis
