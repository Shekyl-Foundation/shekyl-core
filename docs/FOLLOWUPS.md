...
- **Re-derive `drop_connections(address)`'s host-keyed severing together with the anonymity-zone inbound bound.** It drops **every** connection sharing a host and adds a host-fail score of 5 (`src/cryptonote_protocol/cryptonote_protocol_handler.inl:2821-2825`), which is the **same address-family problem** as PWC-E11: `is_same_host` needs an IP an onion address does not have, so on anonymity zones the mechanism cannot key on host at all. **Deferred by PWD-B7 to the PWD-I4 sub-round** so the two are re-derived once rather than twice. PWC-E9 — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B7, PWD-I4
  - Target: pre-genesis
