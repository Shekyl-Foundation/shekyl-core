...
- **CEN-B5's post-connect header-root check compares the wrong state — S1; every non-FAKECHAIN chain halts at height 60.** `handle_block_to_main_chain` compares the header `curve_tree_root` against `get_curve_tree_root()` read *after* `add_block` (`blockchain.cpp:6413`), but `create_block_template` fills the header from the root *before* the add (`:2006`) — the state at chain height N that the per-height record (key N), the wallet client (`drained_through = N − 1`) and the CT-2 KAT all name. **Observed** on real LMDB by `archival_substrate_lmdb.cpp` `cen_b5_post_add_root_is_not_the_state_the_header_was_filled_from`: at the first block where a leaf drains, the pre-add root equals the record under key N and the post-add root does not. **Derived** from it (no non-FAKECHAIN fixture exists to observe it — that fixture is this fix's regression): the genesis coinbase (stored maturity `0 + 1 + 60`) drains when block 60 connects, so mainnet/testnet/stagenet reject block 60 and halt. The rule-71 skip (`m_nettype != FAKECHAIN` gating the check; census R9) is the cause of the blindness, not context: every Blockchain-level test and every `--regtest` run (`cryptonote_core.cpp:423–426` sets `FAKECHAIN`) took the arm without the check. **Fix-or-accept is Rick's** (DRS §7.2). Sketch: compare the header against the pre-add root (record key N, or `get_curve_tree_root()` read before `add_block`), before the write txn opens as the register's REWRITE-NOTE already says; correct `FCMP_PLUS_PLUS.md` §5's "after processing all transactions in the block" to the state the block was built on; red-first core_test on a non-FAKECHAIN nettype, which needs the test generator to produce real roots (the R9 work) — a non-FAKECHAIN Blockchain fixture is the only thing that can turn the current pin into a fix regression. The alternative (template predicts the post-drain root) breaks the CT-2 KAT and the wallet contract. (owner: [`CONSENSUS_RULE_CENSUS.md`](design/CONSENSUS_RULE_CENSUS.md) CEN-B5, §7 #17; register [`CONSENSUS_STORE_RECONCILIATION.md`](design/CONSENSUS_STORE_RECONCILIATION.md) §5.4.1)
  - Target: pre-genesis
...
- **Re-derive `drop_connections(address)`'s host-keyed severing together with the anonymity-zone inbound bound.** It drops **every** connection sharing a host and adds a host-fail score of 5 (`src/cryptonote_protocol/cryptonote_protocol_handler.inl:2821-2825`). *Blocker corrected 2026-09-04 (review finding, verified at source): the earlier "`is_same_host` needs an IP" claim was wrong — the method dispatches to the concrete type (`contrib/epee/src/net_utils_base.cpp:90-98`) and Tor/I2P compare hostname strings (`src/net/tor_address.cpp:181-184`, `src/net/i2p_address.cpp:165-167`).* The actual blockers are identity-policy: every inbound anonymity-zone peer is recorded as the zone's `unknown()` sentinel (`src/p2p/net_node.cpp:336`, `:340`), so host-keyed severing would sever **all** inbound anon peers as one host; and a learnable onion identity is free to mint, so a per-host key prices nothing. Same identity problem as PWC-E11's inbound cap (absent via `has_too_many_connections`' explicit non-public early return, `src/p2p/net_node.inl:3091-3092`) — **deferred by PWD-B7 to the PWD-I4 sub-round** so the two re-derive once, count- or work-based. PWC-E9 — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B7, PWD-I4
  - Target: pre-genesis
