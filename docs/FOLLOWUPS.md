...
- **Implement PWD-B7's affirmative drop verdict — internal failures must not sever peers.** **Three sites, all verified at source:** *(1)* two our-own-state paths return false from `add_tx` with `m_no_drop_offense` unset (`src/cryptonote_core/tx_pool.cpp:785-787`, message literally "internal error"; and the catch-alls `:344`, `:506-509`) and fall through `!tvc.m_no_drop_offense` (`cryptonote_protocol_handler.inl:909-913`) to `drop_connection`; *(2)* the announce path's `check_incoming_block_size` arm (`:536-540`) compares against OUR limit — wrong axis, narrow window; *(3)* the block-sync `prepare_handle_incoming_blocks` arm, whose ten `return false` sites split almost evenly between our state (`m_cancel`, `!waiter.wait()`) and the sender's input (unparseable block or tx, duplicate tx or key image, empty span) — **the boolean cannot say which fired, so neither the sever nor the score is separable without the type**. #620 removed the score on the wrong premise and #628 restored it; that reversal is the sharpest evidence for the tri-state. The fix is a **typed tri-state verdict** (attributable form failure = drop / policy-or-state = no drop / internal = no drop, loud log) so an unclassified arm defaults to no-drop. The parse arm stays a drop. Rust-side per rule 20. PWC-E7, PWC-E8 (**not** PWC-E2 — that is the rate-limit absence owned by the PWD-B1 item; the announce-size arm has no census row of its own) — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B7
  - Target: pre-genesis
