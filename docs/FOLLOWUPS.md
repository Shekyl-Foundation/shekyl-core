...
- **Relay: a transaction mined while the carrier holds it is still SENT, one wasted window.** The verdict-time `pool_has_tx` gate stops the record and the F-10 observation, but the windows are already on the wire by then. Named blocker: cancelling earlier needs an enqueue-cancellation API `NoiseQueues` does not have, and `unbind` clears a whole channel, so cancelling one message would discard its channel-mates. Cost is one window of cover carrying a transaction peers already hold and drop — which is what cover traffic is for. Reopen only if another caller needs cancellation [`COVER_TRAFFIC_RESTORATION.md` §3.1e](design/COVER_TRAFFIC_RESTORATION.md)
  - Target: pre-genesis
