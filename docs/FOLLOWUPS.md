...
- **Block-weight penalty and fee floor price the same expansion off different medians during a surge.** Penalty uses the effective median (up to `S·LTEM`); the fee floor/ladder uses `Mfw = min(Mnw, Mlw) = LTEM` because the surge clamp's lower arm makes `Mnw ≥ Mlw`. Conservative (the floor over-charges relative to the penalty, up to 16× at `S = 4`). Same class as FL-V1: two paths, two operands. Owner: the fee lane; [FEE_LADDER_DERIVATION.md](design/FEE_LADDER_DERIVATION.md). Found implementing C2-R2 Q3 (`S = 4`).
  - Target: pre-genesis
