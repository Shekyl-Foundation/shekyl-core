...
- **PDM-Q-F18: delete the ring-era `check_tx_input` chain.** `Blockchain::check_tx_input` (`src/cryptonote_core/blockchain.cpp:4522`) has zero callers; its body branches on `tx_version == 1` and validates `key_offsets` against ring members. It is the sole caller of `scan_outputkeys_for_indexes` (`:262`), which is the only consensus-shaped reader of `output_metadata`. Rule-60 deletion: `check_tx_input`, `scan_outputkeys_for_indexes`, the `outputs_visitor` struct, the `blockchain.h:1332`/`:1355` declarations, and the comment at `:3514`. Carrier: a dedicated C++ PR. Falsify by `rg -n 'check_tx_input\b|scan_outputkeys_for_indexes' src tests` returning nothing. Owner: [`ARCHIVAL_PRUNED_DAEMON_MODE.md`](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) `PDM-Q-F18`.
  - Target: pre-genesis
- **PDM-Q-F23: delete the runtime JSON checkpoint channel (`load_checkpoints_from_json`).** Live at `src/cryptonote_core/blockchain.cpp:6635` (`Blockchain::update_checkpoints`) and reloaded every ten minutes from `src/cryptonote_protocol/cryptonote_protocol_handler.inl:698`; the DNS half is already gone (comment at the same line). Under `PDM-Q5`'s anchor model the release-carried checkpoint is an `assumevalid` anchor, and a runtime-loadable checkpoint file is a trust channel that bypasses it — rule 15/60 shape, the same surface the DNS deletion removed. Carrier: a dedicated C++ PR, landing no later than the PR that populates `init_default_checkpoints` (`src/checkpoints/checkpoints.cpp:163-172`, a no-op at the pin); the surface includes `core::update_checkpoints` (`cryptonote_core.cpp:250`, `:644`), the JSON-path tests (`tests/unit_tests/checkpoint_uniformity.cpp:81`, `tests/core_tests/checkpoint_conflict_rollback.cpp`) and the test-double stubs. Blocked on `PDM-Q5` ruling the anchor — falsify by that ruling landing. Then falsify the deletion by `rg -n 'load_checkpoints_from_json|update_checkpoints|checkpoints_json' src tests` returning nothing. Owner: [`ARCHIVAL_PRUNED_DAEMON_MODE.md`](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) `PDM-Q-F23` / `PDM-Q5` item 3.
- **PDM-Q-F7: correct the stale "recomputed from leaves" comments on the intermediate-layer prune.** `src/blockchain_db/lmdb/db_lmdb.cpp:9932-9935` and `:9978-9980`, and `src/blockchain_db/blockchain_db.h:2841-2842` (at `4da609cbd`; `:9922-9925` / `:9970` / `:2838` at `edb35dbb1`) say pruned layer entries "can be recomputed from leaves". No such function exists: the only upper-layer rebuild is `trim_curve_tree`, which recomposes from **layer 0** (the leaf-chunk hash layer, never pruned), not from leaves ([ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) `PDM-Q-F7`). A comment that names a mechanism the code does not have is the rule-16 "comment that outlived its architecture" shape and steers a reader to the wrong retention boundary. Carrier: a dedicated C++ PR alongside F9/F18/F23 — three comment edits, falsified by `rg 'recomputed from leaves' src/blockchain_db`. Independent of every `PDM-Q` ruling. **Target:** pre-genesis.
  - Target: pre-genesis
...
- **Every FCMP++ spend is linkable to the output it spends, by the public 4th leaf scalar `h_pqc` — tree-wide privacy defect, found 2026-09-13; needs its own round.** The spend carries `tx.pqc_auths[i].hybrid_public_key` in cleartext (`tx_pqc_verify.cpp:128`); consensus hashes it with the leaf-hash function and passes it to the FCMP++ verifier as a **public input** (`blockchain.cpp:4348–4356`; `fcmps/src/lib.rs:102–111` `extra_leaf_scalars: "Public values"`); the circuit blinds `O, I, C` but constrains the leaf's 4th scalar to **equal** that public value unblinded (`fcmps/src/circuit.rs:153–163`). `h_pqc` is per-output unique (`derivation.rs:87–135`) and published per output in `tx_extra 0x07`, copied into the 128-byte leaf at `[96..128]` (`blockchain_db.cpp:528–556`). So the spent leaf is the unique leaf whose `[96..128]` equals `H(revealed key)` — anonymity set **one**, no proof analysis needed. `FCMP_PLUS_PLUS.md:102–104`, `POST_QUANTUM_CRYPTOGRAPHY.md:735–737`, `MERKLE_TREE.md:235` assert the opposite; `REWARD_EMISSION_LEG.md` §7.3 found the enumerability for the emission backing vin and rested its safety on ordinary spends being hidden — its tripwire has fired. Not a soundness defect; funds are safe; mission priority 2. Fix shape is a round's to rule (in-circuit or algebraic hash of the key; a re-randomised commitment to `h_pqc`; or a key structure that is not per-output-revealed) — every option keeps the ML-DSA key in cleartext at spend, which is why `ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md` §2.2 rules the witness key against it. Falsify by a test that spends an output under FCMP++ and asserts `H(pqc_auths[i].hybrid_public_key) ≠ leaf[96..128]` for the spent leaf, or that the leaf cannot be located from the tx. Owner: `ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md` §1.5 (F5) until a round is opened; identifier and round are Rick's (rule 94 §1).
  - Target: pre-genesis
- **`SO-D9` — `ERR_EPOCH_MISMATCH` on the serve-credit admission path is a tautology; RULED (i) 2026-09-13, implementation owed.** `serve_credit.rs:168` refuses when `response.settlement_epoch != ctx.settlement_epoch`, but `ctx.settlement_epoch` is set at `blockchain.cpp:5304` from `sc_settlement_epoch`, parsed out of the same record at `:5124`; `shekyl_archival_verify_serve_credit_vin` has one caller (`:5315`). The check compares the record to a copy of itself — rule 50 "check that cannot fail", second instance in the SO arc. **Ruled (i):** populate `ctx.settlement_epoch` from `shekyl_archival_settlement_epoch_at_height(current_height)` (`shekyl_ffi.h:2594`), an explicit single-site rule that the record's epoch is the block's — not the *when*-bound `h_close` leaves implicit. Consensus tightening; one C++ line plus a test. Ordering to preserve: the C++ `h_close` gate (`:5186`) runs before the FFI, so a stale-epoch header is refused by `h_close` and a **future**-epoch header is the case `EPOCH_MISMATCH` refuses. Falsify by `rg -n 'ctx.settlement_epoch = sc_settlement_epoch' src/cryptonote_core/blockchain.cpp` returning nothing AND a test that a record whose header names `epoch(current_height) + 1` is refused with `SHEKYL_ARCHIVAL_VERIFY_ERR_EPOCH_MISMATCH`. Owner: [`ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md`](design/ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md) `SO-D9`; [`ARCHIVAL_SETTLEMENT_WRITER.md`](design/ARCHIVAL_SETTLEMENT_WRITER.md) §13.
  - Target: pre-genesis
- **`CHALLENGE_RESPONSE_BLOCKS` has no consensus consumer today; under the ratified R-B shape it becomes the per-challenge deadline and the const-assert coupling it to `CHALLENGE_RESOLUTION_BLOCKS` becomes load-bearing.** Verified 2026-09-13: `constants.rs:171`, `:183–185`, `:194–195` are const-asserts and `lib.rs:139` the re-export; nothing on the admission path reads it, so its doc string (*"blocks after a challenge's issuing block to accept its serve-credit response"*) names a gate that does not exist. R-B ratified in direction 2026-09-13 (`ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md` §2): the admission window `h_incl ≤ h + CHALLENGE_RESPONSE_BLOCKS` is the referent, and the slash-pass emission gather (§5) depends on `CHALLENGE_RESOLUTION_BLOCKS ≥ CHALLENGE_RESPONSE_BLOCKS`. Until the cutover lands the doc string is false and the assert is dangling; this row closes when the deadline gate is the reader. Falsify by `rg -n CHALLENGE_RESPONSE_BLOCKS rust src` showing an admission-path reader. Owner: `ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md` §1.1 / §2.1 item 6.
  - Target: pre-genesis
- **`lambda_target` is a bare parameter; `CHALLENGES_PER_PAIR_PER_EPOCH` does not reach the urn.** `challenge_assignment.rs:152` and `:264` take `lambda_target: u32`; the constant (`constants.rs:27`) is read only by `shekyl-economics-sim` and the `attestation.rs:77,:82` const-asserts. Nothing connects the two, so the urn and the settlement threshold can disagree silently. Pin before any production caller of `assign_epoch` / the `EpochAssignmentCache` (`SO-D8e`): the cache's `open()` reads the constant and takes no `λ` argument. Falsify by `rg -n 'lambda_target' rust/shekyl-archival-retention/src` returning only test-internal uses. Owner: `ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md` §7.3 / Q4.
  - Target: pre-genesis
...
- **FL-R3-STORE: O(1) cumulative transaction count on the Rust chain store** — `Blockchain::get_tx_volume_window` (renamed from `get_tx_volume_avg` by FL-R24 PR A) walks a 720-block window and `get_block_from_height` loads and parses each full block blob to read `tx_hashes.size()`; #640 memoized the repeat case but a cold call still parses 720 blocks, and it is **consensus** (`get_block_reward` in `validate_miner_transaction`). **Promoted at FL round 18, specified at round 2, DEMOTED at round 19 (§11.2 FL-R21):** the fee round no longer blocks on it — FL-R23's cold cost is one `720 + G` scan, at the 720 budget — so this is the storage lane's performance item on its own merits (a consensus operand parsing 720 full blocks cold). The specification stands as written: the grid-anchored fold at `P` = 720 needs raw `C` at 720 heights — 1 440 cold parses against a 720 budget (2.0×) — and a `cumulative_tx_count(h)` read on `shekyl-chain-store`'s S-CHAIN-R surface takes both to O(1). Requirement, bit-identity gate and the recorded LMDB alternative at [FEE_LADDER_DERIVATION.md](design/FEE_LADDER_DERIVATION.md) §10.12.2; the store's shape is the storage lane's, per [DAEMON_REDB_STORE.md](design/DAEMON_REDB_STORE.md)'s 2026-09-01 countermand (RECORD-AND-SPECIFY, no C++ refactor). **Amended 2026-09-11 (FL-R23 ring, PR B):** the same row now owes a second per-block field — the **long-term effective median** `M(h)` — for the same reason. FL-R23's `(height, F)` ring is pushed per block connect at O(1), but its COLD path (startup, reorg) must reconstruct `F(h−k) = R·C·w_ref/M²` at `G + 1` historical heights, and while `already_generated_coins` is stored per block, `M(h)` is not: it is a 100 000-block rolling median that the daemon overwrites each block and cannot step backwards. The cold path therefore costs **two terms in two units** — `720 + G` full block-blob **parses** for the `C` windows (the §10.12.1 figure) **plus** ~100 000 long-term-weight **field reads** to seed a rolling median that is then stepped forward `G` times (one insert, one median per step). Stating one number in both units would invite the reader to conclude the ring costs 100 000 blob parses. **This reconstruction is a WORKAROUND with a trigger, not architecture:** once `cumulative_tx_count(h)` AND `long_term_effective_median(h)` are both stored per block, the ring's cold path is `G + 1` field reads and `Blockchain::rebuild_relay_floor_ring`'s stepped-median code is **deleted**. Without the trigger written down, a 100 000-entry stepped median becomes permanent because it worked. Falsify by `rg cumulative_tx_count rust/shekyl-chain-store` and `rg long_term_effective_median rust/shekyl-chain-store`.
  - Target: pre-genesis
...
- **Implement the daemon shard-fetch client (`SF-` round RULED, halt lifted 2026-09-13; SPLIT at the verify seam 2026-09-13 per `PDM-Q-F25`)** — **Sub-PR 1, unblocked:** dial, header, envelope, SOCKS reuse, in-flight cap, miss/timeout taxonomy, **and the envelope's countersignature verify in full** (P's `HybridSignature` over `nonce ‖ height ‖ shard_id` under the bond-record key — transport authenticity, unit-independent), against a body of `Vec<u8>` and one **content-verify** hole `verify(body, expected) -> Result` — the body type is `Vec<u8>` and nothing else (a unit-aware type or leaf/tx polymorphism means the split leaked and is reverted), and `expected` is opaque until `PDM-Q6` names it. The countersignature check is **not** inside the hole. **Sub-PR 2, blocked on `PDM-Q6` (PR #723):** the frame codec and the content-verify — today `ServedFrameHeader` (`RF-D4`) + `recompute_segment_r_k` over a leaf shard, which `PDM-Q6` is expected to retire for a tx-range body verified per-tx against `txs_prunable_hash`. Falsify the block by `PDM-Q6`'s row in `IMPLEMENTATION_INDEX.md` reading RULED. The ruled contract (leaf-shaped items marked **[sub-PR 2]** below; everything else is sub-PR 1's): one `shekyl-p-fetch` client for challenge and organic reads ([ARCHIVAL_SHARD_FETCH.md](design/ARCHIVAL_SHARD_FETCH.md), Round 1 CLOSED 2026-09-13). Every `SF-D` question is disposed; `SF-D9` is done; `SF-D11` is withdrawn; `SF-D12` is a corollary of `SF-D10`. The request carrier is closed in shape: tor-zone SOCKS, `GET /shard/{id}` on port 80, one required header decoding to `nonce[32] ‖ height_le[8]` (fresh random plus published-tip height, **both** callers — not `attestation_nonce`), and one shared admission path (`fetch(destination, shard_id, header)`; schedulers name `P`). The implementation PR pins the exact header name/encoding in code plus tests and updates the `RF-R1` living contract in the same change. Verification is P's hybrid countersignature under the stable bond-record identity key (sub-PR 1) plus content-verify — local `R_k` today **[sub-PR 2]**; parse and root mismatch **[sub-PR 2]** and bad countersignature (sub-PR 1) remain typed. `SF-D8`: `P` signs `nonce[32] ‖ height_le[8] ‖ shard_id_le[8]` (server-parsed route id); the challenge tuple and `cb_out_key` stay off the fetch — the mechanism proves `P` served, not which miner asked; the pass record carries the 32-byte random; the HTTP body is a fixed-length outer envelope holding the canonical `HybridSignature` (sub-PR 1 parses and verifies it) followed by the `RF-D4` frame — unchanged by `SF-`, but the frame is `ServedFrameHeader` **[sub-PR 2]**; sub-PR 1 hands its bytes to the hole unparsed. **Sub-PR 1 not blocked.** The implementation PR owes: header spelling/encoding plus `RF-R1` contract update, the signature domain string and KAT, the `verify_pass_countersignature` v2 amendment (carried nonce; signed height = predecessor height), the `PServeEndpoint` signer callback (crate still holds no keys; SH-2 wires the live secret), in-flight cap `N` (the Pi 4 memory floor was derived as `N × SHARD_BYTES`, the leaf-shard figure — **re-derive under `PDM-Q6`'s unit** once the body size is a tx-range's, not before; sub-PR 1 ships the cap with the leaf figure as its stated provisional bound; no unbounded buffer, schedulers wait for a slot) with its lower-bound rationale, the `SF-D4` dep-cut gate, and the W₂ measurement as its Round 0. Organic draw bound `k` is `TJ-D`'s (the fill scheduler names the next `P`); not this PR. Archive the round doc to `docs/completed/` when the client lands.

  - Target: pre-genesis

- **Re-examine the C++-anchored CI gates at the LMDB cutover** — **tracked, not scoped** (2026-09-13). Eight of the 59 gates in `scripts/ci/` encode *"C++ is canonical, Rust mirrors it"*: `check_redb_schema_bijection.py`, `check_redb_schema_key_types.py`, `check_lmdb_schema_coverage.py` (+ `test_check_lmdb_schema_coverage.py`), `check_consensus_invariants.sh`, `check_levin_constant_parity.sh`, `check_segment_freeze_sites.sh`, `test_check_doc_code_citations.py`. Each is **correct now and wrong later**: at the cutover the relation inverts and they become obsolete or backwards. The bijection gate is simply first to surface it — it is symmetric (`extra = set(defined) − set(censused)`, `:119–123`), so the first **redb-only** table fails CI, which is why `PDM-Q-F14`'s `txs_pqc_auths_hash` cannot be added without a second change. **The first redb-only table is the event that retires the mirror assumption**, and that cost belongs to the **E-series**, not to any one round that happens to arrive first — design the mirror→lead transition once at DRS-E1 and let `PDM-Q6` point at it, or it is re-derived at every subsequent redb-only table. Any in-flight exception must be **blocker-keyed, not dated**: the repo's own `DEFERRED_DOCS` shape, whose self-expiry FATAL fires when the blocker clears — *"a gate that quietly excludes what it cannot pass cannot fail"*. The natural key already exists as a genesis-gate observable: [DAEMON_REDB_STORE.md](design/DAEMON_REDB_STORE.md) §8.1, *"production `shekyld` does not link liblmdb"*. Not scoped here deliberately — the cheap moment to name the scope is while it is known which eight and why, not after the cutover when they are failing and someone is deciding under pressure whether a red is real. Falsify by `rg -l 'SHEKYL_LMDB_TABLES|db_lmdb|LMDB_SCHEMA|cryptonote_config' scripts/ci/` returning fewer than eight files, or by `shekyld` no longer linking liblmdb while those gates still assume it does.
    - Target: pre-genesis
