...
- **Count-versus-height: height-semantics Phase 2e wrapped the C4 offset and the wallet-ledger tip.** Census has no unclear family; wire table and inland/wire convention are in [`HEIGHT_SEMANTICS.md`](design/HEIGHT_SEMANTICS.md) §3. Phase 2e: `bond_post_offset_blocks` is `BlockCount` (`PENDING_POST_VERSION` 12); `LedgerEngine::synced_height` / `BlockchainTip.synced_height` / `LedgerBlock::height` / reorg window are `BlockHeight` (`LEDGER_BLOCK_VERSION` 12, `WALLET_LEDGER_FORMAT_VERSION` 19). No numeric change; wire/FFI stay `u64` (C1). Remainders: stamp-clock COUNT→ORDINAL conversion is optional-not-owed; `SyncStateBlock.restore_from_height` still `u64`; `get_version` `target_height` wire `0` still `RK-`. Falsify C4 by `bond_post_offset_blocks` still `u64`. Falsify the ledger wrap by `LedgerEngine::synced_height` still returning `u64`. Falsify Phase 1 by a `daemon_claimed_tip` consumer missing from that table, or a consumer whose protocol quantity is ordinal and whose stamp is compared to a count from a different clock.
