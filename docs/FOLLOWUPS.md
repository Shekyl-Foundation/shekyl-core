...
- **The shard partition is a consensus surface — one constant, one home, const-asserted; nothing mints a partition of its own.** *(Re-keyed 2026-09-18 under `PDM-Q6` item 4 / F32 / Q12 — `PDM-Q-F33`, #774 landed second; the leaf-unit text this row carried from #775 is superseded in place.)* Under Q6 item 4 the partition is `b_*` over the retained per-tx length rows (S-CHAIN-W amendment A4), each shard `[b_k, b_{k+1})` closing at `SHARD_BYTES` from the previous boundary (F32); it is consensus for the reason #775 gave, which is unit-independent — admission validates `shard_id` against closed shards, so a divergent partition forks at admission and at pop revert. The hazard #775 named survives re-keyed: **`SHARD_BYTES` has one home, const-asserted, never a `consensus_constants.json` value that reads as a tune, no "provisional" marker**; `b_*` is derived only from the A4 rows by S-PRUNE's forward pass at ingest — DRS-E3 S-CURVE, the wallet-side store (Q12's rebuild) and `shekyl-archival-retention`'s verifier all *read* it, none derives its own. **Owed:** (a) `SHARD_BYTES`'s home and const-assert land with A4 (owner DRS-E, S-CHAIN-W); (b) the verifier reads `(b_k, b_{k+1})` from the retained rows, never from the prover (Q6 item 4, at S-ARCH). **The leaf partition — `SEGMENT_LEAF_COUNT`, `frozen_segment_count`, `SEGMENT_LAYER_J`, `outputs_per_node` — is a deletion surface at E4 / S-ARCH (Q12).** Until that deletion it is live consensus, so #775's interim tie stands as #775 scoped it: `const _: () = assert!(leaves_per_segment() as u64 == SEGMENT_LEAF_COUNT)` in `shekyl-archival-retention` (the crate that sees both), same PR as the CT-1 dedup row below — one line that dies with the freeze; the `const fn` rewrite and marker removal #775 listed as (a) are not worth doing on a deletion target (rule 15), so a `#[test]` tie is an acceptable fallback here where #775 called it strictly weaker. Owner of the interim tie: the wallet lane. Falsify by `SHARD_BYTES` declared in more than one crate or present in `consensus_constants.json`, or by any Rust deriving a shard boundary from anything but the A4 rows. Contract: [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) Q6 item 4 / F32; [DRS_E1_SPRUNE.md](design/DRS_E1_SPRUNE.md) §2.
  - Target: pre-genesis
