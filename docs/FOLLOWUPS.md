...
- **`block_verification_context::m_verifivation_failed` drives a drop on set-ness, and nothing has asked whether its setters are attributable.** PWD-B7 scoped the `tvc` sites and `prepare_handle_incoming_blocks`; the `bvc` side was **not** in scope and is not overlooked residue of that unit but a separate surface (rule 19: different validation surface, different tests). The flag's own comment still reads *"bad block, should drop connection"* (`src/cryptonote_basic/verification_context.h:69`) and it gates two drops — `cryptonote_protocol_handler.inl:696` on the announce path and `:1590` on the sync path, the latter charging `P2P_IP_FAILS_BEFORE_BLOCK` on `m_bad_pow`. **This is NOT the same defect** PWD-B7 removed: `tvc`'s flag was a carve-out where *absence* meant droppable, whereas this one is set affirmatively, so a new arm defaults to no-drop already. The open question is narrower and is the one PWD-B7 makes askable: does any setter of `bvc.m_verifivation_failed` describe **our own state** rather than the block? Enumerate the setters and classify them; if any do, they need the same typed verdict (`shekyl-peer-policy`), which already exists. — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B7
  - Target: pre-genesis
...
- **Restore the §7 hysteresis band to the served fee correction** — round 18 measured the shape: the grid-anchored fold at `P` = 720 restores the oscillation *amplitude* exactly (worst 24, the banded figure) but leaves 20 cells oscillating against the band's 14, and it **exceeds the cold-path cost budget by 2.0×** (1 440 cold parses against 720; round 1 recorded 1.33×, which was P = 240's figure — corrected at §10.12.1). BLOCKED on **FL-R3-STORE**, the O(1) cumulative tx count specified for `shekyl-chain-store`'s S-CHAIN-R surface at §10.12.2 — falsify by `rg cumulative_tx_count rust/shekyl-chain-store` returning the read. Owner `hysteresis_fold` landed; `P` and the anchor await R18-M1. [FEE_LADDER_DERIVATION.md](design/FEE_LADDER_DERIVATION.md) §10.12 (round 2), §10.10 (round-1 results), §8 FL-R3.
  - Target: pre-genesis
