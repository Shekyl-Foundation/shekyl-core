...
- **`tx_extra` `0x02` Nonce: shed from the genesis grammar — FA-10's uniform 8-byte `enc_label` is the sanctioned tagging channel, and `0x02`'s presence and length are cleartext observables that partition users.** Precedent `0x03`/`0xDE`. The only production producer is the coinbase (`construct_miner_tx`'s `extra_nonce`, the `get_block_template` `reserve_size` mechanism); wallet payment-ids die with `construct_tx*` (`TXE-Q1`). **RULED as `TXE-Q6`** ([`TX_EXTRA_RUST_CUTOVER.md`](design/TX_EXTRA_RUST_CUTOVER.md) §6, 2026-09-22): shed, on the fingerprint ground, with the 32-bit-header-nonce cost (≈ 35.8 MH/s per-template exhaustion at `T = 120 s`) priced there; a bounded structured field is the fallback, never `0x02`. Lands in the cutover's commit 3. *(Row body restored 2026-09-22 from `c7df00c55c`; it had been truncated to its heading.)*
  - Target: pre-genesis
  - Owner: [`TX_EXTRA_RUST_CUTOVER.md`](design/TX_EXTRA_RUST_CUTOVER.md) TXE-Q6
...
- **`--max-connections-per-ip` defaults to `1`, so co-residents behind one address hold permanently DISJOINT peer sets — and a node starves when the reachable set falls below what the partition needs.** Measured 2026-09-21 on two daemons sharing one WAN address: **4 and 2 of six seeds, zero overlap, union 6** — forced at cap 1, where no cap would give near-total overlap. Earlier, with one seed down, the other daemon held enough of the remainder that this one got **none**: 34 handshakes ending `LEVIN_ERROR_CONNECTION_DESTROYED`, `white_list: 0 / gray_list: 0`, height 1. `has_too_many_connections` ([`net_node.inl`](../src/p2p/net_node.inl):3236, called `:241`) counts **inbound** same-host connections against `max_connections`, default `1` ([`net_node.cpp`](../src/p2p/net_node.cpp):191); `is_same_host` is `ip()`-only ([`net_utils_base.h`](../contrib/epee/include/net/net_utils_base.h):83), so a distinct `--p2p-bind-port` does not separate them. **Not PWD-I1's cap** ([`net_node.h`](../src/p2p/net_node.h):144, outbound-only). **Tor is not an escape hatch** — [`cryptonote_protocol_handler.inl`](../src/cryptonote_protocol/cryptonote_protocol_handler.inl):452 refuses chain sync on exactly the zones the cap exempts. Anchors pinned to `dev` `059aca264`. **The analysis — why no value of the cap works, the eclipse consequence, and the inverted trade against PWD-E4 — lives in the owning plan doc** (PWD-I7/PWD-I8), per this file's header. **FALSIFIER RUN AND PASSED 2026-09-21** (Rick, verified independently off the seeds): the cap was raised on **three of six** seeds, and overlap went to **exactly the raised set** — those three hold 2 connections from the one address, the three unraised still hold 1, with the unraised remainder serving as a control arm on the same network at the same moment. The starved node reached height 7496, `synchronized=true`. Subsequently raised on **all six**: overlap **6 of 6**, two connections per seed, B at 7505 `synchronized=true`. **The row stays OPEN and its target is unchanged.** The value is now empirically adequate for this estate and still wrong as a design: **8 is no more principled than 1**, merely larger than the node count behind this particular address, and under CGNAT it fails exactly as 1 did. **Three seeds are still partitioning right now.** The green result makes PWD-I8's endpoint-categorisation harder to fund, not easier, which is the outcome this row flagged as the dangerous one.
  - Target: pre-genesis
