...
- **Derive PWD-B1's token-bucket parameters — all four, since a refill rate alone does not define a bucket.** *(1)* refill rate; *(2)* capacity and initial fill, which *is* the burst allowance and decides whether a peer that connects and immediately syncs is throttled; *(3)* per-command token cost, since a handshake and a timed-sync are not the same work and an attacker picks whichever is mispriced; *(4)* **the action on exhaustion — throttle or disconnect**, which is **wire-observable**, so leaving it open means conforming peers disagree about whether a slow peer is a hostile one. **Unruled, not blocked.** Needs the honest-sync measurements PWD-B1's falsifier names. PWC-E2 — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B1
  - Target: pre-genesis

- **Derive PWD-B2's timed-sync `min`/`jitter` split.** The distribution is ruled — bounded uniform `min + U(0, jitter)`, per-connection, re-drawn only on its own fire — and the constraint is fixed: **the mean stays at `P2P_DEFAULT_HANDSHAKE_INTERVAL` (60 s)**. **That constraint is necessary and not sufficient** — `min = 60 s, jitter = 0` satisfies it and is exactly the fixed phase PWD-B2 exists to remove, and a sufficiently small window fails the correlation falsifier for the same reason. Conformance is two conditions: the mean is preserved, **and** the window is non-zero and wide enough that measured phase correlation falls below the falsifier's threshold — a **measurement**, not an algebraic choice, which is why the split is owed rather than picked. The trade is worst-case staleness against phase separation. **Unruled, not blocked.** PWC-E1, PWC-E3 — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B2
  - Target: pre-genesis
