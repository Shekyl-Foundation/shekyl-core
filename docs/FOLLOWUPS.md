...
- **Three FFI exports lost their only C++ caller with `construct_tx*` and are callerless: `shekyl_construct_output_labeled`, `shekyl_label_plaintext_for_payment_uri`, `shekyl_compute_output_key_image_from_ho` (`rust/shekyl-ffi/src/legacy_tx.rs`).** `legacy_tx.rs` is wallet-era surface (`shekyl_scan_output`, `shekyl_sign_pqc_auth`, …) whose live-caller census is one job; deleting three lines of it inside the tx_extra cutover would be the partial sweep that leaves the file looking audited. Falsify by `rg` over `src/ tests/` for each `shekyl_*` export in the file; delete every zero-hit export in one commit under the `nm` gate F-7 builds.
  - Target: pre-genesis
  - Owner: [`F7_TEST_ONLY_FFI_EXPORTS.md`](design/F7_TEST_ONLY_FFI_EXPORTS.md) §3 (the derived symbol list)
