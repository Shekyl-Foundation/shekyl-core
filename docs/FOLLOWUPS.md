...
- ~~**F26 landing: the `txs_pqc_auth_hash` store row (DRS §7.7 item 3)**~~ — **DONE 2026-09-17 on PR #772** (`PDM-Q-F26`, [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) §6 / §8; DRS §7.7). Identity landed #768 (`TxIdentity { hash, pqc_auth_hash: Option<PqcAuthHash>, prunable_hash }` from `Transaction::txid_parts()`). The sparse row is `schema.rs` `TableDefinition` `txs_pqc_auth_hash` (`Coded<PqcAuthHash>`, A3): present ⇔ 4-part, never deleted by a prune; hash row without segment is *discarded*. Prune consumption of the segment is F31. Falsify by `rg 'TableDefinition.*"txs_pqc_auth_hash"' rust/shekyl-chain-store/src/schema.rs` returning nothing.
  - Target: pre-genesis
