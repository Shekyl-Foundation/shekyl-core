...
- **Count-versus-height: height-semantics Phase 2d retyped countersign clocks and difference constants; C4 residue named.** Census has no unclear family; wire table and inland/wire convention are in [`HEIGHT_SEMANTICS.md`](design/HEIGHT_SEMANTICS.md) §3. Phase 2d: `own_height` / `anchor_height` / `predecessor_height` are `BlockHeight`; `PASS_ANCHOR_DEPTH_BLOCKS` / `PASS_ANCHOR_LAG_BLOCKS` / `max_reorg_depth` / `BlockHeaderFacts.depth` are `BlockCount` inland (no numeric change; wire/FFI stay `u64`, C1). Named C4 residue: persisted `pending_post_block.bond_post_offset_blocks` stays `u64` (`PENDING_POST_VERSION` = 11 — a typed wrap is a schema bump to 12). Falsify 2d by `PassSigner::own_height` returning anything but `Option<BlockHeight>`, or `PASS_ANCHOR_DEPTH_BLOCKS` anything but `BlockCount`. Falsify the residue by the field typed `BlockCount` at schema v11. Falsify Phase 1 by a `daemon_claimed_tip` consumer missing from that table, or a consumer whose protocol quantity is ordinal and whose stamp is compared to a count from a different clock.
