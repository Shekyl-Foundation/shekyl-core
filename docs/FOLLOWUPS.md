...
- **FA-6b — v31 multisig `tx_extra_pqc_view_tag_hints`: audit whether the hints are classically linkable (per-wallet-constant clustering without a view key) before the multisig receive path is production-load-bearing.** Separate from FA-6's account-path closure; spec pointer `FA-6_VIEW_TAG_ML_KEM.md` §3.2, §5.1.
  - Target: pre-genesis

- **`tx_extra` `0x02` Nonce: shed from the genesis grammar — FA-10 is the sanctioned tagging channel.**
  - Target: pre-genesis
