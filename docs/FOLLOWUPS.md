...
- **Carry the peer's endpoint in the handshake, and delete the `unknown()` sentinel with it.** `basic_node_data` announces a **port and no address** — a clearnet assumption that has nothing to combine the port with on an overlay, so a zone-wide sentinel was filled in instead of the missing field. Every peer has an endpoint (`.onion`, `.b32.i2p`, or IP and port) because a peer must be reachable to serve. **This deletes work, it does not add it:** the sentinel goes, and with it `identifies_a_host()` in `contrib/epee/src/net_utils_base.cpp`, the `drop_connections` early return, and their comments — all of which exist only to describe a field that should have been there. Verification needs no new mechanism: PWD-B10 already ruled that **the outbound dial is the verification**, so a false announcement produces a failed dial and ages out. Admission policy (no endpoint ⇒ not a peer) is a separate ruling with a `--hide-my-port` consequence, with Rick. — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-I1, PWD-B10
  - Target: pre-genesis
