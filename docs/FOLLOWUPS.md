...
- **Twelve crates meet the feature-governance trigger ungoverned** — each declares a Cargo feature another workspace crate enables, which is the rule for joining `GOVERNED_OWNERS` in `scripts/ci/check_test_only_features.py` (fourth limb, PR #786), and none has: they sit in that file's dated, shrink-only `MET_TRIGGER_UNGOVERNED_AT_REGISTRATION` list. **Three are findings, not chores, and go first:** `shekyl-crypto-pq/test-utils` is enabled on a *normal* edge by `shekyl-ffi`, `shekyl-p-serve/test-signer` by `shekyl-sp-t3-spike`, `shekyl-pow-randomx/test-internals` by `shekyl-randomx-differential` — the exact shape `TEST_ONLY` refuses, on crates it does not govern; each needs the feature moved to a dev edge or ruled `PERMANENT` with the reason. The other nine are categorization (`std`, `schema`, `compile-time-generators`, `gf7-hooks`, test helpers on dev edges). A crate leaves the list by joining `GOVERNED_OWNERS` with every feature categorized; the gate refuses a stale or double entry, so the list only shrinks. Falsify by `python3 scripts/ci/check_test_only_features.py` reporting `trigger met and grandfathered (shrink-only): 0`.
  - Target: pre-genesis
