...
- **`PDM-Q6` items 1–2 ruled before DRS-E2's first production writer; `TxIdentity` and the store carry `pqc_auth_hash`** (`PDM-Q-F26`, 2026-09-16, [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) §6 / §8). Two owed items with one clock. **(1) DRS-E lane:** `TxIdentity { hash, prunable_hash }` (`rust/shekyl-chain-rules/src/block.rs`) carries one of Q6's two occupants; add `pqc_auth_hash` — the spend txid's third component, `H(varint(count) ‖ auths)` as `Transaction::hash()` computes it, **not** `keccak256` of the raw `txs_pqc_auths` segment — with a KAT against the txid, and a store row beside `txs_prunable_hash` (SCW-11's `RUST_ONLY_TABLES` map admits it). Contract on the row before the implementation that omits it (SCW-7's standard). Not optional: a Q6 ruling that keeps `pqc_auths` universal retires the row, not the component. **(2) Steering:** rule Q6 items 1–3 (a transcription of F13/F14/F17 with rule-21 shape against §3's six adversarial items) and name a consensus-side owner for `PDM-Q11` (`D_max`) — the shape is written, the person is not. **SO- contact:** the serve-credit tx's prunable region is F13's good; the SO-D8 writer re-homed onto chain-rules inherits the same E2 ordering (PDM §5 first bullet). Falsify (1) by `rg pqc_auth_hash rust/shekyl-chain-rules/src/block.rs rust/shekyl-chain-store/src/schema.rs` returning both files. **Expiry, not just pending:** if a DRS-E2 replay PR opens while the `PDM-Q6` index row reads OPEN, this deferral is void — that PR states which Q6 items it rules by construction, or it does not merge.
  - Target: pre-genesis
