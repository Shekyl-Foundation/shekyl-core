...
- **Rule the request half of the archival serving route, or ratify `/x-provisional/v0/shard/` as permanent — it is currently ruled into neither.** `ROUTE_PREFIX` ([`rust/shekyl-p-serve/src/serve.rs`](../rust/shekyl-p-serve/src/serve.rs)) is live on **every bonded persona's `.onion`** and appears on 44 lines across three crates — 37 code, 7 comment (`shekyl-p-serve`, `shekyl-p-host`, `shekyl-sp-t3-spike`; instrument: `grep -rn 'ROUTE_PREFIX\|x-provisional' --include=*.rs rust/`, re-run it rather than trusting the tally). **This is not an unfiled item — it is a filed *negative* one, which is worse:** §9.5 ruled that *"the provisional framing is THROWAWAY and gets no vote in the format round"*, whose purpose was to stop a convenient implementation from shaping the frozen format. That ruling removed the route from the only round which would have decided it and assigned it to no other, so under [`23-disposition-visibility`](../.cursor/rules/23-disposition-visibility.mdc) it now matches **no class at all**: that is manifestly not the **zero** code symbols REJECTED/DEFERRED/RESERVED require, and §9.5's own effect is the absent named-consumer-in-a-live-plan that STAGED requires. At genesis an undecided path segment freezes, so the word `provisional` becomes the part that can never change. (§9.5 says "framing"; the enumeration *status line + header set + route* is the crate doc's, sound because `RF-D4` separately froze the payload — but the word "route" appears nowhere in §9.5 itself.) **Not to be conflated with [`ARCHIVAL_RESPONSE_FORMAT.md`](design/ARCHIVAL_RESPONSE_FORMAT.md) (CLOSED 2026-08-21, `RF-D1`…`RF-D10`)**: that round ruled the RESPONSE body and never names a route, request line or URL (grepped: zero hits for `x-provisional`) — and it did not merely omit the HTTP layer, it reasoned it out of scope: `content-length` was refused as TJ-H's reserved field partly because *"it is a property of this transport, not of the format … anything living only in HTTP/1.1 headers does not survive a transport change"* (§ on the served frame). **So two rulings both say "not here" and neither says "over there"** — which is the whole defect, and why it cannot be assumed that some other owner has it. **Falsifier — resolved** iff a dated ruling names the request line and the status/header contract *and* `ROUTE_PREFIX` either cites that ruling or is gone; **not resolved** iff `grep -rn x-provisional rust/` is non-empty while no ruling names it. `x-provisional` surviving to a genesis-freeze tag is the loud form of the failure. `RF-R1` — [`ARCHIVAL_CHALLENGE_MECHANISM.md`](design/ARCHIVAL_CHALLENGE_MECHANISM.md) §9.5
  - Target: pre-genesis
