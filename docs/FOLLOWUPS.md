...
- **`ARCHIVAL_P_DERIVE_V1`'s regenerator is not citation-gated** — its manifest's `regeneration_command` predates rule 50's `SHEKYL_PINNED_REGEN_DECISION` requirement; arm it the way the gate-4 lifecycle regenerator is armed. Surfaced by the withdrawn V1 retirement ([`ARCHIVAL_ENDPOINT_UPDATE.md`](design/ARCHIVAL_ENDPOINT_UPDATE.md) §5, 2026-09-13).
  - Target: pre-genesis
- **PDM-Q-F18 residue: `prepare_handle_incoming_blocks` still pre-fetches ring members into a table nothing reads.** With `check_tx_input` / `scan_outputkeys_for_indexes` deleted, `Blockchain::m_scan_table` (`src/cryptonote_core/blockchain.h:1218`) has **zero readers**: `prepare_handle_incoming_blocks` (`src/cryptonote_core/blockchain.cpp:6708-6905`) still walks every `txin_to_key`'s `key_offsets`, bulk-fetches the referenced output keys through `output_scan_worker` (`:6504`, threaded), and stores them per key image — ring-era work on the hot block-ingest path whose only consumer was the deleted chain. Delete the pre-fetch (`offset_map` / `tx_map` / `output_scan_worker` / the `m_scan_table` member and its three `clear()`s). **Entanglement that must survive the deletion:** the same loop carries the duplicate-tx (`:6756-6758`) and duplicate-key-image (`:6776-6778`) checks, each firing `SHEKYL_DROP_VERDICT_ATTRIBUTABLE_FORM` through `SCAN_TABLE_QUIT` — those are P2P-2 cluster-B verdicts (`PWD-B*`) and need a home (a plain `unordered_set` per batch) before the table goes. Scoped out of the F7/F9/F18/F23 PR because the verdict relocation wants a `PWD-B` owner's read, not because anything blocks the deletion. Falsify by `rg -n 'm_scan_table|output_scan_worker' src` returning nothing **and** the two `ATTRIBUTABLE_FORM` drops still present in `prepare_handle_incoming_blocks`. Owner: [`ARCHIVAL_PRUNED_DAEMON_MODE.md`](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) `PDM-Q-F18`.
  - Target: pre-genesis
