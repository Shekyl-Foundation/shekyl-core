...
- **Count-versus-height: height-semantics Phase 2a walked; Phase 2b blocked on PR #792.** Census has no unclear family; wire table and inland/wire convention are in [`HEIGHT_SEMANTICS.md`](design/HEIGHT_SEMANTICS.md) §3. Remaining: Phase 2b dispatch-clock retype (`ChainCount`, no numeric change) after #792 merges, then Phase 2c wire/FFI decode and Phase 2d inland bare-`u64` tail, each with `compile_fail` per new conversion boundary. Falsify the 2b block by `gh pr view 792 --json state` reporting `MERGED`. Falsify Phase 1 by a `daemon_claimed_tip` consumer missing from that table, or a consumer whose protocol quantity is ordinal and whose stamp is compared to a count from a different clock.
  - Target: pre-genesis
...
- **The open-edge bench has no corpus with realistic block weights, so its volume term is untested rather than measured** (2026-09-20, found by running it; owner **the `WSS-Q1`(b) bench**, [WSS_Q1B_BENCH_SPEC.md](design/WSS_Q1B_BENCH_SPEC.md) §7.2). The first live run against `shekyld --regtest` projected **0.197 s** over the 790-block buffer against a 5 s threshold and attributed the cost **round-trip bound** — which is the model correction §6.3.4 row 3's amendment was made for, and is real as far as it goes. It does not go far enough to read as headroom, for a reason the record states in its own numbers: **regtest blocks are coinbase-only**, so the fetch makes exactly `2.0` round trips per block instead of three (`get_transactions` is never called), and each block carries **1 432 B** decoded. A worst-case block is three orders of magnitude larger and pays the third call, so the measured attribution is as much an artifact of an empty corpus as a finding about the path. **What is owed:** a regtest corpus whose blocks carry non-miner transactions at realistic weights, which coinbase mining cannot produce — it needs a wallet spending into the blocks, the machinery `fcmp_spend_e2e` and the regtest e2e suite already have. Until then the open edge's round-trip term is measured and its volume term is not, and the harness should not be read as having graded the latter. Falsify by an `open_edge` record whose `projected_round_trips / blocks_projected` is 3.0 and whose per-block decoded bytes are within an order of magnitude of the worst-case block weight.
  - Target: pre-genesis
