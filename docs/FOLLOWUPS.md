...
- **The ~42 GB/month cover-traffic budget is signed off PROVISIONALLY; the §3.1c three-arm wire measurement is complete (2026-09-12) and defects 1–3 were not observed.** Condition of the 2026-08-29 sign-off was: build the carrier so a real transaction rides it, then compare actual bytes on the wire against the ceiling. Expected discrepancy and the bar for a real defect are pre-registered [`COVER_TRAFFIC_RESTORATION.md` §3.1c](design/COVER_TRAFFIC_RESTORATION.md). **RUNNABLE as of 2026-09-05** — `shekyld --carrier-development` (hidden) arms the carrier, which nothing outside the gtests could do before; the three-arm method, the exact framed-`WINDOW_BYTES` filter (`HEADER_SIZE + m_cb`, not `m_cb` itself), the count-at-levin-payload trap and the 2 h window are at §3.1c(i); the counter is `utils/carrier/count_windows.py`. **Arm A rehearsal 2026-09-10** (`seedusw`, `--expect-zero`): `windows=0` over 13 node→proxy flows, 20 877 packets, 0 kernel drops. **Arm A 2 h baseline 2026-09-11** (`seedusw`, alpha.8): `windows=0`. **Arm B 2026-09-11/12** (carrier ARMED): first count `windows=0` was the counter still using the pre-PWD-B5 33-byte header; 29-byte re-count `windows=2505 rate_Bps=7128` (87 % of the Tor-only two-channel mean). Defects 1–2 not observed. **Arm C 2026-09-12** (loaded, 153 miner self-transfers): `windows=2577 rate_Bps=7809` (95 % of two-channel mean); per-flow `below_min=0 in_jitter=2563`; C vs B interval histogram unchanged. Defect 3 not observed. Three-arm measurement complete.
  - Target: pre-genesis
