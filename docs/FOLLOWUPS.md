...
- **#774 / #775 mutual re-key — whichever lands second** (`PDM-Q-F33`, 2026-09-18; [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) Q9 and the F33 register row; #775's own entry in [V3_WALLET_DECISION_LOG.md](V3_WALLET_DECISION_LOG.md), 2026-09-17). PR #775 (two stores by obligation; daemon uniformity; all daemons prune) is written against the **leaf unit**: its partition-tie row asserts `SEGMENT_LEAF_COUNT == leaves_per_segment()` and says E3 defines PDM's discard unit in `frozen_segment_count` / `SegmentId`. Under #774 (`PDM-Q6` item 4, `F32`, `Q12`) the freeze retires and the unit is `b_*` over retained length rows closing at `SHARD_BYTES`. #775's *architecture* stands and #774 rules **Q9 on it**; #775's leaf-shaped *consequences* re-key: partition constant → `SHARD_BYTES` (one home, const-asserted, not a tune — the same hazard #775 names), `R_k` → the two hash rows, the E3 sentence → S-PRUNE's predicate on `b_*`. Falsify by both merged and `SEGMENT_LEAF_COUNT` still named as the partition tie in FOLLOWUPS, or `frozen_segment_count` still named as PDM's discard unit anywhere in `docs/design/`.
  - Target: pre-genesis
...
- **The archiver serving-store rebuild — a wallet-lane design round, unbuilt by design** (`PDM-Q12` amended on #775, 2026-09-18; owner **the wallet lane**). Its active round is `CTS-` — [CURVE_TREE_STORE_SHAPES.md](design/CURVE_TREE_STORE_SHAPES.md), PR #776 — a typed-shape rewrite of the *leaf-unit* store that keeps `open_frozen_segment_body` / `FrozenSegmentPruned` (both on Q12's deletion surface); the body-unit rebuild is not in its scope as of #776. The CTS- round owes a scope amendment naming the body unit or a successor round; either registers at birth per rule 94. `PDM-Q6`/`Q12` rebuild `shekyl-curve-tree`'s `LeafStore` around prunable bodies rather than deleting it, and PR #775 makes it *the* serving store; the charter names the substitute and, until this row, no builder. Every decision is unmade: fill from the local daemon during the specified-to-scarce window (`PDM-Q9`) through the ordinary split tx read (`PDM-Q10`) and verify on fill against the daemon's hash rows; key by shard over `[b_k, b_{k+1})` (`PDM-Q-F32`); serve per-tx through `shekyl-p-serve` (`SF-D8` content half, sub-PR 2); its own lapse tail (#775's retention-horizon row is one input); recovery intake from the daemon's episodic fetch; the Foundation `CompleteTree` behind a persona (Q9's structural floor). Inputs cited from [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md). Falsify by `redb_backend.rs` still exposing `open_frozen_segment_body` / leaf-order streaming when `SF` sub-PR 2's per-tx `ContentVerify` lands, or by a `PersonaServing` that serves bodies from any store this row does not name.
  - Target: pre-genesis
