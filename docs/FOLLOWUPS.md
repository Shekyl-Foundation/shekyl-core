...
- **Live-oracle spend-hash KAT: a daemon-captured blob for the FCMP++/PQC spend arms.** Struct-derived cross-language hash parity landed with PR #576 (`pruned_tx_hash_parity.rs`/`.cpp` for the 4-part spend arm, `serve_credit_tx_parity.rs` + C++ leg for the 3-part), binding the two implementations to each other but not yet to a chain. The blocker this entry carried — "the FCMP++ spend path has not produced a daemon-accepted transaction to capture" — **was false from 2026-06-27**, when PR #193 landed `e2e_fcmp_spend_accepted_by_daemon`: a production-`Engine` spend that a live `shekyld` accepts. The entry inherited it from `TRACK2_NORTHSTAR_SAL.md`'s stale `Open` banner. What remains is the capture itself, alongside `capture_coinbase.py`'s corpus (owner: [`GENESIS_TX_WIRE_FORMAT.md`](design/GENESIS_TX_WIRE_FORMAT.md) §11).
  - Target: pre-genesis
