...
- **CEN-B5's post-connect header-root check compares the wrong state — S1; every non-FAKECHAIN chain halts at height 60.** `handle_block_to_main_chain` compares the header `curve_tree_root` against `get_curve_tree_root()` read *after* `add_block` (`blockchain.cpp:6413`), but `create_block_template` fills the header from the root *before* the add (`:2006`) — the state at chain height N that the per-height record (key N), the wallet client (`drained_through = N − 1`) and the CT-2 KAT all name. The operands differ at every block where a leaf drains; the genesis coinbase (stored maturity `0 + 1 + 60`) drains when block 60 connects, so mainnet/testnet/stagenet reject block 60 and halt. Masked by the FAKECHAIN skip (rule 71, census R9) on the only chain type ever run past 59; observed on real LMDB by `archival_substrate_lmdb.cpp` `cen_b5_post_add_root_is_not_the_state_the_header_was_filled_from`. **Fix-or-accept is Rick's** (DRS §7.2). Sketch: compare the header against the pre-add root (record key N, or `get_curve_tree_root()` read before `add_block`), before the write txn opens as the register's REWRITE-NOTE already says; correct `FCMP_PLUS_PLUS.md` §5's "after processing all transactions in the block" to the state the block was built on; red-first core_test on a non-FAKECHAIN nettype, which needs the test generator to produce real roots (the R9 work) — a non-FAKECHAIN Blockchain fixture is the only thing that can turn the current pin into a fix regression. The alternative (template predicts the post-drain root) breaks the CT-2 KAT and the wallet contract. (owner: [`CONSENSUS_RULE_CENSUS.md`](design/CONSENSUS_RULE_CENSUS.md) CEN-B5, §7 #17; register [`CONSENSUS_STORE_RECONCILIATION.md`](design/CONSENSUS_STORE_RECONCILIATION.md) §5.4.1)
  - Target: pre-genesis
