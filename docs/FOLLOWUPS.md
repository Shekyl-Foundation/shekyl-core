...
- **DRS-BENCH redb arm: add when a redb-backed `shekyld` build target exists.** The bench compares **two binaries**, one backend per build — the daemon never compiles two store engines into one (ruled 2026-09-14), so the former `--engine` flag and its `new_db()`-switch probe were deleted as guarding a transition not on the roadmap. Blocked on: a build target producing a redb-backed daemon that **reports its backend** (today `shekyld` reports none; the harness labels every artifact `lmdb` from the only selection mechanism that exists). Falsify by: `cmake --build` producing a second daemon target whose `--version` (or equivalent) names `redb`. `check` already refuses two same-backend artifacts, so the ratio cannot be faked before then. **And when it does exist: the first real ratio must be taken from two artifacts, not a fabricated pair, and the verdict recorded with both artifacts' loads** — because today `check` validates both artifacts before comparing and no selector can yield `redb`, so `compare()` is unreachable on any pair it accepts; §1.3's verdict logic is covered by selftests over fabricated redb fixtures and has never run on real data. `test_redb_artifacts_are_unreachable_until_a_selector_exists` turns red the moment a redb selector is added, which is precisely when to ask whether `check` has ever seen two real artifacts.
  - Target: pre-genesis
