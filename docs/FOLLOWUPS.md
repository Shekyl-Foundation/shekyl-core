...
- **CEN-B5's FAKECHAIN skip survives the S1 fix because the core_tests generator cannot produce real roots.** The header-root check now runs at admission before `add_block` (PR #TBD-B5, 2026-09-05; census §7 #18), but it is still wrapped in `m_nettype != FAKECHAIN` — nettype selecting control flow, rule 71's counter-example. It cannot simply be deleted: core_tests replay every generated block into a real LMDB-backed `core` (`tests/core_tests/chaingen.h`, `c.init(vm, test_options_)`) while `chaingen.cpp` fills headers with the empty-tree root (`shekyl_curve_tree_selene_hash_init`), and ten of those chains rewind through the 60-block maturity window across five files, so the check would reject every generated draining block. **Named blocker:** a generator that computes real roots — a replica of the KAT-verified Rust `CurveTreeClient` (`shekyl-curve-tree`, not FFI-exported today) exposed to chaingen: ingest each constructed block's output leaves, fill the header from `root_at(height)`, replay the ancestor chain from `m_blocks_info` on forks. Then delete the skip and the caption above the check. (owner: [`CONSENSUS_RULE_CENSUS.md`](design/CONSENSUS_RULE_CENSUS.md) R9, CEN-B5; register [`CONSENSUS_STORE_RECONCILIATION.md`](design/CONSENSUS_STORE_RECONCILIATION.md) §5.4.1)
  - Target: pre-genesis
