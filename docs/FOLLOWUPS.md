...
- **`claim.rs:157` logs `skipped` at `debug` — a per-epoch skip list that narrows which persona a log line belongs to.** `skipped = ?derived.skipped` is a `Vec<(u64, EpochSkip)>`: the epochs a persona did not claim in, with a verdict each. `AlreadyClaimed` correlated against the public bond record narrows the persona — the same channel `WSS-20` removed from two sites, weaker in degree but the same kind. It was **not** in `WSS-20`'s enumeration, which is why it is filed rather than fixed: unlike the two sites that were repaired, the repair here costs something real, because a reason-tally drops the per-epoch detail an operator debugging *"why was epoch E not claimed"* actually needs. The decision is whether the correlation is worth that, not how to write it.
  - Owner: the staking lane; surfaced 2026-09-20 in review of PR #793 (`WSS-20`).
  - Target: pre-genesis

- **Nothing drives `refresh` on a timer, so `ServingConfig::refresh_cadence`'s stated premise is false — and that premise is the input `WALLET_SIDE_STORE.md` §6.7.2 says is owed.** The field's doc justifies its value as following the chain *"at the rate the wallet learns about it"*; measurement refuted it. The only production entry points are the `refresh` and `rescan_blockchain` JSON-RPC methods, and the `auto_refresh` preference has **zero consumers** — its declaration and default, nothing else. So `WSS-24` removed the slash exposure, but a wallet nobody calls `refresh` on still carries a stale store. **Filed as one row, not two, deliberately:** §6.7.2 owes *"the principal's refresh cadence in production and the resulting distribution of lag at signing time"* as a measurement, and the cadence is that measurement's input — a distribution computed over a cadence nothing drives would measure a mechanism that is not running. Whoever takes the §6.7.2 measurement must resolve this first or state that it did not.
  - Owner: [`WALLET_SIDE_STORE.md`](design/WALLET_SIDE_STORE.md) §6.7.2 — the section that owes the measurement owns its input.
  - Target: pre-genesis
