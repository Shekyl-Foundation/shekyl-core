...
- **`LmdbHashKey::from_bytes` / `Key::compare` still `expect` on redb's fixed-width promise.** The `Tagged<V>` retrofit that folds that expect is the named landing; RTN-7's `From<BlockHash>` / `From<TxHash>` on `Hash32` is the signature it lands against ([`completed/RTN_7_WIRE_HASH_TYPES.md`](completed/RTN_7_WIRE_HASH_TYPES.md) §2). Falsify by `rg 'Tagged<' docs/ rust/` returning a row that names this expect. Owner: `shekyl-chain-store` / DRS.
  - Target: pre-genesis
