...
- **Delete `get_version`'s `target_height` zero-sentinel at the wire boundary.** `shekyl-daemon-rpc/src/methods.rs:138` writes `if tip.synchronized { 0 } else { tip.target_height.to_raw() }`, overloading a height field to carry a second fact. **This is a wire-surface wart, not a contract change:** `ChainTip` (`rust/shekyl-daemon-rpc/src/chain_facts.rs:53-56`) already models it correctly — a raw `target_height` beside a separate `synchronized` bool — and its own doc already disclaims the sentinel as *"the handler's"*. The sentinel survives only where the POD is flattened into `GetVersionResponse`. **Owner: the daemon-RPC surface's lane** (`RK-`), whose gate obligations this carries — a JSON-RPC response-shape change, not the wallet's. **Discharge condition:** when it lands, `SyncedChainFacts`'s sentinel arm has nothing left to absorb and the constructor collapses from `synchronized && (target_height == 0 || height >= target_height)` to the flag alone (`rust/shekyl-engine-core/src/engine/daemon/synced_chain_facts.rs`, PR #792, ratified 2026-09-19). Note the wallet's constructor reads **`get_info`**, which has no Rust handler yet, so this deletion does not by itself reach the wallet — the two move independently and the conjunction retires when `get_info` migrates. Falsify by `rg -n 'synchronized \{ 0 \}|if tip.synchronized' rust/shekyl-daemon-rpc/src/methods.rs` returning nothing.
  - Target: pre-genesis

- **Count-versus-height audit: `daemon_claimed_tip` labels a block count as a `BlockHeight`, and it is the third instance in a week.** `Rpc::get_height` returns the **count** (`rust/shekyl-rpc-client/src/lib.rs:380-384`: *"the amount of blocks… for a blockchain with only its genesis block, the height will be 1"*), and `daemon_claimed_tip` has always wrapped it straight into `BlockHeight::from_raw` (`rust/shekyl-engine-core/src/engine/pscan/block_source.rs`). Six consumers read their clock through it — `anchor_t0`, the claim / drain / release dispatch stamps, and both `BlockSource::tip_height` impls, whence the pscan finality horizon — so reading `.tip()` would move every one of those stamps down one block. **Rick's ruled order, binding (2026-09-19): numerics frozen, consumers walked, then types make the wrong quantity uncompilable — the value is never flipped first,** because that puts a consensus-adjacent change inside a PR scoped for something else. **Step 1 is done (PR #792):** the arithmetic is frozen and pinned by `a_synchronized_daemon_reports_the_unchanged_clock`, which asserts a 3-block chain still reports 3, so the shift cannot happen by accident. **Step 2 is the open work** — walk each of the six consumers and decide per site; that is where the behavior decision actually happens. **Step 3 lands the type only after that**, so the wrong quantity stops compiling rather than merely being documented. **Why a consumer walk and not just a newtype:** the three instances are WSS-24's producer-side conversion, PR #792's own fixture answering `get_info` with a tip where a count was required, and this one. A type protects the consumers that *hold* it; `daemon_claimed_tip` is precisely a caller that never held one, which is how it survived. A newtype alone would repeat that. **Discharged when** every consumer's site is decided and the count/height distinction is carried by a type at this seam. Owner: this lane (PR #792's finding).
  - Target: pre-genesis
