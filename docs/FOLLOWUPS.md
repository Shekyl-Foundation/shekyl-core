...
- **PR-P4 slice 3 is only PARTLY discharged: the engine walk asserts the wipe and the funded gate at the actor; the seal-then-act ordering is unobservable at that level.** `store.save` and `dispatch_retires` both sit inside `pscan_sweep`'s per-batch loop, so reaching them needs a chain longer than the ~270k-block cursor the claim-window expiry requires. Closing it needs a seam reaching that pair without the scan, or the daemon walk. **The reachability gate is NOT lifted** — `IMPLEMENTATION_INDEX.md` and `api/wallet_rpc.yaml` correctly still say the gating walk has not run. [`retire_walk.rs`](../rust/shekyl-engine-core/src/engine/retire_walk.rs)
  - Target: pre-genesis

- **The DAEMON walk: no test anywhere puts an `Unbond` on a wire.** The engine walk never assembles or encodes one — it builds a `RetirementWitness` and sends it to the actor — so *the bytes we assemble are the bytes consensus accepts* is unjudged, cross-language and cross-process. `RF-D9` is the precedent (the serve-credit wire had never round-tripped through the C++ oracle). Blocked: native `/submit_transaction` refuses `Unbond` as `Malformed`; the construction-leg reopen fired, the submit fact set has not. Rides that slice. Absence is invisible — everything is green.
  - Target: pre-genesis
