...
- **`lambda_target` is a bare parameter; `CHALLENGES_PER_PAIR_PER_EPOCH` does not reach the urn.** `challenge_assignment.rs:152` and `:264` take `lambda_target: u32`; the constant (`constants.rs:27`) is read only by `shekyl-economics-sim` and the `attestation.rs:77,:82` const-asserts. Nothing connects the two, so the urn and the settlement threshold can disagree silently. Pin before any production caller of `assign_epoch` / the `EpochAssignmentCache` (`SO-D8e`): the cache's `open()` reads the constant and takes no `λ` argument. Falsify by a non-test caller of `assign_epoch` / `ChallengeUrn::new` (`rg -n 'assign_epoch\(|ChallengeUrn::new\(' rust --type rust`; today all callers are inside `#[cfg(test)]`) that passes `CHALLENGES_PER_PAIR_PER_EPOCH`, or by the parameter being removed in favour of the constant read at the definition. Owner: `ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md` §7.3 / Q4.
  - Target: pre-genesis
- **Serve-credit inclusion incentive and archival weight pricing — the fee-and-weight round Q9 names but nothing scheduled.** Q9 (RULED 2026-09-16, `ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md` §7.6) leaves three items to a round that did not exist as a work item: (1) **unpaid inclusion** — the witness is unpaid by ruling and the carrier is structurally fee-less (`txnFee != 0` refuses), so inclusion rests on miners carrying zero-fee weight, and a miner at the median is priced to omit; (2) a **prunable-weight discount** for archival bytes (the lever if (1) bites — same weight-path fact, other side); (3) the **cap-raise** to admit one 97-record carrier (~11 KB/block ≈ 2.9 GB/year vs the 39/39/19 split). Blocked on: the economics sim's inclusion-rate dynamics at median-bound blocks (not on Slice C). Falsify by a sim run showing the inclusion rate of zero-fee carriers at blocks at or above the effective median; if omission is material, (2) is the lever and this row becomes its design round; if not, close (1) as won't-fix and carry (3) alone. Owner: `ARCHIVAL_SETTLEMENT_SO_D8_PROPOSAL.md` §7.6 ("The residual, held").
  - Target: pre-genesis
