...
- **S0 (§7.2 ladder): the FCMP++ proof-skip at block connect is presence-gated, not hash-gated — an invalid proof can connect (`CEN-M8` DIVERGENT).** `can_skip_fcmp = found_tx_in_pool && is_fcmp` (`blockchain.cpp:6128`) never consults `meta.fcmp_verified`, while `add_tx(kept_by_block)` tolerates a failed input check and inserts with `fcmp_verified = 0` (`tx_pool.cpp:304–330`) — so a block-carried tx whose membership/spend-auth proof failed at admission gets the proof skipped at connect. The ratified rule (CEN-M8) requires the skip only "when the hash still matches", and the exact check exists unused on this path (`is_fcmp_verification_cached`, `tx_pool.cpp:1735–1743`). Fix ≈ read the meta before `take_tx` and require the cached check in `can_skip_fcmp`; the D++ embargo semantics survive (verified txs still skip). Per the ladder: fix or explicit risk-accept in the decision log — **ruled FIX 2026-09-03 (Rick)**; fix = **PR #602** (hash-gated skip via `take_tx`, red-observed tests), and CEN-G4/J26 are coupled to this row (round 2), so all three re-review together at a sha containing the merged fix. Owner: [`CONSENSUS_STORE_RECONCILIATION.md`](design/CONSENSUS_STORE_RECONCILIATION.md) §5.4.1 slice 8.
  - Target: pre-genesis

- **S1 (§7.2 ladder): the PoW-failure sentinel fails open at difficulty 1 (`CEN-D2` DIVERGENT).** On verifier FFI failure the longhash is forced to `0xff…` and the validation paths rely on `check_hash` rejecting it — but `check_hash(0xff…, 1)` passes ((2²⁵⁶−1)·1 < 2²⁵⁶; the crate's own test documents it), both call sites ignore the verifier's returned bool (`blockchain.cpp:5899–5902` main, `:2414–2416` alt), and difficulty 1 is reachable — `--fixed-difficulty` is a general daemon arg on any nettype, and organic LWMA-1 has no floor above CEN-D6's zero-guard. Under (FFI failure ∧ difficulty 1) a block connects with its PoW never verified. Fix ≈ consume the verifier's bool at both validation sites and reject on failure as a verification error (sentinel stays as a belt); correct the in-tree comment that claims the sentinel "guarantees" rejection. Per the ladder S1 blocks DRS-0: fix or accept before engine work — Rick's call. Owner: [`CONSENSUS_STORE_RECONCILIATION.md`](design/CONSENSUS_STORE_RECONCILIATION.md) §5.4.1.
  - Target: pre-genesis
