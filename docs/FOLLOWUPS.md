...
- **The segment partition is a consensus surface with a thin tie between its two derivations — assert it in `shekyl-archival-retention`; DRS-E3 S-CURVE consumes `frozen_segment_count`, never a partition of its own.** `frozen_segment_count = ⌊leaf_count / SEGMENT_LEAF_COUNT⌋` (`segment_freeze.rs:12–13`, O-1/O-3) decides admissible `shard_id`s and what pop revert deletes, so a divergent partition forks at admission and revert. Today `SEGMENT_LEAF_COUNT` is generated (`build.rs:249`) and const-asserted to the `shekyl-fcmp` width product (`segment_freeze.rs:44–46`), while the wallet-side store computes `outputs_per_node(SEGMENT_LAYER_J)` from the same widths independently (`shekyl-curve-tree/src/segment.rs:64`); they agree through the widths, not through each other. `SEGMENT_LAYER_J` is marked "provisional" (`segment.rs:17`) while being de facto consensus-frozen — the marker invites the fork; `consensus_constants.json` is the more dangerous door (a Rust-free edit that reads as a tune); and the config value also reaches challenge leaf selection through the segment registry row written at freeze (`blockchain.cpp:5050–5073`). Owed, one PR in three pieces: (0) make `outputs_per_node` / `leaves_per_segment` `const fn` in curve-tree (rewrite the loop over a `u8` counter — `Result::expect` is not const) and drop or justify the "provisional" marker; (1) a **compile-time** `const _: () = assert!(leaves_per_segment() as u64 == SEGMENT_LEAF_COUNT)` in archival-retention (the crate that sees both; the dependency runs archival-retention → curve-tree) — a test is the fallback a PR settles for if it skips (0), and is strictly weaker for a consensus invariant; (2) E3 imports `SEGMENT_LEAF_COUNT` / `SegmentId` and defines PDM's discard unit in them; (3) E3 runs the CT-0 freeze KAT (`shekyl-fcmp/tests/curve_tree_freeze.rs`) against its own skeleton. Precedent: RF-D8 at `segment_freeze.rs:89–92` — the same drift class at the same boundary, caught once by the cross-language path. `recon` unification is ruled after E3, conditional on a CT-2 pin for global-vs-segment-relative index confusion (`V3_WALLET_DECISION_LOG.md` 2026-09-17, two stores). Falsify by `rg -n 'SEGMENT_LAYER_J|outputs_per_node|struct SegmentId|SEGMENT_LEAF_COUNT' rust/shekyl-chain-store` returning a definition rather than an import, or by the equality assertion being absent from archival-retention.
  - Target: pre-genesis

- **The archiver's store has its own retention horizon for the path-assembly serving purpose, and no owner.** Bond-side retention past a drop is zero (Q3 §7.4 (3.3): the witness skips the fetch for a pair not held at the fire height). What is unstated is the wallet-service half — how long an archiver keeps serving a shard it no longer holds a bond for, and what the route answers once the store has let it go. Owner: `ARCHIVAL_SERVING_ROUTE.md` with `shekyl-curve-tree`'s store as the enforcing site. Falsify by a documented rule in that contract and a test in `rust/shekyl-curve-tree/src/store/` for the serving reader's answer past the horizon.
  - Target: pre-genesis

- **Daemon uniformity: "no archival serving state in the daemon, ever" must be written where DRS-E4 S-ARCH will read it.** Ruled 2026-09-17 (`V3_WALLET_DECISION_LOG.md`, two stores): the daemon holds archival consensus state (bonds, serve credits, settlement, slash) and no serving state, because an archiver-backed daemon that behaves differently from a plain one fingerprints the Principal's public address. The archival fast path in the daemon is REJECTED at any performance argument; the criterion is persistent posture-correlated state (forbidden) versus episodic, universally available actions such as the witness fetch (allowed). Pruning posture is part of uniformity: all daemons prune uniformly (PDM owns the default). The constraint belongs in `DAEMON_REDB_STORE.md`'s S-ARCH / E4 design so the port enforces it, not only the decision log. Carrier: the E4 S-ARCH pre-flight (the DRS lane's doc, rule 94 §6). Falsify by `rg -n 'serving|segment|LeafStore|shard bytes' rust/shekyl-chain-store/src` returning nothing that serves archival content.
  - Target: pre-genesis
...
- **`SEGMENT_FREEZE_REORG_MARGIN_BLOCKS` dedup (CT-1).** Lands with PHASE_2B, in [`config/consensus_constants.json`](../config/consensus_constants.json) **Amended 2026-09-18 (two-store review, `V3_WALLET_DECISION_LOG.md` 2026-09-17):** the hazard is now concrete — with the segments in the wallet-side store and the `R_k` skeleton in the daemon's, a config move of `ARCHIVAL_REORG_DEPTH_BLOCKS` (`shekyl-archival-retention/build.rs:210`) leaves segments freezing at the hardcoded 720 (`segment.rs:22`) while `SO-D8e`'s fork analysis, `bond_assembly.rs:400` and `pass_anchor.rs` move; a segment can then be frozen and served inside the depth the rest of the system treats as reorg-able. If PHASE_2B codegen is not imminent, the interim fix is `const _: () = assert!(SEGMENT_FREEZE_REORG_MARGIN_BLOCKS == ARCHIVAL_REORG_DEPTH_BLOCKS)` **in `shekyl-archival-retention`** — the crate that sees both constants; the dependency runs archival-retention → curve-tree, so it cannot live in `segment.rs`. Same PR as the `SEGMENT_LEAF_COUNT == outputs_per_node(SEGMENT_LAYER_J)` tie (partition row). Falsify by that assertion or the import existing.
  - Target: pre-genesis
...
- **Daemon shard-fetch client (`SF-` round): sub-PR 1 BUILT 2026-09-13 as one PR stacked on #734; §9.1 (c) DONE 2026-09-16 (`N = 8`) — what remains** — `shekyl-p-fetch` (dial, `shekyl-pass-request` header, envelope, SOCKS5h reuse, `MAX_INFLIGHT = 8` pinned, `SF-D6` taxonomy, countersignature verify via the shared `verify_pass_transcript`, `Vec<u8>` body, `ContentVerify` hole), the serve-side `SF-D8` countersignature and pre-sign gate, the `PassSigner`/`PassKey` seam, the `serving_route` home, the `RF-R1` update, and `check_p_fetch_dep_cut.py` all landed in that PR plus #746 ([ARCHIVAL_SHARD_FETCH.md](design/ARCHIVAL_SHARD_FETCH.md) §9.1). Open, each with its carrier:
  - Target: pre-genesis
  - **Sub-PR 2 — UNBLOCKED 2026-09-17 (`PDM-Q6` items 1–3 RULED; was blocked on it since PR #723):** the frame codec and the content-verify are **per-tx** over a **`tx_id`-range** body `[k·T, (k+1)·T)`: each fetched tx is re-hashed with `Transaction::txid_parts()` and its components checked against what the node retains — `txs_prunable_hash` (exists) and `txs_pqc_auth_hash` (**the target verifier once its store row lands**, DRS §7.7 item 3 / S-CHAIN-W A3; until then a full body still verifies through the recorded txid, which commits both). **That per-tx verifier** is what plugs into `ContentVerify`. The leaf-shard scheme — `ServedFrameHeader` (`RF-D4`) + `recompute_segment_r_k` — **does not survive** (`PDM-Q-F25`, ruled) and is deleted, not adapted. Then the scheduler that constructs `PFetchClient` (challenge and organic callers, `SF-D1`, `SF-D10`); `max_body_bytes()` re-derived as `T × max_tx_size` (the ruling's pricing falsifier). Still owed *to* it from Q6 item 4: the re-key of `SF-D7`/`SF-D8`/`SF-D1`'s addressing clause.
...
- **F26 landing: the `txs_pqc_auth_hash` store row (DRS §7.7 item 3) still owed; the identity landed. `PDM-Q6` items 1–3 RULED 2026-09-17.** (`PDM-Q-F26`, [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) §6 / §8; DRS §7.7). **Landed 2026-09-17 on #768:** `TxIdentity { hash, pqc_auth_hash: Option<PqcAuthHash>, prunable_hash }` from `Transaction::txid_parts()` (one construction; `None` ⇔ 3-part txid), plus `hash_with_supplied_components` so a skeleton reconstructs the 4-part txid. **Still owed:** the sparse `txs_pqc_auth_hash` row beside `txs_prunable_hash` (S-CHAIN-W amendment A3 on S-CHAIN-R's layout commit, one `SCHEMA_VERSION` bump) — present ⇔ 4-part, never deleted; hash row without segment is *discarded*, the steady state below `W`, not a fault. **Steering — DONE 2026-09-17:** `PDM-Q6` items 1–3 RULED (item 2 *ratifies* the landed identity; item 3 makes a shard a `tx_id` range — not the store row, which is DRS §7.7's item 3; Q6 item 4 OPEN by name) and `PDM-Q11` RULED (shape frozen, home `CEN-E2`, 720 PROVISIONAL) — the owner ask is discharged by the ruling. The E2-writer deadline is met. Falsify the remaining store half: `rg 'TableDefinition.*"txs_pqc_auth_hash"' rust/shekyl-chain-store/src/schema.rs` matches a table constant. **Expiry (now moot for items 1–3, live for the row):** an E2 replay PR that opens while the `txs_pqc_auth_hash` row is absent from `schema.rs` states so, or it does not merge.
  - Target: pre-genesis
...
- **S-PRUNE needs a rule-26 plan doc before its first increment** (`PDM-Q-F31`, 2026-09-17; owner **DRS-E lane**). The store-side implementation items in [ARCHIVAL_PRUNED_DAEMON_MODE.md](design/ARCHIVAL_PRUNED_DAEMON_MODE.md) §8 — the discard predicate on `eligible_height` vs `tip − W` (F10), retention exceptions (Q9), the undo-log floor (SCW-7), the `pqc_auths` discard consuming `txs_pqc_auth_hash` (F26), `W` entering D11 — land on or adjacent to S-PRUNE (**not** F27's below-anchor mode, which is E6's, nor F28's wire field, which is `LV-`/`PWC-`'s), which is deliberately not extracted ([DAEMON_REDB_STORE.md](design/DAEMON_REDB_STORE.md) §7) and already carries two contracts on its row with no home. E1 got `DRS_E1_SCHAIN_W.md` before its writers; S-PRUNE's successor gets `DRS_E*_SPRUNE.md` before its first increment, and it is where the three-leg invariant, the predicate's enforcement point, the exceptions structure and §3's reorg discard-side half stop being scattered. Writable now against the charter with `W` and the exceptions' binding as named inputs; compatible with `PDM-Q-S0` (design, not implementation). Falsify by `ls docs/design/DRS_E*_SPRUNE.md` returning a file with a rule-26 Round-0 pre-flight, before any commit touches the S-PRUNE row's status.
  - Target: pre-genesis
