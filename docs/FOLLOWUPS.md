...
- **The segment partition is a consensus surface with a thin tie between its two derivations — assert it in `shekyl-archival-retention`; DRS-E3 S-CURVE consumes `frozen_segment_count`, never a partition of its own.** `frozen_segment_count = ⌊leaf_count / SEGMENT_LEAF_COUNT⌋` (`segment_freeze.rs:12–13`, O-1/O-3) decides admissible `shard_id`s and what pop revert deletes, so a divergent partition forks at admission and revert. Today `SEGMENT_LEAF_COUNT` is generated (`build.rs:249`) and const-asserted to the `shekyl-fcmp` width product (`segment_freeze.rs:44–46`), while the wallet-side store computes `outputs_per_node(SEGMENT_LAYER_J)` from the same widths independently (`shekyl-curve-tree/src/segment.rs:64`); they agree through the widths, not through each other. `SEGMENT_LAYER_J` is marked "provisional" (`segment.rs:17`) while being de facto consensus-frozen — the marker invites the fork; `consensus_constants.json` is the more dangerous door (a Rust-free edit that reads as a tune); and the config value also reaches challenge leaf selection through the segment registry row written at freeze (`blockchain.cpp:5050–5073`). **Owed now — the partition PR, one PR in three pieces, all in the crate the dependency direction forces (archival-retention → curve-tree):** (a) make `outputs_per_node` / `leaves_per_segment` `const fn` in curve-tree (rewrite the loop over a `u8` counter — `Result::expect` is not const) and drop or justify the "provisional" marker; (b) a **compile-time** `const _: () = assert!(leaves_per_segment() as u64 == SEGMENT_LEAF_COUNT)` in archival-retention — a test is the fallback a PR settles for if it skips (a), and is strictly weaker for a consensus invariant; (c) the `SEGMENT_FREEZE_REORG_MARGIN_BLOCKS == ARCHIVAL_REORG_DEPTH_BLOCKS` assert at the same site (the CT-1 dedup row below). **Owed later, by DRS-E3 S-CURVE, not this PR:** import `SEGMENT_LEAF_COUNT` / `SegmentId` and define PDM's discard unit in them; run the CT-0 freeze KAT (`shekyl-fcmp/tests/curve_tree_freeze.rs`) against its own skeleton. Precedent: RF-D8 at `segment_freeze.rs:89–92` — the same drift class at the same boundary, caught once by the cross-language path. `recon` unification is ruled after E3, conditional on a CT-2 pin for global-vs-segment-relative index confusion. Owner: [`V3_WALLET_DECISION_LOG.md`](V3_WALLET_DECISION_LOG.md) (2026-09-17, two stores) for the ruling; [`DAEMON_REDB_STORE.md`](design/DAEMON_REDB_STORE.md) §7.5 for the E3 half. **Discharged when** the compile-time assertion exists in archival-retention (partition PR) **and** `rg -n 'SEGMENT_LAYER_J|outputs_per_node|struct SegmentId|SEGMENT_LEAF_COUNT' rust/shekyl-chain-store` returns imports only (E3). **Violation signal:** that `rg` returning a definition.
  - Target: pre-genesis

- **The archiver's store has its own retention horizon for the path-assembly serving purpose, and no owner.** Bond-side retention past a drop is zero (Q3 §7.4 (3.3): the witness skips the fetch for a pair not held at the fire height). What is unstated is the wallet-service half — how long an archiver keeps serving a shard it no longer holds a bond for, and what the route answers once the store has let it go. Owner: [`ARCHIVAL_SERVING_ROUTE.md`](design/ARCHIVAL_SERVING_ROUTE.md), with `shekyl-curve-tree`'s store as the enforcing site. **Discharged when** that contract carries a documented retention rule for the serving purpose and `rust/shekyl-curve-tree/src/store/` has a test for the serving reader's answer past the horizon. Ruling: [`V3_WALLET_DECISION_LOG.md`](V3_WALLET_DECISION_LOG.md) (2026-09-17, two stores).
  - Target: pre-genesis
