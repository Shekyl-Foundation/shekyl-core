...
- **Derive PWD-B2's timed-sync `min`/`jitter` split.** The distribution is ruled — bounded uniform `min + U(0, jitter)`, per-connection, re-drawn only on its own fire — and the constraint is fixed: **the mean stays at `P2P_DEFAULT_HANDSHAKE_INTERVAL` (60 s)**, so any pair with `min + jitter/2 = 60 s` conforms. Which pair is a trade between worst-case staleness and how much phase separation the window buys. **Unruled, not blocked.** PWC-E1, PWC-E3 — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B2
  - Target: pre-genesis
