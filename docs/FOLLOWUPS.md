...
- **The `shekyl-fcmp` fuzz crate is built nowhere in CI, so a target has been uncompilable undetected** — `fuzz_curve_tree_leaf_hash.rs:33` calls `construct_leaf` with two arguments (it takes three since the 4th-scalar parameter); the only gate is the file-presence inventory in `rust-audit-test.yml`. Build the fuzz crate in CI (`cargo fuzz build` or `--features fuzzing` check) so a stale target is red; the target itself is rewritten by the `PL-D3` implementation ([`design/FCMP_SPEND_LINKABILITY.md`](design/FCMP_SPEND_LINKABILITY.md) §9 d-1).
  - Target: pre-genesis
