...
- **DRS-P0 multi-PR — blocks DRS-0.** **P0a** schema+CI — **DELIVERED 2026-09-05** (49-row reconciliation registry + heading/registry gate legs; see the P0a registry in [`DAEMON_REDB_STORE.md`](design/DAEMON_REDB_STORE.md)); **P0b** atomicity+journals+transcriptions — **DELIVERED 2026-09-05** ([`LMDB_WRITE_ATOMICITY_AUDIT.md`](LMDB_WRITE_ATOMICITY_AUDIT.md) rewritten in place, §10 matrix gate-pinned, A-2/A-4/A-6 transcribed, DRS-W1…DRS-W15 recorded — DRS-W1 through DRS-W11 by P0b, DRS-W12 through DRS-W15 by P0c); **P0c** wart register — **DELIVERED 2026-09-08** (A-1/A-3/A-5 and the `hf_versions` pop row inverted to RECORD-AND-SPECIFY under the 2026-09-01 countermand, no C++ written; the A3 narrow exception tested against `hf_versions` and did not fire — no ratified, conformance-checked row covers post-reorg hardfork reconstruction, CEN-B3 being bucket 4 and held for R4; the read-back is established load-bearing, **not** established correct. "FIX or REPLICATE" closes on REPLICATE, with both the open correctness question and the mechanism routed to census R4); **P0d**
  - Target: pre-genesis
