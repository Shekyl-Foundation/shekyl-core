...
- **Replace `block_verification_context`'s bag of bools with a discriminant, and classify each arm for PWD-B7 drop.** Independent flags (`m_added_to_main_chain` vs `m_verifivation_failed`, plus orphan / already-exists / bad-pow / missing-txs) encode one outcome as overlapping true/false pairs; both-false is a real third class (orphan, alt-chain, prune-watermark refused switch). PWD-B7 (#674) typed the *tx* path; `bvc` was out of scope (rule 19). The misspelled flag still drops on set-ness (`cryptonote_protocol_handler.inl:633` announce, `:1571` sync, heavier score on `m_bad_pow`). New arms already default to no-drop (set affirmatively), unlike the old `tvc` absence-means-droppable. Name the enum arms from the setters: if any describe our state, they write `shekyl-peer-policy` and do not sever. Rename `m_verifivation_failed` with the struct. — [`SHEKYL_P2P_PROTOCOL.md`](design/SHEKYL_P2P_PROTOCOL.md) PWD-B7
  - Target: pre-genesis
