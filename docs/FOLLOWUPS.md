...
- **Daemon shard-fetch client (`SF-` round): sub-PR 1 BUILT 2026-09-13 as one PR stacked on #734 — what remains** — `shekyl-p-fetch` (dial, `shekyl-pass-request` header, envelope, SOCKS5h reuse, `MAX_INFLIGHT = 4` SPIKE-PIN, `SF-D6` taxonomy, countersignature verify via the shared `verify_pass_transcript`, `Vec<u8>` body, `ContentVerify` hole), the serve-side `SF-D8` countersignature and pre-sign gate, the `PassSigner`/`PassKey` seam, the `serving_route` home, the `RF-R1` update, and `check_p_fetch_dep_cut.py` all landed in that PR ([ARCHIVAL_SHARD_FETCH.md](design/ARCHIVAL_SHARD_FETCH.md) §9.1 (a)+(b)). Open, each with its carrier:
  ...
  - **§9.1 (c), W₂ then pin `N` — rig re-based 2026-09-14, run pending:** the SP-T3 rig's client leg is on `shekyl-p-fetch` (daemon→wallet topology, one client tor, `NEWNYM` as the cold arm, `Refused` split from `Circuit`/`Stall`), the concurrency sweep produces the `SF-D7` churn table and the cold arm's p99 is read against the `L` note's two thresholds; `live_apparatus` passed over real Tor (110 s) — the stale-leg claim is closed. **What remains is the run:** it serves a REAL regtest shard by design (no synthetic fallback, `fixture.rs`), so it needs a regtest chain mined past the shard-0 freeze (~5 h) and `extract-shard`, then `pd-f2-measure` with `SHEKYL_SPIKE_HOURS ≥ 24` for the soak. Then read `N` off the churn table (min of largest non-churning width and Pi 4 memory fit), replace the SPIKE-PIN, and apply the `L` verdict. Falsify "run pending" by `rg SPIKE-PIN rust/shekyl-p-fetch/src` returning nothing.
