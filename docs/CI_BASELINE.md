...
| Verification | `wallet_storage.*` 3 PASSED, 0 SKIPPED, 0 failed (the three previously-skipped Monero-era keys-file fixtures and their tests were deleted in the same branch — see CHANGELOG `### Removed`). `account.*` 8 PASSED, 0 failed (the 6 new entries cover the full permitted `(network, format)` matrix — `rederive_from_bip39_reproduces_account_mainnet`, `rederive_from_bip39_reproduces_account_stagenet`, `rederive_from_raw_seed_reproduces_account_testnet`, `bip39_passphrase_changes_account_mainnet`, `generate_from_bip39_rejects_fakechain_and_testnet`, `generate_from_raw_seed_rejects_mainnet_and_stagenet`). Rust: `shekyl-crypto-pq` 215/215, `shekyl-ffi` (incl. signing FFI round-trip and the two new FFI constant equality assertions, `ffi_classical_address_bytes_matches_rust_authority` and `ffi_seed_format_constants_match_rust_authority`) all green, `shekyl-engine-file` 55/55. |
| Migration concern? | None. Pre-V3 launch (per `15-deletion-and-debt.mdc`), no on-disk wallets exist. Both the RAW32 and BIP-39 paths were broken under the buggy header (RAW32 silently mis-decoded as BIP39; BIP-39 written as `0` which Rust rejects as "unset"). Neither produced a loadable file that needs migration, so the rule "rm -rf and re-sync" applies trivially. |
